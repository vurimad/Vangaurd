...
namespace vanguard::runtime
{
        ...
        application::StateOperationStatus StopSession(application::StateContext& context) noexcept
        {
            ...
            if (!session->RequestStop(engine::WorldSessionStopMode::ReleaseEverything, &failure))
                return application::StateOperationStatus::Failure(
                    failure.message != nullptr ? failure.message : "world session stop request failed");
            ...
            {
                const engine::WorldSessionFailure& reported = session->LastFailure();
                VG_LOG_ERROR(diagnostics::Category::Engine,
                             "world session stop failed: code=%u packageResult=%u resourceFailure=%u message=%s",
                             static_cast<u32>(reported.code), static_cast<u32>(reported.packageResult),
                             static_cast<u32>(reported.resourceFailure),
                             reported.message != nullptr ? reported.message : "<none>");
                return application::StateOperationStatus::Failure(
                    reported.message != nullptr ? reported.message : "world session stop failed");
            }
    ...
    application::StateOperationStatus StartupSessionState::OnEnter(application::StateContext& context) noexcept
    {
        ...
        if (!session->Begin(request, &failure))
        {
            VG_LOG_ERROR(diagnostics::Category::Resources,
                         "runtime session start failed: root=%s code=%u packageResult=%u resourceFailure=%u message=%s",
                         request.gameDirectory.ToDebugString(), static_cast<u32>(failure.code),
            ...
            return application::StateOperationStatus::Failure(
                failure.message != nullptr ? failure.message : "runtime session start failed");
        }
    ...
    application::StateTickStatus StartupSessionState::OnTick(application::StateContext& context) noexcept
    {
        ...
        if (status == engine::WorldSessionStatus::Failed)
        {
            const engine::WorldSessionFailure& reported = session->LastFailure();
            VG_LOG_ERROR(diagnostics::Category::Resources,
                         "runtime session loading failed: code=%u packageResult=%u resourceFailure=%u message=%s",
                         static_cast<u32>(reported.code), static_cast<u32>(reported.packageResult),
                         static_cast<u32>(reported.resourceFailure),
                         reported.message != nullptr ? reported.message : "<none>");
            return application::StateTickStatus::Failure(
                reported.message != nullptr ? reported.message : "runtime session loading failed");
        }
    ...
    application::StateTickStatus RunningState::OnTick(application::StateContext& context) noexcept
    {
        ...
        {
            VG_LOG_ERROR(diagnostics::Category::Engine,
                         "engine frame failed: frame=%llu code=%u phase=%u participant=%llu related=%llu name=%s message=%s",
                         failure.frame, static_cast<u32>(failure.code), static_cast<u32>(failure.phase), failure.participant,
                         failure.relatedParticipant, failure.participantName != nullptr ? failure.participantName : "<none>",
                         failure.message != nullptr ? failure.message : "<none>");
            return application::StateTickStatus::Failure(
                failure.message != nullptr ? failure.message : "engine frame failed");
        }
        return application::StateTickStatus::Success();
    }
        ...
        application::ApplicationStateMachine& states) noexcept
    {
        ...
        if (!engine::RegisterEngineModule(services, &failure) || !engine::RegisterIoService(services, &failure) ||
            ...
            return application::CompositionStatus::Failure(
                failure.message != nullptr ? failure.message : "runtime engine service registration failed");
