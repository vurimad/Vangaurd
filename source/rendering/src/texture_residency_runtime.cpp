...
namespace vanguard::rendering
{
    namespace
    {
        ...
        [[nodiscard]] bool Fail(TextureResidencyRuntimeFailure* const failure,
                                const TextureResidencyRuntimeFailureCode code, const char* const message) noexcept
        {
            if (failure != nullptr)
            {
                failure->code = code;
        ...
        [[nodiscard]] bool FailResidency(TextureResidencyRuntimeFailure* const failure,
                                         const TextureResidencyFailure& residencyFailure,
        ...
        [[nodiscard]] bool FailUploader(TextureResidencyRuntimeFailure* const failure,
                                        const TextureUploadFailure& uploadFailure,
                                        const char* const message) noexcept
        {
            if (failure != nullptr)
            {
        ...
        [[nodiscard]] bool FailGpuScene(TextureResidencyRuntimeFailure* const failure,
                                        const GpuSceneRuntimeFailure& gpuSceneFailure,
                                        const char* const message) noexcept
        {
            if (failure != nullptr)
            {
        ...
        [[nodiscard]] constexpr GpuSceneContributionToken EncodeTextureBatch(
            const TextureResidencyBatch batch) noexcept
        {
            return {static_cast<u64>(batch.serial) |
                        (static_cast<u64>(batch.installationCount) << 32u),
                    0x5458545245534944ull};
        }

        [[nodiscard]] constexpr TextureResidencyBatch DecodeTextureBatch(
            const GpuSceneContributionToken token) noexcept
        {
            return {static_cast<u32>(token.value0), static_cast<u32>(token.value0 >> 32u)};
        }

        [[nodiscard]] bool WriteTextureContribution(
            void* const owner, const GpuSceneContributionToken token,
            const containers::ArraySpan<const GpuSceneUploadReservation> reservations,
            const char*& failureMessage) noexcept
        {
            ...
            const bool result = runtime != nullptr && runtime->GetResidencyManager().WriteBatch(
                                                        DecodeTextureBatch(token), reservations, &failure);
            failureMessage = result ? nullptr : failure.message;
            return result;
        }

        void AcceptTextureContribution(void* const owner, const GpuSceneContributionToken token,
                                       const rhi::GpuFence sharedCompletion) noexcept
        {
            auto* const runtime = static_cast<TextureResidencyRuntime*>(owner);
            if (runtime != nullptr)
                runtime->GetResidencyManager().AcceptSubmittedBatch(DecodeTextureBatch(token),
                                                                   sharedCompletion);
        }

        [[nodiscard]] bool RetryTextureContribution(void* const owner,
                                                    const GpuSceneContributionToken token,
                                                    const char*& failureMessage) noexcept
        {
            auto* const runtime = static_cast<TextureResidencyRuntime*>(owner);
            TextureResidencyFailure failure;
            const bool result = runtime != nullptr && runtime->GetResidencyManager().RetryBatch(
                                                        DecodeTextureBatch(token), &failure);
            failureMessage = result ? nullptr : failure.message;
            return result;
        }
    } // namespace
    ...
    struct TextureResidencyRuntime::Impl
    {
        ...
        explicit Impl(const TextureResidencyRuntimeConfig& runtimeConfig) noexcept
            : residencies(memory::pools::Rendering::GetInstance()), recycledResidencies(memory::pools::Rendering::GetInstance()),
              residencyByPath(memory::pools::Rendering::GetInstance()), residencyByGpuIndex(memory::pools::Rendering::GetInstance()),
              demands(memory::pools::Rendering::GetInstance()), recycledDemands(memory::pools::Rendering::GetInstance()),
        ...
        {
            return index < residencies.Size() && residencies[index].active && residencies[index].generation == generation
                       ? &residencies[index]
                       : nullptr;
        }
    ...
    TextureDemandHandle::TextureDemandHandle(TextureResidencyRuntime& runtime, const TextureDemandId demand,
                                             const GpuTextureResidencyHandle residency) noexcept
    ...
    TextureDemandHandle::TextureDemandHandle(TextureDemandHandle&& other) noexcept
        : m_runtime(other.m_runtime), m_demand(other.m_demand), m_residency(other.m_residency)
    {
    ...
    {
        return m_runtime != nullptr && m_demand.IsValid() && m_residency.IsValid();
    }
    ...
    {
        VG_ASSERT_MSG(m_impl == nullptr && !m_residency.IsInitialized() && !m_uploader.IsInitialized(),
                      "texture residency runtime must be shut down before destruction");
    }

    bool TextureResidencyRuntime::Initialize(GpuSceneLifetime& lifetime, const rhi::DescriptorDomainRef resourceDescriptors,
                                             ...
                                             TextureResidencyRuntimeFailure* const failure) noexcept
    {
        ...
        if (m_impl != nullptr || m_residency.IsInitialized() || m_uploader.IsInitialized())
            return Fail(failure, TextureResidencyRuntimeFailureCode::AlreadyInitialized,
                        "texture residency runtime is already initialized");
        if (!concurrency::IsMainThread())
            return Fail(failure, TextureResidencyRuntimeFailureCode::WrongThread,
                        "texture residency runtime must initialize on the main thread");
        if (!lifetime.IsInitialized() || !resourceDescriptors.IsValid())
            return Fail(failure, TextureResidencyRuntimeFailureCode::InvalidDependency,
                        "texture residency runtime requires GPU Scene lifetime and the global resource descriptor domain");
        if (config.maximumResidencyRecords == 0 || config.maximumDemands == 0 ||
            config.maximumInstallationsPerTick == 0 ||
            config.maximumTableInstallationsPerFrame == 0 ||
            config.maximumStateChecksPerTick == 0 || config.maximumResidencyRecords > config.residency.maximumTextures)
            return Fail(failure, TextureResidencyRuntimeFailureCode::InvalidConfiguration,
                        "texture residency runtime configuration is invalid");
        ...
        if (m_impl == nullptr)
        {
            ...
            static_cast<void>(m_residency.Shutdown());
            return Fail(failure, TextureResidencyRuntimeFailureCode::CapacityExceeded,
                        "texture residency runtime storage allocation failed");
        }
        return true;
    }
    ...
    bool TextureResidencyRuntime::Shutdown(TextureResidencyRuntimeFailure* const failure) noexcept
    {
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, TextureResidencyRuntimeFailureCode::WrongThread,
                        "texture residency runtime must shut down on the main thread");
        if (m_impl != nullptr)
        {
            concurrency::ScopedLock<concurrency::SpinLock> guard(m_impl->lock);
            if (m_impl->demandCount != 0 || m_impl->residencyCount != 0)
                return Fail(failure, TextureResidencyRuntimeFailureCode::LiveResidencyRemains,
                            "texture residency runtime still owns live demands or residency records");
        }
        TextureUploadFailure uploadFailure;
    ...
    bool TextureResidencyRuntime::RequestTexture(const resources::ResourceHandle& resource, TextureDemandHandle& demand,
                                                 TextureResidencyRuntimeFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (!IsInitialized())
            return Fail(failure, TextureResidencyRuntimeFailureCode::NotInitialized,
                        "texture residency runtime is not initialized");
        if (!concurrency::IsMainThread())
            return Fail(failure, TextureResidencyRuntimeFailureCode::WrongThread,
                        "texture residency demand must be issued on the main thread");
        if (demand.IsValid() || !resource.IsValid() || resource.GetType() != textures::TextureResourceType)
            return Fail(failure, TextureResidencyRuntimeFailureCode::InvalidArgument,
                        "texture demand requires a live texture resource and empty output handle");
        ...
        if (texture == nullptr || !texture->IsOpen() || texture->GetMetadata().GetMipCount() == 0)
            return Fail(failure, TextureResidencyRuntimeFailureCode::InvalidArgument,
                        "texture demand references closed or empty texture metadata");

        {
            ...
            if (m_impl->residencyByPath.Find(resource.GetPath().Id(), candidate))
            {
                while (candidate != InvalidRuntimeRecordIndex)
                {
                    ...
                    if (slot.active && slot.resource.GetGeneration() == resource.GetGeneration() &&
                        slot.state != TextureRuntimeState::Failed && slot.state != TextureRuntimeState::Cancelling &&
                        slot.state != TextureRuntimeState::Retiring)
                    {
                        if (m_impl->demandCount >= m_impl->config.maximumDemands || slot.demandCount == 0xffffffffu)
                            return Fail(failure, TextureResidencyRuntimeFailureCode::CapacityExceeded,
                                        "texture demand capacity is exhausted");
            ...
            if (m_impl->residencyCount >= m_impl->config.maximumResidencyRecords ||
                m_impl->demandCount >= m_impl->config.maximumDemands)
                return Fail(failure, TextureResidencyRuntimeFailureCode::CapacityExceeded,
                            "texture residency or demand capacity is exhausted");
        }
        ...
        TextureUploadRequestId uploadRequest;
        TextureUploadFailure uploadFailure;
        if (!m_uploader.RequestMipTail(resource, m_residency, residency, uploadRequest,
                                       io::eAsyncPriority_Streaming, &uploadFailure))
        {
        ...
        slot.active = true;
        const bool pathIndexed = previousHead == InvalidRuntimeRecordIndex
    ...
    bool TextureResidencyRuntime::CancelDemand(TextureDemandHandle& demand,
                                               TextureResidencyRuntimeFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (demand.m_runtime != this || !demand.m_demand.IsValid())
            return Fail(failure, TextureResidencyRuntimeFailureCode::InvalidArgument,
                        "texture demand does not belong to this residency runtime");
        if (m_impl == nullptr)
            return Fail(failure, TextureResidencyRuntimeFailureCode::NotInitialized,
                        "texture residency runtime is not initialized");
        {
            ...
            if (m_impl->FindDemand(demand.m_demand) == nullptr)
                return Fail(failure, TextureResidencyRuntimeFailureCode::StaleDemand,
                            "texture demand handle is stale");
        }
        demand.Reset();
        return true;
    }
    ...
    bool TextureResidencyRuntime::Tick(const rhi::ResidencyFenceSet& prerequisiteFences,
                                       TextureResidencyRuntimeFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (!IsInitialized())
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, TextureResidencyRuntimeFailureCode::WrongThread,
                        "texture residency runtime progress must run on the main thread");
            ...
            for (const Impl::WorkRef work : m_impl->workScratch)
            {
                ...
                if (state == TextureRuntimeState::Retiring)
                {
                    ...
                    if (!m_residency.GetInfo(residency, lowerInfo, &queryFailure) &&
                        queryFailure.code == TextureResidencyFailureCode::StaleHandle)
                    {
                        concurrency::ScopedLock<concurrency::SpinLock> guard(m_impl->lock);
                        Impl::ResidencySlot* const slot = m_impl->FindRecord(work.index, work.generation);
                        if (slot != nullptr && slot->demandCount == 0)
                ...
                if (state == TextureRuntimeState::InstallationPending)
                {
                    ...
                    TextureResidencyFailure queryFailure;
                    if (!m_residency.GetInfo(residency, lowerInfo, &queryFailure))
                        return FailResidency(failure, queryFailure,
                                             "pending texture residency query failed");
                    if (lowerInfo.state == TextureResidencyState::BindlessReady)
                    {
                        concurrency::ScopedLock<concurrency::SpinLock> guard(m_impl->lock);
                        Impl::ResidencySlot* const slot =
                            m_impl->FindRecord(work.index, work.generation);
                        if (slot != nullptr)
                        {
                ...
                else if (uploadState == TextureUploadState::Invalid)
                {
                    ...
                    {
                        slot->uploadRequest = {};
                        slot->state = lowerInfo.state == TextureResidencyState::BindlessReady
                                          ? TextureRuntimeState::BindlessReady
                                          : TextureRuntimeState::InstallationPending;
        ...
        u32 installed = 0;
        if (!m_uploader.InstallReadyCandidates(m_residency, m_impl->config.maximumInstallationsPerTick,
                                                installed, &uploadFailure))
    ...
    bool TextureResidencyRuntime::StageGpuSceneContribution(
        GpuSceneRuntime& gpuScene, TextureResidencyRuntimeFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (!IsInitialized())
            return Fail(failure, TextureResidencyRuntimeFailureCode::NotInitialized,
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, TextureResidencyRuntimeFailureCode::WrongThread,
                        "texture GPU Scene contribution must stage on the main thread");
        if (!gpuScene.IsInitialized())
            return Fail(failure, TextureResidencyRuntimeFailureCode::InvalidDependency,
                        "texture GPU Scene contribution requires an initialized GPU Scene runtime");
        if (m_residency.GetStats().pendingInstallations == 0)
            return true;

        TextureResidencyBatch batch;
        TextureResidencyFailure residencyFailure;
        if (!m_residency.PrepareBatch(m_impl->config.maximumTableInstallationsPerFrame, batch,
                                      &residencyFailure))
            return FailResidency(failure, residencyFailure,
                                 "texture GPU Scene contribution preparation failed");
        if (batch.installationCount == 0)
        {
            m_residency.AcceptSubmittedBatch(batch, {});
            return true;
        }
        const containers::ArraySpan<GpuSceneUploadRequest> requests{
            m_impl->contributionRequests.TypedData(), batch.installationCount};
        if (!m_residency.BuildUploadRequests(batch, requests, &residencyFailure))
        {
            static_cast<void>(m_residency.RetryBatch(batch));
            return FailResidency(failure, residencyFailure,
                                 "texture GPU Scene upload request planning failed");
        }
        ...
        {
            static_cast<void>(m_residency.RetryBatch(batch));
            return FailGpuScene(failure, gpuSceneFailure,
                                "texture GPU Scene contribution staging failed");
        }
        return true;
    }

    bool TextureResidencyRuntime::SealRetirements(const rhi::ResidencyFenceSet& safeAfter,
                                                  TextureResidencyRuntimeFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (!IsInitialized())
            return Fail(failure, TextureResidencyRuntimeFailureCode::NotInitialized,
                        "texture residency runtime is not initialized");
        TextureResidencyFailure residencyFailure;
        return m_residency.SealRetirements(safeAfter, &residencyFailure)
                   ? true
                   : FailResidency(failure, residencyFailure, "texture residency retirement sealing failed");
    }
    ...
    u32 TextureResidencyRuntime::CollectRetirements(TextureResidencyRuntimeFailure* const failure) noexcept
    {
        ...
        if (!IsInitialized())
        {
            static_cast<void>(Fail(failure, TextureResidencyRuntimeFailureCode::NotInitialized,
                                   "texture residency runtime is not initialized"));
            return 0;
        }
        TextureResidencyFailure residencyFailure;
        ...
        if (residencyFailure.code != TextureResidencyFailureCode::None)
        {
            static_cast<void>(FailResidency(failure, residencyFailure,
                                            "texture residency retirement collection failed"));
            return 0;
        }
        return collected;
    }

    bool TextureResidencyRuntime::GetInfo(const GpuTextureResidencyHandle residency, TextureRuntimeInfo& info,
                                          TextureResidencyRuntimeFailure* const failure) const noexcept
    {
        ClearFailure(failure);
        info = {};
        if (m_impl == nullptr)
        ...
        if (!residency.IsValid() || !m_impl->residencyByGpuIndex.Find(residency.index, recordIndex) ||
            recordIndex >= m_impl->residencies.Size())
            return Fail(failure, TextureResidencyRuntimeFailureCode::InvalidArgument,
                        "texture runtime query references an unknown residency identity");
        const Impl::ResidencySlot& slot = m_impl->residencies[recordIndex];
        if (!slot.active || slot.residency != residency)
            return Fail(failure, TextureResidencyRuntimeFailureCode::InvalidArgument,
                        "texture runtime query references a stale residency identity");
