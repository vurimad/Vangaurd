...
namespace vanguard::rendering
{
    namespace
    {
        ...
        [[nodiscard]] bool Fail(ViewportFailure* const failure, const ViewportFailureCode code,
                                const char* const message, const RenderViewportHandle renderViewport = {},
                                const EngineViewportHandle engineViewport = {},
                                const rhi::Failure* const rhiFailure = nullptr) noexcept
        {
            if (failure != nullptr)
            {
                ...
                if (rhiFailure != nullptr) failure->rhiFailure = *rhiFailure;
            }
            return false;
        }
        ...
        {
        public:
            explicit RetainedFrame(const RenderFrameInfo& source) noexcept : frame(source)
            {
                if (frame.Payload().data != nullptr)
                {
                    frame.Payload().retain(frame.Payload().data);
                    retained = true;
                }
            }

            RetainedFrame(RetainedFrame&& other) noexcept : frame(other.frame), retained(other.retained)
            {
            ...
            {
                if (!retained) return;
                frame.Payload().release(frame.Payload().data);
                retained = false;
            }
    ...
    struct RenderFrameDispatcher::Impl
    {
        VANGUARD_USE_MEMORY_POOL(memory::pools::Rendering);

        ExecuteRenderFrame execute = nullptr;
        void* userData = nullptr;
        jobs::JobName renderFrameJobName{"RenderFrame"};
        jobs::Counter flushCounter;
    ...
    bool RenderFrameDispatcher::Initialize(const ExecuteRenderFrame execute, void* const userData,
                                           ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (m_impl != nullptr)
            return Fail(failure, ViewportFailureCode::AlreadyInitialized, "render frame dispatcher is already initialized");
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread, "render frame dispatcher must initialize on the main thread");
        if (execute == nullptr || !jobs::IsInitialized())
            return Fail(failure, ViewportFailureCode::InvalidDescriptor,
                        "render frame dispatcher requires an execution callback and initialized Jobs");

        m_impl = VANGUARD_NEW(Impl);
        if (m_impl == nullptr)
            return Fail(failure, ViewportFailureCode::CapacityExceeded, "render frame dispatcher allocation failed");
        m_impl->execute = execute;
        m_impl->userData = userData;
        m_impl->initialized = true;
        return true;
    }

    bool RenderFrameDispatcher::Shutdown(ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (m_impl == nullptr) return true;
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread, "render frame dispatcher must shut down on the main thread");
        if (!Flush(failure)) return false;
    ...
    bool RenderFrameDispatcher::IsInitialized() const noexcept
    {
        return m_impl != nullptr && m_impl->initialized;
    }

    bool RenderFrameDispatcher::Submit(const RenderFrameInfo& frame, RenderFrameSubmission& submission,
                                       ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        submission = {};
        if (!IsInitialized())
            return Fail(failure, ViewportFailureCode::NotInitialized, "render frame dispatcher is not initialized");
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread, "render frames must be submitted from the main thread",
                        frame.OutputViewport(), frame.EngineViewport());
        if (frame.Serial() == 0 || !frame.EngineViewport().IsValid() || !frame.OutputViewport().IsValid() ||
            !frame.RenderExtent().IsValid() || !frame.Payload().IsValid())
            return Fail(failure, ViewportFailureCode::InvalidDescriptor, "render frame packet is invalid",
                        frame.OutputViewport(), frame.EngineViewport());

        RetainedFrame retained(frame);
        jobs::Task task = jobs::Task::Create(
            [impl = m_impl, retained = static_cast<RetainedFrame&&>(retained)](const jobs::JobContext& context) mutable noexcept
            {
                const RenderFrameExecutionStatus status = impl->execute(retained.frame, context, impl->userData);
                if (!status) static_cast<void>(impl->failedFrames.Increment());
                static_cast<void>(impl->completedFrames.Increment());
                impl->lastCompletedSerial.SetValue(retained.frame.Serial());
                retained.Release();
            });
        if (!task)
            return Fail(failure, ViewportFailureCode::SubmissionFailure, "render frame job allocation failed",
                        frame.OutputViewport(), frame.EngineViewport());

        jobs::Builder builder({jobs::Priority::RenderPath, jobs::Affinity::AnyWorker}, this);
        if (!builder.IsValid())
            return Fail(failure, ViewportFailureCode::SubmissionFailure, "render frame job builder creation failed",
                        frame.OutputViewport(), frame.EngineViewport());
        if (m_impl->flushCounter.IsValid()) builder.AddDependency(m_impl->flushCounter);
        if (!builder.Dispatch(m_impl->renderFrameJobName, static_cast<jobs::Task&&>(task), jobs::Fence::Full))
            return Fail(failure, ViewportFailureCode::SubmissionFailure, "render frame job dispatch failed",
                        frame.OutputViewport(), frame.EngineViewport());

        m_impl->flushCounter = builder.ExtractCounter();
        if (!m_impl->flushCounter.IsValid())
            return Fail(failure, ViewportFailureCode::SubmissionFailure, "render frame completion counter extraction failed",
                        frame.OutputViewport(), frame.EngineViewport());
        static_cast<void>(m_impl->submittedFrames.Increment());
        submission.serial = frame.Serial();
        return true;
    }

    bool RenderFrameDispatcher::Flush(ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (!IsInitialized())
            return Fail(failure, ViewportFailureCode::NotInitialized, "render frame dispatcher is not initialized");
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread, "render frame dispatcher flush must run on the main thread");
        if (m_impl->flushCounter.IsValid() && !m_impl->flushCounter.Wait())
            return Fail(failure, ViewportFailureCode::SubmissionFailure, "render frame completion wait failed");
        m_impl->flushCounter = {};
        return true;
    }

    bool RenderFrameDispatcher::IsIdle() const noexcept
    {
        return IsInitialized() && (!m_impl->flushCounter.IsValid() || m_impl->flushCounter.IsReady());
    }

    RenderFrameDispatcherStats RenderFrameDispatcher::GetStats() const noexcept
    {
        RenderFrameDispatcherStats stats;
        if (!IsInitialized()) return stats;
        stats.submittedFrames = m_impl->submittedFrames.GetValue();
        stats.completedFrames = m_impl->completedFrames.GetValue();
        stats.failedFrames = m_impl->failedFrames.GetValue();
        stats.lastCompletedSerial = m_impl->lastCompletedSerial.GetValue();
        stats.initialized = true;
        stats.workOutstanding = m_impl->flushCounter.IsValid() && !m_impl->flushCounter.IsReady();
        return stats;
    }
    ...
    struct ViewportManager::Impl
    {
        ...
        RenderFrameDispatcher* dispatcher = nullptr;
        ...
        void Reject() noexcept { ++stats.rejectedOperations; }
    };
    ...
    bool ViewportManager::Initialize(RenderFrameDispatcher& dispatcher, ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (m_impl != nullptr)
        ...
        if (!dispatcher.IsInitialized())
            return Fail(failure, ViewportFailureCode::InvalidDescriptor,
                        "viewport manager requires an initialized render frame dispatcher");

        m_impl = VANGUARD_NEW(Impl);
        if (m_impl == nullptr)
            return Fail(failure, ViewportFailureCode::CapacityExceeded, "viewport manager allocation failed");
        m_impl->dispatcher = &dispatcher;
        m_impl->initialized = true;
        return true;
    }
    ...
    bool ViewportManager::Shutdown(ViewportFailure* const failure) noexcept
    {
        ...
        if (m_impl->dispatcher == nullptr || !m_impl->dispatcher->IsInitialized())
            return Fail(failure, ViewportFailureCode::InvalidState,
                        "render frame dispatcher was shut down before the viewport manager");
        if (!m_impl->dispatcher->Flush(failure)) return false;
        ...
        for (u32 index = 0; index < MaximumRenderViewports; ++index)
        {
            ...
            if (slot.snapshot.swapChain.IsValid()) static_cast<void>(rhi::SafeRelease(slot.snapshot.swapChain));
            if (slot.snapshot.outputTexture.IsValid()) static_cast<void>(rhi::SafeRelease(slot.snapshot.outputTexture));
            slot.active = false;
        }
        m_impl->initialized = false;
        VANGUARD_DELETE(m_impl);
    ...
    bool ViewportManager::CreateRenderViewport(const RenderViewportDesc& desc, RenderViewportHandle& viewport,
                                               ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        viewport = {};
        ...
        const bool descriptorValid = CopyName(validatedName, desc.name) && ValidOutputKind(desc.outputKind) &&
                                     desc.renderExtent.IsValid() && desc.outputExtent.IsValid() &&
            ...
            (!rhi::IsInitialized() || !rhi::IsResourceReferenceValid(rhi::ResourceRef(desc.outputTexture))))
        {
            m_impl->Reject();
            return Fail(failure, ViewportFailureCode::InvalidDescriptor,
                        "texture output must be a live RHI texture reference");
        }
        ...
        for (u32 index = 0; index < MaximumRenderViewports; ++index)
        {
            ...
            slot.snapshot.state = desc.outputKind == RenderViewportOutputKind::Presentation
                                      ? RenderViewportState::AwaitingOutput
                                      : RenderViewportState::Ready;
            ...
            if (desc.outputTexture.IsValid()) rhi::AddRef(desc.outputTexture);
            slot.active = true;
    ...
    bool ViewportManager::DestroyRenderViewport(const RenderViewportHandle viewport,
                                                ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (!IsInitialized())
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "render viewport destruction must run on the main thread", viewport);
        ...
        {
            m_impl->Reject();
            return Fail(failure, ViewportFailureCode::OutputStillReferenced,
                        "engine viewports must be destroyed before their render output", viewport);
        }
        if (!m_impl->dispatcher->IsIdle())
        {
            m_impl->Reject();
            return Fail(failure, ViewportFailureCode::Busy,
                        "render frame work must be flushed before destroying a render viewport", viewport);
        }
            ...
            rhi::GetSwapChainStats(slot->snapshot.swapChain).state == rhi::SwapChainState::Acquired)
        {
            m_impl->Reject();
            return Fail(failure, ViewportFailureCode::Busy,
                        "the acquired presentation output must be presented or abandoned before destroying its viewport",
                        viewport);
        }
        if (slot->snapshot.swapChain.IsValid()) static_cast<void>(rhi::SafeRelease(slot->snapshot.swapChain));
        if (slot->snapshot.outputTexture.IsValid()) static_cast<void>(rhi::SafeRelease(slot->snapshot.outputTexture));
    ...
    bool ViewportManager::BindSwapChain(const RenderViewportHandle viewport, const rhi::SwapChainRef swapChain,
                                        ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (!IsInitialized())
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "swapchain binding must run on the main thread", viewport);
        Impl::RenderSlot* const slot = m_impl->Find(viewport);
        if (slot == nullptr)
            return Fail(failure, ViewportFailureCode::InvalidHandle, "invalid render viewport handle", viewport);
        if (slot->snapshot.outputKind != RenderViewportOutputKind::Presentation || slot->snapshot.swapChain.IsValid() ||
            !swapChain.IsValid() || !rhi::IsInitialized() || !rhi::IsResourceReferenceValid(rhi::ResourceRef(swapChain)))
        ...
        if (slot->snapshot.requiredPixelExtentRevision == 0 || slot->snapshot.requiredSurfaceRevision == 0 ||
            !slot->snapshot.requestedOutputExtent.IsValid() || swapChainStats.state == rhi::SwapChainState::Failed ||
            swapChainStats.width == 0 || swapChainStats.height == 0)
            return Fail(failure, ViewportFailureCode::InvalidState,
                        "swapchain binding requires current presentation state and a valid back buffer", viewport);
    ...
    bool ViewportManager::UnbindSwapChain(const RenderViewportHandle viewport,
                                          ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (!IsInitialized())
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "swapchain unbinding must run on the main thread", viewport);
        Impl::RenderSlot* const slot = m_impl->Find(viewport);
        if (slot == nullptr)
            return Fail(failure, ViewportFailureCode::InvalidHandle, "invalid render viewport handle", viewport);
        if (slot->snapshot.outputKind != RenderViewportOutputKind::Presentation || !slot->snapshot.swapChain.IsValid())
        ...
        if (!m_impl->dispatcher->IsIdle())
            return Fail(failure, ViewportFailureCode::Busy,
                        "render frame work must be flushed before unbinding a swapchain", viewport);
        if (rhi::GetSwapChainStats(slot->snapshot.swapChain).state == rhi::SwapChainState::Acquired)
            return Fail(failure, ViewportFailureCode::Busy,
                        "the acquired presentation output must be presented or abandoned before unbinding its swapchain",
                        viewport);
    ...
    bool ViewportManager::UpdatePresentation(const RenderViewportHandle viewport,
                                             ...
                                             ViewportFailure* const failure) noexcept
    {
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "presentation updates must run on the main thread", viewport);
        Impl::RenderSlot* const slot = m_impl->Find(viewport);
        if (slot == nullptr)
            return Fail(failure, ViewportFailureCode::InvalidHandle, "invalid render viewport handle", viewport);
        if (slot->snapshot.outputKind != RenderViewportOutputKind::Presentation ||
            presentation.handle != slot->snapshot.presentation ||
            presentation.surfaceKind == window::PresentationSurfaceKind::None || !presentation.pixelExtent.IsValid())
            return Fail(failure, ViewportFailureCode::InvalidDescriptor,
                        "presentation snapshot does not belong to this render viewport", viewport);
        if (presentation.requiredPixelExtentRevision < slot->snapshot.requiredPixelExtentRevision ||
            ...
            return Fail(failure, ViewportFailureCode::InvalidDescriptor,
                        "presentation snapshot revisions are stale or inconsistent", viewport);

        const bool suspended = !presentation.visible || presentation.minimized ||
                               window::HasRequirement(presentation.requirements,
                                                      window::PresentationRequirement::Suspended);
        ...
        const bool surfaceChanged =
            presentation.requiredSurfaceRevision > slot->snapshot.appliedSurfaceRevision;
        const bool extentChanged =
            presentation.requiredPixelExtentRevision > slot->snapshot.appliedPixelExtentRevision;
        ...
        if (slot->snapshot.swapChain.IsValid() && (surfaceChanged || (extentChanged && !suspended)) &&
            !m_impl->dispatcher->IsIdle())
            return Fail(failure, ViewportFailureCode::Busy,
                        surfaceChanged
                            ? "render frame work must be flushed before replacing a swapchain surface"
        ...
        if (slot->snapshot.swapChain.IsValid() && (surfaceChanged || extentChanged) &&
            rhi::GetSwapChainStats(slot->snapshot.swapChain).state == rhi::SwapChainState::Acquired)
            return Fail(failure, ViewportFailureCode::Busy,
                        "the acquired presentation output must be presented or abandoned before changing its surface",
                        viewport);
        ...
        if (slot->snapshot.swapChain.IsValid() && extentChanged && !suspended)
        {
            ...
            if (!rhi::ResizeBackbuffer(requestedExtent.width, requestedExtent.height, slot->snapshot.swapChain, &rhiFailure))
            {
                ...
                return Fail(failure, ViewportFailureCode::BackendFailure, "swapchain resize failed", viewport, {},
                            &rhiFailure);
            }
        ...
        else if (slot->snapshot.swapChain.IsValid() &&
                 slot->snapshot.appliedPixelExtentRevision == slot->snapshot.requiredPixelExtentRevision &&
                 slot->snapshot.appliedSurfaceRevision == slot->snapshot.requiredSurfaceRevision)
        ...
        if (previousState != slot->snapshot.state || previousRequestedExtent != slot->snapshot.requestedOutputExtent ||
            previousOutputExtent != slot->snapshot.outputExtent ||
            previousRequiredPixelRevision != slot->snapshot.requiredPixelExtentRevision ||
            previousRequiredSurfaceRevision != slot->snapshot.requiredSurfaceRevision ||
            previousVisible != slot->snapshot.visible || previousOccluded != slot->snapshot.occluded ||
            (hadSwapChain && !slot->snapshot.swapChain.IsValid()))
            ++slot->snapshot.outputRevision;
        return true;
    }

    bool ViewportManager::GetPresentationAcknowledgement(
        const RenderViewportHandle viewport, window::PresentationAcknowledgement& acknowledgement) const noexcept
    {
        acknowledgement = {};
        if (!IsInitialized()) return false;
        const Impl::RenderSlot* const slot = m_impl->Find(viewport);
        if (slot == nullptr || slot->snapshot.outputKind != RenderViewportOutputKind::Presentation) return false;
    ...
    bool ViewportManager::RequestRenderExtent(const RenderViewportHandle viewport, const ViewportExtent extent,
                                              ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (!IsInitialized())
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "render extent requests must run on the main thread", viewport);
        Impl::RenderSlot* const slot = m_impl->Find(viewport);
        if (slot == nullptr)
            return Fail(failure, ViewportFailureCode::InvalidHandle, "invalid render viewport handle", viewport);
        if (!extent.IsValid())
    ...
    bool ViewportManager::AcquireOutput(const RenderViewportHandle viewport, RenderOutputAcquisition& acquisition,
                                        ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        acquisition = {};
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "render-output acquisition must run on the main thread", viewport);
        Impl::RenderSlot* const slot = m_impl->Find(viewport);
        if (slot == nullptr)
            return Fail(failure, ViewportFailureCode::InvalidHandle, "invalid render viewport handle", viewport);
        if (slot->snapshot.state != RenderViewportState::Ready)
            return Fail(failure, ViewportFailureCode::OutputUnavailable,
                        "render viewport output is not ready for acquisition", viewport);
        ...
        if (slot->snapshot.outputKind != RenderViewportOutputKind::Presentation ||
            !slot->snapshot.swapChain.IsValid() || slot->snapshot.occluded)
            return Fail(failure, ViewportFailureCode::OutputUnavailable,
                        "presentation output is unavailable", viewport);

        rhi::Failure rhiFailure;
        rhi::AcquiredBackBuffer backBuffer;
        if (!rhi::AcquireBackBuffer(slot->snapshot.swapChain, backBuffer, &rhiFailure))
            return Fail(failure, rhiFailure.code == rhi::FailureCode::Busy ? ViewportFailureCode::Busy
                                                                          : ViewportFailureCode::BackendFailure,
    ...
    bool ViewportManager::AbandonOutput(RenderOutputAcquisition& acquisition,
                                        ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (!IsInitialized())
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "render-output abandonment must run on the main thread", acquisition.viewport);
        ...
        if (slot == nullptr || !acquisition.IsValid() || acquisition.outputRevision != slot->snapshot.outputRevision)
            return Fail(failure, ViewportFailureCode::InvalidHandle,
                        "stale or foreign render-output acquisition", acquisition.viewport);
        if (acquisition.backBuffer.IsValid())
        {
            rhi::Failure rhiFailure;
            if (!rhi::AbandonBackBuffer(acquisition.backBuffer, &rhiFailure))
                return Fail(failure, ViewportFailureCode::BackendFailure,
                            "swapchain back-buffer abandonment failed", acquisition.viewport, {}, &rhiFailure);
        }
        acquisition = {};
        return true;
    }
    ...
    bool ViewportManager::Present(RenderOutputAcquisition& acquisition, ViewportFailure* const failure) noexcept
    {
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "presentation must run on the main thread", acquisition.viewport);
        Impl::RenderSlot* const slot = m_impl->Find(acquisition.viewport);
        if (slot == nullptr || !acquisition.IsValid() || acquisition.outputRevision != slot->snapshot.outputRevision ||
            slot->snapshot.outputKind != RenderViewportOutputKind::Presentation ||
            slot->snapshot.state != RenderViewportState::Ready || !acquisition.backBuffer.IsValid())
            return Fail(failure, ViewportFailureCode::InvalidHandle,
                        "stale or foreign presentation acquisition", acquisition.viewport);
        rhi::Failure rhiFailure;
        if (!rhi::Present(acquisition.backBuffer, &rhiFailure))
        {
            slot->snapshot.state = RenderViewportState::Failed;
            return Fail(failure, ViewportFailureCode::BackendFailure, "swapchain presentation failed",
                        acquisition.viewport, {}, &rhiFailure);
        }
    ...
    bool ViewportManager::CreateEngineViewport(const EngineViewportDesc& desc, EngineViewportHandle& viewport,
                                               ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        viewport = {};
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "engine viewport creation must run on the main thread");
        char validatedName[MaximumViewportNameBytes]{};
    ...
    bool ViewportManager::DestroyEngineViewport(const EngineViewportHandle viewport,
                                                ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (!IsInitialized())
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "engine viewport destruction must run on the main thread", {}, viewport);
        ...
        if (slot->snapshot.buildingFrameSerial != 0)
            return Fail(failure, ViewportFailureCode::FrameAlreadyBuilding,
                        "building frame must be submitted or abandoned before destroying its engine viewport",
                        slot->snapshot.output, viewport);
        if (!m_impl->dispatcher->IsIdle())
            return Fail(failure, ViewportFailureCode::Busy,
                        "render frame work must be flushed before destroying an engine viewport",
                        slot->snapshot.output, viewport);
    ...
    bool ViewportManager::BeginFrame(const EngineViewportHandle viewport, const RenderFrameSetup& setup,
                                     RenderFrameInfo& frame, ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (!IsInitialized())
            return Fail(failure, ViewportFailureCode::NotInitialized, "viewport manager is not initialized", {}, viewport);
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "render frame construction must begin on the main thread", {}, viewport);
        Impl::EngineSlot* const engine = m_impl->Find(viewport);
        if (engine == nullptr)
            return Fail(failure, ViewportFailureCode::InvalidHandle, "invalid engine viewport handle", {}, viewport);
        Impl::RenderSlot* const output = m_impl->Find(engine->snapshot.output);
        if (output == nullptr || output->snapshot.state == RenderViewportState::AwaitingOutput ||
            output->snapshot.state == RenderViewportState::Failed)
            return Fail(failure, ViewportFailureCode::OutputUnavailable,
                        "engine viewport output is unavailable", engine->snapshot.output, viewport);
        if (engine->snapshot.buildingFrameSerial != 0)
            return Fail(failure, ViewportFailureCode::FrameAlreadyBuilding,
                        "engine viewport already has a building frame", engine->snapshot.output, viewport);
        if (!ValidFrameSetup(setup))
            return Fail(failure, ViewportFailureCode::InvalidDescriptor, "invalid render frame setup",
                        engine->snapshot.output, viewport);

        const u64 serial = m_impl->nextFrameSerial++;
        if (m_impl->nextFrameSerial == 0) m_impl->nextFrameSerial = 1;
        frame = {};
        ...
        frame.m_present = setup.present && engine->snapshot.presentByDefault &&
                          output->snapshot.outputKind == RenderViewportOutputKind::Presentation &&
    ...
    bool ViewportManager::SubmitFrame(const EngineViewportHandle viewport, RenderFrameInfo& frame,
                                      RenderFrameSubmission& submission, ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        submission = {};
        if (!IsInitialized())
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "render frame submission must run on the main thread", {}, viewport);
        Impl::EngineSlot* const engine = m_impl->Find(viewport);
        if (engine == nullptr)
            return Fail(failure, ViewportFailureCode::InvalidHandle, "invalid engine viewport handle", {}, viewport);
        if (frame.m_engineViewport.IsValid() && frame.m_engineViewport != viewport)
            return Fail(failure, ViewportFailureCode::ForeignFrame,
                        "render frame does not belong to this engine viewport", engine->snapshot.output, viewport);
        if (engine->snapshot.buildingFrameSerial == 0)
            return Fail(failure, ViewportFailureCode::FrameNotBuilding,
                        "engine viewport has no building frame", engine->snapshot.output, viewport);
        if (frame.m_engineViewport != viewport || frame.m_renderViewport != engine->snapshot.output ||
            frame.m_serial != engine->snapshot.buildingFrameSerial)
            return Fail(failure, ViewportFailureCode::ForeignFrame,
                        "render frame does not belong to this engine viewport", engine->snapshot.output, viewport);
        if (!frame.m_payload.IsValid())
            return Fail(failure, ViewportFailureCode::InvalidDescriptor, "render frame payload is invalid",
                        engine->snapshot.output, viewport);
        if (!m_impl->dispatcher->Submit(frame, submission, failure)) return false;

        Impl::RenderSlot* const output = m_impl->Find(engine->snapshot.output);
        if (output != nullptr) ++output->snapshot.renderedFrames;
    ...
    bool ViewportManager::AbandonFrame(const EngineViewportHandle viewport, RenderFrameInfo& frame,
                                       ViewportFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (!IsInitialized())
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "render frame abandonment must run on the main thread", {}, viewport);
        Impl::EngineSlot* const engine = m_impl->Find(viewport);
        if (engine == nullptr)
            return Fail(failure, ViewportFailureCode::InvalidHandle, "invalid engine viewport handle", {}, viewport);
        if (engine->snapshot.buildingFrameSerial == 0)
    ...
    bool ViewportManager::FlushFrame(const EngineViewportHandle viewport, ViewportFailure* const failure) noexcept
    {
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, ViewportFailureCode::WrongThread,
                        "render frame flush must run on the main thread", {}, viewport);
        const Impl::EngineSlot* const engine = m_impl->Find(viewport);
        if (engine == nullptr)
            return Fail(failure, ViewportFailureCode::InvalidHandle, "invalid engine viewport handle", {}, viewport);
        return m_impl->dispatcher->Flush(failure);
    }

    bool ViewportManager::Snapshot(const RenderViewportHandle viewport, RenderViewportSnapshot& snapshot) const noexcept
    {
        snapshot = {};
        if (!IsInitialized()) return false;
        const Impl::RenderSlot* const slot = m_impl->Find(viewport);
    ...
    bool ViewportManager::Snapshot(const EngineViewportHandle viewport, EngineViewportSnapshot& snapshot) const noexcept
    {
        snapshot = {};
        if (!IsInitialized()) return false;
    ...
    {
        viewport = {};
        if (!IsInitialized() || m_impl->Find(handle) == nullptr) return false;
    ...
    {
        if (!IsInitialized() || visitor == nullptr) return;
        for (u32 index = 0; index < MaximumRenderViewports; ++index)
            if (m_impl->renderSlots[index].active) visitor(m_impl->renderSlots[index].snapshot, userData);
    }
    ...
    void ViewportManager::VisitEngineViewports(const VisitEngineViewport visitor, void* const userData) const noexcept
    {
        if (!IsInitialized() || visitor == nullptr) return;
        for (u32 index = 0; index < MaximumEngineViewports; ++index)
            if (m_impl->engineSlots[index].active) visitor(m_impl->engineSlots[index].snapshot, userData);
    }
    ...
    bool RenderViewport::Snapshot(RenderViewportSnapshot& snapshot) const noexcept
    {
        snapshot = {};
        return m_manager != nullptr && m_manager->Snapshot(m_handle, snapshot);
    }

    bool RenderViewport::AcquireOutput(RenderOutputAcquisition& acquisition, ViewportFailure* const failure) noexcept
    {
        return m_manager != nullptr
                   ? m_manager->AcquireOutput(m_handle, acquisition, failure)
    ...
    {
        return m_manager != nullptr
                   ? m_manager->RequestRenderExtent(m_handle, extent, failure)
                   : Fail(failure, ViewportFailureCode::InvalidHandle, "render viewport facade is invalid", m_handle);
    }
    ...
    {
        return m_manager != nullptr
                   ? m_manager->BindSwapChain(m_handle, swapChain, failure)
                   : Fail(failure, ViewportFailureCode::InvalidHandle, "render viewport facade is invalid", m_handle);
    }
    ...
    {
        return m_manager != nullptr
                   ? m_manager->UnbindSwapChain(m_handle, failure)
                   : Fail(failure, ViewportFailureCode::InvalidHandle, "render viewport facade is invalid", m_handle);
    }

    bool RenderViewport::UpdatePresentation(const window::PresentationAttachmentSnapshot& presentation,
                                            ViewportFailure* const failure) noexcept
    {
        return m_manager != nullptr
                   ? m_manager->UpdatePresentation(m_handle, presentation, failure)
                   : Fail(failure, ViewportFailureCode::InvalidHandle, "render viewport facade is invalid", m_handle);
    }

    bool RenderViewport::GetPresentationAcknowledgement(
    ...
    bool RenderViewport::AbandonOutput(RenderOutputAcquisition& acquisition, ViewportFailure* const failure) noexcept
    {
        if (acquisition.viewport != m_handle)
            return Fail(failure, ViewportFailureCode::InvalidHandle,
                        "render-output acquisition belongs to another viewport", m_handle);
        return m_manager != nullptr
                   ? m_manager->AbandonOutput(acquisition, failure)
                   : Fail(failure, ViewportFailureCode::InvalidHandle, "render viewport facade is invalid", m_handle);
    }
    ...
    bool RenderViewport::Present(RenderOutputAcquisition& acquisition, ViewportFailure* const failure) noexcept
    {
        if (acquisition.viewport != m_handle)
            return Fail(failure, ViewportFailureCode::InvalidHandle,
                        "presentation acquisition belongs to another viewport", m_handle);
        return m_manager != nullptr
                   ? m_manager->Present(acquisition, failure)
                   : Fail(failure, ViewportFailureCode::InvalidHandle, "render viewport facade is invalid", m_handle);
    }
    ...
    bool EngineViewport::Snapshot(EngineViewportSnapshot& snapshot) const noexcept
    {
        snapshot = {};
        return m_manager != nullptr && m_manager->Snapshot(m_handle, snapshot);
    }

    bool EngineViewport::BeginFrame(const RenderFrameSetup& setup, RenderFrameInfo& frame,
                                    ViewportFailure* const failure) noexcept
    {
        return m_manager != nullptr
                   ? m_manager->BeginFrame(m_handle, setup, frame, failure)
                   : Fail(failure, ViewportFailureCode::InvalidHandle, "engine viewport facade is invalid", {}, m_handle);
    }

    bool EngineViewport::SubmitFrame(RenderFrameInfo& frame, RenderFrameSubmission& submission,
                                     ViewportFailure* const failure) noexcept
    {
        return m_manager != nullptr
                   ? m_manager->SubmitFrame(m_handle, frame, submission, failure)
                   : Fail(failure, ViewportFailureCode::InvalidHandle, "engine viewport facade is invalid", {}, m_handle);
    }
    ...
    {
        return m_manager != nullptr
                   ? m_manager->AbandonFrame(m_handle, frame, failure)
                   : Fail(failure, ViewportFailureCode::InvalidHandle, "engine viewport facade is invalid", {}, m_handle);
    }
    ...
    {
        return m_manager != nullptr
                   ? m_manager->FlushFrame(m_handle, failure)
                   : Fail(failure, ViewportFailureCode::InvalidHandle, "engine viewport facade is invalid", {}, m_handle);
    }
} // namespace vanguard::rendering
