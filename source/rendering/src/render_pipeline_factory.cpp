...
namespace vanguard::rendering
{
    namespace
    {
        [[nodiscard]] const RenderShader* ResolveShader(const RenderPipelineRequest& request,
                                                        const pipelines::ShaderReference& reference) noexcept
        {
        ...
        {
            return shader.IsLoaded() && shader.Permutation() == reference.permutation &&
                   shader.BindingLayoutFingerprint() == reference.bindingLayout &&
                   shader.PipelineInterfaceFingerprint() == reference.pipelineInterface;
        }
        ...
        {
            for (const pipelines::ShaderReference& reference : request.pipeline->Shaders())
            {
                ...
                if (!assign(output.vertexShader, shader->Stage(shaders::ShaderStage::Vertex)) ||
                    !assign(output.hullShader, shader->Stage(shaders::ShaderStage::Hull)) ||
                    !assign(output.domainShader, shader->Stage(shaders::ShaderStage::Domain)) ||
                    !assign(output.geometryShader, shader->Stage(shaders::ShaderStage::Geometry)) ||
                    !assign(output.pixelShader, shader->Stage(shaders::ShaderStage::Fragment)) ||
                    !assign(compute.computeShader, shader->Stage(shaders::ShaderStage::Compute)))
                    return RenderPipelineResult::DuplicateStage;
                if (shader->Stage(shaders::ShaderStage::Task) || shader->Stage(shaders::ShaderStage::Mesh) ||
                    shader->Stage(shaders::ShaderStage::Library))
        ...
        [[nodiscard]] bool ConvertBlendFactor(const pipelines::BlendFactor source, rhi::BlendFactor& destination) noexcept
        {
            switch (source)
            {
            case pipelines::BlendFactor::Zero: destination = rhi::BlendFactor::Zero; return true;
            case pipelines::BlendFactor::One: destination = rhi::BlendFactor::One; return true;
            case pipelines::BlendFactor::SourceColor: destination = rhi::BlendFactor::SourceColor; return true;
            case pipelines::BlendFactor::OneMinusSourceColor: destination = rhi::BlendFactor::OneMinusSourceColor; return true;
            case pipelines::BlendFactor::DestinationColor: destination = rhi::BlendFactor::DestinationColor; return true;
            case pipelines::BlendFactor::OneMinusDestinationColor: destination = rhi::BlendFactor::OneMinusDestinationColor; return true;
            case pipelines::BlendFactor::SourceAlpha: destination = rhi::BlendFactor::SourceAlpha; return true;
            case pipelines::BlendFactor::OneMinusSourceAlpha: destination = rhi::BlendFactor::OneMinusSourceAlpha; return true;
            case pipelines::BlendFactor::DestinationAlpha: destination = rhi::BlendFactor::DestinationAlpha; return true;
            case pipelines::BlendFactor::OneMinusDestinationAlpha: destination = rhi::BlendFactor::OneMinusDestinationAlpha; return true;
            case pipelines::BlendFactor::ConstantColor: destination = rhi::BlendFactor::ConstantColor; return true;
            case pipelines::BlendFactor::OneMinusConstantColor: destination = rhi::BlendFactor::OneMinusConstantColor; return true;
            case pipelines::BlendFactor::SourceAlphaSaturate: destination = rhi::BlendFactor::SourceAlphaSaturate; return true;
            case pipelines::BlendFactor::SourceOneColor: destination = rhi::BlendFactor::SourceOneColor; return true;
            case pipelines::BlendFactor::OneMinusSourceOneColor: destination = rhi::BlendFactor::OneMinusSourceOneColor; return true;
            case pipelines::BlendFactor::SourceOneAlpha: destination = rhi::BlendFactor::SourceOneAlpha; return true;
            case pipelines::BlendFactor::OneMinusSourceOneAlpha: destination = rhi::BlendFactor::OneMinusSourceOneAlpha; return true;
        ...
        {
            const pipelines::GraphicsState& source = request.pipeline->Graphics();
            ...
            bindings.Reserve(request.pipeline->VertexStreams().Size());
            attributes.Reserve(request.pipeline->VertexAttributes().Size());
            for (const pipelines::VertexStream& stream : request.pipeline->VertexStreams())
            {
                ...
                bindings.PushBack({static_cast<u8>(stream.binding), static_cast<u16>(stream.stride),
                                   stream.inputRate == pipelines::InputRate::PerInstance ? rhi::VertexInputRate::PerInstance
                                                                                       : rhi::VertexInputRate::PerVertex,
                                   static_cast<u16>(stream.instanceStepRate)});
            }
            for (const pipelines::VertexAttribute& attribute : request.pipeline->VertexAttributes())
            {
                if (attribute.location > 0xffu || attribute.streamBinding > 0xffu || attribute.byteOffset > 0xffffu)
                    return false;
                attributes.PushBack({static_cast<u8>(attribute.location), static_cast<u8>(attribute.streamBinding),
                                     static_cast<u16>(attribute.byteOffset), ConvertFormat(attribute.format), attribute.semanticName,
    ...
    const char* ToString(const RenderPipelineResult result) noexcept
    {
        switch (result)
        {
        case RenderPipelineResult::Success: return "Success";
        case RenderPipelineResult::InvalidArgument: return "InvalidArgument";
        case RenderPipelineResult::MissingShader: return "MissingShader";
        case RenderPipelineResult::StaleShader: return "StaleShader";
        case RenderPipelineResult::DuplicateStage: return "DuplicateStage";
        case RenderPipelineResult::UnsupportedState: return "UnsupportedState";
        case RenderPipelineResult::NativeObjectFailure: return "NativeObjectFailure";
        case RenderPipelineResult::CacheFailure: return "CacheFailure";
        }
        return "Unknown";
    }

    RenderPipelineResult RequestRenderPipeline(const RenderPipelineRequest& request, PipelineCache& cache,
                                               PipelineRequest& output) noexcept
    {
        ...
        if (request.pipeline->Kind() == pipelines::PipelineKind::Graphics)
        {
        ...
        if (request.pipeline->Kind() == pipelines::PipelineKind::Compute)
        {
            ...
            return cache.RequestCompute(key, compute, output, request.priority) == pipeline_cache::Result::Success
                       ? RenderPipelineResult::Success
