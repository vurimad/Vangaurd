...
namespace vanguard::rendering
{
    namespace
    {
        ...
        bool Fail(PresentationFailure* const failure, const PresentationFailureCode code,
                  const char* const message, const PresentationOutputHandle output = {},
                  const window::Failure* const windowFailure = nullptr,
                  const ViewportFailure* const viewportFailure = nullptr,
                  const rhi::Failure* const rhiFailure = nullptr) noexcept
        {
            if (failure != nullptr)
            {
                ...
                if (windowFailure != nullptr) failure->windowFailure = *windowFailure;
                if (viewportFailure != nullptr) failure->viewportFailure = *viewportFailure;
        ...
        {
            return policy.bufferCount >= 2 && policy.bufferCount <= rhi::MaximumSwapChainBuffers &&
                   policy.presentMode != rhi::PresentMode::Mailbox &&
                   (!policy.enableFrameLatencyPacing || (policy.maximumFramesInFlight != 0 &&
                    policy.maximumFramesInFlight <= policy.bufferCount && policy.frameLatencyWaitTimeoutMilliseconds != 0));
        }

        [[nodiscard]] bool IsRequiredHdr(const SwapChainPolicy::ColorPreference preference) noexcept
        {
            return preference == SwapChainPolicy::ColorPreference::RequireHdr10 ||
                   preference == SwapChainPolicy::ColorPreference::RequireScRgb;
        }

        void ResolveColorConfiguration(const SwapChainPolicy& policy, const bool hdrActive,
                                       rhi::Format& format, rhi::ColorSpace& colorSpace) noexcept
        {
            format = rhi::Format::B8G8R8A8UNorm;
            colorSpace = rhi::ColorSpace::Srgb;
            if (!hdrActive || policy.colorPreference == SwapChainPolicy::ColorPreference::Sdr) return;
        ...
        [[nodiscard]] bool ConvertSurface(const window::NativePresentationSurface& source,
                                          rhi::PresentationSurface& destination) noexcept
        {
            destination = {};
    ...
    struct PresentationService::Impl
    {
        ...
        void RefreshStateCounts() noexcept
        {
            ...
            for (const Record& record : records)
            {
                ...
                if (record.snapshot.state == PresentationOutputState::Ready) ++stats.readyOutputs;
                if (record.snapshot.state == PresentationOutputState::Suspended) ++stats.suspendedOutputs;
            }
        }
    ...
    bool PresentationService::Initialize(window::WindowManager& windows, ViewportManager& viewports,
                                         PresentationFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (m_impl != nullptr)
            return Fail(failure, PresentationFailureCode::AlreadyInitialized,
                        "presentation service is already initialized");
        if (!concurrency::IsMainThread())
            return Fail(failure, PresentationFailureCode::WrongThread,
                        "presentation service initialization requires the main thread");
        if (!windows.IsInitialized() || !viewports.IsInitialized() || !rhi::IsInitialized())
            return Fail(failure, PresentationFailureCode::InvalidDescriptor,
                        "presentation service requires initialized window, viewport, and RHI systems");
        memory::MemoryBlock block = memory::Allocate(memory::PoolId::Rendering, sizeof(Impl), alignof(Impl));
        if (!block)
            return Fail(failure, PresentationFailureCode::CapacityExceeded,
                        "presentation service storage allocation failed");
    ...
    bool PresentationService::Shutdown(PresentationFailure* const failure) noexcept
    {
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, PresentationFailureCode::WrongThread,
                        "presentation service shutdown requires the main thread");
        if (m_impl->stats.activeOutputs != 0)
        {
            m_impl->Reject();
            return Fail(failure, PresentationFailureCode::OutputsRemainAlive,
                        "all presentation outputs must be explicitly destroyed before shutdown");
        }
        m_impl->~Impl();
    ...
    bool PresentationService::IsInitialized() const noexcept { return m_impl != nullptr; }

    bool PresentationService::CreateOutput(const PresentationOutputDesc& desc, PresentationOutputHandle& output,
                                           PresentationFailure* const failure) noexcept
    {
        output = {};
        ClearFailure(failure);
        if (m_impl == nullptr)
            return Fail(failure, PresentationFailureCode::NotInitialized,
                        "presentation service is not initialized");
        if (!concurrency::IsMainThread())
            return Fail(failure, PresentationFailureCode::WrongThread,
                        "presentation output creation requires the main thread");
        if (desc.name == nullptr || desc.name[0] == '\0' || !desc.window.IsValid() ||
            !desc.renderExtent.IsValid() || !IsPolicyValid(desc.swapChain))
        {
            m_impl->Reject();
            return Fail(failure, PresentationFailureCode::InvalidDescriptor,
                        "presentation output descriptor is invalid");
        }
        window::WindowSnapshot windowSnapshot;
        if (!m_impl->windows->Snapshot(desc.window, windowSnapshot) ||
            windowSnapshot.lifecycle != window::WindowLifecycleState::Alive)
        {
            m_impl->Reject();
            return Fail(failure, PresentationFailureCode::InvalidDescriptor,
                        "presentation output requires a live window");
        }
        ...
        if (record == nullptr)
        {
            m_impl->Reject();
            return Fail(failure, PresentationFailureCode::CapacityExceeded,
                        "presentation output capacity is exhausted");
        }
        ...
        if (!m_impl->windows->AttachPresentation(desc.window, attachment, &windowFailure))
            return Fail(failure, PresentationFailureCode::WindowFailure,
                        "window presentation attachment failed", {}, &windowFailure);
        ...
        if (!m_impl->viewports->CreateRenderViewport(viewportDesc, renderViewport, &viewportFailure))
        {
            ...
            return Fail(failure, PresentationFailureCode::ViewportFailure,
                        "presentation render viewport creation failed", {}, nullptr, &viewportFailure);
        }
    ...
    bool PresentationService::DestroyOutput(const PresentationOutputHandle output,
                                            PresentationFailure* const failure) noexcept
    {
        ClearFailure(failure);
        if (m_impl == nullptr)
            return Fail(failure, PresentationFailureCode::NotInitialized,
                        "presentation service is not initialized", output);
        if (!concurrency::IsMainThread())
            return Fail(failure, PresentationFailureCode::WrongThread,
                        "presentation output destruction requires the main thread", output);
        Impl::Record* const record = m_impl->Find(output);
        if (record == nullptr)
        {
            m_impl->Reject();
            return Fail(failure, PresentationFailureCode::InvalidHandle,
                        "presentation output handle is stale or invalid", output);
        }
        RenderViewportSnapshot viewportSnapshot;
        if (!m_impl->viewports->Snapshot(record->snapshot.renderViewport, viewportSnapshot))
        {
            m_impl->RecordFailure(*record);
            return Fail(failure, PresentationFailureCode::ViewportFailure,
                        "presentation render viewport is unavailable", output);
        }
        if (viewportSnapshot.engineViewportReferences != 0)
        {
            m_impl->Reject();
            return Fail(failure, PresentationFailureCode::Busy,
                        "engine viewports must be destroyed before their presentation output", output);
        }
        if (viewportSnapshot.swapChain.IsValid())
        {
            ViewportFailure viewportFailure;
            if (!m_impl->viewports->UnbindSwapChain(record->snapshot.renderViewport, &viewportFailure))
                return Fail(failure, viewportFailure.code == ViewportFailureCode::Busy
                                         ? PresentationFailureCode::Busy
                                         : PresentationFailureCode::ViewportFailure,
        ...
        if (!m_impl->viewports->DestroyRenderViewport(record->snapshot.renderViewport, &viewportFailure))
            return Fail(failure, PresentationFailureCode::ViewportFailure,
                        "presentation render viewport destruction failed", output, nullptr, &viewportFailure);
        window::Failure windowFailure;
        if (!m_impl->windows->DetachPresentation(record->snapshot.attachment, &windowFailure))
            return Fail(failure, PresentationFailureCode::WindowFailure,
                        "window presentation detachment failed after viewport destruction", output, &windowFailure);
    ...
    bool PresentationService::Tick(PresentationFailure* const failure) noexcept
    {
        ...
        if (!concurrency::IsMainThread())
            return Fail(failure, PresentationFailureCode::WrongThread,
                        "presentation reconciliation requires the main thread");
        ...
        for (Impl::Record& record : m_impl->records)
        {
            ...
            if (!m_impl->windows->Snapshot(record.snapshot.attachment, attachment))
            {
                record.snapshot.state = PresentationOutputState::Failed;
                m_impl->RecordFailure(record);
                if (success)
                    Fail(failure, PresentationFailureCode::WindowFailure,
                         "presentation attachment disappeared during reconciliation", record.snapshot.handle);
                success = false;
                continue;
            }
            ...
            static_cast<void>(m_impl->viewports->Snapshot(record.snapshot.renderViewport, before));
            ViewportFailure viewportFailure;
            if (!m_impl->viewports->UpdatePresentation(record.snapshot.renderViewport, attachment, &viewportFailure))
            {
                ...
                if (success)
                    Fail(failure, viewportFailure.code == ViewportFailureCode::Busy
                                      ? PresentationFailureCode::Busy
                                      : PresentationFailureCode::ViewportFailure,
                         "presentation viewport reconciliation failed", record.snapshot.handle, nullptr,
                         &viewportFailure);
                success = false;
                continue;
            }
            ...
            if (!m_impl->viewports->Snapshot(record.snapshot.renderViewport, current))
            {
                record.snapshot.state = PresentationOutputState::Failed;
                m_impl->RecordFailure(record);
                if (success)
                    Fail(failure, PresentationFailureCode::ViewportFailure,
                         "presentation viewport disappeared during reconciliation", record.snapshot.handle);
                success = false;
                continue;
            }
            ...
            if (current.requiredSurfaceRevision > before.appliedSurfaceRevision && before.swapChain.IsValid() &&
                !current.swapChain.IsValid())
            {
            ...
            if (!current.swapChain.IsValid())
            {
                if (before.swapChain.IsValid())
                {
                    ...
                    if (!rhi::FlushRetiredResources(&rhiFailure))
                    {
                        ...
                        m_impl->RecordFailure(record);
                        if (success)
                            Fail(failure, PresentationFailureCode::RhiFailure,
                                 "retired swap-chain destruction failed before surface replacement",
                ...
                window::Failure windowFailure;
                if (!m_impl->windows->ResolvePresentationSurface(record.snapshot.attachment, nativeSurface,
                                                                 &windowFailure))
                {
                    record.snapshot.state = PresentationOutputState::AwaitingSurface;
                    m_impl->RecordFailure(record);
                    if (success)
                        Fail(failure, PresentationFailureCode::WindowFailure,
                             "native presentation surface resolution failed", record.snapshot.handle,
                             &windowFailure);
                    success = false;
                    continue;
                }
                ...
                if (!ConvertSurface(nativeSurface, surface))
                {
                    ...
                    m_impl->RecordFailure(record);
                    if (success)
                        Fail(failure, PresentationFailureCode::UnsupportedSurface,
                             "native presentation surface is unsupported by the active RHI",
                ...
                ResolveColorConfiguration(record.snapshot.swapChain, attachment.hdrCapable,
                                          swapChainDesc.format, swapChainDesc.colorSpace);
                ...
                swapChainDesc.frameLatency.waitTimeoutMilliseconds =
                    record.snapshot.swapChain.frameLatencyWaitTimeoutMilliseconds;
                swapChainDesc.allowTearing = record.snapshot.swapChain.allowTearing;
                if (!attachment.hdrCapable && IsRequiredHdr(record.snapshot.swapChain.colorPreference) &&
                    !record.snapshot.swapChain.allowSdrFallback)
                {
                    record.snapshot.state = PresentationOutputState::AwaitingSurface;
                    m_impl->RecordFailure(record);
                    if (success)
                        Fail(failure, PresentationFailureCode::RhiFailure,
                             "the presentation policy requires HDR but the active window output is SDR",
                             record.snapshot.handle);
                    success = false;
                    continue;
                }
                ...
                if (!swapChain.IsValid() && swapChainDesc.colorSpace != rhi::ColorSpace::Srgb &&
                    record.snapshot.swapChain.allowSdrFallback)
                {
                ...
                if (!swapChain.IsValid())
                {
                    ...
                    if (success)
                        Fail(failure, PresentationFailureCode::RhiFailure,
                             "swap-chain creation failed", record.snapshot.handle, nullptr, nullptr,
                             &rhiFailure);
                    success = false;
                    continue;
                }
                if (!m_impl->viewports->BindSwapChain(record.snapshot.renderViewport, swapChain, &viewportFailure))
                {
                    ...
                    if (success)
                        Fail(failure, PresentationFailureCode::ViewportFailure,
                             "swap-chain binding failed", record.snapshot.handle, nullptr, &viewportFailure);
                    success = false;
                    continue;
                }
                static_cast<void>(rhi::SafeRelease(swapChain));
                ...
                static_cast<void>(m_impl->viewports->Snapshot(record.snapshot.renderViewport, current));
            }
            ...
            {
                window::Failure windowFailure;
                if (!m_impl->windows->AcknowledgePresentation(record.snapshot.attachment, acknowledgement,
                                                              &windowFailure))
                {
                    record.snapshot.state = PresentationOutputState::Failed;
                    m_impl->RecordFailure(record);
                    if (success)
                        Fail(failure, PresentationFailureCode::WindowFailure,
                             "presentation acknowledgement failed", record.snapshot.handle, &windowFailure);
                    success = false;
                    continue;
                }
            }
    ...
    bool PresentationService::Snapshot(const PresentationOutputHandle output,
