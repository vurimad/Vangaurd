...
namespace
{
    ...
    constexpr u16 KnownPageFlags = static_cast<u16>(mesh::PageFlags::RequiredForLowestLod) | static_cast<u16>(mesh::PageFlags::DirectGpuUpload);
    constexpr u16 KnownSubmeshFlags = static_cast<u16>(mesh::SubmeshFlags::CastsShadow) | static_cast<u16>(mesh::SubmeshFlags::RayTracing) | static_cast<u16>(mesh::SubmeshFlags::TwoSided);
    ...
    struct CanonicalData
    {
        CanonicalData() noexcept
            : buffers(memory::pools::Rendering::GetInstance()), pages(memory::pools::Rendering::GetInstance()), vertexLayouts(memory::pools::Rendering::GetInstance()),
              vertexStreams(memory::pools::Rendering::GetInstance()), materialSlots(memory::pools::Rendering::GetInstance()), lods(memory::pools::Rendering::GetInstance()),
              submeshes(memory::pools::Rendering::GetInstance()), bufferRecords(memory::pools::Rendering::GetInstance()), pageRecords(memory::pools::Rendering::GetInstance()),
              layoutRecords(memory::pools::Rendering::GetInstance()), streamRecords(memory::pools::Rendering::GetInstance()), materialRecords(memory::pools::Rendering::GetInstance()),
    ...
    {
        return writer.IsGood() ? mesh::Result::Success : ConvertSerializationResult(writer.GetStatus());
    }

    [[nodiscard]] mesh::Result ReaderResult(const serialization::BinaryReader& reader) noexcept
    {
        return reader.IsGood() ? mesh::Result::Success : ConvertSerializationResult(reader.GetStatus());
    }
    ...
    [[nodiscard]] bool IsValidBounds(const mesh::Bounds& bounds) noexcept
    {
        ...
        {
            if (!IsFinite(bounds.minimum[axis]) || !IsFinite(bounds.maximum[axis]) || !IsFinite(bounds.sphereCenter[axis]) || bounds.minimum[axis] > bounds.maximum[axis])
            {
                return false;
            }
        }
    ...
    [[nodiscard]] bool RequiredPagesCoverRange(const containers::ArraySpan<const mesh::BufferRecord> buffers, const containers::ArraySpan<const mesh::PageRecord> pages,
                                               const u32 bufferIndex, const u64 byteOffset, const u64 byteSize) noexcept
    {
        if (bufferIndex >= buffers.Size() || byteSize == 0 || byteOffset > buffers[bufferIndex].byteSize || byteSize > buffers[bufferIndex].byteSize - byteOffset)
        {
            return false;
        }
        ...
        for (u32 pageOffset = 0; pageOffset < buffer.pageCount; ++pageOffset)
        {
            ...
            if (page.bufferOffset < rangeEnd && byteOffset < pageEnd && !mesh::HasFlag(page.flags, mesh::PageFlags::RequiredForLowestLod))
            {
                return false;
            }
        }
    ...
    [[nodiscard]] bool WriteQuantization(serialization::BinaryWriter& writer, const mesh::PositionQuantization& quantization) noexcept
    {
        for (u32 axis = 0; axis < 3; ++axis)
        {
    ...
    [[nodiscard]] bool ReadQuantization(serialization::BinaryReader& reader, mesh::PositionQuantization& quantization) noexcept
    {
        for (u32 axis = 0; axis < 3; ++axis)
        {
    ...
    [[nodiscard]] mesh::Result ValidateAndBuildRecords(const mesh::BuildDescription& description, CanonicalData& data) noexcept
    {
        ...
        if (!IsValidMeshKind(description.kind) || description.name == 0 || !IsValidBounds(description.bounds) || !IsValidQuantization(description.positionQuantization) ||
            description.buffers.Empty() || description.pages.Empty() || description.vertexLayouts.Empty() || description.vertexStreams.Empty() || description.lods.Empty() ||
            description.submeshes.Empty())
        {
            return mesh::Result::InvalidArgument;
        }
        if ((description.kind == mesh::MeshKind::Skinned) != description.skeleton.IsValid() || (description.skeleton.IsValid() && !description.skeleton.IsTyped()))
        {
            return mesh::Result::InvalidArgument;
        }
        if (description.buffers.Size() > limits.maximumBuffers || description.pages.Size() > limits.maximumPages || description.vertexLayouts.Size() > limits.maximumVertexLayouts ||
            description.vertexStreams.Size() > limits.maximumVertexStreams || description.materialSlots.Size() > limits.maximumMaterialSlots ||
            description.lods.Size() > limits.maximumLods || description.submeshes.Size() > limits.maximumSubmeshes)
        {
            return mesh::Result::LimitExceeded;
        }
        ...
        std::sort(data.buffers.Begin(), data.buffers.End(), [](const mesh::BufferBuildRecord& left, const mesh::BufferBuildRecord& right) { return left.id < right.id; });
        ...
        std::sort(data.vertexLayouts.Begin(), data.vertexLayouts.End(),
                  [](const mesh::VertexLayoutBuildRecord& left, const mesh::VertexLayoutBuildRecord& right) { return left.id < right.id; });
        ...
        std::sort(data.lods.Begin(), data.lods.End(), [](const mesh::LodBuildRecord& left, const mesh::LodBuildRecord& right) { return left.level < right.level; });
        std::sort(data.submeshes.Begin(), data.submeshes.End(),
                  [](const mesh::SubmeshBuildRecord& left, const mesh::SubmeshBuildRecord& right) { return left.lod != right.lod ? left.lod < right.lod : left.stableId < right.stableId; });

        data.bufferRecords.Resize(data.buffers.Size());
        for (u32 index = 0; index < data.buffers.Size(); ++index)
        {
            const mesh::BufferBuildRecord& source = data.buffers[index];
            if (source.id == 0 || source.byteSize == 0 || source.stride == 0 || !IsValidBufferKind(source.kind) || (index != 0 && data.buffers[index - 1].id == source.id))
            {
                return index != 0 && data.buffers[index - 1].id == source.id ? mesh::Result::DuplicateIdentifier : mesh::Result::InvalidBuffer;
            }
            if ((source.kind == mesh::BufferKind::Index && source.stride != 2 && source.stride != 4) || source.byteSize % source.stride != 0)
            {
        ...
        for (u32 index = 0; index < data.pages.Size(); ++index)
        {
            ...
            if (bufferIndex == mesh::InvalidRecordIndex || source.data == nullptr || source.byteSize == 0 || source.byteSize > limits.maximumPageBytes || source.alignmentLog2 < 4 ||
                source.alignmentLog2 > 20 || (static_cast<u16>(source.flags) & ~KnownPageFlags) != 0)
            {
            ...
            const u64 expectedBufferOffset = bufferRecord.pageCount == 0 ? 0
        ...
        for (u32 index = 0; index < data.vertexLayouts.Size(); ++index)
        {
            if (data.vertexLayouts[index].id == 0 || (index != 0 && data.vertexLayouts[index - 1].id == data.vertexLayouts[index].id))
            {
                return index != 0 && data.vertexLayouts[index - 1].id == data.vertexLayouts[index].id ? mesh::Result::DuplicateIdentifier : mesh::Result::InvalidLayout;
            }
        }
        ...
        for (u32 index = 0; index < data.vertexStreams.Size(); ++index)
        {
            ...
            if (layoutIndex == mesh::InvalidRecordIndex || !IsValidSemantic(source.semantic) || !IsValidVertexFormat(source.format) || source.stride == 0 ||
                mesh::GetVertexFormatByteSize(source.format) == 0)
            {
                return mesh::Result::InvalidLayout;
            }
            if (index != 0 && data.vertexStreams[index - 1].layoutId == source.layoutId && data.vertexStreams[index - 1].semantic == source.semantic &&
                data.vertexStreams[index - 1].semanticIndex == source.semanticIndex)
            {
                return mesh::Result::DuplicateStream;
            }
            const u32 bufferIndex = FindBufferIndex(data, source.bufferId);
            if (bufferIndex == mesh::InvalidRecordIndex || data.buffers[bufferIndex].kind != mesh::BufferKind::Vertex || source.stride != data.buffers[bufferIndex].stride ||
                source.byteOffset > source.stride || mesh::GetVertexFormatByteSize(source.format) > source.stride - source.byteOffset)
            {
                return mesh::Result::InvalidBuffer;
            }
        ...
        for (mesh::VertexLayoutRecord& layout : data.layoutRecords)
        {
            ...
            for (u32 streamOffset = 0; streamOffset < layout.streamCount; ++streamOffset)
            {
                ...
                if (!fingerprintWriter.WriteU8(static_cast<u8>(stream.semantic)) || !fingerprintWriter.WriteU8(stream.semanticIndex) ||
                    !fingerprintWriter.WriteU8(static_cast<u8>(stream.format)) || !fingerprintWriter.WriteU8(stream.binding) || !fingerprintWriter.WriteU32(stream.byteOffset) ||
        ...
        for (u32 index = 0; index < data.lods.Size(); ++index)
        {
            ...
            if (source.level != index || !IsFinite(source.minimumScreenCoverage) || source.minimumScreenCoverage <= 0.0f || source.minimumScreenCoverage > 1.0f ||
        ...
        for (u32 index = 0; index < data.submeshes.Size(); ++index)
        {
            ...
            if (source.stableId == 0 || source.name == 0 || source.lod >= data.lods.Size() || !IsValidIndexFormat(source.indexFormat) || !IsValidTopology(source.topology) ||
                !IsValidBounds(source.bounds) || source.vertexCount == 0 || source.indexCount == 0 || (static_cast<u16>(source.flags) & ~KnownSubmeshFlags) != 0 ||
                (index != 0 && data.submeshes[index - 1].lod == source.lod && data.submeshes[index - 1].stableId == source.stableId))
            {
            ...
            if (materialIndex == mesh::InvalidRecordIndex || vertexLayout == mesh::InvalidRecordIndex || indexBuffer == mesh::InvalidRecordIndex ||
                data.buffers[indexBuffer].kind != mesh::BufferKind::Index || data.buffers[indexBuffer].stride != (source.indexFormat == mesh::IndexFormat::UInt16 ? 2u : 4u))
            {
                return mesh::Result::InvalidSubmesh;
            }
        ...
        for (u32 submeshOffset = 0; submeshOffset < lowestLod.submeshCount; ++submeshOffset)
        {
            ...
            if (!RequiredPagesCoverRange(buffers, pages, submesh.indexBuffer, submesh.firstIndex * indexBuffer.stride, static_cast<u64>(submesh.indexCount) * indexBuffer.stride))
            {
                return mesh::Result::InvalidPage;
            }
            ...
            {
                const mesh::VertexStream& stream = data.streamRecords[layout.firstStream + streamOffset];
                if (!RequiredPagesCoverRange(buffers, pages, stream.buffer, submesh.firstVertex * stream.stride, static_cast<u64>(submesh.vertexCount) * stream.stride))
                {
                    return mesh::Result::InvalidPage;
                }
            }
    ...
    [[nodiscard]] mesh::Result WriteMetadata(ByteArray& metadata, const mesh::BuildDescription& description, const CanonicalData& data, const crypto::Digest256& contentFingerprint) noexcept
    {
        metadata.Clear();
        filesystem::MemoryFileWriter file(metadata);
        serialization::BinaryWriter writer(file);
        if (!writer.WriteU32(MetadataWireVersion) || !writer.WriteU8(static_cast<u8>(description.kind)) || !writer.WriteU8(0) || !writer.WriteU16(0) || !writer.WriteU64(description.name) ||
            !WriteBounds(writer, description.bounds) || !WriteQuantization(writer, description.positionQuantization) || !WriteDigest(writer, description.sourceFingerprint) ||
            !WriteDigest(writer, contentFingerprint) || !writer.WriteU64(description.skeleton.GetPath().Id()) || !writer.WriteU32(description.skeleton.ExpectedType()) ||
            !writer.WriteU32(data.bufferRecords.Size()) || !writer.WriteU32(data.pageRecords.Size()) || !writer.WriteU32(data.layoutRecords.Size()) ||
            !writer.WriteU32(data.streamRecords.Size()) || !writer.WriteU32(data.materialRecords.Size()) || !writer.WriteU32(data.lodRecords.Size()) ||
        ...
        {
            if (!writer.WriteU8(static_cast<u8>(record.kind)) || !writer.WriteU8(0) || !writer.WriteU16(0) || !writer.WriteU32(record.stride) || !writer.WriteU64(record.byteSize) ||
                !writer.WriteU32(record.firstPage) || !writer.WriteU32(record.pageCount))
            {
                return WriterResult(writer);
            }
        }
        ...
        {
            if (!writer.WriteU32(record.buffer) || !writer.WriteU16(static_cast<u16>(record.flags)) || !writer.WriteU8(record.alignmentLog2) || !writer.WriteU8(0) ||
                !writer.WriteU64(record.bufferOffset) || !writer.WriteU64(record.dataOffset) || !writer.WriteU64(record.byteSize) || !WriteDigest(writer, record.digest))
            {
                return WriterResult(writer);
            }
        }
        ...
        {
            if (!writer.WriteU32(record.firstStream) || !writer.WriteU32(record.streamCount) || !WriteDigest(writer, record.fingerprint))
            {
                return WriterResult(writer);
            }
        }
        ...
        {
            if (!writer.WriteU8(static_cast<u8>(record.semantic)) || !writer.WriteU8(record.semanticIndex) || !writer.WriteU8(static_cast<u8>(record.format)) ||
                !writer.WriteU8(record.binding) || !writer.WriteU32(record.buffer) || !writer.WriteU32(record.byteOffset) || !writer.WriteU32(record.stride))
            {
                return WriterResult(writer);
            }
        }
        ...
        {
            if (!writer.WriteU64(record.name) || !writer.WriteU64(record.material.GetPath().Id()) || !writer.WriteU32(record.material.ExpectedType()) || !writer.WriteU32(0))
            {
                return WriterResult(writer);
            }
        }
        ...
        {
            if (!writer.WriteF32(record.minimumScreenCoverage) || !writer.WriteU32(record.firstSubmesh) || !writer.WriteU32(record.submeshCount) || !writer.WriteU32(0))
            {
                return WriterResult(writer);
            }
        }
        ...
        {
            if (!writer.WriteU64(record.stableId) || !writer.WriteU64(record.name) || !writer.WriteU32(record.materialSlot) || !writer.WriteU32(record.vertexLayout) ||
                !writer.WriteU32(record.indexBuffer) || !writer.WriteU32(0) || !writer.WriteU8(static_cast<u8>(record.indexFormat)) || !writer.WriteU8(static_cast<u8>(record.topology)) ||
                !writer.WriteU16(static_cast<u16>(record.flags)) || !writer.WriteU64(record.firstVertex) || !writer.WriteU32(record.vertexCount) || !writer.WriteU32(0) ||
                !writer.WriteU64(record.firstIndex) || !writer.WriteU32(record.indexCount) || !writer.WriteU32(0) || !WriteBounds(writer, record.bounds))
            {
                return WriterResult(writer);
            }
        }
    ...
    [[nodiscard]] mesh::Result WriteDocument(filesystem::IFile& file, const ByteArray& metadata, const CanonicalData& data) noexcept
    {
        serialization::BinaryWriter writer(file);
        ...
        for (u32 index = 0; index < data.pageRecords.Size(); ++index)
        {
            ...
            if (record.dataOffset < geometryPosition || !WriteZeroBytes(writer, record.dataOffset - geometryPosition, geometryCrc))
            {
                return mesh::Result::IoFailure;
            }
            const mesh::PageBuildRecord& source = data.pages[index];
        ...
        if (!writer.Seek(0) || serialization::WriteDocumentHeader(writer, header) != serialization::Result::Success || !writer.Seek(header.fileSize) || !writer.Flush())
        {
            return WriterResult(writer);
        }
        return mesh::Result::Success;
    ...
    [[nodiscard]] const serialization::SectionDescriptor* FindSection(const containers::ArraySpan<const serialization::SectionDescriptor> sections, const u32 id) noexcept
    {
        for (const serialization::SectionDescriptor& section : sections)
        {
            if (section.id == id)
    ...
    {
        const auto buffers = file.GetBuffers();
        const auto pages = file.GetPages();
        const auto layouts = file.GetVertexLayouts();
        const auto streams = file.GetVertexStreams();
        const auto materials = file.GetMaterialSlots();
        const auto lods = file.GetLods();
        const auto submeshes = file.GetSubmeshes();
        if (buffers.Empty() || pages.Empty() || layouts.Empty() || streams.Empty() || lods.Empty() || submeshes.Empty() || !IsValidMeshKind(file.GetKind()) ||
            !IsValidBounds(file.GetMeshBounds()) || !IsValidQuantization(file.GetQuantization()))
        {
            return mesh::Result::InvalidLayout;
        }
        if ((file.GetKind() == mesh::MeshKind::Skinned) != file.GetSkeleton().IsValid() || (file.GetSkeleton().IsValid() && !file.GetSkeleton().IsTyped()))
        {
            return mesh::Result::InvalidLayout;
        }
        ...
        {
            const mesh::BufferRecord& buffer = buffers[bufferIndex];
            if (!IsValidBufferKind(buffer.kind) || buffer.stride == 0 || buffer.byteSize == 0 || buffer.pageCount == 0 || buffer.firstPage > pages.Size() ||
                buffer.pageCount > pages.Size() - buffer.firstPage || (buffer.kind == mesh::BufferKind::Index && buffer.stride != 2 && buffer.stride != 4) ||
            ...
            {
                const mesh::PageRecord& page = pages[buffer.firstPage + pageOffset];
                if (page.buffer != bufferIndex || page.bufferOffset != expectedOffset || page.byteSize == 0 || page.bufferOffset > buffer.byteSize ||
                    page.byteSize > buffer.byteSize - page.bufferOffset || page.alignmentLog2 < 4 || page.alignmentLog2 > 20 || (static_cast<u16>(page.flags) & ~KnownPageFlags) != 0)
                {
                    return mesh::Result::InvalidPage;
                }
        ...
        for (u32 index = 0; index < streams.Size(); ++index)
        {
            ...
            if (!IsValidSemantic(stream.semantic) || !IsValidVertexFormat(stream.format) || stream.buffer >= buffers.Size() || buffers[stream.buffer].kind != mesh::BufferKind::Vertex ||
                stream.stride != buffers[stream.buffer].stride || stream.byteOffset > stream.stride || mesh::GetVertexFormatByteSize(stream.format) > stream.stride - stream.byteOffset)
            {
                return mesh::Result::InvalidLayout;
            }
        }
        ...
        {
            if (layout.streamCount == 0 || layout.firstStream != expectedFirstStream || layout.firstStream > streams.Size() || layout.streamCount > streams.Size() - layout.firstStream)
            {
                return mesh::Result::InvalidLayout;
            }
            bool hasPosition = false;
        ...
        {
            if (!IsFinite(lod.minimumScreenCoverage) || lod.minimumScreenCoverage <= 0.0f || lod.minimumScreenCoverage > 1.0f || lod.minimumScreenCoverage >= previousCoverage ||
                lod.submeshCount == 0 || lod.firstSubmesh != expectedFirstSubmesh || lod.firstSubmesh > submeshes.Size() || lod.submeshCount > submeshes.Size() - lod.firstSubmesh)
            {
                return mesh::Result::InvalidLod;
            }
        ...
        for (u32 submeshIndex = 0; submeshIndex < submeshes.Size(); ++submeshIndex)
        {
            ...
            if (submesh.stableId == 0 || submesh.name == 0 || submesh.materialSlot >= materials.Size() || submesh.vertexLayout >= layouts.Size() || submesh.indexBuffer >= buffers.Size() ||
                buffers[submesh.indexBuffer].kind != mesh::BufferKind::Index || !IsValidIndexFormat(submesh.indexFormat) || !IsValidTopology(submesh.topology) ||
                !IsValidBounds(submesh.bounds) || submesh.vertexCount == 0 || submesh.indexCount == 0 || (static_cast<u16>(submesh.flags) & ~KnownSubmeshFlags) != 0 ||
...
namespace vanguard::meshes
{
    ...
    u32 GetVertexFormatByteSize(const VertexFormat format) noexcept
    {
        switch (format)
        {
    ...
    MeshFile::MeshFile() noexcept
        : m_buffers(memory::pools::Rendering::GetInstance()), m_pages(memory::pools::Rendering::GetInstance()), m_vertexLayouts(memory::pools::Rendering::GetInstance()),
          m_vertexStreams(memory::pools::Rendering::GetInstance()), m_materialSlots(memory::pools::Rendering::GetInstance()), m_lods(memory::pools::Rendering::GetInstance()),
          m_submeshes(memory::pools::Rendering::GetInstance())
    {
    }

    MeshFile::MeshFile(MeshFile&& other) noexcept
        : m_open(other.m_open), m_kind(other.m_kind), m_name(other.m_name), m_bounds(other.m_bounds), m_quantization(other.m_quantization),
          m_sourceFingerprint(other.m_sourceFingerprint), m_contentFingerprint(other.m_contentFingerprint), m_skeleton(other.m_skeleton),
          m_geometryOffset(other.m_geometryOffset), m_geometrySize(other.m_geometrySize), m_buffers(static_cast<containers::DynamicArray<BufferRecord>&&>(other.m_buffers)),
          m_pages(static_cast<containers::DynamicArray<PageRecord>&&>(other.m_pages)),
          m_vertexLayouts(static_cast<containers::DynamicArray<VertexLayoutRecord>&&>(other.m_vertexLayouts)),
          m_vertexStreams(static_cast<containers::DynamicArray<VertexStream>&&>(other.m_vertexStreams)),
          m_materialSlots(static_cast<containers::DynamicArray<MaterialSlot>&&>(other.m_materialSlots)),
          m_lods(static_cast<containers::DynamicArray<LodRecord>&&>(other.m_lods)), m_submeshes(static_cast<containers::DynamicArray<SubmeshRecord>&&>(other.m_submeshes))
    {
        other.Close();
    }

    MeshFile& MeshFile::operator=(MeshFile&& other) noexcept
    {
        if (this != &other)
        {
            Close();
            ...
            m_geometryOffset = other.m_geometryOffset;
            m_geometrySize = other.m_geometrySize;
            m_buffers = static_cast<containers::DynamicArray<BufferRecord>&&>(other.m_buffers);
            m_pages = static_cast<containers::DynamicArray<PageRecord>&&>(other.m_pages);
            m_vertexLayouts = static_cast<containers::DynamicArray<VertexLayoutRecord>&&>(other.m_vertexLayouts);
            m_vertexStreams = static_cast<containers::DynamicArray<VertexStream>&&>(other.m_vertexStreams);
            m_materialSlots = static_cast<containers::DynamicArray<MaterialSlot>&&>(other.m_materialSlots);
            m_lods = static_cast<containers::DynamicArray<LodRecord>&&>(other.m_lods);
            m_submeshes = static_cast<containers::DynamicArray<SubmeshRecord>&&>(other.m_submeshes);
            other.Close();
        }
        return *this;
    }
    ...
    Result MeshFile::Open(filesystem::IFile& reader, const ReadLimits& limits) noexcept
    {
        ...
        const serialization::Result headerResult = serialization::ReadDocumentHeader(documentReader, MeshMagic, {1, 0, 0}, documentLimits, header);
        ...
        const serialization::Result sectionResult = serialization::ReadSectionTable(documentReader, header, documentLimits, sections);
        ...
        if (metadataSection == nullptr || geometrySection == nullptr || metadataSection->version != serialization::Version{1, 0} ||
            geometrySection->version != serialization::Version{1, 0} || metadataSection->codec != serialization::Codec::None || geometrySection->codec != serialization::Codec::None ||
            metadataSection->storedSize != metadataSection->logicalSize || geometrySection->storedSize != geometrySection->logicalSize ||
            (static_cast<u32>(geometrySection->flags) & static_cast<u32>(serialization::SectionFlags::Streamable)) == 0 || metadataSection->storedSize > limits.maximumMetadataBytes ||
            geometrySection->storedSize > limits.maximumGeometryBytes || metadataSection->storedSize > ~u32{0})
        {
        ...
        u32 lodCount = 0;
        u32 submeshCount = 0;
        if (!metadataReader.ReadU32(metadataVersion) || !metadataReader.ReadU8(kind) || !metadataReader.ReadU8(reserved8) || !metadataReader.ReadU16(reserved16) ||
            !metadataReader.ReadU64(m_name) || !ReadBounds(metadataReader, m_bounds) || !ReadQuantization(metadataReader, m_quantization) ||
            !ReadDigest(metadataReader, m_sourceFingerprint) || !ReadDigest(metadataReader, m_contentFingerprint) || !metadataReader.ReadU64(skeletonPath) ||
            !metadataReader.ReadU32(skeletonType) || !metadataReader.ReadU32(bufferCount) || !metadataReader.ReadU32(pageCount) || !metadataReader.ReadU32(layoutCount) ||
            !metadataReader.ReadU32(streamCount) || !metadataReader.ReadU32(materialCount) || !metadataReader.ReadU32(lodCount) || !metadataReader.ReadU32(submeshCount))
        {
            Close();
            return ReaderResult(metadataReader);
        }
        if (metadataVersion != MetadataWireVersion || reserved8 != 0 || reserved16 != 0 || !IsValidMeshKind(static_cast<MeshKind>(kind)) || m_name == 0)
        {
            Close();
            return Result::InvalidLayout;
        }
        if (!ResizeChecked(m_buffers, bufferCount, limits.maximumBuffers) || !ResizeChecked(m_pages, pageCount, limits.maximumPages) ||
            !ResizeChecked(m_vertexLayouts, layoutCount, limits.maximumVertexLayouts) || !ResizeChecked(m_vertexStreams, streamCount, limits.maximumVertexStreams) ||
            !ResizeChecked(m_materialSlots, materialCount, limits.maximumMaterialSlots) || !ResizeChecked(m_lods, lodCount, limits.maximumLods) ||
        ...
        {
            u8 bufferKind = 0;
            if (!metadataReader.ReadU8(bufferKind) || !metadataReader.ReadU8(reserved8) || !metadataReader.ReadU16(reserved16) || !metadataReader.ReadU32(record.stride) ||
                !metadataReader.ReadU64(record.byteSize) || !metadataReader.ReadU32(record.firstPage) || !metadataReader.ReadU32(record.pageCount) || reserved8 != 0 || reserved16 != 0)
            {
                Close();
                return Result::InvalidLayout;
            }
        ...
        {
            u16 flags = 0;
            if (!metadataReader.ReadU32(record.buffer) || !metadataReader.ReadU16(flags) || !metadataReader.ReadU8(record.alignmentLog2) || !metadataReader.ReadU8(reserved8) ||
                !metadataReader.ReadU64(record.bufferOffset) || !metadataReader.ReadU64(record.dataOffset) || !metadataReader.ReadU64(record.byteSize) ||
                !ReadDigest(metadataReader, record.digest) || reserved8 != 0 || record.byteSize > limits.maximumPageBytes || record.dataOffset > geometrySection->storedSize ||
        ...
        {
            if (!metadataReader.ReadU32(record.firstStream) || !metadataReader.ReadU32(record.streamCount) || !ReadDigest(metadataReader, record.fingerprint))
            {
                Close();
                return Result::InvalidLayout;
            }
        ...
        for (VertexStream& record : m_vertexStreams)
        {
            u8 semantic = 0;
            u8 format = 0;
            if (!metadataReader.ReadU8(semantic) || !metadataReader.ReadU8(record.semanticIndex) || !metadataReader.ReadU8(format) || !metadataReader.ReadU8(record.binding) ||
                !metadataReader.ReadU32(record.buffer) || !metadataReader.ReadU32(record.byteOffset) || !metadataReader.ReadU32(record.stride))
            {
                Close();
                return Result::InvalidLayout;
            }
        ...
        for (MaterialSlot& record : m_materialSlots)
        {
            ...
            u32 type = 0;
            u32 reserved = 0;
            if (!metadataReader.ReadU64(record.name) || !metadataReader.ReadU64(path) || !metadataReader.ReadU32(type) || !metadataReader.ReadU32(reserved) || reserved != 0)
            {
                Close();
                return Result::InvalidLayout;
            }
        ...
        {
            u32 reserved = 0;
            if (!metadataReader.ReadF32(record.minimumScreenCoverage) || !metadataReader.ReadU32(record.firstSubmesh) || !metadataReader.ReadU32(record.submeshCount) ||
        ...
        for (SubmeshRecord& record : m_submeshes)
        {
            ...
            u16 flags = 0;
            u32 reserved = 0;
            if (!metadataReader.ReadU64(record.stableId) || !metadataReader.ReadU64(record.name) || !metadataReader.ReadU32(record.materialSlot) ||
                !metadataReader.ReadU32(record.vertexLayout) || !metadataReader.ReadU32(record.indexBuffer) || !metadataReader.ReadU32(reserved) || reserved != 0 ||
                !metadataReader.ReadU8(indexFormat) || !metadataReader.ReadU8(topology) || !metadataReader.ReadU16(flags) || !metadataReader.ReadU64(record.firstVertex) ||
                !metadataReader.ReadU32(record.vertexCount) || !metadataReader.ReadU32(reserved) || reserved != 0 || !metadataReader.ReadU64(record.firstIndex) ||
                !metadataReader.ReadU32(record.indexCount) || !metadataReader.ReadU32(reserved) || reserved != 0 || !ReadBounds(metadataReader, record.bounds))
            {
                Close();
                return Result::InvalidLayout;
            }
    ...
    MeshKind MeshFile::GetKind() const noexcept
    {
        return m_kind;
    }

    u64 MeshFile::GetName() const noexcept
    {
        return m_name;
    }

    const Bounds& MeshFile::GetMeshBounds() const noexcept
    {
        return m_bounds;
    }

    const PositionQuantization& MeshFile::GetQuantization() const noexcept
    {
        return m_quantization;
    }

    const crypto::Digest256& MeshFile::GetSourceFingerprint() const noexcept
    {
        return m_sourceFingerprint;
    }

    const crypto::Digest256& MeshFile::GetContentFingerprint() const noexcept
    {
        return m_contentFingerprint;
    }

    resources::ResourceReference MeshFile::GetSkeleton() const noexcept
    {
        return m_skeleton;
    }

    containers::ArraySpan<const BufferRecord> MeshFile::GetBuffers() const noexcept
    {
        return m_buffers;
    }

    containers::ArraySpan<const PageRecord> MeshFile::GetPages() const noexcept
    {
        return m_pages;
    }

    containers::ArraySpan<const VertexLayoutRecord> MeshFile::GetVertexLayouts() const noexcept
    {
        return m_vertexLayouts;
    }

    containers::ArraySpan<const VertexStream> MeshFile::GetVertexStreams() const noexcept
    {
        return m_vertexStreams;
    }

    containers::ArraySpan<const MaterialSlot> MeshFile::GetMaterialSlots() const noexcept
    {
        return m_materialSlots;
    }

    containers::ArraySpan<const LodRecord> MeshFile::GetLods() const noexcept
    {
        return m_lods;
    }

    containers::ArraySpan<const SubmeshRecord> MeshFile::GetSubmeshes() const noexcept
    {
        return m_submeshes;
    }

    u64 MeshFile::GetGeometryOffset() const noexcept
    {
        return m_geometryOffset;
    }

    u64 MeshFile::GetGeometrySize() const noexcept
    {
        return m_geometrySize;
    }

    Result BuildStorageSegments(const MeshFile& mesh, const u64 documentSize, containers::DynamicArray<StorageSegment>& segments, const u32 maximumSegments) noexcept
    {
        segments.Clear();
        if (!mesh.IsOpen() || maximumSegments == 0 || mesh.GetPages().Empty() || mesh.GetPages().Size() >= maximumSegments || mesh.GetGeometryOffset() > documentSize ||
            mesh.GetGeometrySize() > documentSize - mesh.GetGeometryOffset())
        {
            return Result::InvalidArgument;
        }

        containers::DynamicArray<u32> pageOrder(memory::pools::Serialization::GetInstance());
        pageOrder.Resize(mesh.GetPages().Size());
        if (pageOrder.Size() != mesh.GetPages().Size())
        {
            return Result::LimitExceeded;
        }
        ...
        for (u32 index = 1; index < pageOrder.Size(); ++index)
        {
            ...
            u32 insertion = index;
            while (insertion > 0 && mesh.GetPages()[page].dataOffset < mesh.GetPages()[pageOrder[insertion - 1u]].dataOffset)
            {
        ...
        const u64 firstPageOffset = mesh.GetGeometryOffset() + mesh.GetPages()[pageOrder[0]].dataOffset;
        ...
        for (u32 orderIndex = 0; orderIndex < pageOrder.Size(); ++orderIndex)
        {
            ...
            const PageRecord& page = mesh.GetPages()[pageIndex];
            const u64 begin = mesh.GetGeometryOffset() + page.dataOffset;
            const u64 end = orderIndex + 1u < pageOrder.Size() ? mesh.GetGeometryOffset() + mesh.GetPages()[pageOrder[orderIndex + 1u]].dataOffset : documentSize;
    ...
    Result CollectLodPages(const MeshFile& mesh, const u16 lod, containers::DynamicArray<u32>& pages, const u32 maximumPages) noexcept
    {
        pages.Clear();
        if (!mesh.IsOpen() || lod >= mesh.GetLods().Size() || maximumPages == 0)
        {
            return Result::InvalidArgument;
        }

        const auto addBufferPages = [&](const u32 bufferIndex, const u64 firstElement, const u32 elementCount) -> Result
        {
            if (bufferIndex >= mesh.GetBuffers().Size())
            {
                return Result::InvalidBuffer;
            }
            const BufferRecord& buffer = mesh.GetBuffers()[bufferIndex];
            if (firstElement > ~u64{0} / buffer.stride || elementCount > (~u64{0} - firstElement * buffer.stride) / buffer.stride)
            {
                return Result::InvalidBuffer;
            }
        ...
        const LodRecord& selectedLod = mesh.GetLods()[lod];
        for (u32 localSubmesh = 0; localSubmesh < selectedLod.submeshCount; ++localSubmesh)
        {
            const SubmeshRecord& submesh = mesh.GetSubmeshes()[selectedLod.firstSubmesh + localSubmesh];
            const VertexLayoutRecord& layout = mesh.GetVertexLayouts()[submesh.vertexLayout];
            for (u32 localStream = 0; localStream < layout.streamCount; ++localStream)
            {
                const VertexStream& stream = mesh.GetVertexStreams()[layout.firstStream + localStream];
    ...
    Result MeshFile::ReadPage(filesystem::IFile& reader, const u32 pageIndex, void* const destination, const usize capacity) const noexcept
    {
        if (!m_open)
        {
        ...
        if (!binaryReader.Seek(m_geometryOffset + page.dataOffset) || !binaryReader.ReadBytes(destination, static_cast<usize>(page.byteSize)))
        {
            return ReaderResult(binaryReader);
        }
        return crypto::Sha256(destination, static_cast<usize>(page.byteSize)) == page.digest ? Result::Success : Result::IntegrityFailure;
    }
