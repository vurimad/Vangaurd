...
namespace
{
    ...
    [[nodiscard]] Type* AllocateCellStreamingObject(const memory::PoolId pool, Args&&... args) noexcept
    {
    ...
    void DeleteCellStreamingObject(const memory::PoolId pool, Type* const object) noexcept
    {
    ...
    [[nodiscard]] resources::Failure ToResourceFailure(const world::Result result) noexcept
    {
        switch (result)
        {
        case world::Result::Success: return resources::Failure::None;
        case world::Result::InvalidMagic:
        case world::Result::InvalidLayout: return resources::Failure::DeserializationFailure;
        case world::Result::UnsupportedVersion: return resources::Failure::UnsupportedVersion;
        case world::Result::IntegrityFailure: return resources::Failure::IntegrityFailure;
        case world::Result::IoFailure: return resources::Failure::IoFailure;
        default: return resources::Failure::DeserializationFailure;
        }
    }

    [[nodiscard]] resources::ResourceObject* DecodeCell(const resources::ResourceReference reference,
                                                        const void* const data, const usize size,
                                                        const resources::LoadContext& loadContext, resources::Failure& failure,
                                                        void* const userData) noexcept
    {
        ...
        if (result == world::Result::Success && resource->BindDependencies(loadContext)) return resource;
        failure = ToResourceFailure(result);
        if (result == world::Result::Success) failure = resources::Failure::DependencyFailure;
    ...
    [[nodiscard]] resources::Failure ToResourceFailure(const entity::Result result) noexcept
    {
        switch (result)
        {
        case entity::Result::OutOfMemory: return resources::Failure::OutOfMemory;
        case entity::Result::UnsupportedSchemaVersion: return resources::Failure::UnsupportedVersion;
        case entity::Result::SchemaFailure:
        case entity::Result::InvalidPrefab:
        case entity::Result::InvalidReference: return resources::Failure::DeserializationFailure;
        case entity::Result::MissingPrefab:
        case entity::Result::UnknownSchema: return resources::Failure::DependencyFailure;
        default: return resources::Failure::InternalError;
        }
    }
} // namespace
...
namespace vanguard::entities
{
    ...
    namespace
    {
        ...
        [[nodiscard]] const prefabs::PrefabFile* ResolveCellPrefab(
            const resources::ResourceReference reference, void* const userData) noexcept
        {
            const auto* const context = static_cast<const CellPrefabResolverContext*>(userData);
            if (context == nullptr || context->resource == nullptr) return nullptr;
            if (const prefabs::PrefabFile* const prefab = context->resource->ResolvePrefab(reference)) return prefab;
    ...
    {
        if (m_impl != nullptr || m_config.executor == nullptr || !m_config.executor->IsInitialized()) return false;
        Impl* const impl = AllocateCellStreamingObject<Impl>(memory::PoolId::Gameplay);
        if (impl == nullptr || !impl->components.Initialize(gameWorld.Entities()) ||
            !impl->references.Initialize(gameWorld.Entities()))
        {
            DeleteCellStreamingObject(memory::PoolId::Gameplay, impl);
            return false;
        }
        if ((m_config.registerComponents != nullptr &&
             !m_config.registerComponents(impl->components, m_config.userData)) ||
            !impl->materializer.Initialize(impl->components, impl->references, m_config.materialization))
        {
    ...
    {
        if (m_impl == nullptr) return;
        if (!m_impl->operations.Empty() || !m_impl->materializer.Shutdown() || !m_impl->references.Shutdown() ||
            !m_impl->components.Shutdown())
        {
            m_impl->failed = true;
            return;
        }
    ...
    {
        if (m_impl == nullptr || input.observers.Size() > world::MaximumStreamingObservers ||
            input.globalDistanceScale <= 0.0f) return false;
        m_impl->observers.Clear();
        for (const world::StreamingObserver& observer : input.observers) m_impl->observers.PushBack(observer);
        m_impl->input = input;
    ...
    {
        if (m_impl == nullptr || m_impl->failed || !m_impl->hasInput ||
            !m_config.executor->Process(m_impl->input, m_impl->events))
        {
            if (m_impl != nullptr && m_impl->hasInput) m_impl->failed = true;
            return;
        }
        for (const world::StreamingResourceEvent& event : m_impl->events)
        {
            ...
            {
                if (m_config.forwardNonCellEvent != nullptr) m_config.forwardNonCellEvent(event, m_config.userData);
                continue;
            }
            if (event.type == world::StreamingResourceEventType::ResourceAvailable)
            {
                const resources::ResourceHandle* const handle = m_config.executor->Resource(event.key);
                const auto* const resource = handle != nullptr && handle->Type() == world::CellResourceType
                                                 ? static_cast<const world::CellResource*>(handle->Get()) : nullptr;
                if (resource == nullptr || resource->File().CellId() != event.key.id || m_impl->Find(event.key.id) != nullptr)
                {
                ...
                operation->generation = handle->Generation();
                CellPrefabResolverContext resolver{resource, m_config.resolvePrefab, m_config.userData};
                const Result result = m_impl->materializer.QueueCell(resource->File(), operation->generation,
                                                                     &ResolveCellPrefab, &resolver);
            ...
            else if (event.type == world::StreamingResourceEventType::ReleaseRequested)
            {
                ...
                if (operation == nullptr) { m_impl->failed = true; continue; }
                Result result = Result::InvalidState;
                const CellState state = m_impl->materializer.State(event.key.id);
                ...
                if (result != Result::Success) { m_impl->failed = true; continue; }
                if (m_impl->materializer.State(event.key.id) == CellState::Unknown)
                {
                    m_impl->completed.PushBack(event.key.id);
                    if (!m_config.executor->CompleteRelease(event.key)) m_impl->failed = true;
                }
                else operation->phase = Impl::Phase::AwaitingRelease;
            }
        }
    ...
    {
        if (m_impl == nullptr || m_impl->failed) return;
        for (u32 pass = 0; pass < 2; ++pass)
        {
            for (auto iterator = m_impl->operations.Begin(), end = m_impl->operations.End(); iterator != end; ++iterator)
            {
                Impl::Operation* const operation = iterator.Value();
                if (operation->phase != Impl::Phase::AwaitingActivation) continue;
                const Result result = m_impl->materializer.CompleteActivation(operation->key.id, operation->generation);
                if (result == Result::Success)
                {
        ...
        for (auto iterator = m_impl->operations.Begin(), end = m_impl->operations.End(); iterator != end; ++iterator)
        {
            ...
            {
                const Result groupResult = m_impl->materializer.SynchronizeActivationGroups(
                    operation->key.id, operation->generation);
                if (groupResult != Result::Success && groupResult != Result::NotReady)
                {
                    m_impl->failed = true;
                    return;
                }
                const bool ready = m_impl->materializer.State(operation->key.id) == CellState::Active;
                if (!m_config.executor->SetReady(operation->key, ready)) { m_impl->failed = true; return; }
                continue;
            }
            if (operation->phase != Impl::Phase::AwaitingRelease && operation->phase != Impl::Phase::FailureRelease) continue;
            const Result result = m_impl->materializer.CompleteRelease(operation->key.id, operation->generation);
            if (result == Result::NotReady) continue;
            if (result != Result::Success) { m_impl->failed = true; return; }
            const bool acknowledged = operation->phase == Impl::Phase::AwaitingRelease
                                          ? m_config.executor->CompleteRelease(operation->key)
                                          : m_config.executor->FailResident(operation->key, operation->failure);
            if (!acknowledged) { m_impl->failed = true; return; }
            m_impl->completed.PushBack(operation->key.id);
            ++m_impl->releasedCells;
            if (operation->phase == Impl::Phase::FailureRelease) ++m_impl->downstreamFailures;
        }
        for (const u64 cellId : m_impl->completed)
        {
            Impl::Operation* const operation = m_impl->Find(cellId);
    ...
    Result CellStreamingSystem::AcquireActivationGroup(const u64 cellId, const u64 groupId,
                                                       const ActivationOwnerId ownerId) noexcept
    {
        if (m_impl == nullptr) return Result::InvalidState;
        Impl::Operation* const operation = m_impl->Find(cellId);
        if (operation == nullptr || operation->phase != Impl::Phase::Active) return Result::InvalidState;
        const resources::ResourceHandle* const handle = m_config.executor->Resource(operation->key);
        const auto* const resource = handle != nullptr && handle->Type() == world::CellResourceType
                                         ? static_cast<const world::CellResource*>(handle->Get()) : nullptr;
        if (resource == nullptr) return Result::InvalidState;
        CellPrefabResolverContext resolver{resource, m_config.resolvePrefab, m_config.userData};
        return m_impl->materializer.AcquireActivationGroup(resource->File(), operation->generation, groupId, ownerId,
                                                           &ResolveCellPrefab, &resolver);
    }

    Result CellStreamingSystem::ReleaseActivationGroup(const u64 cellId, const u64 groupId,
                                                       const ActivationOwnerId ownerId) noexcept
    {
        if (m_impl == nullptr) return Result::InvalidState;
        Impl::Operation* const operation = m_impl->Find(cellId);
    ...
    ComponentRegistry* CellStreamingSystem::Components() noexcept { return m_impl != nullptr ? &m_impl->components : nullptr; }
    EntityReferenceRegistry* CellStreamingSystem::References() noexcept { return m_impl != nullptr ? &m_impl->references : nullptr; }
    CellMaterializer* CellStreamingSystem::Materializer() noexcept { return m_impl != nullptr ? &m_impl->materializer : nullptr; }
    ...
    CellStreamingSystemStats CellStreamingSystem::GetStats() const noexcept
    {
        ...
        for (auto iterator = m_impl->operations.Begin(), end = m_impl->operations.End(); iterator != end; ++iterator)
        {
            ...
            stats.awaitingRelease += operation->phase == Impl::Phase::AwaitingRelease ||
                                     operation->phase == Impl::Phase::FailureRelease;
    ...
    {
        if (m_impl == nullptr) return "cell streaming is not initialized";
        if (m_impl->failed) return "cell streaming lifecycle failure";
        for (auto iterator = m_impl->operations.Begin(), end = m_impl->operations.End(); iterator != end; ++iterator)
            if (iterator.Value()->phase == Impl::Phase::AwaitingActivation) return "streamed cells are awaiting activation";
        return nullptr;
    }
} // namespace vanguard::entities
