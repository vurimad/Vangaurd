...
namespace
{
    ...
    [[nodiscard]] Type* AllocateMaterializerObject(Args&&... args) noexcept
    {
    ...
    void DeleteMaterializerObject(Type* const object) noexcept
    {
    ...
    [[nodiscard]] constexpr bool HasFlag(const Enum value, const Enum flag) noexcept
    {
    ...
    [[nodiscard]] bool ShouldMaterializeEntity(const prefabs::PrefabFile& prefab,
                                               const prefabs::EntityRecord& entity,
                                               const bool includeEditorData) noexcept
    {
        if (includeEditorData) return true;
        const prefabs::EntityRecord* cursor = &entity;
        while (cursor != nullptr)
        {
            if (HasFlag(cursor->flags, prefabs::EntityFlags::EditorOnly)) return false;
            cursor = cursor->parentStableId != prefabs::InvalidStableId
                         ? prefab.FindEntity(cursor->parentStableId)
                         : nullptr;
        }
        return true;
    }

    [[nodiscard]] bool IsInitiallyActive(const world::CellFile& cell, const world::PlacementRecord& placement,
                                         const bool includeEditorData, const bool includeInactiveGroups) noexcept
    {
        const world::ActivationGroupRecord* const group = cell.FindActivationGroup(placement.activationGroup);
        if (group == nullptr) return false;
        if (!includeEditorData && HasFlag(group->flags, world::ActivationGroupFlags::EditorOnly)) return false;
        return includeInactiveGroups || HasFlag(group->flags, world::ActivationGroupFlags::DefaultActive);
    }

    [[nodiscard]] bool IsEligiblePlacement(const world::CellFile& cell, const world::PlacementRecord& placement,
                                           const bool includeEditorData) noexcept
    {
        if (!includeEditorData && HasFlag(placement.flags, world::PlacementFlags::EditorOnly)) return false;
        const world::ActivationGroupRecord* const group = cell.FindActivationGroup(placement.activationGroup);
...
{
    ecs::EntityId DeriveEntityId(const ecs::EntityId instanceId, const u64 prefabEntityStableId,
                                 const u64 prefabRootStableId) noexcept
    {
        if (instanceId == ecs::InvalidEntityId || prefabEntityStableId == prefabs::InvalidStableId ||
            prefabRootStableId == prefabs::InvalidStableId) return ecs::InvalidEntityId;
        if (prefabEntityStableId == prefabRootStableId) return instanceId;
    ...
    struct CellMaterializer::Impl
    {
        ...
        {
            CellRecord() noexcept
                : entities(memory::pools::World::GetInstance()), expected(memory::pools::World::GetInstance()),
                  references(memory::pools::World::GetInstance()), baseGroups(memory::pools::World::GetInstance()),
        ...
        struct GroupRecord
        {
            GroupRecord() noexcept
                : entities(memory::pools::World::GetInstance()), expected(memory::pools::World::GetInstance()),
                  references(memory::pools::World::GetInstance()) {}
            u64 groupId = 0;
            u32 retainCount = 0;
        ...
        Impl(ComponentRegistry& componentRegistry, EntityReferenceRegistry& referenceRegistry,
             const MaterializationConfig& value) noexcept
            : components(&componentRegistry), references(&referenceRegistry), world(componentRegistry.RegisteredWorld()), config(value),
        ...
        [[nodiscard]] GroupLease* FindLease(CellRecord& cell, const u64 groupId,
                                            const ActivationOwnerId owner) const noexcept
        {
            for (GroupLease* const lease : cell.leases)
                if (lease->requestedGroup == groupId && lease->owner == owner) return lease;
            return nullptr;
        }

        [[nodiscard]] bool AddExpected(containers::DynamicArray<ExpectedComponent>& expected,
                                       const ExpectedComponent value) const noexcept
        {
            for (const ExpectedComponent& existing : expected)
                if (existing.entity == value.entity && existing.component == value.component) return false;
            const u32 size = expected.Size();
            expected.PushBack(value);
            return expected.Size() == size + 1u;
        }

        [[nodiscard]] Result PlanSelection(
            const world::CellFile& cell, const containers::HashMap<ecs::EntityId, u8>& selectedPlacements,
            const PrefabResolver resolvePrefab, void* const userData,
            containers::DynamicArray<PlannedEntity>& plannedEntities,
            containers::DynamicArray<PlannedComponent>& plannedComponents,
            containers::DynamicArray<ExpectedComponent>& expected, MaterializationReport& report) noexcept
        {
            containers::HashMap<ecs::EntityId, u8> identities(memory::pools::World::GetInstance());
            identities.Reserve(selectedPlacements.Size());
            for (const world::PlacementRecord& placement : cell.Placements())
            {
                u8 selected = 0;
                ...
                if (prefab == nullptr) return Result::MissingPrefab;
                if (!prefab->IsOpen() || prefab->Entities().Empty()) return Result::InvalidPrefab;

                const prefabs::EntityRecord* root = nullptr;
                for (const prefabs::EntityRecord& prefabEntity : prefab->Entities())
                {
                    if (prefabEntity.parentStableId != prefabs::InvalidStableId) continue;
                    if (root != nullptr) return Result::InvalidPrefab;
                    root = &prefabEntity;
                }
                if (root == nullptr) return Result::InvalidPrefab;
                ...
                for (const prefabs::EntityRecord& prefabEntity : prefab->Entities())
                {
                    ...
                    u64 reservingCell = 0;
                    if (identity == ecs::InvalidEntityId || world->Resolve(identity) ||
                        reservedIdentities.Find(identity, reservingCell) || !identities.Insert(identity, 1).IsSuccessful())
                    ...
                    plannedEntities.PushBack({identity, parent, placement.entityId, prefabEntity.stableId,
                                              &placement, prefabEntity.stableId == root->stableId, disabled});
                    if (plannedEntities.Size() > config.maximumEntitiesPerCell) return Result::LimitExceeded;

                    const containers::ArraySpan<const world::ComponentOverrideRecord> overrides = cell.OverridesFor(placement);
                    for (u32 local = 0; local < prefabEntity.componentCount; ++local)
                    {
                        const prefabs::ComponentRecord& component = prefab->Components()[prefabEntity.firstComponent + local];
                        ...
                        for (const world::ComponentOverrideRecord& candidate : overrides)
                            if (candidate.componentStableId == component.stableId) { overrideRecord = &candidate; break; }
                        ...
                        containers::ArraySpan<const u8> data = prefab->ComponentData(component);
                        if (overrideRecord != nullptr)
                        {
                            ...
                            data = cell.OverrideData(*overrideRecord);
                        ...
                        if (decoded != Result::Success) return decoded;
                        planned.runtimeComponent = components->RuntimeComponent(schema);
                        if (!AddExpected(expected, {identity, planned.runtimeComponent})) return Result::DuplicateIdentifier;
                        plannedComponents.PushBack(static_cast<PlannedComponent&&>(planned));
                        if (plannedComponents.Size() > config.maximumComponentsPerCell) return Result::LimitExceeded;
                    }

                    if (prefabEntity.stableId != root->stableId) continue;
                    for (const world::ComponentOverrideRecord& overrideRecord : overrides)
                    {
                        bool targetsExisting = false;
                        for (const prefabs::ComponentRecord& component : prefab->Components())
                            targetsExisting |= component.stableId == overrideRecord.componentStableId;
                        if (overrideRecord.mode != world::OverrideMode::Add)
                        {
                            if (!targetsExisting) return Result::MissingOverrideTarget;
                            continue;
                        }
                        if (targetsExisting) return Result::MissingOverrideTarget;
                        ...
                        const Result decoded = components->Decode(overrideRecord.schema, overrideRecord.schemaVersion,
                                                                  cell.OverrideData(overrideRecord), planned.value);
                        if (decoded != Result::Success) return decoded;
                        planned.runtimeComponent = components->RuntimeComponent(overrideRecord.schema);
                        if (!AddExpected(expected, {planned.entity, planned.runtimeComponent}))
        ...
        [[nodiscard]] Result QueueGroupPlan(const world::CellFile& cell,
                                            const containers::DynamicArray<PlannedEntity>& plannedEntities,
                                            const containers::DynamicArray<PlannedComponent>& plannedComponents,
                                            containers::DynamicArray<ExpectedComponent>& expected,
                                            GroupRecord& group) noexcept
        {
            ...
            {
                if (!reservedIdentities.Insert(entity.identity, cell.CellId()).IsSuccessful())
                {
            ...
            for (const PlannedEntity& entity : plannedEntities)
            {
                if (world->QueueCreate(commandBatch, entity.identity)) continue;
            ...
            for (const PlannedEntity& entity : plannedEntities)
            {
                const EntityOrigin origin{cell.CellId(), entity.instanceId, entity.prefabEntityStableId};
                if (!world->QueueAddComponent(commandBatch, entity.identity, originType) ||
                    !world->QueueSetComponent(commandBatch, entity.identity, originType, origin) ||
                    !AddExpected(expected, {entity.identity, originType.id}))
                {
                    static_cast<void>(world->CancelCommandBatch(commandBatch));
                    ReleaseReservations(plannedEntities, reservedCount);
                ...
                if (entity.root)
                {
                    ...
                    {
                        placement.translation[axis] = cell.Origin()[axis] + entity.placement->transform.translation[axis];
                ...
                if (entity.disabled && (!world->QueueAddComponent(commandBatch, entity.identity, disabledType) ||
                                        !AddExpected(expected, {entity.identity, disabledType.id})))
                {
                    static_cast<void>(world->CancelCommandBatch(commandBatch));
                    ReleaseReservations(plannedEntities, reservedCount);
                    return Result::QueueFailure;
            ...
            for (const PlannedComponent& component : plannedComponents)
            {
                if (components->Queue(commandBatch, component.entity, component.value, component.enabled)) continue;
                static_cast<void>(world->CancelCommandBatch(commandBatch));
                ReleaseReservations(plannedEntities, reservedCount);
                return Result::QueueFailure;
            }
            ...
            for (const PlannedEntity& entity : plannedEntities) group.entities.PushBack(entity.identity);
        ...
        {
            for (const ecs::EntityId identity : record.entities) static_cast<void>(reservedIdentities.Remove(identity));
        }
        ...
        void DeleteGroups(CellRecord& cell) noexcept
        {
            ...
            for (GroupLease* const lease : cell.leases) DeleteMaterializerObject(lease);
            cell.groups.Clear();
            cell.leases.Clear();
        }
    ...
    bool CellMaterializer::Initialize(ComponentRegistry& components, EntityReferenceRegistry& references,
                                      const MaterializationConfig& config) noexcept
    {
        if (m_impl != nullptr || !components.IsInitialized() || config.maximumEntitiesPerCell == 0 ||
            config.maximumComponentsPerCell == 0 || references.RegisteredWorld() != components.RegisteredWorld() ||
            !components.Seal()) return false;
        Impl* const impl = AllocateMaterializerObject<Impl>(components, references, config);
    ...
    bool CellMaterializer::Shutdown() noexcept
    {
        ...
        if (!m_impl->lookup.Empty() || !m_impl->reservedIdentities.Empty()) return false;
        DeleteMaterializerObject(m_impl);
        m_impl = nullptr;
        return true;
    }

    bool CellMaterializer::IsInitialized() const noexcept { return m_impl != nullptr; }

    Result CellMaterializer::QueueCell(const world::CellFile& cell, const u32 generation,
                                       const PrefabResolver resolvePrefab, void* const userData,
                                       MaterializationReport* const output) noexcept
    {
        if (output != nullptr) *output = {};
        if (m_impl == nullptr || !cell.IsOpen() || cell.CellId() == 0 || generation == 0 || resolvePrefab == nullptr)
            return Result::InvalidArgument;
        if (m_impl->Find(cell.CellId()) != nullptr) return Result::InvalidState;
        ...
        plannedEntities.Reserve(cell.Placements().Size());
        selectedPlacements.Reserve(cell.Placements().Size());

        MaterializationReport report;
        report.cellId = cell.CellId();
        report.generation = generation;

        for (const world::PlacementRecord& placement : cell.Placements())
        {
            if (IsEligiblePlacement(cell, placement, m_impl->config.includeEditorData) &&
                IsInitiallyActive(cell, placement, m_impl->config.includeEditorData,
                                  m_impl->config.includeInactiveActivationGroups))
        ...
        while (selectionChanged)
        {
            ...
            for (const world::EntityReferenceRecord& reference : cell.EntityReferences())
            {
                u8 selected = 0;
                if (reference.kind != world::EntityReferenceKind::RequiredLocal ||
                    !selectedPlacements.Find(reference.sourceEntityId, selected) ||
                    selectedPlacements.Find(reference.targetEntityId, selected)) continue;
            ...
            {
                u8 selected = 0;
                if (placement.parentEntityId == ecs::InvalidEntityId ||
                    !selectedPlacements.Find(placement.entityId, selected) ||
                    selectedPlacements.Find(placement.parentEntityId, selected)) continue;
                const world::PlacementRecord* const parent = cell.FindPlacement(placement.parentEntityId);
                if (parent == nullptr) continue;
                if (!IsEligiblePlacement(cell, *parent, m_impl->config.includeEditorData)) return Result::InvalidReference;
                if (!selectedPlacements.Insert(parent->entityId, 1).IsSuccessful()) return Result::OutOfMemory;
                selectionChanged = true;
            }
        }

        const Result planResult = m_impl->PlanSelection(cell, selectedPlacements, resolvePrefab, userData,
                                                        plannedEntities, plannedComponents, expected, report);
        if (planResult != Result::Success) return planResult;

        u32 reservedCount = 0;
        for (const Impl::PlannedEntity& entity : plannedEntities)
        {
            if (!m_impl->reservedIdentities.Insert(entity.identity, cell.CellId()).IsSuccessful())
            {
        ...
        for (const Impl::PlannedEntity& entity : plannedEntities)
        {
            ...
            if (entity.root)
            {
                ...
                for (u32 axis = 0; axis < 4; ++axis) placement.rotation[axis] = entity.placement->transform.rotation[axis];
        ...
        for (const Impl::PlannedEntity& entity : plannedEntities) record->entities.PushBack(entity.identity);
        record->expected = static_cast<containers::DynamicArray<Impl::ExpectedComponent>&&>(expected);
        record->baseGroups.Reserve(cell.ActivationGroups().Size());
        for (const world::PlacementRecord& placement : cell.Placements())
        {
            u8 selected = 0;
            if (selectedPlacements.Find(placement.entityId, selected))
        ...
        {
            u8 selected = 0;
            if (selectedPlacements.Find(reference.sourceEntityId, selected)) record->references.PushBack(reference);
        }
    ...
    Result CellMaterializer::CompleteActivation(const u64 cellId, const u32 generation) noexcept
    {
        if (m_impl == nullptr || cellId == 0 || generation == 0) return Result::InvalidArgument;
        ...
        if (record == nullptr || (record->state != CellState::PendingActivation &&
                                  ...
                                  record->state != CellState::Active)) return Result::InvalidState;
        if (record->generation != generation) return Result::StaleGeneration;
        if (record->state == CellState::Active)
            return m_impl->references->IsPartitionReady(cellId, generation, EntityReferenceRegistry::BasePartition)
                       ? Result::Success : Result::NotReady;
        ...
        if (batchStatus != ecs::CommandBatchStatus::Succeeded) return Result::InvalidState;
        for (const ecs::EntityId identity : record->entities)
        {
            if (m_impl->world->Resolve(identity)) continue;
            static_cast<void>(m_impl->world->RetireCommandBatch(record->commandBatch));
            record->commandBatch = {};
            record->state = CellState::Failed;
            return Result::QueueFailure;
        ...
        for (const Impl::ExpectedComponent& expected : record->expected)
        {
            ...
            if (!entity || !ecs_has_id(m_impl->world->Native(), entity.value, expected.component))
            {
                static_cast<void>(m_impl->world->RetireCommandBatch(record->commandBatch));
                record->commandBatch = {};
                record->state = CellState::Failed;
        ...
        if (!m_impl->world->RetireCommandBatch(record->commandBatch)) return Result::InvalidState;
        record->commandBatch = {};
        const ReferenceResult referenceResult = m_impl->references->RegisterCell(
            cellId, generation, containers::ArraySpan<const ecs::EntityId>(record->entities),
    ...
    Result CellMaterializer::QueueRelease(const u64 cellId, const u32 generation) noexcept
    {
        if (m_impl == nullptr || cellId == 0 || generation == 0) return Result::InvalidArgument;
        Impl::CellRecord* const record = m_impl->Find(cellId);
        if (record == nullptr || (record->state != CellState::Active && record->state != CellState::PendingReferences &&
                                  record->state != CellState::Failed))
            return Result::InvalidState;
        if (record->generation != generation) return Result::StaleGeneration;
        for (auto iterator = record->groups.Begin(), end = record->groups.End(); iterator != end; ++iterator)
            if (iterator.Value()->state == ActivationGroupState::PendingActivation ||
                iterator.Value()->state == ActivationGroupState::PendingRelease) return Result::InvalidState;
        const ecs::CommandBatch commandBatch = m_impl->world->BeginCommandBatch();
        if (!commandBatch) return Result::OutOfMemory;
        for (auto iterator = record->groups.Begin(), end = record->groups.End(); iterator != end; ++iterator)
        {
            Impl::GroupRecord* const group = iterator.Value();
            for (u32 index = group->entities.Size(); index > 0; --index)
                if (m_impl->world->Resolve(group->entities[index - 1u]) &&
                    !m_impl->world->QueueDestroy(commandBatch, group->entities[index - 1u]))
                {
                    static_cast<void>(m_impl->world->CancelCommandBatch(commandBatch));
                    return Result::QueueFailure;
                }
        ...
        for (u32 index = record->entities.Size(); index > 0; --index)
            if (m_impl->world->Resolve(record->entities[index - 1u]) &&
                !m_impl->world->QueueDestroy(commandBatch, record->entities[index - 1u]))
            {
                static_cast<void>(m_impl->world->CancelCommandBatch(commandBatch));
                return Result::QueueFailure;
            }
    ...
    Result CellMaterializer::CompleteRelease(const u64 cellId, const u32 generation) noexcept
    {
        if (m_impl == nullptr || cellId == 0 || generation == 0) return Result::InvalidArgument;
        Impl::CellRecord* const record = m_impl->Find(cellId);
        if (record == nullptr || record->state != CellState::PendingRelease) return Result::InvalidState;
        if (record->generation != generation) return Result::StaleGeneration;
        const ecs::CommandBatchStatus batchStatus = m_impl->world->GetCommandBatchStatus(record->commandBatch);
        if (batchStatus == ecs::CommandBatchStatus::Open || batchStatus == ecs::CommandBatchStatus::Pending)
            return Result::NotReady;
        ...
        for (auto iterator = record->groups.Begin(), end = record->groups.End(); iterator != end; ++iterator)
            for (const ecs::EntityId identity : iterator.Value()->entities)
                if (m_impl->world->Resolve(identity)) return Result::QueueFailure;
        if (!m_impl->world->RetireCommandBatch(record->commandBatch)) return Result::InvalidState;
        record->commandBatch = {};
    ...
    Result CellMaterializer::Cancel(const u64 cellId, const u32 generation) noexcept
    {
        ...
        Impl::CellRecord* const record = m_impl->Find(cellId);
        if (record == nullptr || (record->state != CellState::PendingActivation &&
                                  record->state != CellState::PendingReferences)) return Result::InvalidState;
        if (record->generation != generation) return Result::StaleGeneration;
        if (record->state == CellState::PendingReferences)
        {
        ...
        if (!m_impl->world->CancelCommandBatch(record->commandBatch)) return Result::InvalidState;
        record->commandBatch = {};
        bool anyLive = false;
        for (const ecs::EntityId identity : record->entities)
    ...
    Result CellMaterializer::AcquireActivationGroup(const world::CellFile& cell, const u32 generation,
                                                    const u64 groupId, const ActivationOwnerId ownerId,
                                                    const PrefabResolver resolvePrefab, void* const userData) noexcept
    {
        if (m_impl == nullptr || !cell.IsOpen() || cell.CellId() == 0 || generation == 0 ||
            groupId == world::AlwaysActiveGroup || ownerId == InvalidActivationOwnerId || resolvePrefab == nullptr)
            return Result::InvalidArgument;
        Impl::CellRecord* const record = m_impl->Find(cell.CellId());
        ...
        if (lease == nullptr) return Result::OutOfMemory;
        ...
        while (changed)
        {
            ...
            for (const world::EntityReferenceRecord& reference : cell.EntityReferences())
            {
                if (reference.kind != world::EntityReferenceKind::RequiredLocal) continue;
                const world::PlacementRecord* const source = cell.FindPlacement(reference.sourceEntityId);
                const world::PlacementRecord* const target = cell.FindPlacement(reference.targetEntityId);
                if (source == nullptr || target == nullptr) { DeleteMaterializerObject(lease); return Result::InvalidReference; }
            ...
            for (const world::PlacementRecord& placement : cell.Placements())
            {
                ...
                if (!sourceGroupSelected || placement.parentEntityId == ecs::InvalidEntityId) continue;
                const world::PlacementRecord* const parent = cell.FindPlacement(placement.parentEntityId);
                if (parent == nullptr) continue;
                bool alreadySelected = false;
                for (const u64 selectedGroup : lease->groups) alreadySelected |= selectedGroup == parent->activationGroup;
                if (!alreadySelected)
                {
        ...
        {
            u8 base = 0;
            if (record->baseGroups.Find(dependencyGroup, base)) continue;
            Impl::GroupRecord* existing = m_impl->FindGroup(*record, dependencyGroup);
            if (existing != nullptr)
            {
                if (existing->state == ActivationGroupState::PendingRelease ||
                    existing->state == ActivationGroupState::Failed)
                {
            ...
            for (const world::PlacementRecord& placement : cell.PlacementsInGroup(*authoredGroup))
                if (IsEligiblePlacement(cell, placement, m_impl->config.includeEditorData))
            ...
            report.cellId = cell.CellId();
            report.generation = generation;
            const Result planResult = m_impl->PlanSelection(cell, selected, resolvePrefab, userData, plannedEntities,
                                                            plannedComponents, expected, report);
            ...
            {
                u8 sourceSelected = 0;
                if (selected.Find(reference.sourceEntityId, sourceSelected)) group->references.PushBack(reference);
            }
        ...
        for (const u64 dependencyGroup : lease->groups)
        {
            ...
            if (group != nullptr) ++group->retainCount;
        }
        record->leases.PushBack(lease);
        return Result::Success;
    }

    Result CellMaterializer::SynchronizeActivationGroups(const u64 cellId, const u32 generation) noexcept
    {
        if (m_impl == nullptr || cellId == 0 || generation == 0) return Result::InvalidArgument;
        Impl::CellRecord* const record = m_impl->Find(cellId);
        if (record == nullptr) return Result::InvalidState;
        if (record->generation != generation) return Result::StaleGeneration;
        bool pending = false;
        ...
        for (auto iterator = record->groups.Begin(), end = record->groups.End(); iterator != end; ++iterator)
        {
            ...
            if (group->state == ActivationGroupState::PendingActivation)
            {
                ...
                for (const ecs::EntityId identity : group->entities)
                    if (!m_impl->world->Resolve(identity)) { group->state = ActivationGroupState::Failed; return Result::QueueFailure; }
                ...
                if (!m_impl->world->RetireCommandBatch(group->commandBatch)) return Result::InvalidState;
                group->commandBatch = {};
                if (m_impl->references->RegisterPartition(
                        cellId, generation, group->groupId, containers::ArraySpan<const ecs::EntityId>(group->entities),
                        containers::ArraySpan<const world::EntityReferenceRecord>(group->references)) != ReferenceResult::Success)
                {
                    group->state = ActivationGroupState::Failed;
                    return Result::InvalidReference;
                }
                group->referencesPublished = true;
                group->state = m_impl->references->IsPartitionReady(cellId, generation, group->groupId)
                                   ? ActivationGroupState::Active : ActivationGroupState::PendingReferences;
            ...
            if (group->state == ActivationGroupState::PendingRelease)
            {
                ...
                if (status != ecs::CommandBatchStatus::Succeeded) return Result::QueueFailure;
                for (const ecs::EntityId identity : group->entities)
                    if (m_impl->world->Resolve(identity)) return Result::QueueFailure;
                if (!m_impl->world->RetireCommandBatch(group->commandBatch)) return Result::InvalidState;
                group->commandBatch = {};
        ...
        for (auto iterator = record->groups.Begin(), end = record->groups.End(); iterator != end; ++iterator)
        {
            Impl::GroupRecord* const group = iterator.Value();
            if (group->state == ActivationGroupState::PendingReferences &&
                m_impl->references->IsPartitionReady(cellId, generation, group->groupId))
                group->state = ActivationGroupState::Active;
            pending |= group->state == ActivationGroupState::PendingActivation ||
                       group->state == ActivationGroupState::PendingReferences ||
                       group->state == ActivationGroupState::PendingRelease;
        }
        return pending ? Result::NotReady : Result::Success;
    }

    Result CellMaterializer::ReleaseActivationGroup(const u64 cellId, const u32 generation, const u64 groupId,
                                                    const ActivationOwnerId ownerId) noexcept
    {
        if (m_impl == nullptr || cellId == 0 || generation == 0 || groupId == world::AlwaysActiveGroup ||
            ownerId == InvalidActivationOwnerId) return Result::InvalidArgument;
        Impl::CellRecord* const record = m_impl->Find(cellId);
        if (record == nullptr) return Result::InvalidState;
        if (record->generation != generation) return Result::StaleGeneration;
        Impl::GroupLease* const lease = m_impl->FindLease(*record, groupId, ownerId);
        if (lease == nullptr) return Result::InvalidState;

        containers::DynamicArray<u64> removeImmediately(memory::pools::World::GetInstance());
        for (const u64 dependencyGroup : lease->groups)
        {
            Impl::GroupRecord* const group = m_impl->FindGroup(*record, dependencyGroup);
            if (group == nullptr || group->retainCount > 1u) continue;
            if (group->state == ActivationGroupState::PendingActivation)
            {
                if (!m_impl->world->CancelCommandBatch(group->commandBatch)) return Result::InvalidState;
                group->commandBatch = {};
                m_impl->ReleaseReservations(*group);
                removeImmediately.PushBack(dependencyGroup);
                continue;
            }
            if (group->state != ActivationGroupState::Active &&
                group->state != ActivationGroupState::PendingReferences &&
                group->state != ActivationGroupState::Failed) return Result::InvalidState;
            const ecs::CommandBatch batch = m_impl->world->BeginCommandBatch();
            if (!batch) return Result::OutOfMemory;
            for (u32 index = group->entities.Size(); index > 0; --index)
            {
                if (!m_impl->world->Resolve(group->entities[index - 1u]) ||
                    m_impl->world->QueueDestroy(batch, group->entities[index - 1u])) continue;
            ...
            if (group->referencesPublished &&
                m_impl->references->UnregisterPartition(cellId, generation, group->groupId) != ReferenceResult::Success)
            {
                static_cast<void>(m_impl->world->CancelCommandBatch(batch));
                return Result::InvalidReference;
            }
        ...
        {
            Impl::GroupRecord* const group = m_impl->FindGroup(*record, dependencyGroup);
            if (group != nullptr && group->retainCount > 0) --group->retainCount;
        }
        ...
        {
            if (record->leases[index] != lease) continue;
    ...
    ActivationGroupState CellMaterializer::ActivationState(const u64 cellId, const u64 groupId) const noexcept
    {
        if (m_impl == nullptr || cellId == 0) return ActivationGroupState::Inactive;
        const Impl::CellRecord* const record = m_impl->Find(cellId);
        if (record == nullptr) return ActivationGroupState::Inactive;
        u8 base = 0;
        if (record->baseGroups.Find(groupId, base)) return ActivationGroupState::Active;
    ...
    u32 CellMaterializer::ActivationOwnerCount(const u64 cellId, const u64 groupId) const noexcept
    {
        if (m_impl == nullptr || cellId == 0) return 0;
        const Impl::CellRecord* const record = m_impl->Find(cellId);
        if (record == nullptr) return 0;
        const Impl::GroupRecord* const group = m_impl->FindGroup(*record, groupId);
        if (group != nullptr) return group->retainCount;
        u32 owners = 0;
        for (const Impl::GroupLease* const lease : record->leases)
            for (const u64 retainedGroup : lease->groups) owners += retainedGroup == groupId;
        return owners;
    }

    CellState CellMaterializer::State(const u64 cellId) const noexcept
    {
        if (m_impl == nullptr) return CellState::Unknown;
        const Impl::CellRecord* const record = m_impl->Find(cellId);
        if (record == nullptr) return CellState::Unknown;
    ...
    u32 CellMaterializer::Generation(const u64 cellId) const noexcept
    {
    ...
    MaterializerStats CellMaterializer::GetStats() const noexcept
    {
        ...
        for (auto iterator = m_impl->lookup.Begin(), end = m_impl->lookup.End(); iterator != end; ++iterator)
        {
            ...
            const CellState visibleState = State(record->cellId);
            ...
            for (auto groupIterator = record->groups.Begin(), groupEnd = record->groups.End();
                 groupIterator != groupEnd; ++groupIterator)
            {
