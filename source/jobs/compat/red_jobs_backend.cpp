...
namespace vanguard::jobs
{
    struct BackendContextAccess
    {
        ...
        [[nodiscard]] static const void* Get(
...
namespace
{
    ...
    T* AllocateEngineObject(Args&&... args) noexcept
    {
        using Envelope = EngineObjectEnvelope<T>;

        vanguard::memory::MemoryBlock allocation =
            vanguard::memory::Allocate(
                vanguard::memory::PoolId::Jobs,
                sizeof(Envelope),
                alignof(Envelope));
        if (!allocation)
        {
    ...
    template<typename T>
    void FreeEngineObject(T* const object) noexcept
    {
        if (object == nullptr)
        {
            return;
        ...
        using Envelope = EngineObjectEnvelope<T>;
        auto* const envelope = reinterpret_cast<Envelope*>(
            reinterpret_cast<vanguard::byte*>(object) -
            offsetof(Envelope, object));
    ...
    {
        explicit JobNameBackend(const char* staticName) noexcept
            : instrumentation(staticName != nullptr ? staticName : "")
        {
        }
    ...
    [[nodiscard]] job::Priority ToRedPriority(
        const vanguard::jobs::Priority priority) noexcept
    {
    ...
    [[nodiscard]] job::ScheduleParam ToRedSchedule(
        const vanguard::jobs::Schedule schedule) noexcept
    {
        return {
            ToRedPriority(schedule.priority),
            job::Affinity::All
        };
    }

    [[nodiscard]] vanguard::jobs::JobContext MakeContext(
        const job::RunContext& redContext) noexcept
    {
    ...
    void RunParallelTask(
        const ParallelTaskPacket task,
        const vanguard::u32 index,
        const job::RunContext& redContext) noexcept
    {
        const vanguard::jobs::JobContext context = MakeContext(redContext);
    ...
    void RunParallelEpilogue(
        const ParallelTaskPacket parallelTask,
        const TaskPacket epilogue,
        JobNameBackend& name,
        const job::RunContext& redContext) noexcept
    {
...
namespace vanguard::jobs::backend
{
    ...
    bool Initialize(const Config& config) noexcept
    {
        ...
        if (config.maxLatentJobs == 0 ||
            config.maxCriticalPathJobs == 0 ||
            config.maxImmediateJobs == 0 ||
            config.workerStackSizeKiB == 0 ||
    ...
    bool Shutdown() noexcept
    {
        ...
        if (g_liveBuilders.GetValue() != 0 ||
            g_liveCounters.GetValue() != 0 ||
            g_activeDeferrals.GetValue() != 0 ||
            g_outstandingJobs.GetValue() != 0)
        {
            return false;
        }
    ...
    u32 WorkerCount() noexcept
    {
        return IsInitialized()
            ? static_cast<u32>(job::GetNumDispatcherThreads())
            : 0;
    }
    ...
    SchedulerStats GetSchedulerStats() noexcept
    {
        ...
        stats.workerCount = stats.initialized ? WorkerCount() : 0;
        stats.outstandingJobs =
            static_cast<u32>(g_outstandingJobs.GetValue());
        stats.liveBuilders = static_cast<u32>(g_liveBuilders.GetValue());
        stats.liveCounters = static_cast<u32>(g_liveCounters.GetValue());
        stats.activeDeferrals =
            static_cast<u32>(g_activeDeferrals.GetValue());
        ...
        {
            stats.queued.latent = static_cast<u32>(
                job::GetApproximateQueueDepth(job::Priority::Latent));
            stats.queued.renderPath = static_cast<u32>(
                job::GetApproximateQueueDepth(job::Priority::RenderPath));
            stats.queued.criticalPath = static_cast<u32>(
                job::GetApproximateQueueDepth(job::Priority::CriticalPath));
            stats.queued.immediate = static_cast<u32>(
                job::GetApproximateQueueDepth(job::Priority::Immediate));
        }
        return stats;
    }
    ...
    {
        return IsInitialized() && DispatcherThreadIndex() != 0 &&
               DispatcherThreadIndex() != UINT32_MAX;
    }
    ...
    void RegisterCurrentThread(const char* name) noexcept
    {
        if (IsInitialized())
        {
            red::memory::RegisterCurrentThread(
                name != nullptr ? name : "VanguardJobProducer");
        }
    }
    ...
    {
        static_assert(
            sizeof(JobNameBackend) <= 64,
            "Vanguard JobName storage is too small for RED instrumentation.");
        static_assert(
            alignof(JobNameBackend) <= 16,
            "Vanguard JobName alignment is too small for RED instrumentation.");
    ...
    void DestroyJobName(void* storage) noexcept
    {
        ...
        RED_FATAL_ASSERT(
            name->inFlight.GetValue() == 0,
            "A Vanguard JobName was destroyed while jobs still referenced it.");
        name->~JobNameBackend();
    }

    const char* GetJobName(const void* storage) noexcept
    {
        return static_cast<const JobNameBackend*>(storage)
            ->instrumentation.m_name;
    }

    void* CreateBuilder(
        const Schedule schedule,
        const void* debugUserData) noexcept
    {
        if (!IsInitialized())
        {
            return nullptr;
        ...
        auto* builder = AllocateEngineObject<job::Builder>(
            ToRedSchedule(schedule),
            debugUserData);
        if (builder == nullptr)
        {
            return nullptr;
    ...
    void* CreateContinuationBuilder(const JobContext& context) noexcept
    {
        ...
        const auto* redContext = static_cast<const job::RunContext*>(
            BackendContextAccess::Get(context));
        ...
        const Fence fence) noexcept
    {
        ...
        const auto callable =
            [task, name](const job::RunContext& context) noexcept
            {
                RunTask(task, *name, context);
            };
        ...
        if (fence == Fence::Full)
        {
            importedBuilder.DispatchJob<job::Fence::Full>(
                name->instrumentation,
                callable);
        }
        else
        {
            importedBuilder.DispatchJob<job::Fence::None>(
                name->instrumentation,
                callable);
        }
        return true;
    ...
    bool DispatchAfter(
        ...
        const Fence fence) noexcept
    {
        if (builder == nullptr || dependency == nullptr ||
            jobName == nullptr || task.state == nullptr)
        {
            return false;
        }

        auto& importedBuilder = *static_cast<job::Builder*>(builder);
        const auto& importedDependency =
            *static_cast<const job::Counter*>(dependency);
        auto* name = static_cast<JobNameBackend*>(jobName);

        name->inFlight.Increment();
        g_outstandingJobs.Increment();
        g_submittedJobs.Increment();
        importedBuilder.DispatchJobAfterWait_NoFence(
            importedDependency,
            name->instrumentation,
            [task, name](const job::RunContext& context) noexcept
            {
    ...
    bool DispatchParallel(
        ...
        const detail::ParallelTaskPacket task,
        const detail::TaskPacket epilogue,
        const u32 maximumBatchSize,
        const Fence fence) noexcept
    {
        if (builder == nullptr || jobName == nullptr || task.state == nullptr)
        {
        ...
        const auto parallelCallable =
            [task](
                const u32 index,
                const job::RunContext& context) noexcept
            {
                RunParallelTask(task, index, context);
            };
        const auto epilogueCallable =
            [task, epilogue, name](
                const job::RunContext& context) noexcept
            {
                RunParallelEpilogue(task, epilogue, *name, context);
            };
        ...
        if (maximumBatchSize != 0)
        {
            ...
            {
                importedBuilder
                    .DispatchParallelForJobWithEpilogueWithBatchSize<
                        job::Fence::Full>(
                        name->instrumentation,
                        job::ImmediateValue{elementCount},
                        parallelCallable,
                        epilogueCallable,
                        maximumBatchSize);
            }
        ...
        {
            importedBuilder.DispatchParallelForJobWithEpilogue<
                job::Fence::Full>(
                name->instrumentation,
                job::ImmediateValue{elementCount},
                parallelCallable,
    ...
    {
        static_cast<job::Builder*>(builder)->DispatchWait(
            *static_cast<const job::Counter*>(dependency));
    }
    ...
    {
        auto* counter = AllocateEngineObject<job::Counter>(
            static_cast<job::Builder*>(builder)->ExtractWaitCounter());
        if (counter == nullptr)
        {
            return nullptr;
        }
    ...
    {
        return static_cast<const job::Counter*>(counter)
            ->Internal_IsZeroSnapshot();
    }

    bool WaitCounter(
        const void* counter,
        const bool processLatent,
        const i32 timeoutMilliseconds) noexcept
    {
        g_waitCalls.Increment();
        const bool completed = job::FlushCounter(
            *static_cast<const job::Counter*>(counter),
            processLatent,
            timeoutMilliseconds);
        if (!completed)
        {
        ...
        const void* debugUserData) noexcept
    {
        ...
        auto* deferral = AllocateEngineObject<job::CompletionDeferral>(
            importedCounter.CreateDeferral(
                debugUserData,
                staticDebugName != nullptr ? staticDebugName : ""));
        if (deferral == nullptr)
        {
            return nullptr;
        }
    ...
    void DestroyDeferral(void* deferral) noexcept
    {
        ...
        auto* imported =
            static_cast<job::CompletionDeferral*>(deferral);
    ...
    {
        return deferral != nullptr &&
               static_cast<const job::CompletionDeferral*>(deferral)
                   ->GetDebugIsFinished();
    }
