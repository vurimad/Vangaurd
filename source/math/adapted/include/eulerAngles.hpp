...
namespace vanguard::math
{
    ...
    EulerAngles::EulerAngles(const Float roll, const Float pitch, const Float yaw) : Roll(roll), Pitch(pitch), Yaw(yaw) {}

    EulerAngles::EulerAngles(const EulerAngles& ea) : EulerAngles{ea.Roll, ea.Pitch, ea.Yaw} {}

    EulerAngles::EulerAngles(const Vector3& v) : EulerAngles{v.X, v.Y, v.Z} {}

    EulerAngles::EulerAngles(const Float f[3]) : EulerAngles{f[0], f[1], f[2]} {}
    ...
    RED_INLINE Bool EulerAngles::AlmostEquals(const EulerAngles& a, const Float epsilon /* = 0.01f */) const
    {
        ...
        return (r < epsilon || r > hl) && (p < epsilon || p > hl) && (y < epsilon || y > hl);
    }
    ...
    RED_INLINE Float EulerAngles::NormalizeAngle(Float angle)
    {
        ...
        if (cycles != 0)
            angle -= cycles * 360.0f; // angle e ( -360, 360 )
        if (angle < 0)
            angle += 360; // angle e ( 0, 360 )
        return angle;
    }
