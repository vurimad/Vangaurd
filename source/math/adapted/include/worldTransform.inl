...
namespace vanguard::math
{
    ...
    RED_INLINE WorldTransform::WorldTransform(EIdentity) : m_position(WorldPosition::ZEROS()), m_orientation(Quaternion::IDENTITY()) {}

    RED_INLINE WorldTransform::WorldTransform(const WorldPosition& p) : m_position(p), m_orientation(Quaternion::IDENTITY()) {}
    ...
    RED_INLINE WorldTransform::WorldTransform(const WorldPosition& p, const Quaternion& q) : m_position(p), m_orientation(q.Normalized())
    {
        RED_MATH_PARANOID_SANITY_CHECK(IsOk());
    }

    RED_INLINE WorldTransform::WorldTransform(const Quaternion& q) : m_position(WorldPosition::ZEROS()), m_orientation(q.Normalized())
    {
        RED_MATH_PARANOID_SANITY_CHECK(IsOk());
    }
    ...
    RED_INLINE WorldTransform::WorldTransform(const Vector3& p) : m_position(p), m_orientation(Quaternion::IDENTITY())
    {
        RED_MATH_PARANOID_SANITY_CHECK(IsOk());
    }
    RED_INLINE WorldTransform::WorldTransform(const Vector3& p, const Quaternion& q) : m_position(p), m_orientation(q.Normalized())
    {
        RED_MATH_PARANOID_SANITY_CHECK(IsOk());
    }
    ...
    RED_INLINE Box WorldTransform::TransformBox(const Box& box) const
    {
        ...
        return Box(posV3 + Vector3::Min(xa, xb) + Vector3::Min(ya, yb) + Vector3::Min(za, zb),
