...
{
    template <typename T> RED_INLINE constexpr const T& Max(const T& a, const T& b)
    {
        return std::max(a, b);
    }

    template <typename T> RED_INLINE constexpr const T& Min(const T& a, const T& b)
    {
        return std::min(a, b);
    }

    template <typename T, typename... Tx> RED_INLINE constexpr const T& Max(const T& a, const T& b, Tx&&... args)
    {
        return Max(Max(a, b), std::forward<Tx>(args)...);
    }

    template <typename T, typename... Tx> RED_INLINE constexpr const T& Min(const T& a, const T& b, Tx&&... args)
    {
        return Min(Min(a, b), std::forward<Tx>(args)...);
    }

    template <typename T> RED_INLINE constexpr T Clamp(const T& x, const T& min, const T& max)
    {
        return Min(Max(x, min), max);
    }

    template <typename T> RED_INLINE constexpr T ClampFromAbove(const T& x, const T& max)
    {
        return Min(x, max);
    }

    template <typename T> RED_INLINE constexpr T ClampFromBelow(const T& x, const T& min)
    {
        return Max(x, min);
    }

    template <typename T> RED_INLINE constexpr T Abs(const T& a)
    {
        return (a < T{0}) ? -a : a;
    }

    template <typename T> RED_INLINE constexpr int Sgn(const T& a)
    {
        return (a > T{0}) ? 1 : (a < T{0} ? -1 : 0);
    }

    template <typename T> RED_INLINE void Swap(T& a, T& b)
    {
        std::swap(a, b);
    }
    ...
    template <typename T> constexpr T Lerp(float t, const T& src, const T& dst)
    {
    ...
    template <typename T>
    RED_FORCE_INLINE constexpr bool RangesOverlap(const std::pair<T, T>& rangeA, const std::pair<T, T>& rangeB,
                                                  const T& eps = std::numeric_limits<T>::epsilon())
    {
        return (AlmostEqualsOrSmaller(rangeA.first, rangeB.second) && AlmostEqualsOrGreater(rangeA.second, rangeB.first));
    }

    template <typename T> RED_FORCE_INLINE constexpr bool EqualsZero(const T& val1, const T& eps = std::numeric_limits<T>::epsilon())
    {
    ...
    template <typename T, typename Q> RED_INLINE T Pow(const T& a, const Q& b)
    {
