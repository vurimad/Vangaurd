...
namespace
{
    ...
    struct ManagedValue
    {
        ...
        ManagedValue() noexcept
        {
            ++liveInstances;
        }
        explicit ManagedValue(const int initialValue) noexcept : value(initialValue)
        {
            ++liveInstances;
        }
        ManagedValue(const ManagedValue& other) noexcept : value(other.value)
        {
            ++liveInstances;
        }
        ManagedValue(ManagedValue&& other) noexcept : value(other.value)
        {
            ++liveInstances;
        }
        ManagedValue& operator=(const ManagedValue& other) noexcept
        {
            value = other.value;
            return *this;
        }
...
int main()
{
    ...
    Check(vanguard::diagnostics::Initialize(vanguard::diagnostics::Mode::Synchronous, "ecsTests"), "diagnostics initialize");
    ...
    Check(world.FlushActions(&report) && report.created == 2 && report.rejected == 0, "flush deterministic entity creation batch");
    const ecs::Entity first = world.Resolve(100);
    Check(first && world.IsAlive(first) && world.GetIdentity(first) == 100, "stable identity resolves to live runtime entity");
    ...
    Check(!world.QueueAddComponent(100, foreignPositionType), "reject a component token registered by a different ECS world");
    Check(secondWorld.Shutdown(), "shutdown the empty token ownership test world");
    ecs::ComponentActionReport componentReport;
    Check(world.QueueAddComponent(100, positionType) && world.FlushComponentActions(&componentReport) && componentReport.added == 1 &&
              componentReport.rejected == 0,
          "add a component at an explicit structural synchronization point");
    Check(world.QueueSetComponent(100, positionType, Position{4.0f}) && world.FlushComponentActions(&componentReport) && componentReport.set == 1,
          "copy a queued component value into the live entity");

    const ecs::CommandBatch materializationBatch = world.BeginCommandBatch();
    Check(materializationBatch && world.QueueCreate(materializationBatch, 300) && world.QueueAddComponent(materializationBatch, 300, positionType) &&
              world.QueueSetComponent(materializationBatch, 300, positionType, Position{30.0f}) && world.SealCommandBatch(materializationBatch),
          "seal one owned entity-and-component command batch");
    ecs::CommandBatchReport batchReport;
    Check(!world.QueueCreate(300) && world.GetCommandBatchStatus(materializationBatch, &batchReport) == ecs::CommandBatchStatus::Pending &&
              batchReport.queued == 3 && batchReport.completed == 0,
          "pending identity reservation rejects competing entity creation");
    Check(world.FlushActions(&report) && world.GetCommandBatchStatus(materializationBatch, &batchReport) == ecs::CommandBatchStatus::Pending &&
    ...
    Check(world.FlushComponentActions(&componentReport) &&
              world.GetCommandBatchStatus(materializationBatch, &batchReport) == ecs::CommandBatchStatus::Succeeded && batchReport.completed == 3 &&
              batchReport.rejected == 0 && world.RetireCommandBatch(materializationBatch),
    ...
    Check(cancelledBatch && survivingBatch && world.QueueCreate(cancelledBatch, 400) && world.QueueAddComponent(cancelledBatch, 400, positionType) &&
              world.QueueSetComponent(cancelledBatch, 400, positionType, Position{40.0f}) && world.SealCommandBatch(cancelledBatch) &&
              world.QueueCreate(survivingBatch, 500) && world.SealCommandBatch(survivingBatch) && world.CancelCommandBatch(cancelledBatch),
          "cancel only one producer-owned command batch");
    const ecs::WorldStats cancelledStats = world.GetStats();
    Check(cancelledStats.pendingActions == 1 && cancelledStats.pendingComponentActions == 0 && cancelledStats.commandBatches == 1 &&
              world.FlushActions(&report) && !world.Resolve(400) && world.Resolve(500) &&
              world.GetCommandBatchStatus(survivingBatch) == ecs::CommandBatchStatus::Succeeded && world.RetireCommandBatch(survivingBatch),
    ...
    Check(failedBatch && world.QueueCreate(failedBatch, 100) && world.SealCommandBatch(failedBatch) && world.FlushActions(&report) && report.rejected == 1 &&
              world.GetCommandBatchStatus(failedBatch, &batchReport) == ecs::CommandBatchStatus::Failed && batchReport.rejected == 1 &&
              world.RetireCommandBatch(failedBatch),
    ...
    Check(world.QueueAddComponent(200, managedType) && world.FlushComponentActions(&componentReport), "add a component with non-trivial lifecycle hooks");
    {
        ...
        flecs::world native = ecs::GetNative(world);
    ...
    Check(world.QueueRemoveComponent(200, managedType) && world.FlushComponentActions(&componentReport) && ManagedValue::liveInstances == 0,
    ...
    Check(world.GetProgress(1.0f / 60.0f), "progress Flecs system pipeline");
    {
        flecs::world native = ecs::GetNative(world);
    ...
    Check(world.QueueAddComponent(100, positionType) && world.QueueRemoveComponent(100, positionType) &&
              world.QueueSetComponent(100, positionType, Position{8.0f}) && world.FlushComponentActions(&componentReport) && componentReport.removed == 1 &&
              componentReport.rejected == 2,
          "duplicate add and set-after-remove are rejected without repairing caller ordering");

    Check(world.QueueDestroy(100) && world.FlushActions(&report) && report.destroyed == 1 && !world.IsAlive(first) && !world.Resolve(100),
          "destroy invalidates previous generational runtime entity");
    Check(world.QueueCreate(100) && world.FlushActions(&report) && report.created == 1, "stable identity may be materialized again after unload");
    const ecs::Entity replacement = world.Resolve(100);
    Check(replacement && replacement != first && world.GetIdentity(replacement) == 100, "rematerialized identity receives a different Flecs generation");
    ...
    for (vanguard::u32 thread = 0; thread < threadCount; ++thread)
    {
        threads[thread] = std::thread(
            [thread, &world, &queuedSuccessfully, positionType]()
            {
                ...
                for (vanguard::u32 index = 0; index < entitiesPerThread; ++index)
                {
                    ...
                    queuedSuccessfully[thread] = world.QueueCreate(identity) && world.QueueAddComponent(identity, positionType) &&
                                                 world.QueueSetComponent(identity, positionType, position) && queuedSuccessfully[thread];
                }
            });
    }
    ...
    Check(world.FlushActions(&report) && report.created == threadCount * entitiesPerThread, "serialized sync point commits concurrent entity requests");
    Check(world.FlushComponentActions(&componentReport) && componentReport.added == threadCount * entitiesPerThread &&
    ...
    Check(activeStats.addedComponents == 3 + threadCount * entitiesPerThread && activeStats.setComponents == 3 + threadCount * entitiesPerThread &&
    ...
    Check(world.FlushActions(&report) && report.destroyed == 2 + threadCount * entitiesPerThread, "destroy every stable entity at explicit sync point");
    ...
    Check(activeMetrics.allocationCount > beforeMetrics.allocationCount, "Flecs allocations are visible through the Gameplay memory pool telemetry");
#endif
    ...
    Check(journalWorld.Initialize(journalConfig) && journalWorld.QueueCreate(1) && journalWorld.QueueCreate(2) && journalWorld.QueueCreate(3) &&
              journalWorld.FlushActions(),
    ...
    vanguard::containers::DynamicArray<ecs::CommittedChange> journalChanges(vanguard::memory::pools::Gameplay::GetInstance());
    Check(journalWorld.ReadCommittedChanges(slowCursor, journalChanges, 8, &journalRead) && journalRead.lostRecords == 1 && journalRead.records == 2 &&
    ...
    Check(journalWorld.ReadCommittedChanges(independentCursor, journalChanges, 8, &journalRead) && journalRead.lostRecords == 0 && journalRead.records == 2,
          "retain independent committed-change cursors without destructive consumption");
    Check(journalWorld.QueueDestroy(1) && journalWorld.QueueDestroy(2) && journalWorld.QueueDestroy(3) && journalWorld.FlushActions() &&
              journalWorld.Shutdown(),
