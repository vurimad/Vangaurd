...
namespace vanguard::rhi::d3d12
{
    namespace
    {
        IBackend* CreateBackend() noexcept
        {
            memory::MemoryBlock block = memory::Allocate(memory::PoolId::Rendering, sizeof(Backend), alignof(Backend));
            return block ? ::new (block.address) Backend() : nullptr;
        }

        void DestroyBackend(IBackend* const backend) noexcept
        {
            if (backend == nullptr)
                return;
            static_cast<Backend*>(backend)->~Backend();
            memory::MemoryBlock block{backend, sizeof(Backend), memory::PoolId::Rendering};
            memory::Free(block);
        }
    } // namespace

    BackendFactory GetBackendFactory() noexcept
    {
        return {&CreateBackend, &DestroyBackend};
    }
    ...
    namespace
    {
        ...
        [[nodiscard]] bool IsPresentParametersValid(const PresentParameters& parameters) noexcept
        {
            ...
            return parameters.synchronizationInterval >= 1 && parameters.synchronizationInterval <= 4 && !parameters.allowTearing;
        }
        ...
        {
            const u64 frequency = system::GetMonotonicFrequency();
            if (frequency == 0)
                return 0;
            constexpr u64 NanosecondsPerSecond = 1'000'000'000ull;
            return (ticks / frequency) * NanosecondsPerSecond + ((ticks % frequency) * NanosecondsPerSecond) / frequency;
        }
        ...
        [[nodiscard]] DisplayColorCapabilities QueryDisplayColorCapabilities(IDXGISwapChain4& swapChain) noexcept
        {
            ...
            output->Release();
            if (FAILED(query) || output6 == nullptr)
                return capabilities;
            ...
            capabilities.hdrActive = capabilities.hdr10Output || desc.ColorSpace == DXGI_COLOR_SPACE_RGB_FULL_G10_NONE_P709;
            return capabilities;
        }
        ...
        {
            if (payload.native == nullptr || payload.debugName[0] == '\0')
                return;
            ...
            for (u32 index = 0; index < payload.bufferCount; ++index)
            {
                ...
                for (u32 suffixIndex = 0; suffixIndex + 1u < static_cast<u32>(_countof(suffix)) && write + 1u < static_cast<u32>(_countof(name)); ++suffixIndex)
                    name[write++] = suffix[suffixIndex];
                if (write >= 2u)
                    name[write - 2u] = static_cast<wchar_t>(L'0' + index);
                name[write] = L'\0';
        ...
        [[nodiscard]] bool WaitForPresentFence(SwapChainPayload& payload, const u64 value, bool* const waited = nullptr) noexcept
        {
            if (waited != nullptr)
                *waited = false;
            if (value == 0)
                return true;
            if (payload.presentFence == nullptr || payload.presentFenceEvent == nullptr)
                return false;
            if (payload.presentFence->GetCompletedValue() >= value)
                return true;
            if (waited != nullptr)
                *waited = true;
            if (FAILED(payload.presentFence->SetEventOnCompletion(value, payload.presentFenceEvent)))
                return false;
        ...
        [[nodiscard]] bool WaitForAllPresents(SwapChainPayload& payload) noexcept
        {
            u64 latestFence = 0;
            for (u32 index = 0; index < payload.bufferCount; ++index)
                if (latestFence < payload.presentFenceValues[index])
                    latestFence = payload.presentFenceValues[index];
        ...
        BackendStatus ExecutePresent(void* const address) noexcept
        {
                ...
                FAILED(payload.presentationQueue->Signal(payload.presentFence, fenceValue)))
            {
                ++payload.stats.presentationFailures;
                payload.stats.state = SwapChainState::Failed;
                return BackendStatus::Failure(FailureCode::DeviceLost, 0, "failed to signal the swap-chain presentation fence");
            }
        ...
        [[nodiscard]] bool CreateBackBufferHandles(SwapChainPayload& payload) noexcept
        {
            ...
            for (u32 index = 0; index < payload.bufferCount; ++index)
            {
                ...
                payload.backBuffers[index] = payload.common->AdoptNativeTexture(static_cast<nvrhi::TextureHandle&&>(native), textureDesc);
        ...
        void DestroySwapChainPayload(void*, ResourceRef, void* const address) noexcept
        {
            ...
            ReleaseBackBufferHandles(*payload);
            if (payload->presentFenceEvent != nullptr)
                CloseHandle(payload->presentFenceEvent);
            payload->presentFenceEvent = nullptr;
            if (payload->frameLatencyWaitableObject != nullptr)
                CloseHandle(payload->frameLatencyWaitableObject);
            payload->frameLatencyWaitableObject = nullptr;
            if (payload->presentFence != nullptr)
                payload->presentFence->Release();
            payload->presentFence = nullptr;
            if (payload->native != nullptr)
                payload->native->Release();
                                                          ...
                                                          ID3D12Pageable** const pageables, u32& count) noexcept
        {
            ...
            for (const ResourceRef resource : resources)
            {
                ...
                if (!allocation.IsValid())
                    return BackendStatus::Failure(FailureCode::IncompatibleBinding, 0, "only textures, buffers and heaps have explicit residency");
                ID3D12Pageable* pageable = nullptr;
                if (allocation.GetKind() == ResourceKind::Heap)
                {
                    auto* const heap = static_cast<nvrhi::d3d12::Heap*>(common.GetNativeHeap(CastResourceRef<HeapRef>(allocation)));
                    pageable = heap != nullptr ? heap->heap.Get() : nullptr;
                }
                else
                    pageable = common.GetNativeObject(allocation, nvrhi::ObjectTypes::D3D12_Resource);
                if (pageable == nullptr)
                    return BackendStatus::Failure(FailureCode::InvalidReference, 0, "GPU residency batch contains a stale or unsupported resource");
                bool duplicate = false;
                for (u32 index = 0; index < count; ++index)
                    duplicate = duplicate || pageables[index] == pageable;
                if (!duplicate)
                    pageables[count++] = pageable;
            }
            return BackendStatus::Success();
        }
...
#pragma warning(disable : 4324)
    ...
    {
        enum class AllocationResidency : u8
        {
            Resident,
            Evicted
        };
        ...
        {
            if (allocation.GetKind() == ResourceKind::Texture && allocation.Index() < TextureResidencyCapacity)
                return allocation.Index();
            if (allocation.GetKind() == ResourceKind::Buffer && allocation.Index() < BufferResidencyCapacity)
                return TextureResidencyCapacity + allocation.Index();
            if (allocation.GetKind() == ResourceKind::Heap && allocation.Index() < HeapResidencyCapacity)
        ...
        [[nodiscard]] ResidencyRecord* FindResidencyRecord(const ResourceRef allocation) noexcept
        {
            ...
            if (residencyRecords == nullptr || index == InvalidReferenceIndex)
                return nullptr;
        ...
        {
            if (residencyRecords != nullptr)
                return false;
            residencyRecordBlock = memory::Allocate(memory::PoolId::Rendering, sizeof(ResidencyRecord) * ResidencyRecordCapacity, alignof(ResidencyRecord));
            if (!residencyRecordBlock)
                return false;
        ...
        void RemoveRecordAccounting(const ResidencyRecord& record) noexcept
        {
            ...
            if (record.pinned)
                --residencyStats.policyPinnedObjects;
        }

        [[nodiscard]] bool RegisterAllocation(const ResourceRef allocation, const u64 size, const ResidencyFenceSet& initialUse = {}) noexcept
        {
            const u32 index = ResidencyRecordIndex(allocation);
            if (residencyRecords == nullptr || index == InvalidReferenceIndex || size == 0)
                return false;
        ...
        void MarkResident(const ResourceRef allocation) noexcept
        {
            ...
            if (record == nullptr || record->state == AllocationResidency::Resident)
                return;
        ...
        static BackendStatus PrepareWorkingSet(void* const context, const containers::ArraySpan<const ResourceRef> resources) noexcept
        {
            ...
            for (const ResourceRef allocation : resources)
                impl.MarkResident(allocation);
            return BackendStatus::Success();
        }

        static void CommitWorkingSet(void* const context, const containers::ArraySpan<const ResourceRef> resources, const GpuFence completion) noexcept
        {
        ...
        static void ReleaseResidencyRecord(void* const context, const ResourceRef resource) noexcept
        {
            ...
            if (impl.residencyRecords == nullptr || index == InvalidReferenceIndex)
                return;
            concurrency::ScopedLock guard(impl.residencyLock);
        ...
        [[nodiscard]] static bool BetterEvictionCandidate(const EvictionCandidate& left, const EvictionCandidate& right) noexcept
        {
        ...
        static BackendStatus MaintainResidencyPolicy(void* const context) noexcept
        {
            ...
            const HRESULT budgetResult = impl.budgetAdapter->QueryVideoMemoryInfo(0, DXGI_MEMORY_SEGMENT_GROUP_LOCAL, &info);
            if (FAILED(budgetResult))
            {
                concurrency::ScopedLock guard(impl.residencyLock);
                ++impl.residencyStats.policyFailures;
                ++impl.residencyStats.residencyFailures;
                return BackendStatus::Failure(FailureCode::BackendFailure, budgetResult, "DXGI memory-budget query failed during residency maintenance");
            }
            ...
            const auto worse = [](const EvictionCandidate& left, const EvictionCandidate& right) noexcept { return BetterEvictionCandidate(right, left); };
            const auto siftDown = [&](const u32 start) noexcept
            {
                ...
                while (true)
                {
                    ...
                    u32 worst = left;
                    if (right < candidateCount && worse(candidates[right], candidates[left]))
                        worst = right;
                    if (!worse(candidates[worst], candidates[parent]))
                        break;
...

            {
                ...
                for (u32 index = 0; index < ResidencyRecordCapacity; ++index)
                {
                    ...
                    if (record.state != AllocationResidency::Resident || record.pinned)
                        continue;
                    if ((record.lastUse.graphics != 0 && !impl.common.IsGpuFenceComplete({QueueType::Graphics, record.lastUse.graphics})) ||
                        (record.lastUse.compute != 0 && !impl.common.IsGpuFenceComplete({QueueType::Compute, record.lastUse.compute})) ||
                        (record.lastUse.copy != 0 && !impl.common.IsGpuFenceComplete({QueueType::Copy, record.lastUse.copy})))
                    {
                    ...
                    {
                        if (!impl.common.TryAddRef(candidate.resource))
                            continue;
                        ...
                        while (child != 0)
                        {
                            ...
                            if (!worse(candidates[child], candidates[parent]))
                                break;
                            const EvictionCandidate temporary = candidates[parent];
                    ...
                    else if (BetterEvictionCandidate(candidate, candidates[0]) && impl.common.TryAddRef(candidate.resource))
                    {
            ...
            u32 pageableCount = 0;
            const BackendStatus resolution = ResolveResidencyBatch(impl.common, {resources, selectedCount}, pageables, pageableCount);
...

            {
                ...
                if (status)
                {
                    for (u32 index = 0; index < selectedCount; ++index)
                        impl.MarkEvicted(resources[index]);
        ...
        {
            if (!reference.IsValid() || reference.index >= MaximumQueryPools)
                return nullptr;
        ...
        static void QueryPoolSubmitted(void* const context, const GpuFence completion, const u64 resolved) noexcept
        {
            ...
            if (resolved != 0)
                payload->lastResolveFence = completion;
            if (payload->pendingSubmissions != 0)
                --payload->pendingSubmissions;
        }

        [[nodiscard]] bool TrackQueryPoolUse(const CommandListRef commandList, QueryPoolPayload& payload, const bool resolvesResults = false) noexcept
        {
            bool added = false;
            if (!common.RegisterCommandSubmissionCallback(commandList, &QueryPoolSubmitted, &payload, resolvesResults ? 1u : 0u, &added))
                return false;
            if (added)
                ++payload.pendingSubmissions;
            return true;
                                                 ...
                                                 const SubresourceRange& range, const Rect& rectangle) noexcept
        {
            ...
            const nvrhi::Object view = texture->getNativeView(nvrhi::ObjectTypes::D3D12_RenderTargetViewDescriptor, nvrhi::Format::UNKNOWN, subresources);
            if (nativeCommandList == nullptr || view.integer == 0)
                return false;
            const D3D12_CPU_DESCRIPTOR_HANDLE descriptor{view.integer};
            const D3D12_RECT nativeRectangle{rectangle.x, rectangle.y, rectangle.x + rectangle.width, rectangle.y + rectangle.height};
                                                        ...
                                                        const Rect& rectangle) noexcept
        {
            ...
            ID3D12GraphicsCommandList* const nativeCommandList = commandList->getNativeObject(nvrhi::ObjectTypes::D3D12_GraphicsCommandList);
            const nvrhi::TextureSubresourceSet subresources(range.firstMip, 1, range.firstSlice, range.sliceCount);
            const nvrhi::Object view = texture->getNativeView(nvrhi::ObjectTypes::D3D12_DepthStencilViewDescriptor, nvrhi::Format::UNKNOWN, subresources);
            if (nativeCommandList == nullptr || view.integer == 0)
                return false;
...
#pragma warning(pop)
    ...
    Backend::~Backend()
    {
        ...
        if (m_impl->initialized)
        {
            ...
            if (!shutdown)
            {
                VG_LOG_ERROR(diagnostics::Category::Rendering, "D3D12 backend required emergency teardown because live GPU resource owners remained");
    ...
    BackendStatus Backend::Initialize(const DeviceParams& params, Capabilities& capabilities) noexcept
    {
        ...
        if (!m_impl->InitializeResidencyPolicy(params.residencyPolicy))
            return BackendStatus::Failure(FailureCode::OutOfMemory, 0, "could not allocate the GPU residency-policy registry");
        ...
        const DXGI_GPU_PREFERENCE preference = params.preferHighPerformanceAdapter ? DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE : DXGI_GPU_PREFERENCE_UNSPECIFIED;
        ...
        for (u32 enumerationIndex = 0;; ++enumerationIndex)
        {
            ...
            const bool supported = hardware && SUCCEEDED(D3D12CreateDevice(candidate, D3D_FEATURE_LEVEL_12_0, __uuidof(ID3D12Device), nullptr));
        ...
        const D3D12_COMMAND_QUEUE_DESC graphicsDesc{D3D12_COMMAND_LIST_TYPE_DIRECT, D3D12_COMMAND_QUEUE_PRIORITY_NORMAL, D3D12_COMMAND_QUEUE_FLAG_NONE, 0};
        const D3D12_COMMAND_QUEUE_DESC computeDesc{D3D12_COMMAND_LIST_TYPE_COMPUTE, D3D12_COMMAND_QUEUE_PRIORITY_NORMAL, D3D12_COMMAND_QUEUE_FLAG_NONE, 0};
        const D3D12_COMMAND_QUEUE_DESC copyDesc{D3D12_COMMAND_LIST_TYPE_COPY, D3D12_COMMAND_QUEUE_PRIORITY_NORMAL, D3D12_COMMAND_QUEUE_FLAG_NONE, 0};
    ...
    BackendStatus Backend::SetResidencyPinned(const ResourceRef resource, const bool pinned) noexcept
    {
        ...
        Impl::ResidencyRecord* const record = m_impl->FindResidencyRecord(allocation);
        if (record == nullptr)
            return BackendStatus::Failure(FailureCode::IncompatibleBinding, 0, "GPU allocation is not managed by the residency policy");
    ...
    BackendStatus Backend::MakeResident(const containers::ArraySpan<const ResourceRef> resources) noexcept
    {
        if (m_impl == nullptr || !m_impl->initialized || m_impl->residencyDevice == nullptr)
            return BackendStatus::Failure(FailureCode::Unsupported, 0, "explicit D3D12 residency is unavailable");
        memory::MemoryBlock pageableBlock = memory::Allocate(memory::PoolId::Rendering, sizeof(ID3D12Pageable*) * resources.Size(), alignof(ID3D12Pageable*));
        if (!pageableBlock)
    ...
    BackendStatus Backend::Evict(const containers::ArraySpan<const ResourceRef> resources, const ResidencyFenceSet& safeAfter) noexcept
    {
        if (m_impl == nullptr || !m_impl->initialized || m_impl->residencyDevice == nullptr)
            return BackendStatus::Failure(FailureCode::Unsupported, 0, "explicit D3D12 residency is unavailable");
        memory::MemoryBlock pageableBlock = memory::Allocate(memory::PoolId::Rendering, sizeof(ID3D12Pageable*) * resources.Size(), alignof(ID3D12Pageable*));
        ...
        const BackendStatus status = m_impl->common.ExecuteSerializedQueueOperation(
            ...
            {
                auto& value = *static_cast<Operation*>(context);
                if ((value.safeAfter.graphics != 0 && !value.impl->common.IsGpuFenceComplete({QueueType::Graphics, value.safeAfter.graphics})) ||
                    (value.safeAfter.compute != 0 && !value.impl->common.IsGpuFenceComplete({QueueType::Compute, value.safeAfter.compute})) ||
                    (value.safeAfter.copy != 0 && !value.impl->common.IsGpuFenceComplete({QueueType::Copy, value.safeAfter.copy})))
                    return BackendStatus::Failure(FailureCode::Busy, 0, "GPU resources are still in flight and cannot be evicted");
                {
                    ...
                    for (const ResourceRef resource : value.resources)
                    {
                        ...
                        if (record != nullptr && record->pinned)
                            return BackendStatus::Failure(FailureCode::Busy, 0, "pinned GPU allocations cannot be evicted");
                    }
                }
                const HRESULT result = value.impl->residencyDevice->Evict(value.count, value.pageables);
                if (FAILED(result))
                    return BackendStatus::Failure(FailureCode::BackendFailure, result, "D3D12 GPU-memory eviction failed");
                concurrency::ScopedLock guard(value.impl->residencyLock);
                for (const ResourceRef resource : value.resources)
    ...
    BackendStatus Backend::WriteDescriptor(const DescriptorDomainRef domain, const DescriptorHandle descriptor, const SamplerStateRef sampler) noexcept
    {
    ...
    AccelerationStructureRef Backend::CreateAccelerationStructure(const AccelerationStructureDesc&) noexcept
    {
        return {};
    }
    ShaderTableRef Backend::CreateShaderTable(const ShaderTableDesc&) noexcept
    {
        return {};
    }
    BackendStatus Backend::GetAccelerationStructureDeviceAddress(AccelerationStructureRef, u64&) noexcept
    {
        return BackendStatus::Failure(FailureCode::Unsupported, 0, "acceleration-structure device addresses are not implemented");
    }
    QueryPoolRef Backend::CreateQueryPool(const QueryPoolDesc& desc) noexcept
    {
        ...
        if (slotIndex == MaximumQueryPools)
            return {};
    ...
    BackendStatus Backend::ResolveQueries(const CommandListRef commandList, const QueryPoolRef queryPool, const u32 start, const u32 count) noexcept
    {
        concurrency::ScopedLock guard(m_impl->queryPoolLock);
        QueryPoolPayload* const payload = m_impl->FindQueryPool(queryPool);
        nvrhi::ICommandList* const command = m_impl->common.GetNativeCommandList(commandList);
    ...
    BackendStatus Backend::AcquireQueries(const QueryPoolRef queryPool, const u32 start, const u32 count) noexcept
    {
        ...
        QueryPoolPayload* const payload = m_impl->FindQueryPool(queryPool);
        if (payload == nullptr)
            return BackendStatus::Failure(FailureCode::InvalidReference, 0, "invalid query pool");
        if (count == 0 || start >= payload->capacity || count > payload->capacity - start || payload->mapped != nullptr)
            return BackendStatus::Failure(FailureCode::InvalidArgument, 0, "invalid query acquisition range");
        if (payload->pendingSubmissions != 0 || !payload->lastResolveFence.IsValid() || !m_impl->common.IsGpuFenceComplete(payload->lastResolveFence))
            return BackendStatus::Failure(FailureCode::Busy, 0, "query results are not ready");
        D3D12_RANGE range{static_cast<SIZE_T>(start) * payload->resultStride, static_cast<SIZE_T>(start + count) * payload->resultStride};
    ...
    void Backend::ReleaseQueries(const QueryPoolRef queryPool) noexcept
    {
        concurrency::ScopedLock guard(m_impl->queryPoolLock);
        QueryPoolPayload* const payload = m_impl->FindQueryPool(queryPool);
        if (payload == nullptr || payload->mapped == nullptr)
            return;
    ...
    BackendStatus Backend::GetQueryResult(const QueryPoolRef queryPool, const u32 index, PipelineStatistics& result) noexcept
    {
        ...
        result = {native.IAVertices,  native.IAPrimitives,  native.VSInvocations, native.GSInvocations, native.GSPrimitives, native.CInvocations,
                  native.CPrimitives, native.PSInvocations, native.HSInvocations, native.DSInvocations, native.CSInvocations};
        return BackendStatus::Success();
    }
    ...
    {
        return BackendStatus::Failure(FailureCode::Unsupported, 0, "variable-rate shading backend support is not implemented");
    }
    ...
    BackendStatus Backend::BindIndirectArguments(const CommandListRef commandList, const BufferRef arguments, const BufferRef count) noexcept
    {
    ...
    BackendStatus Backend::ClearTextureUav(const CommandListRef commandList, const TextureRef texture, const u32 value, const SubresourceRange& range) noexcept
    {
        return m_impl->common.ClearTextureUav(commandList, texture, value, range);
    }
    ...
    BackendStatus Backend::CopyAccelerationStructure(CommandListRef, AccelerationStructureRef, AccelerationStructureRef, AccelerationStructureCopyMode) noexcept
    {
        return BackendStatus::Failure(FailureCode::Unsupported, 0, "acceleration-structure copy and compaction are not implemented");
    }
    BackendStatus Backend::WriteAccelerationStructureCompactedSize(CommandListRef, AccelerationStructureRef, QueryPoolRef, u32) noexcept
    {
        return BackendStatus::Failure(FailureCode::Unsupported, 0, "acceleration-structure compaction queries are not implemented");
    }
    ...
    BackendStatus Backend::WriteTexture(const CommandListRef commandList, const TextureRef texture, const TextureSubresourceData& data) noexcept
    {
    ...
    {
        if (m_impl == nullptr || !m_impl->initialized || !IsSwapChainDescriptorSupported(desc) || !IsWindow(static_cast<HWND>(desc.surface.nativeWindow)))
            return {};
        ...
        if (FAILED(result) || nativeSwapChain == nullptr)
            return {};
        ...
        if (desc.frameLatency.enabled)
        {
            ...
            if (SUCCEEDED(result))
                frameLatencyWaitableObject = nativeSwapChain->GetFrameLatencyWaitableObject();
        ...
        static_cast<void>(m_impl->factory->MakeWindowAssociation(static_cast<HWND>(desc.surface.nativeWindow), DXGI_MWA_NO_ALT_ENTER));
        ...
        if (SUCCEEDED(result))
            payload->presentFenceEvent = CreateEventW(nullptr, FALSE, FALSE, nullptr);
        ...
        const ResourceRef resource = m_impl->common.CreateBackendResource(ResourceKind::SwapChain, payload, &DestroySwapChainPayload);
        if (!resource)
        {
            DestroySwapChainPayload(nullptr, {}, payload);
            m_impl->common.DrainRetiredResourcesAfterGpuIdle();
    ...
    BackendStatus Backend::ResizeBackbuffer(const SwapChainRef swapChain, const u32 width, const u32 height) noexcept
    {
        ...
        if (payload->stats.state == SwapChainState::Acquired)
        {
            ...
            return BackendStatus::Failure(FailureCode::Busy, 0, "swap-chain resize requires the active back-buffer acquisition to be presented or abandoned");
        }
        ...
        for (u32 index = 0; index < payload->bufferCount; ++index)
            ...
            {
                ++payload->stats.rejectedOperations;
                return BackendStatus::Failure(FailureCode::Busy, 0, "swap-chain back buffers are still retained by recorded or external work");
            }
        ...
        {
            payload->stats.state = SwapChainState::Failed;
            return BackendStatus::Failure(FailureCode::DeviceLost, 0, "presentation fence wait failed before swap-chain resize");
        }
        ...
        for (u32 index = 0; index < payload->bufferCount; ++index)
            payload->presentFenceValues[index] = 0;
        if (!CreateBackBufferHandles(*payload))
        {
            payload->stats.state = SwapChainState::Failed;
            return BackendStatus::Failure(FailureCode::BackendFailure, 0, "failed to wrap resized DXGI back buffers with NVRHI");
        }
    ...
    BackendStatus Backend::SetSwapChainPresentParameters(const SwapChainRef swapChain, const PresentParameters& parameters) noexcept
    {
        if (m_impl == nullptr || !m_impl->initialized)
            return BackendStatus::Failure(FailureCode::NotInitialized, 0, "D3D12 backend is not initialized");
        auto* const payload = static_cast<SwapChainPayload*>(m_impl->common.GetResourcePayload(ResourceRef(swapChain)));
        ...
        if (payload->stats.state != SwapChainState::Available)
        {
            ...
            return BackendStatus::Failure(payload->stats.state == SwapChainState::Acquired ? FailureCode::Busy : FailureCode::BackendFailure, 0,
                                          "swap-chain presentation parameters require an available swap chain");
        }
        if (parameters.allowTearing && !payload->tearingEnabled)
        {
            ++payload->stats.rejectedOperations;
            return BackendStatus::Failure(FailureCode::Unsupported, 0, "tearing was not enabled for this swap chain");
        }
        payload->stats.presentParameters = parameters;
        return BackendStatus::Success();
    }

    BackendStatus Backend::AcquireBackBuffer(const SwapChainRef swapChain, AcquiredBackBuffer& acquisition) noexcept
    {
        acquisition = {};
        if (m_impl == nullptr || !m_impl->initialized)
            return BackendStatus::Failure(FailureCode::NotInitialized, 0, "D3D12 backend is not initialized");
        ...
        {
            const u64 waitStart = system::GetMonotonicTicks();
            const DWORD waitResult = WaitForSingleObjectEx(payload->frameLatencyWaitableObject, payload->desc.frameLatency.waitTimeoutMilliseconds, FALSE);
            ...
            if (waitResult == WAIT_TIMEOUT)
            {
                ...
                ++payload->stats.rejectedOperations;
                return BackendStatus::Failure(FailureCode::Timeout, WAIT_TIMEOUT, "DXGI frame-latency pacing timed out");
            }
        ...
        if (serial == 0)
        {
            ...
            {
                payload->stats.state = SwapChainState::Failed;
                return BackendStatus::Failure(FailureCode::CapacityExceeded, 0, "swap-chain acquisition serial space was exhausted");
            }
        }
    ...
    BackendStatus Backend::AbandonBackBuffer(const AcquiredBackBuffer& acquisition) noexcept
    {
        if (m_impl == nullptr || !m_impl->initialized)
            return BackendStatus::Failure(FailureCode::NotInitialized, 0, "D3D12 backend is not initialized");
        auto* const payload = static_cast<SwapChainPayload*>(m_impl->common.GetResourcePayload(ResourceRef(acquisition.swapChain)));
        if (payload == nullptr)
    ...
    BackendStatus Backend::TransitionSwapChainPresent(const CommandListRef commandList, const AcquiredBackBuffer& acquisition) noexcept
    {
        if (m_impl == nullptr || !m_impl->initialized)
            return BackendStatus::Failure(FailureCode::NotInitialized, 0, "D3D12 backend is not initialized");
        auto* const payload = static_cast<SwapChainPayload*>(m_impl->common.GetResourcePayload(ResourceRef(acquisition.swapChain)));
        ...
        {
            ++payload->stats.rejectedOperations;
            return BackendStatus::Failure(FailureCode::InvalidArgument, 0, "back-buffer present transition was already recorded");
        }
        if (m_impl->common.GetCommandListType(commandList) != CommandListType::Default)
        {
            ++payload->stats.rejectedOperations;
            return BackendStatus::Failure(FailureCode::InvalidCommandList, 0, "back-buffer presentation requires a graphics command list");
        }
        const BackendStatus transition = m_impl->common.TransitionTexture(commandList, acquisition.texture, ResourceState::Unknown, ResourceState::Present, {});
        if (!transition)
            return transition;
        if (!m_impl->common.RetainCommandResource(commandList, ResourceRef(acquisition.swapChain)))
            return BackendStatus::Failure(FailureCode::OutOfMemory, 0, "failed to retain the swap chain for command submission");
        if (!m_impl->common.RegisterCommandSubmissionCallback(commandList, &MarkPresentTransitionSubmitted, payload, acquisition.serial))
            return BackendStatus::Failure(FailureCode::OutOfMemory, 0, "failed to register the swap-chain submission acknowledgement");
    ...
    BackendStatus Backend::Present(const AcquiredBackBuffer& acquisition) noexcept
    {
        ...
        {
            ++payload->stats.rejectedOperations;
            return BackendStatus::Failure(FailureCode::InvalidArgument, 0, "presentation requires an explicit back-buffer present transition");
        }
        if (payload->submittedAcquisitionSerial.GetValue() != acquisition.serial)
        {
            ++payload->stats.rejectedOperations;
            return BackendStatus::Failure(FailureCode::Busy, 0, "the back-buffer present transition has not been submitted");
        }
        ...
        if (currentIndex != acquisition.bufferIndex)
        {
            ++payload->stats.presentationFailures;
            payload->stats.state = SwapChainState::Failed;
            return BackendStatus::Failure(FailureCode::BackendFailure, 0, "DXGI current back buffer changed before presentation");
        }
    ...
    {
        if (m_impl == nullptr || !m_impl->initialized || name == nullptr)
            return;
        auto* const payload = static_cast<SwapChainPayload*>(m_impl->common.GetResourcePayload(ResourceRef(swapChain)));
        if (payload == nullptr)
            return;
