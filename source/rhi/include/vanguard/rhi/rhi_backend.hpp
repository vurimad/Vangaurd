...
namespace vanguard::rhi
{
    ...
    public:
        ...
        [[nodiscard]] virtual BackendStatus QueryMemoryBudget(MemorySegment segment,
                                                              MemoryBudgetSnapshot& budget) noexcept = 0;
        [[nodiscard]] virtual BackendStatus SetResidencyPriority(ResourceRef resource,
                                                                 ResidencyPriority priority) noexcept = 0;
        ...
        [[nodiscard]] virtual BackendStatus Evict(containers::ArraySpan<const ResourceRef> resources,
                                                  const ResidencyFenceSet& safeAfter) noexcept = 0;
        ...
        [[nodiscard]] virtual AccelerationStructureRef CreateAccelerationStructure(
            const AccelerationStructureDesc& desc) noexcept = 0;
        [[nodiscard]] virtual ShaderTableRef CreateShaderTable(const ShaderTableDesc& desc) noexcept = 0;
        [[nodiscard]] virtual BackendStatus GetAccelerationStructureDeviceAddress(
            AccelerationStructureRef accelerationStructure, u64& address) noexcept = 0;
        ...
        [[nodiscard]] virtual BackendStatus ResolveQueries(CommandListRef commandList, QueryPoolRef queryPool, u32 start,
        ...
        [[nodiscard]] virtual BackendStatus GetQueryResult(QueryPoolRef queryPool, u32 index,
                                                           PipelineStatistics& result) noexcept = 0;
        [[nodiscard]] virtual BackendStatus GetTimestampFrequency(QueueType queue, u64& frequency) const noexcept = 0;
        [[nodiscard]] virtual BackendStatus CalibrateTimestamps(QueueType queue,
                                                                TimestampCalibration& calibration) const noexcept = 0;
        ...
        [[nodiscard]] virtual BackendStatus AddToResidencyWorkingSet(CommandListRef commandList,
                                                                     ResourceRef resource) noexcept = 0;
        ...
        [[nodiscard]] virtual BackendStatus SetVariableRateShading(
            CommandListRef commandList, const VariableRateShadingState& state) noexcept = 0;
        ...
        [[nodiscard]] virtual BackendStatus ClearColorTarget(CommandListRef commandList, TextureRef target,
                                                             const ColorValue& value, const SubresourceRange& range,
                                                             const Rect* rectangle) noexcept = 0;
        [[nodiscard]] virtual BackendStatus ClearDepthStencilTarget(CommandListRef commandList, TextureRef target,
                                                                    bool clearDepth, f32 depth, bool clearStencil,
                                                                    u8 stencil, const SubresourceRange& range,
                                                                    const Rect* rectangle) noexcept = 0;
        [[nodiscard]] virtual BackendStatus ClearTextureUav(CommandListRef commandList, TextureRef texture,
                                                            const ColorValue& value, const SubresourceRange& range) noexcept = 0;
        ...
        [[nodiscard]] virtual BackendStatus DiscardTexture(CommandListRef commandList, TextureRef texture,
                                                           const SubresourceRange& range) noexcept = 0;
        ...
        [[nodiscard]] virtual BackendStatus BuildBottomLevelAccelerationStructure(
            CommandListRef commandList, AccelerationStructureRef destination,
            containers::ArraySpan<const RayTracingGeometryDesc> geometries,
            AccelerationStructureBuildMode mode) noexcept = 0;
        [[nodiscard]] virtual BackendStatus BuildTopLevelAccelerationStructure(
            CommandListRef commandList, AccelerationStructureRef destination,
            containers::ArraySpan<const RayTracingInstanceDesc> instances,
            AccelerationStructureBuildMode mode) noexcept = 0;
        [[nodiscard]] virtual BackendStatus BuildTopLevelAccelerationStructureIndirect(
            CommandListRef commandList, AccelerationStructureRef destination, BufferRef instanceBuffer,
            u64 offset, u32 instanceCount, AccelerationStructureBuildMode mode) noexcept = 0;
        [[nodiscard]] virtual BackendStatus CopyAccelerationStructure(
            CommandListRef commandList, AccelerationStructureRef destination, AccelerationStructureRef source,
            AccelerationStructureCopyMode mode) noexcept = 0;
        [[nodiscard]] virtual BackendStatus WriteAccelerationStructureCompactedSize(
            CommandListRef commandList, AccelerationStructureRef accelerationStructure,
        ...
        [[nodiscard]] virtual BackendStatus RequestTextureReadback(CommandListRef commandList, TextureRef source,
                                                                   const TextureReadbackRegion& region,
                                                                   TextureReadbackRef& readback) noexcept = 0;
        [[nodiscard]] virtual BackendStatus GetTextureReadbackInfo(TextureReadbackRef readback,
                                                                   TextureReadbackInfo& info) noexcept = 0;
        [[nodiscard]] virtual BackendStatus MapTextureReadback(TextureReadbackRef readback,
                                                               TextureReadbackMapping& mapping) noexcept = 0;
        ...
        [[nodiscard]] virtual BackendStatus SetSwapChainPresentParameters(SwapChainRef swapChain,
                                                                          const PresentParameters& parameters) noexcept = 0;
        [[nodiscard]] virtual BackendStatus AcquireBackBuffer(SwapChainRef swapChain,
                                                              AcquiredBackBuffer& acquisition) noexcept = 0;
        [[nodiscard]] virtual BackendStatus AbandonBackBuffer(const AcquiredBackBuffer& acquisition) noexcept = 0;
        [[nodiscard]] virtual BackendStatus TransitionSwapChainPresent(CommandListRef commandList,
                                                                        const AcquiredBackBuffer& acquisition) noexcept = 0;
