...
namespace vanguard::rhi
{
    ...
    struct GpuCounterSystem::Impl
    {
        ...
        {
            if (!handle.IsValid() || handle.index >= config.bufferedFrames) return nullptr;
        ...
        [[nodiscard]] u32 SampleBase(const u32 frameIndex) const noexcept { return frameIndex * config.maximumScopesPerFrame; }
    ...
    bool GpuCounterSystem::Initialize(const GpuCounterConfig& config, Failure* const failure) noexcept
    {
        ...
        if (config.bufferedFrames < 2 || config.bufferedFrames > MaximumGpuCounterBufferedFrames ||
            config.maximumScopesPerFrame == 0 || config.maximumRegisteredScopes == 0 ||
            config.queue > QueueType::Copy || (config.collectPipelineStatistics && config.queue != QueueType::Graphics) ||
        ...
        memory::MemoryBlock sampleBlock = memory::Allocate(memory::PoolId::Rendering,
                                                          sizeof(GpuCounterSample) * config.bufferedFrames * config.maximumScopesPerFrame,
                                                          alignof(GpuCounterSample));
        memory::MemoryBlock stateBlock = memory::Allocate(memory::PoolId::Rendering,
                                                         sizeof(SampleState) * config.bufferedFrames * config.maximumScopesPerFrame,
                                                         alignof(SampleState));
        memory::MemoryBlock definitionBlock = memory::Allocate(memory::PoolId::Rendering,
                                                              sizeof(ScopeDefinition) * config.maximumRegisteredScopes,
                                                              alignof(ScopeDefinition));
        if (!frameBlock || !sampleBlock || !stateBlock || !definitionBlock)
        {
            if (definitionBlock) memory::Free(definitionBlock);
            if (stateBlock) memory::Free(stateBlock);
            if (sampleBlock) memory::Free(sampleBlock);
            if (frameBlock) memory::Free(frameBlock);
            m_impl->~Impl();
        ...
        for (u32 index = 0; index < config.bufferedFrames; ++index) new (&m_impl->frames[index]) FrameRecord{};
        ...
        for (u32 index = 0; index < config.maximumRegisteredScopes; ++index) new (&m_impl->definitions[index]) ScopeDefinition{};
        ...
        if (config.collectPipelineStatistics)
            m_impl->statisticsPool = QueryPool(AdoptReference,
                                               CreateQueryPool({QueryType::PipelineStatistics, static_cast<u32>(statisticsCapacity)}, failure));
        if (!m_impl->timestampPool || (config.collectPipelineStatistics && !m_impl->statisticsPool))
        {
            ...
            for (u32 index = 0; index < config.maximumRegisteredScopes; ++index) m_impl->definitions[index].~ScopeDefinition();
            for (u32 index = 0; index < sampleCapacity; ++index) m_impl->samples[index].~GpuCounterSample();
            for (u32 index = 0; index < config.bufferedFrames; ++index) m_impl->frames[index].~FrameRecord();
        ...
        if (m_impl->statisticsPool) SetResourceDebugName(m_impl->statisticsPool.GetRef(), "GPU Counter Pipeline Statistics");
        return true;
    }
    ...
    bool GpuCounterSystem::Shutdown(Failure* const failure) noexcept
    {
        ...
        for (u32 index = 0; index < m_impl->config.maximumRegisteredScopes; ++index) m_impl->definitions[index].~ScopeDefinition();
        for (u32 index = 0; index < sampleCapacity; ++index) m_impl->samples[index].~GpuCounterSample();
        for (u32 index = 0; index < m_impl->config.bufferedFrames; ++index) m_impl->frames[index].~FrameRecord();
    ...
    bool GpuCounterSystem::IsInitialized() const noexcept { return m_impl != nullptr; }
    ...
    bool GpuCounterSystem::RegisterScope(const GpuCounterScopeId scope, const char* const name, Failure* const failure) noexcept
    {
        ...
        for (u32 index = 0; index < m_impl->definitionCount; ++index)
            ...
            {
                u32 character = 0;
                while (copiedName[character] == m_impl->definitions[index].name[character] && copiedName[character] != '\0') ++character;
        ...
        definition.id = scope;
        for (u32 index = 0; index < MaximumGpuCounterScopeNameLength; ++index) definition.name[index] = copiedName[index];
    ...
    {
        if (m_impl == nullptr || !scope.IsValid()) return nullptr;
        concurrency::ScopedLock guard(m_impl->lock);
        for (u32 index = 0; index < m_impl->definitionCount; ++index)
            if (m_impl->definitions[index].id == scope) return m_impl->definitions[index].name;
        return nullptr;
    }
    ...
    bool GpuCounterSystem::BeginFrame(const u64 frameNumber, GpuCounterFrameHandle& output, Failure* const failure) noexcept
    {
        ...
        frame.calibration.cpuFrequency = system::MonotonicFrequency();
    ...
    bool GpuCounterSystem::BeginScope(const GpuCounterFrameHandle handle, const GpuCounterScopeId scope,
                                      GpuCounterScopeToken& token, Failure* const failure) noexcept
    {
        token = {};
        ClearFailure(failure);
        if (m_impl == nullptr)
        ...
        for (u32 index = 0; index < m_impl->definitionCount; ++index) registered = registered || m_impl->definitions[index].id == scope;
        if (!registered)
        {
        ...
        sample = {};
        sample.scope = scope;
        sample.cpuBegin = system::MonotonicTicks();
        sample.cpuThread = concurrency::ThreadId::CurrentThread().AsNumber();
    ...
    bool GpuCounterSystem::EndScope(GpuCounterScopeToken& token, Failure* const failure) noexcept
    {
        ...
        m_impl->samples[storageIndex].cpuEnd = system::MonotonicTicks();
    ...
    bool GpuCounterSystem::CommitFrame(const GpuCounterFrameHandle handle, const GpuFence completion, Failure* const failure) noexcept
    {
        ...
        concurrency::ScopedLock guard(m_impl->lock);
        FrameRecord* const frame = m_impl->FindFrame(handle);
        if (frame == nullptr || frame->state != GpuCounterFrameState::AwaitingSubmission || !completion.IsValid() ||
            completion.queue != m_impl->config.queue)
        {
    ...
    u32 GpuCounterSystem::Collect(Failure* const failure) noexcept
    {
        ...
        {
            FrameRecord& frame = m_impl->frames[slot];
            if (frame.state != GpuCounterFrameState::PendingGpu || !IsGpuFenceComplete(frame.completion)) continue;
            ...
            {
                if (queryFailure.code == FailureCode::Busy) continue;
                frame.state = GpuCounterFrameState::Failed;
            ...
            {
                statisticsAcquired = AcquireQueries(m_impl->statisticsPool.GetRef(), m_impl->StatisticsBase(slot), frame.sampleCount,
                                                    &queryFailure);
            ...
            for (u32 index = 0; index < frame.sampleCount && valid; ++index)
            {
                ...
                if (valid && statisticsAcquired)
                    valid = GetQueryResult(m_impl->statisticsPool.GetRef(), m_impl->StatisticsBase(slot) + index,
                                           sample.pipelineStatistics, &queryFailure);
                sample.valid = valid && sample.gpuEnd >= sample.gpuBegin;
            }
            if (statisticsAcquired) ReleaseQueries(m_impl->statisticsPool.GetRef());
            ReleaseQueries(m_impl->timestampPool.GetRef());
            if (!valid)
            {
                frame.state = GpuCounterFrameState::Failed;
    ...
    bool GpuCounterSystem::GetOldestReadyFrame(GpuCounterFrameView& output, Failure* const failure) const noexcept
    {
        ...
        for (u32 index = 0; index < m_impl->config.bufferedFrames; ++index)
        {
            ...
            if (frame.state == GpuCounterFrameState::Ready && (selected == nullptr || frame.sequence < selected->sequence)) selected = &frame;
        }
        if (selected == nullptr)
        {
        ...
        output = {selected->handle, m_impl->samples + m_impl->SampleBase(selected->handle.index), selected->calibration,
                  selected->completion, selected->frameNumber, selected->gpuBegin, selected->gpuEnd, selected->sampleCount,
                  selected->droppedSamples, m_impl->config.queue, m_impl->config.collectPipelineStatistics};
        return true;
    }
    ...
    {
        if (m_impl == nullptr) return GpuCounterFrameState::Empty;
        concurrency::ScopedLock guard(m_impl->lock);
