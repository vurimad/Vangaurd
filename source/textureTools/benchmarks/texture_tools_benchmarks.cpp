...
namespace
{
    ...
    double Run(const tt::SourceTexture& source, const tt::CookSettings& settings, ByteArray& output)
    {
        ...
        for (vanguard::u32 iteration = 0; iteration < Iterations; ++iteration)
        {
            ...
            if (tt::CookTexture(source, writer, settings) != tt::Result::Success) return -1.0;
        }
...
int main()
{
    ...
    if (!memory::Initialize() || !diagnostics::Initialize(diagnostics::Mode::Synchronous, "textureToolsBenchmarks") ||
        !containers::Initialize() || !io::Initialize())
    {
        return 1;
    }
    const filesystem::AbsolutePath root = filesystem::paths::GetCurrentWorkingDirectory();
    if (!filesystem::Initialize({root, root, root}) || !tt::Initialize()) return 2;
    jobs::Config jobsConfig = jobs::ToolConfig();
    jobsConfig.maxWorkers = 0;
    if (!jobs::Initialize(jobsConfig)) return 3;
    ...
    const tt::SourceTexture source{textures::TextureDimension::Texture2D, tt::SourcePixelFormat::R8G8B8A8UNorm,
                                      textures::ColorSpace::SRgb, Extent, Extent, 1, 1,
                                      vanguard::crypto::Sha256(pixels.TypedData(), pixels.Size()), {&image, 1}};
    ...
    if (serialSeconds <= 0.0 || parallelSeconds <= 0.0 || !Equal(serialOutput, parallelOutput)) return 4;
    ...
    std::printf("textureTools BC7 %ux%u x %u: serial %.2f MPix/s, Jobs %.2f MPix/s, speedup %.2fx, workers %u\n",
                Extent, Extent, Iterations, megaPixels / serialSeconds, megaPixels / parallelSeconds,
                serialSeconds / parallelSeconds, jobs::WorkerCount());

    if (!jobs::Shutdown()) return 5;
    filesystem::Shutdown();
    io::Shutdown();
