...
namespace
{
    ...
    [[nodiscard]] bool MapDxgiFormat(const u32 dxgi, textures::PixelFormat& format, textures::ColorSpace& colorSpace) noexcept
    {
        colorSpace = textures::ColorSpace::Linear;
        switch (dxgi)
        {
        case 71:
            format = textures::PixelFormat::BC1UNorm;
            return true;
        case 72:
            format = textures::PixelFormat::BC1UNorm;
            colorSpace = textures::ColorSpace::SRgb;
            return true;
        case 74:
            format = textures::PixelFormat::BC2UNorm;
            return true;
        case 75:
            format = textures::PixelFormat::BC2UNorm;
            colorSpace = textures::ColorSpace::SRgb;
            return true;
        case 77:
            format = textures::PixelFormat::BC3UNorm;
            return true;
        case 78:
            format = textures::PixelFormat::BC3UNorm;
            colorSpace = textures::ColorSpace::SRgb;
            return true;
        case 80:
            format = textures::PixelFormat::BC4UNorm;
            return true;
        case 81:
            format = textures::PixelFormat::BC4SNorm;
            return true;
        case 83:
            format = textures::PixelFormat::BC5UNorm;
            return true;
        case 84:
            format = textures::PixelFormat::BC5SNorm;
            return true;
        case 95:
            format = textures::PixelFormat::BC6HUFloat;
            return true;
        case 96:
            format = textures::PixelFormat::BC6HSFloat;
            return true;
        case 98:
    ...
    [[nodiscard]] u32 ExpectedSubresourceCount(const textures::TextureDimension dimension, const u16 arrayLayers, const u8 mipCount) noexcept
    {
        if (dimension == textures::TextureDimension::Texture3D)
            return mipCount;
...
namespace vanguard::texture_tools
{
    ImportedGpuTexture::ImportedGpuTexture() noexcept
        : m_payload(memory::pools::Assets::GetInstance()), m_ownedSubresources(memory::pools::Assets::GetInstance()),
          m_views(memory::pools::Assets::GetInstance())
    {
    }

    ImportedGpuTexture::ImportedGpuTexture(ImportedGpuTexture&& other) noexcept
        : m_dimension(other.m_dimension), m_format(other.m_format), m_colorSpace(other.m_colorSpace), m_width(other.m_width), m_height(other.m_height),
          m_depth(other.m_depth), m_arrayLayers(other.m_arrayLayers), m_mipCount(other.m_mipCount), m_writeOffset(other.m_writeOffset),
          m_sourceFingerprint(other.m_sourceFingerprint), m_importer(other.m_importer), m_importerVersion(other.m_importerVersion),
    ...
    {
        if (m_width == 0 || m_height == 0 || m_depth == 0 || m_arrayLayers == 0 || m_mipCount == 0 || m_importer == InvalidTextureImporterId ||
            m_importerVersion == 0 || m_writeOffset != m_payload.Size() ||
        ...
        for (u32 index = 0; index < m_ownedSubresources.Size(); ++index)
            if (!m_ownedSubresources[index].initialized)
                return false;
        return true;
    }

    GpuTextureSource ImportedGpuTexture::GetSource() const noexcept
    {
        if (!IsValid())
            return {};
        m_views.Resize(m_ownedSubresources.Size());
        if (m_views.Size() != m_ownedSubresources.Size())
            return {};
        for (u32 index = 0; index < m_ownedSubresources.Size(); ++index)
        {
        ...
        return {m_dimension,
                ...
                m_mipCount,
                m_sourceFingerprint,
                {m_views.TypedData(), m_views.Size()}};
    }

    TextureImporterId ImportedGpuTexture::Importer() const noexcept
    {
        return m_importer;
    }
    u32 ImportedGpuTexture::ImporterVersion() const noexcept
    {
        return m_importerVersion;
    }

    TextureImportResult ImportedGpuTexture::Initialize(const textures::TextureDimension dimension, const textures::PixelFormat format,
                                                       ...
                                                       const TextureImportLimits& limits) noexcept
    {
        ...
        if (dimension > textures::TextureDimension::Cube || format >= textures::PixelFormat::Count || colorSpace > textures::ColorSpace::SRgb ||
            info.bytesPerBlock == 0 || width == 0 || height == 0 || depth == 0 || width > limits.maximumDimension || height > limits.maximumDimension ||
            depth > limits.maximumDimension || arrayLayers == 0 || mipCount == 0 || subresourceCount != expectedCount ||
            subresourceCount > limits.maximumImages || totalBytes == 0 || totalBytes > limits.maximumDecodedBytes || totalBytes > 0xffffffffull ||
            importer == InvalidTextureImporterId || importerVersion == 0 || (colorSpace == textures::ColorSpace::SRgb && !info.supportsSRgb))
            return totalBytes > limits.maximumDecodedBytes || totalBytes > 0xffffffffull || width > limits.maximumDimension ||
                           height > limits.maximumDimension || depth > limits.maximumDimension || subresourceCount > limits.maximumImages
    ...
    TextureImportResult ImportedGpuTexture::SetSubresource(const u32 index, const u8 mipLevel, const u16 arrayLayer, const u8 face, const void* data,
                                                           const usize byteSize, const u32 rowPitch, const u32 slicePitch) noexcept
    {
        if (index >= m_ownedSubresources.Size() || data == nullptr || byteSize == 0 || byteSize > 0xffffffffull || m_ownedSubresources[index].initialized ||
            mipLevel >= m_mipCount || arrayLayer >= m_arrayLayers || face >= (m_dimension == textures::TextureDimension::Cube ? 6u : 1u) || rowPitch == 0 ||
            slicePitch == 0 || byteSize > m_payload.Size() - m_writeOffset)
        ...
        for (u32 byte = 0; byte < copyByteSize; ++byte)
            m_payload[m_writeOffset + byte] = source[byte];
        m_ownedSubresources[index] = {mipLevel, arrayLayer, face, m_writeOffset, static_cast<u32>(byteSize), rowPitch, slicePitch, true};
        m_writeOffset += static_cast<u32>(byteSize);
        return TextureImportResult::Success;
    }

    TextureImportResult ImportDdsTexture(const TextureImportRequest& request, ImportedGpuTexture& output, GpuTextureImportReport* const report) noexcept
    {
        output.Reset();
        ...
        if (request.encoded.Data() == nullptr || request.encoded.Count() < DdsMinimumHeaderBytes || request.colorSpace > ImportedColorSpace::SRgb ||
            request.limits.maximumDimension == 0 || request.limits.maximumImages == 0 || request.limits.maximumDecodedBytes == 0 ||
        ...
        if (ReadU32(bytes) != DdsMagic || ReadU32(bytes + 4) != DdsHeaderSize || ReadU32(bytes + 76) != DdsPixelFormatSize ||
            (ReadU32(bytes + 8) & DdsRequiredFlags) != DdsRequiredFlags)
        ...
        if (hasDx10Header)
        {
            ...
            if (arrayLayers32 == 0 || alphaMode > 4 || alphaMode == 2 || !MapDxgiFormat(dxgiFormat, format, embeddedColorSpace))
            ...
            {
                if ((flags & DdsDepthFlag) == 0 || arrayLayers32 != 1 || depth == 0 || (miscFlag & DdsResourceMiscCube) != 0)
                    return TextureImportResult::DecodeFailure;
        ...
        {
            if ((pixelFormatFlags & DdsPixelFormatFourCc) == 0 || fourCc == FourCc('D', 'X', 'T', '2') || fourCc == FourCc('D', 'X', 'T', '4') ||
                !MapLegacyFormat(fourCc, format))
                return TextureImportResult::UnsupportedFormat;
            if ((flags & DdsDepthFlag) != 0)
            {
                if (depth == 0 || (caps2 & DdsCube) != 0)
                    return TextureImportResult::DecodeFailure;
                dimension = textures::TextureDimension::Texture3D;
            }
            else
        ...
        if (mipCount > textures::CalculateMipCount(width, height, depth) || arrayLayers32 > 65535 || width > request.limits.maximumDimension ||
            height > request.limits.maximumDimension || depth > request.limits.maximumDimension)
        ...
        for (u32 layer = 0; layer < arrayLayers; ++layer)
        {
            ...
            for (u32 face = 0; face < faceCount; ++face)
            {
                for (u8 mip = 0; mip < mipCount; ++mip)
                {
                    ...
                    const u32 mipDepth = dimension == textures::TextureDimension::Texture3D ? textures::CalculateMipExtent(depth, mip) : 1u;
                    ...
                    if (!formatInfo.blockCompressed || rowPitch == 0 || slicePitch == 0 || requiredPayloadBytes > 0xffffffffffffffffull - byteSize)
                        return TextureImportResult::UnsupportedFormat;
        ...
        if (requiredPayloadBytes > request.limits.maximumDecodedBytes || payloadOffset > size || requiredPayloadBytes != size - payloadOffset)
            return requiredPayloadBytes > request.limits.maximumDecodedBytes ? TextureImportResult::LimitExceeded : TextureImportResult::DecodeFailure;

        const textures::ColorSpace colorSpace = request.colorSpace == ImportedColorSpace::Linear ? textures::ColorSpace::Linear
                                                : request.colorSpace == ImportedColorSpace::SRgb ? textures::ColorSpace::SRgb
                                                                                                 : embeddedColorSpace;
        if (colorSpace == textures::ColorSpace::SRgb && !formatInfo.supportsSRgb)
            return TextureImportResult::InvalidArgument;
        TextureImportResult result = output.Initialize(dimension, format, colorSpace, width, height, depth, arrayLayers, mipCount, subresourceCount,
                                                       requiredPayloadBytes, Fingerprint(request), importers::Dds, 1, request.limits);
        ...
        for (u16 layer = 0; result == TextureImportResult::Success && layer < arrayLayers; ++layer)
        {
            ...
            for (u8 face = 0; result == TextureImportResult::Success && face < faceCount; ++face)
            {
                for (u8 mip = 0; result == TextureImportResult::Success && mip < mipCount; ++mip)
                {
                    ...
                    result = output.SetSubresource(subresource++, mip, layer, face, bytes + cursor, byteSize, rowPitch, slicePitch);
                    cursor += byteSize;
                }
            }
            if (dimension == textures::TextureDimension::Texture3D)
        ...
        if (report != nullptr)
            *report = {importers::Dds, 1, dimension, format, colorSpace, width, height, depth, arrayLayers, mipCount, subresourceCount, requiredPayloadBytes};
        return TextureImportResult::Success;
    }

    Result CookGpuTexture(const GpuTextureSource& source, filesystem::IFile& output, const GpuTextureCookSettings& settings, CookReport* const report) noexcept
    {
        if (!IsInitialized())
        ...
        if (source.dimension > textures::TextureDimension::Cube || source.format >= textures::PixelFormat::Count || formatInfo.bytesPerBlock == 0 ||
            source.width == 0 || source.height == 0 || source.depth == 0 || source.arrayLayers == 0 || source.mipCount == 0 ||
            source.subresources.Data() == nullptr || source.subresources.Count() != expectedCount || settings.mipTailCount == 0)
        ...
        for (u32 index = 0; index < expectedCount; ++index)
        {
            ...
            if (subresource.data == nullptr || subresource.byteSize == 0 || sourceBytes > settings.maximumOutputBytes ||
                subresource.byteSize > settings.maximumOutputBytes - sourceBytes)
        ...
        if (settings.streamable)
            description.flags = description.flags | textures::TextureFlags::Streamable;
        ...
        description.mipTailFirstLevel =
            settings.streamable && source.mipCount > settings.mipTailCount ? static_cast<u8>(source.mipCount - settings.mipTailCount) : 0;
        ...
        if (static_cast<u64>(output.GetSize()) > settings.maximumOutputBytes)
            return Result::LimitExceeded;
        if (report != nullptr)
            *report = {0, 0, source.format, source.mipCount, expectedCount, sourceBytes, static_cast<u64>(output.GetSize()), 0, 0, 0.0f, false};
