...
namespace
{
    ...
    void PngFailure(png_structp png, png_const_charp) { png_longjmp(png, 1); }
    ...
    [[nodiscard]] bool ValidatePngStructure(const tools::TextureImportRequest& request) noexcept
    {
        ...
        while (offset <= size && size - offset >= 12)
        {
            ...
            if (static_cast<u64>(length) + 12u > size - offset) return false;
            ...
            if ((!foundHeader && (!isHeader || length != 13)) || (foundHeader && isHeader)) return false;
            const u32 storedCrc = ReadBigEndianU32(bytes + offset + 8u + length);
            if (serialization::Crc32(type, static_cast<usize>(length) + 4u) != storedCrc) return false;
            foundHeader = true;
            foundImageData = foundImageData || isImageData;
            offset += static_cast<usize>(length) + 12u;
            if (isEnd) return length == 0 && foundImageData && offset == size;
        }
        return false;
    }

    [[nodiscard]] tools::TextureImportResult DecodePng(const tools::TextureImportRequest& request,
                                                        tools::ImportedTexture& output, void*) noexcept
    {
        ...
        if (png == nullptr) return tools::TextureImportResult::OutOfMemory;
        ...
        if (setjmp(png_jmpbuf(png)) == 0)
        {
            ...
            if (width == 0 || height == 0 || width > request.limits.maximumDimension ||
                height > request.limits.maximumDimension || (originalBitDepth != 16 && originalBitDepth > 8))
            {
            ...
            else
            {
                ...
                if (originalColorType == PNG_COLOR_TYPE_PALETTE) png_set_palette_to_rgb(png);
                ...
                if (preserve16Bit) png_set_swap(png);
#endif
                ...
                if (png_get_channels(png, info) != 4 || rowPitch != static_cast<u64>(width) * bytesPerPixel ||
                    rowPitch > 0xffffffffull || decodedBytes > request.limits.maximumDecodedBytes)
                {
                    result = decodedBytes > request.limits.maximumDecodedBytes
                        ? tools::TextureImportResult::LimitExceeded : tools::TextureImportResult::DecodeFailure;
                }
                else
                {
                    const tools::SourcePixelFormat format = preserve16Bit
                        ? tools::SourcePixelFormat::R16G16B16A16UNorm : tools::SourcePixelFormat::R8G8B8A8UNorm;
                    const textures::ColorSpace automaticColor = png_get_valid(png, info, PNG_INFO_sRGB)
                        ? textures::ColorSpace::SRgb : textures::ColorSpace::SRgb;
                    result = output.Initialize2D(format, ResolveColorSpace(request, automaticColor), width, height,
                                                 static_cast<u32>(rowPitch), Fingerprint(request),
                                                 tools::ResolveImportedProfile(request.usage, format),
                                                 tools::importers::Png, 1, request.limits);
                    if (result == tools::TextureImportResult::Success)
                    {
                        for (u32 row = 0; row < height; ++row)
                            png_read_row(png, output.MutableImageData() + static_cast<usize>(row) * rowPitch, nullptr);
    ...
    {
        const bool signature = request.encoded.Count() >= 3 && request.encoded[0] == 0xff &&
                               request.encoded[1] == 0xd8 && request.encoded[2] == 0xff;
        return signature ? tools::TextureProbeResult::Exact :
               (IsHint(request.typeHint, "jpg") || IsHint(request.typeHint, "jpeg")
                    ? tools::TextureProbeResult::Possible : tools::TextureProbeResult::NoMatch);
    }

    [[nodiscard]] tools::TextureImportResult DecodeJpeg(const tools::TextureImportRequest& request,
                                                         tools::ImportedTexture& output, void*) noexcept
    {
        if (request.encoded.Count() < 3 || request.encoded[0] != 0xff || request.encoded[1] != 0xd8 ||
            request.encoded[2] != 0xff)
            return tools::TextureImportResult::DecodeFailure;
        ...
        tools::TextureImportResult result = output.Initialize2D(
            tools::SourcePixelFormat::R8G8B8A8UNorm, ResolveColorSpace(request, textures::ColorSpace::SRgb),
        ...
        {
            JSAMPROW row = output.MutableImageData() + static_cast<usize>(decoder.output_scanline) * rowPitch;
            if (jpeg_read_scanlines(&decoder, &row, 1) != 1) result = tools::TextureImportResult::DecodeFailure;
        }
    ...
    const tools::TextureImporterDescriptor g_pngImporter{
        tools::importers::Png, "Portable Network Graphics", 1, &ProbePng, &DecodePng, nullptr};
    const tools::TextureImporterDescriptor g_jpegImporter{
        tools::importers::Jpeg, "JPEG", 1, &ProbeJpeg, &DecodeJpeg, nullptr};
}

namespace vanguard::texture_tools
{
    const TextureImporterDescriptor& PngImporter() noexcept { return g_pngImporter; }
    const TextureImporterDescriptor& JpegImporter() noexcept { return g_jpegImporter; }
} // namespace vanguard::texture_tools
