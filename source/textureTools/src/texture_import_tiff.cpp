...
namespace
{
    ...
    [[nodiscard]] tmsize_t ReadTiff(thandle_t handle, void* destination, const tmsize_t requested) noexcept
    {
        ...
        if (stream == nullptr || destination == nullptr || requested < 0 || stream->position > stream->size)
            return -1;
        ...
        for (u64 byte = 0; byte < count; ++byte)
            output[byte] = stream->data[stream->position + byte];
        stream->position += count;
        return static_cast<tmsize_t>(count);
    }

    [[nodiscard]] tmsize_t WriteTiff(thandle_t, void*, tmsize_t) noexcept
    {
        return 0;
    }
    ...
    [[nodiscard]] toff_t SeekTiff(thandle_t handle, const toff_t offset, const int origin) noexcept
    {
        ...
        if (origin == SEEK_CUR)
            ...
            return static_cast<toff_t>(-1);
        if (offset > ~u64{0} - base || base + offset > stream->size)
            return static_cast<toff_t>(-1);
        stream->position = base + offset;
        return stream->position;
    }

    [[nodiscard]] int CloseTiff(thandle_t) noexcept
    {
        return 0;
    }
    ...
    {
        const auto* const stream = static_cast<const TiffMemory*>(handle);
        if (stream == nullptr || base == nullptr || size == nullptr)
            return 0;
    ...
    [[nodiscard]] int IgnoreTiffMessage(TIFF*, void*, const char*, const char*, va_list) noexcept
    {
        return 1;
    }
    ...
    [[nodiscard]] f32 ReadF32(const u8* bytes) noexcept
    {
        ...
        converted.bits = static_cast<u32>(bytes[0]) | static_cast<u32>(bytes[1]) << 8u | static_cast<u32>(bytes[2]) << 16u | static_cast<u32>(bytes[3]) << 24u;
    ...
    void WriteNormalized(u8* destination, const tools::SourcePixelFormat format, const u32 channel, f32 value) noexcept
    {
        ...
        else if (format == tools::SourcePixelFormat::R16G16B16A16UNorm)
        {
            if (value < 0.0f)
                value = 0.0f;
            WriteU16(destination + channel * 2u, static_cast<u16>(value >= 1.0f ? 65535u : static_cast<u32>(value * 65535.0f + 0.5f)));
        }
        else if (format == tools::SourcePixelFormat::R16G16B16A16Float)
        {
            ...
            WriteF32(destination + channel * 4u, value);
    }

    void TransformCoordinate(const u16 orientation, const u32 width, const u32 height, const u32 x, const u32 y, u32& outputX, u32& outputY) noexcept
    {
        switch (orientation)
        {
    ...
    void StorePixel(u8* output, const u32 outputWidth, const u32 outputBytesPerPixel, const tools::SourcePixelFormat outputFormat, const u16 photometric,
                    const u16 orientation, const u32 sourceWidth, const u32 sourceHeight, const u32 x, const u32 y, const u8* const samples[4],
                    const u16 sampleCount, const u16 bits, const u16 sampleFormat, const bool associatedAlpha) noexcept
    {
        u32 outputX = 0;
        u32 outputY = 0;
        ...
        if (photometric == PHOTOMETRIC_RGB)
        {
            ...
            if (sampleCount == 4)
                rgba[3] = ReadNormalized(samples[3], bits, sampleFormat);
        }
        else
        {
            ...
            if (sampleCount == 2)
                rgba[3] = ReadNormalized(samples[1], bits, sampleFormat);
        }
    ...
    {
        const bool little =
            request.encoded.Count() >= 4 && request.encoded[0] == 'I' && request.encoded[1] == 'I' && request.encoded[2] == 42 && request.encoded[3] == 0;
        const bool big =
            request.encoded.Count() >= 4 && request.encoded[0] == 'M' && request.encoded[1] == 'M' && request.encoded[2] == 0 && request.encoded[3] == 42;
        const bool littleBig =
            request.encoded.Count() >= 4 && request.encoded[0] == 'I' && request.encoded[1] == 'I' && request.encoded[2] == 43 && request.encoded[3] == 0;
        const bool bigBig =
            request.encoded.Count() >= 4 && request.encoded[0] == 'M' && request.encoded[1] == 'M' && request.encoded[2] == 0 && request.encoded[3] == 43;
        return little || big || littleBig || bigBig
                   ? tools::TextureProbeResult::Exact
                   : (IsHint(request.typeHint, "tif") || IsHint(request.typeHint, "tiff") ? tools::TextureProbeResult::Possible
                                                                                          : tools::TextureProbeResult::NoMatch);
    }

    [[nodiscard]] tools::TextureImportResult DecodeTiff(const tools::TextureImportRequest& request, tools::ImportedTexture& output, void*) noexcept
    {
        ...
        const u64 allocationLimit = request.limits.maximumDecodedBytes < 0x7fffffffffffffffull ? request.limits.maximumDecodedBytes : 0x7fffffffffffffffull;
        ...
        TIFF* const tiff =
            TIFFClientOpenExt("Vanguard memory TIFF", "rm", &stream, &ReadTiff, &WriteTiff, &SeekTiff, &CloseTiff, &SizeTiff, &MapTiff, &UnmapTiff, options);
        ...
        {
            result = directoryCount > 1 ? tools::TextureImportResult::MultipleImagesUnsupported : tools::TextureImportResult::DecodeFailure;
            TIFFClose(tiff);
            return result;
        }
        ...
        if (width == 0 || height == 0 || (orientation < ORIENTATION_TOPLEFT || orientation > ORIENTATION_LEFTBOT) ||
            (planar != PLANARCONFIG_CONTIG && planar != PLANARCONFIG_SEPARATE) || (!grayscale && !rgb) ||
            (grayscale && samplesPerPixel != 1 && samplesPerPixel != 2) || (rgb && samplesPerPixel != 3 && samplesPerPixel != 4) ||
        ...
        if ((grayscale && samplesPerPixel == 2) || (rgb && samplesPerPixel == 4))
        {
            ...
            {
                if (extraCount != 1 ||
                    (extraTypes[0] != EXTRASAMPLE_ASSOCALPHA && extraTypes[0] != EXTRASAMPLE_UNASSALPHA && extraTypes[0] != EXTRASAMPLE_UNSPECIFIED))
                {
                    TIFFClose(tiff);
                    return tools::TextureImportResult::UnsupportedFormat;
                }
        ...
        if (sampleFormat == SAMPLEFORMAT_IEEEFP)
            outputFormat = bitsPerSample == 16 ? tools::SourcePixelFormat::R16G16B16A16Float : tools::SourcePixelFormat::R32G32B32A32Float;
        else if (bitsPerSample == 16)
            outputFormat = tools::SourcePixelFormat::R16G16B16A16UNorm;
        else if (grayscale && samplesPerPixel == 1)
            outputFormat = tools::SourcePixelFormat::R8UNorm;
        const u32 outputBytesPerPixel = outputFormat == tools::SourcePixelFormat::R8UNorm             ? 1u
                                        ...
                                        : outputFormat == tools::SourcePixelFormat::R32G32B32A32Float ? 16u
                                                                                                      : 8u;
        ...
        const textures::ColorSpace automaticColor = sampleFormat == SAMPLEFORMAT_IEEEFP ? textures::ColorSpace::Linear : textures::ColorSpace::SRgb;
        result =
            output.Initialize2D(outputFormat, ResolveColorSpace(request, automaticColor), outputWidth, outputHeight, static_cast<u32>(rowPitch),
                                Fingerprint(request), tools::ResolveImportedProfile(request.usage, outputFormat), tools::importers::Tiff, 1, request.limits);
        ...
        if (TIFFIsTiled(tiff))
        {
            u32 tileWidth = 0;
            u32 tileHeight = 0;
            if (!TIFFGetField(tiff, TIFFTAG_TILEWIDTH, &tileWidth) || !TIFFGetField(tiff, TIFFTAG_TILELENGTH, &tileHeight) || tileWidth == 0 || tileHeight == 0)
            ...
            if (result == tools::TextureImportResult::Success)
                scratch.Resize(static_cast<u32>(scratchBytes));
            ...
            for (u32 tileY = 0; result == tools::TextureImportResult::Success && tileY < height; tileY += tileHeight)
            {
                for (u32 tileX = 0; result == tools::TextureImportResult::Success && tileX < width; tileX += tileWidth)
                {
                    ...
                    for (u16 plane = 0; result == tools::TextureImportResult::Success && plane < planes; ++plane)
                    {
                        ...
                        if (TIFFReadEncodedTile(tiff, tile, scratch.TypedData() + static_cast<u64>(plane) * tileBytes, static_cast<tmsize_t>(tileBytes)) < 0)
                            result = tools::TextureImportResult::DecodeFailure;
                    }
                    for (u32 localY = 0; result == tools::TextureImportResult::Success && localY < tileHeight && tileY + localY < height; ++localY)
                    {
                        for (u32 localX = 0; localX < tileWidth && tileX + localX < width; ++localX)
                        {
                            ...
                            for (u16 sample = 0; sample < samplesPerPixel; ++sample)
                            {
                                samples[sample] = planar == PLANARCONFIG_CONTIG
                                                      ...
                                                      : scratch.TypedData() + static_cast<u64>(sample) * tileBytes + static_cast<u64>(localY) * tileRowBytes +
                                                            static_cast<u64>(localX) * inputBytesPerSample;
                            }
                            StorePixel(output.GetMutableImageData(), outputWidth, outputBytesPerPixel, outputFormat, photometric, orientation, width, height,
                                       tileX + localX, tileY + localY, samples, samplesPerPixel, bitsPerSample, sampleFormat, associatedAlpha);
                        }
                    }
                }
            }
                ...
                for (u32 x = 0; result == tools::TextureImportResult::Success && x < width; ++x)
                {
                    ...
                    for (u16 sample = 0; sample < samplesPerPixel; ++sample)
                        samples[sample] = planar == PLANARCONFIG_CONTIG
                                              ...
                                              : scratch.TypedData() + static_cast<u64>(sample) * scanlineBytes + static_cast<u64>(x) * inputBytesPerSample;
                    StorePixel(output.GetMutableImageData(), outputWidth, outputBytesPerPixel, outputFormat, photometric, orientation, width, height, x, y,
    ...
    const tools::TextureImporterDescriptor g_tiffImporter{tools::importers::Tiff, "Tagged Image File Format", 1, &ProbeTiff, &DecodeTiff, nullptr};
} // namespace

namespace vanguard::texture_tools
{
    const TextureImporterDescriptor& TiffImporter() noexcept
    {
        return g_tiffImporter;
    }
} // namespace vanguard::texture_tools
