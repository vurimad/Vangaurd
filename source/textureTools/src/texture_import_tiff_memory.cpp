...
namespace
{
    ...
    [[nodiscard]] TiffAllocationHeader* GetHeader(void* address) noexcept
    {
...
extern "C"
{
    ...
    {
        if (requested <= 0 || static_cast<u64>(requested) > 0xffffffffull - sizeof(TiffAllocationHeader))
            return nullptr;
        const usize size = static_cast<usize>(requested) + sizeof(TiffAllocationHeader);
        const vanguard::memory::MemoryBlock block = vanguard::memory::Allocate(vanguard::memory::PoolId::Assets, size, 16);
        if (!block)
            return nullptr;
    ...
    void _TIFFfree(void* address)
    {
        if (address == nullptr)
            return;
        TiffAllocationHeader* const header = GetHeader(address);
        vanguard::memory::MemoryBlock block{header, static_cast<usize>(header->size), vanguard::memory::PoolId::Assets};
    ...
    {
        if (address == nullptr)
            return _TIFFmalloc(requested);
        if (requested <= 0)
        {
            _TIFFfree(address);
            return nullptr;
        }
        TiffAllocationHeader* const oldHeader = GetHeader(address);
