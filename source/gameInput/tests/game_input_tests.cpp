...
namespace
{
    ...
    void SetKey(input::FrameSnapshot& snapshot, const input::Key key, const bool down) noexcept
    {
        ...
        if (down) snapshot.keyboard.down[value >> 6u] |= mask;
        else snapshot.keyboard.down[value >> 6u] &= ~mask;
    }
    ...
    {
        for (const gi::ActionEvent& event : map.Events()) if (event.type == type) return true;
        return false;
    }
}
...
int main()
{
    Check(vanguard::memory::Initialize(), "memory initialization");
    {
        ...
        gi::ActionMap map;
        Check(map.RegisterContext({gameplay, "gameplay", gi::ContextLayer::Player, 0}) == gi::Result::Success,
              "gameplay context registration");
        Check(map.RegisterContext({menu, "menu", gi::ContextLayer::UserInterface, 0}) == gi::Result::Success,
        ...
        Check(map.RegisterAction({navigate, "navigate", gi::ActionValueType::Axis1D}) == gi::Result::Success,
              "axis action registration");
        Check(map.RegisterBinding({activateBinding, gameplay, activate, gi::Control::Keyboard(input::Key::Space)}) ==
                  gi::Result::Success, "button binding registration");
        ...
        Check(map.PushContext(gameplay) == gi::Result::Success && map.PushContext(menu) == gi::Result::Success,
              "context activation");
        ...
        Check(map.Update(snapshot, {&event, 1}, 0.0f) == gi::Result::Success &&
              HasEvent(map, gi::ActionEventType::Pressed), "press event");
        Check(map.Update(snapshot, {}, 0.26f) == gi::Result::Success &&
              HasEvent(map, gi::ActionEventType::HoldComplete) && HasEvent(map, gi::ActionEventType::Repeat),
              "hold and repeat events");
        event = KeyEvent(input::Key::Space, false);
        SetKey(snapshot, input::Key::Space, false);
        Check(map.Update(snapshot, {&event, 1}, 0.0f) == gi::Result::Success &&
              HasEvent(map, gi::ActionEventType::Released), "release event");

        for (u32 tap = 0; tap < 2; ++tap)
        {
            event = KeyEvent(input::Key::Space, true); SetKey(snapshot, input::Key::Space, true);
            Check(map.Update(snapshot, {&event, 1}, 0.05f) == gi::Result::Success, "multi-tap press");
            event = KeyEvent(input::Key::Space, false); SetKey(snapshot, input::Key::Space, false);
        ...
        Check(map.Update(snapshot, {}, 0.016f) == gi::Result::Success &&
              map.FindAction(navigate) != nullptr && map.FindAction(navigate)->x == 1.0f,
              "active UI axis mapping");
        Check(map.PopContext(gi::ContextLayer::UserInterface, menu) == gi::Result::Success &&
              map.Update(snapshot, {}, 0.016f) == gi::Result::Success && HasEvent(map, gi::ActionEventType::AxisChanged) &&
              map.FindAction(navigate)->x == 0.0f, "context removal publishes axis reset");

        gi::ContextDescriptor storedContexts[]{
            {menu, "menu", gi::ContextLayer::UserInterface, 10},
            {gameplay, "gameplay", gi::ContextLayer::Player, 0}};
        gi::ActionDescriptor storedActions[]{
            {navigate, "navigate", gi::ActionValueType::Axis1D}, action};
        gi::BindingDescriptor storedBindings[]{axis,
            {activateBinding, gameplay, activate, gi::Control::Keyboard(input::Key::Space)}};
        gi::ContextId initialContexts[]{gameplay, menu};
        gi::MappingBuildDescription mappingDescription{
            {storedContexts, 2}, {storedActions, 2}, {storedBindings, 2}, {initialContexts, 2}};
        containers::DynamicArray<u8> cooked(memory::pools::Input::GetInstance());
        filesystem::MemoryFileWriter mappingWriter(cooked);
        Check(gi::CookMapping(mappingDescription, mappingWriter) == gi::MappingResult::Success,
              "cook deterministic mapping resource");
        ...
        gi::MappingBuildDescription reversedDescription{
            {reversedContexts, 2}, {reversedActions, 2}, {reversedBindings, 2}, {initialContexts, 2}};
        containers::DynamicArray<u8> recooked(memory::pools::Input::GetInstance());
        filesystem::MemoryFileWriter recookedWriter(recooked);
        Check(gi::CookMapping(reversedDescription, recookedWriter) == gi::MappingResult::Success &&
              cooked.Size() == recooked.Size(), "recook reordered source mapping");
        bool byteExact = cooked.Size() == recooked.Size();
        for (u32 index = 0; byteExact && index < cooked.Size(); ++index) byteExact = cooked[index] == recooked[index];
        ...
        Check(mappingFile.Open(mappingReader) == gi::MappingResult::Success && mappingFile.Contexts().Size() == 2 &&
              mappingFile.Actions().Size() == 2 && mappingFile.Bindings().Size() == 2,
        ...
        Check(mappingFile.Install(installed) == gi::MappingResult::Success && installed.GetStats().compiled &&
              installed.CurrentContext(gi::ContextLayer::UserInterface) == menu,
        ...
        if (cooked.Size() > vanguard::serialization::DocumentHeader::WireSize + 4u)
        {
            ...
            Check(corrupt.Open(corruptReader) == gi::MappingResult::IntegrityFailure,
                  "reject corrupted mapping resource");
        }
    }
