...
namespace
{
    ...
    constexpr u16 KnownGroupFlags = static_cast<u16>(cell::ActivationGroupFlags::DefaultActive) | static_cast<u16>(cell::ActivationGroupFlags::EditorOnly);
    constexpr u32 KnownPlacementFlags = static_cast<u32>(cell::PlacementFlags::Persistent) | static_cast<u32>(cell::PlacementFlags::InitiallyDisabled) |
                                        static_cast<u32>(cell::PlacementFlags::EditorOnly) | static_cast<u32>(cell::PlacementFlags::NoCollision) |
                                        static_cast<u32>(cell::PlacementFlags::Interior) | static_cast<u32>(cell::PlacementFlags::Mission) |
                                        static_cast<u32>(cell::PlacementFlags::Decoration) | static_cast<u32>(cell::PlacementFlags::TwoDimensionalStreaming) |
    ...
    struct CanonicalData
    {
        CanonicalData() noexcept
            : groups(memory::pools::World::GetInstance()), placements(memory::pools::World::GetInstance()), lookup(memory::pools::World::GetInstance()),
              overrides(memory::pools::World::GetInstance()), references(memory::pools::World::GetInstance()),
              dependencies(memory::pools::Resources::GetInstance()), overrideData(memory::pools::World::GetInstance())
        {
        }
    ...
    [[nodiscard]] cell::Result Convert(const serialization::Result result) noexcept
    {
        ...
        case serialization::Result::InvalidMagic:
            return cell::Result::InvalidMagic;
        ...
        case serialization::Result::UnsupportedHeader:
            return cell::Result::UnsupportedVersion;
        case serialization::Result::IntegrityFailure:
            return cell::Result::IntegrityFailure;
    ...
    [[nodiscard]] cell::Result Convert(const schemas::Result result) noexcept
    {
        ...
        case schemas::Result::Success:
            return cell::Result::Success;
        case schemas::Result::SchemaNotFound:
            return cell::Result::UnknownSchema;
        case schemas::Result::LimitExceeded:
            return cell::Result::LimitExceeded;
        case schemas::Result::IoFailure:
            return cell::Result::IoFailure;
        default:
            return cell::Result::SchemaFailure;
        }
    }

    [[nodiscard]] cell::Result WriterResult(const serialization::BinaryWriter& writer) noexcept
    {
        return writer.IsGood() ? cell::Result::Success : Convert(writer.GetStatus());
    }

    [[nodiscard]] cell::Result ReaderResult(const serialization::BinaryReader& reader) noexcept
    {
        return reader.IsGood() ? cell::Result::Success : Convert(reader.GetStatus());
    }
    ...
    {
        for (const f32 coordinate : value.minimum)
            if (!writer.WriteF32(coordinate))
                return false;
        for (const f32 coordinate : value.maximum)
            if (!writer.WriteF32(coordinate))
                return false;
        return true;
    }
    ...
    {
        for (f32& coordinate : value.minimum)
            if (!reader.ReadF32(coordinate))
                return false;
        for (f32& coordinate : value.maximum)
            if (!reader.ReadF32(coordinate))
                return false;
        return true;
    }
    ...
    {
        for (const f32 coordinate : value.translation)
            if (!writer.WriteF32(coordinate))
                return false;
        for (const f32 coordinate : value.rotation)
            if (!writer.WriteF32(coordinate))
                return false;
        for (const f32 coordinate : value.scale)
            if (!writer.WriteF32(coordinate))
                return false;
        return true;
    }
    ...
    {
        for (f32& coordinate : value.translation)
            if (!reader.ReadF32(coordinate))
                return false;
        for (f32& coordinate : value.rotation)
            if (!reader.ReadF32(coordinate))
                return false;
        for (f32& coordinate : value.scale)
            if (!reader.ReadF32(coordinate))
                return false;
        return true;
    }
    ...
    {
        for (u32 axis = 0; axis < 3; ++axis)
            if (!std::isfinite(transform.translation[axis]) || !std::isfinite(transform.scale[axis]) || transform.scale[axis] == 0.0f)
                return false;
    ...
    {
        if (left.activationGroup != right.activationGroup)
            return left.activationGroup < right.activationGroup;
    ...
    [[nodiscard]] bool ReferenceSourceLess(const ReferenceSource& left, const ReferenceSource& right) noexcept
    {
        if (left.placementIndex != right.placementIndex)
            return left.placementIndex < right.placementIndex;
        if (left.source.slot != right.source.slot)
    ...
    [[nodiscard]] const cell::DependencyRecord* FindDependency(const containers::ArraySpan<const cell::DependencyRecord> dependencies,
                                                               const resources::ResourceReference resource) noexcept
    {
        ...
        while (count != 0)
        {
            ...
            if (candidate.resource.GetPath() < resource.GetPath() ||
                (candidate.resource.GetPath() == resource.GetPath() && candidate.resource.ExpectedType() < resource.ExpectedType()))
            {
                first = index + 1u;
                count -= step + 1u;
            }
    ...
    [[nodiscard]] const cell::ActivationGroupRecord* FindGroup(const containers::DynamicArray<cell::ActivationGroupRecord>& groups, const u64 stableId) noexcept
    {
        u32 first = 0;
        ...
        while (count != 0)
        {
            const u32 step = count / 2u;
            const u32 index = first + step;
            if (groups[index].stableId < stableId)
            {
                first = index + 1u;
                count -= step + 1u;
            }
    ...
    [[nodiscard]] const cell::EntityLookupRecord* FindLookup(const containers::DynamicArray<cell::EntityLookupRecord>& lookup, const u64 entityId) noexcept
    {
        u32 first = 0;
        ...
        while (count != 0)
        {
            const u32 step = count / 2u;
            const u32 index = first + step;
            if (lookup[index].entityId < entityId)
            {
                first = index + 1u;
                count -= step + 1u;
            }
    ...
    [[nodiscard]] bool AddDependency(containers::DynamicArray<cell::DependencyRecord>& dependencies, resources::ResourceReference reference,
                                     resources::DependencyKind kind) noexcept
    {
        if (!reference.IsValid())
            return kind != resources::DependencyKind::Required;
        if (!reference.IsTyped() || kind > resources::DependencyKind::Soft)
            return false;
    ...
    [[nodiscard]] bool DependencyVisitor(resources::ResourceReference reference, resources::DependencyKind kind, void* userData) noexcept
    {
    ...
    [[nodiscard]] bool AlignData(ByteArray& data, const u32 alignment) noexcept
    {
        ...
        if (data.Size() != aligned)
            return false;
        for (u32 index = previous; index < aligned; ++index)
            data[index] = 0;
        return true;
    }
    ...
    {
        if (size > 0xffffffffu - destination.Size())
            return false;
        const u32 offset = destination.Size();
        destination.Resize(offset + size);
        if (destination.Size() != offset + size)
            return false;
        ...
        for (u32 start = 0; start < placements.Size(); ++start)
        {
            ...
            for (;;)
            {
                if (states[cursor] == 2)
                    break;
                if (states[cursor] == 1)
                    return cell::Result::HierarchyCycle;
                states[cursor] = 1;
                ...
                if (parentId == cell::InvalidEntityId)
                    break;
                const cell::EntityLookupRecord* parentLookup = FindLookup(lookup, parentId);
                if (parentLookup == nullptr)
                    return cell::Result::MissingParent;
                const cell::PlacementRecord& parent = placements[parentLookup->placementIndex];
                if (parent.activationGroup != cell::AlwaysActiveGroup && parent.activationGroup != placements[cursor].activationGroup)
                    return cell::Result::InvalidLayout;
                cursor = parentLookup->placementIndex;
            }
            for (const u32 index : path)
                states[index] = 2;
        }
        return cell::Result::Success;
    }
    ...
    {
        if (description.cellId == 0 || description.worldId == 0 || description.placements.Empty() || description.category > cell::CellCategory::Generic ||
            !IsFiniteBounds(description.bounds))
            return cell::Result::InvalidArgument;
        ...
        for (const cell::ActivationGroupBuildRecord& source : description.activationGroups)
        {
            if (source.stableId == cell::AlwaysActiveGroup || (static_cast<u16>(source.flags) & ~KnownGroupFlags) != 0)
                return cell::Result::InvalidArgument;
            if (!description.includeEditorData && (static_cast<u16>(source.flags) & static_cast<u16>(cell::ActivationGroupFlags::EditorOnly)) != 0)
                continue;
        ...
        for (u32 index = 1; index < output.groups.Size(); ++index)
            if (output.groups[index - 1u].stableId == output.groups[index].stableId)
        ...
        {
            if (source.entityId == cell::InvalidEntityId || !source.prefab.IsValid() || source.prefab.ExpectedType() != prefabs::PrefabResourceType ||
                !IsValidTransform(source.transform) || !IsFiniteBounds(source.bounds) || !std::isfinite(source.streamingDistance) ||
                source.streamingDistance < 0.0f || !std::isfinite(source.visibilityDistance) || source.visibilityDistance < 0.0f ||
                (static_cast<u32>(source.flags) & ~KnownPlacementFlags) != 0)
                return cell::Result::InvalidArgument;
            for (const f32 coordinate : source.streamingReferencePoint)
                if (!std::isfinite(coordinate))
                    return cell::Result::InvalidArgument;
            if (!description.includeEditorData && (static_cast<u32>(source.flags) & static_cast<u32>(cell::PlacementFlags::EditorOnly)) != 0)
                continue;
            if (FindGroup(output.groups, source.activationGroup) == nullptr)
                return cell::Result::UnknownActivationGroup;
        ...
        if (placementIndex != output.placements.Size())
            return cell::Result::InvalidLayout;
        ...
        {
            if (source.entityId == cell::InvalidEntityId || source.componentStableId == prefabs::InvalidStableId || source.mode > cell::OverrideMode::Remove ||
                (static_cast<u16>(source.flags) & ~KnownOverrideFlags) != 0)
                return cell::Result::InvalidArgument;
            if (!description.includeEditorData && (static_cast<u16>(source.flags) & static_cast<u16>(cell::OverrideFlags::EditorOnly)) != 0)
                continue;
            ...
            {
                if (source.schema != nullptr || source.object != nullptr)
                    return cell::Result::InvalidArgument;
            }
        ...
        for (u32 index = 0; index < overrideSources.Size(); ++index)
        {
            ...
            if (source.mode != cell::OverrideMode::Remove)
            {
                ...
                if (schemaResult != schemas::Result::Success)
                    return Convert(schemaResult);
                const schemas::Result dependencyResult = schemas::VisitDependencies(*source.schema, source.object, DependencyVisitor, &output.dependencies);
                if (dependencyResult != schemas::Result::Success)
                    return Convert(dependencyResult);
                if (!AlignData(output.overrideData, 8))
                    return cell::Result::LimitExceeded;
                ...
                if (!AppendBytes(output.overrideData, bytes.Data(), bytes.Size()))
                    return cell::Result::LimitExceeded;
            }
        ...
        if (overrideIndex != output.overrides.Size())
            return cell::Result::InvalidLayout;
        ...
        {
            if (source.sourceEntityId == cell::InvalidEntityId || source.targetEntityId == cell::InvalidEntityId || source.slot == 0 ||
                source.kind > cell::EntityReferenceKind::OptionalWorld)
                return cell::Result::InvalidArgument;
            const cell::EntityLookupRecord* sourceLookup = FindLookup(output.lookup, source.sourceEntityId);
            if (sourceLookup == nullptr)
                return cell::Result::InvalidReference;
            if ((source.kind == cell::EntityReferenceKind::RequiredLocal || source.kind == cell::EntityReferenceKind::OptionalLocal) &&
                FindLookup(output.lookup, source.targetEntityId) == nullptr)
                return cell::Result::InvalidReference;
        ...
        for (cell::PlacementRecord& placement : output.placements)
        {
            ...
            while (referenceIndex < output.references.Size() && output.references[referenceIndex].sourceEntityId == placement.entityId)
            {
                ++placement.referenceCount;
                ++referenceIndex;
            }
        }
        if (referenceIndex != output.references.Size())
            return cell::Result::InvalidLayout;
    ...
    [[nodiscard]] bool ReadGroup(serialization::BinaryReader& reader, cell::ActivationGroupRecord& value) noexcept
    {
        u16 flags = 0;
        u16 reserved = 0;
        if (!reader.ReadU64(value.stableId) || !reader.ReadU64(value.name) || !reader.ReadU32(value.firstPlacement) || !reader.ReadU32(value.placementCount) ||
            !reader.ReadU16(flags) || !reader.ReadU16(reserved))
            return false;
    ...
    [[nodiscard]] bool WritePlacement(serialization::BinaryWriter& writer, const cell::PlacementRecord& value) noexcept
    {
        if (!writer.WriteU64(value.entityId) || !writer.WriteU64(value.parentEntityId) || !writer.WriteU64(value.name) ||
            ...
            return false;
        for (const f32 coordinate : value.streamingReferencePoint)
            if (!writer.WriteF32(coordinate))
                return false;
        return writer.WriteF32(value.streamingDistance) && writer.WriteF32(value.visibilityDistance) && writer.WriteU64(value.layerMask) &&
               ...
               writer.WriteU32(value.referenceCount) && writer.WriteU32(static_cast<u32>(value.flags)) && writer.WriteU16(value.timeOfDayVisibilityMask) &&
               writer.WriteU8(value.streamingPriority) && writer.WriteU8(0);
    }
    ...
    [[nodiscard]] bool ReadPlacement(serialization::BinaryReader& reader, cell::PlacementRecord& value) noexcept
    {
        u32 flags = 0;
        u8 reserved = 0;
        if (!reader.ReadU64(value.entityId) || !reader.ReadU64(value.parentEntityId) || !reader.ReadU64(value.name) || !reader.ReadU64(value.activationGroup) ||
            !ReadReference(reader, value.prefab) || !ReadTransform(reader, value.transform) || !ReadBounds(reader, value.bounds))
            return false;
        for (f32& coordinate : value.streamingReferencePoint)
            if (!reader.ReadF32(coordinate))
                return false;
        if (!reader.ReadF32(value.streamingDistance) || !reader.ReadF32(value.visibilityDistance) || !reader.ReadU64(value.layerMask) ||
            ...
            !reader.ReadU8(value.streamingPriority) || !reader.ReadU8(reserved))
            return false;
    ...
    [[nodiscard]] bool WriteOverride(serialization::BinaryWriter& writer, const cell::ComponentOverrideRecord& value) noexcept
    {
        return writer.WriteU64(value.entityId) && writer.WriteU64(value.componentStableId) && writer.WriteU64(value.schema) &&
               writer.WriteU16(value.schemaVersion) && writer.WriteU16(static_cast<u16>(value.flags)) && writer.WriteU8(static_cast<u8>(value.mode)) &&
               writer.WriteU8(0) && writer.WriteU16(0) && writer.WriteU64(value.dataOffset) && writer.WriteU64(value.dataSize) &&
               WriteDigest(writer, value.dataFingerprint);
    }
    ...
    [[nodiscard]] bool ReadOverride(serialization::BinaryReader& reader, cell::ComponentOverrideRecord& value) noexcept
    {
        ...
        if (!reader.ReadU64(value.entityId) || !reader.ReadU64(value.componentStableId) || !reader.ReadU64(value.schema) ||
            !reader.ReadU16(value.schemaVersion) || !reader.ReadU16(flags) || !reader.ReadU8(mode) || !reader.ReadU8(reserved8) ||
            !reader.ReadU16(reserved16) || !reader.ReadU64(value.dataOffset) || !reader.ReadU64(value.dataSize) || !ReadDigest(reader, value.dataFingerprint))
            return false;
    ...
    [[nodiscard]] bool ReadEntityReference(serialization::BinaryReader& reader, cell::EntityReferenceRecord& value) noexcept
    {
        ...
        u16 reserved16 = 0;
        u32 reserved32 = 0;
        if (!reader.ReadU64(value.sourceEntityId) || !reader.ReadU64(value.slot) || !reader.ReadU64(value.targetEntityId) || !reader.ReadU8(kind) ||
            !reader.ReadU8(reserved8) || !reader.ReadU16(reserved16) || !reader.ReadU32(reserved32))
            return false;
    ...
    [[nodiscard]] cell::Result BuildBody(const CanonicalData& data, ByteArray& body, crypto::Digest256& fingerprint) noexcept
    {
        ...
        if (!writer.WriteU64(data.cellId) || !writer.WriteU64(data.worldId))
            return WriterResult(writer);
        for (const i32 coordinate : data.gridCoordinate)
        ...
        for (const f64 coordinate : data.origin)
            if (!writer.WriteF64(coordinate))
                return WriterResult(writer);
        if (!WriteBounds(writer, data.bounds) || !WriteDigest(writer, data.sourceFingerprint) || !writer.WriteU32(data.groups.Size()) ||
            !writer.WriteU32(data.placements.Size()) || !writer.WriteU32(data.lookup.Size()) || !writer.WriteU32(data.overrides.Size()) ||
            !writer.WriteU32(data.references.Size()) || !writer.WriteU32(data.dependencies.Size()) || !writer.WriteU64(data.overrideData.Size()))
            return WriterResult(writer);
        for (const cell::ActivationGroupRecord& value : data.groups)
            if (!WriteGroup(writer, value))
                return WriterResult(writer);
        for (const cell::PlacementRecord& value : data.placements)
            if (!WritePlacement(writer, value))
                return WriterResult(writer);
        for (const cell::EntityLookupRecord& value : data.lookup)
            if (!WriteLookup(writer, value))
                return WriterResult(writer);
        for (const cell::ComponentOverrideRecord& value : data.overrides)
            if (!WriteOverride(writer, value))
                return WriterResult(writer);
        for (const cell::EntityReferenceRecord& value : data.references)
            if (!WriteEntityReference(writer, value))
                return WriterResult(writer);
        for (const cell::DependencyRecord& value : data.dependencies)
            if (!WriteDependency(writer, value))
                return WriterResult(writer);
        if (!writer.WriteBytes(data.overrideData.Data(), data.overrideData.Size()))
            return WriterResult(writer);
        fingerprint = crypto::Sha256(body.Data(), body.Size());
        return cell::Result::Success;
    }

    [[nodiscard]] cell::Result WriteDocument(filesystem::IFile& file, const ByteArray& body, const crypto::Digest256& fingerprint) noexcept
    {
    ...
    [[nodiscard]] cell::Result ValidateLoaded(const cell::CellFile& file, const containers::DynamicArray<cell::EntityLookupRecord>& lookup,
                                              const u64 overrideDataSize) noexcept
    {
        const auto groups = file.GetActivationGroups();
        const auto placements = file.GetPlacements();
        const auto overrides = file.GetOverrides();
        const auto references = file.GetEntityReferences();
        const auto dependencies = file.GetDependencies();
        if (file.GetCellId() == 0 || file.GetWorldId() == 0 || groups.Empty() || placements.Empty() || groups[0].stableId != cell::AlwaysActiveGroup ||
            file.GetCategory() > cell::CellCategory::Generic || !IsFiniteBounds(file.GetCellBounds()))
            return cell::Result::InvalidLayout;
        for (u32 axis = 0; axis < 3; ++axis)
            if (!std::isfinite(file.GetOrigin()[axis]))
                return cell::Result::InvalidLayout;
        u32 expectedPlacement = 0;
        for (u32 index = 0; index < groups.Size(); ++index)
        {
            const cell::ActivationGroupRecord& group = groups[index];
            if ((index > 0 && groups[index - 1u].stableId >= group.stableId) || (static_cast<u16>(group.flags) & ~KnownGroupFlags) != 0 ||
                group.firstPlacement != expectedPlacement || group.placementCount > placements.Size() - expectedPlacement)
                return cell::Result::InvalidLayout;
            for (u32 local = 0; local < group.placementCount; ++local)
            {
                const cell::PlacementRecord& placement = placements[expectedPlacement + local];
                if (placement.activationGroup != group.stableId || (local > 0 && placements[expectedPlacement + local - 1u].entityId >= placement.entityId))
                    return cell::Result::InvalidLayout;
            }
            expectedPlacement += group.placementCount;
        }
        if (expectedPlacement != placements.Size() || lookup.Size() != placements.Size())
            return cell::Result::InvalidLayout;
        for (u32 index = 0; index < lookup.Size(); ++index)
            if (lookup[index].entityId == cell::InvalidEntityId || lookup[index].placementIndex >= placements.Size() ||
                ...
                (index > 0 && lookup[index - 1u].entityId >= lookup[index].entityId))
                return cell::Result::InvalidLayout;
        ...
        for (const cell::PlacementRecord& placement : placements)
            hierarchyPlacements.PushBack(placement);
        if (hierarchyPlacements.Size() != placements.Size())
            return cell::Result::LimitExceeded;
        const cell::Result hierarchyResult = ValidateHierarchy(hierarchyPlacements, lookup);
        if (hierarchyResult != cell::Result::Success)
            return hierarchyResult;
        u32 expectedOverride = 0;
        u32 expectedReference = 0;
        for (const cell::PlacementRecord& placement : placements)
        {
            if (!placement.prefab.IsValid() || placement.prefab.ExpectedType() != prefabs::PrefabResourceType || !IsValidTransform(placement.transform) ||
                !IsFiniteBounds(placement.bounds) || !std::isfinite(placement.streamingDistance) || placement.streamingDistance < 0.0f ||
                !std::isfinite(placement.visibilityDistance) || placement.visibilityDistance < 0.0f ||
                (static_cast<u32>(placement.flags) & ~KnownPlacementFlags) != 0 || placement.firstOverride != expectedOverride ||
                placement.overrideCount > overrides.Size() - expectedOverride || placement.firstReference != expectedReference ||
                placement.referenceCount > references.Size() - expectedReference)
                return cell::Result::InvalidLayout;
            for (const f32 coordinate : placement.streamingReferencePoint)
                if (!std::isfinite(coordinate))
                    return cell::Result::InvalidLayout;
            for (u32 local = 0; local < placement.overrideCount; ++local)
                if (overrides[expectedOverride + local].entityId != placement.entityId)
                    return cell::Result::InvalidLayout;
            for (u32 local = 0; local < placement.referenceCount; ++local)
                if (references[expectedReference + local].sourceEntityId != placement.entityId)
                    return cell::Result::InvalidLayout;
            expectedOverride += placement.overrideCount;
            expectedReference += placement.referenceCount;
        }
        if (expectedOverride != overrides.Size() || expectedReference != references.Size())
            return cell::Result::InvalidLayout;
        ...
        for (u32 index = 0; index < overrides.Size(); ++index)
        {
            ...
            if (record.entityId == cell::InvalidEntityId || record.componentStableId == prefabs::InvalidStableId || record.mode > cell::OverrideMode::Remove ||
                (static_cast<u16>(record.flags) & ~KnownOverrideFlags) != 0 || FindLookup(lookup, record.entityId) == nullptr)
                return cell::Result::InvalidLayout;
            if (record.mode == cell::OverrideMode::Remove)
            {
                if (record.schema != reflection::InvalidSchemaTypeId || record.schemaVersion != 0 || record.dataOffset != 0 || record.dataSize != 0 ||
                    !record.dataFingerprint.IsEmpty())
                    return cell::Result::InvalidLayout;
            }
            else
            {
                expectedDataOffset = (expectedDataOffset + 7u) & ~7ull;
                const auto bytes = file.GetOverrideData(record);
                if (record.schema == reflection::InvalidSchemaTypeId || record.schemaVersion == 0 || record.dataSize == 0 ||
                    record.dataOffset != expectedDataOffset || bytes.Size() != record.dataSize)
                    return cell::Result::InvalidLayout;
                if (crypto::Sha256(bytes.Data(), bytes.Size()) != record.dataFingerprint)
                    return cell::Result::IntegrityFailure;
                expectedDataOffset += record.dataSize;
            }
            if (index > 0 && overrides[index - 1u].entityId == record.entityId && overrides[index - 1u].componentStableId >= record.componentStableId)
                return cell::Result::InvalidLayout;
        }
        if (expectedDataOffset != overrideDataSize)
            return cell::Result::InvalidLayout;
        for (u32 index = 0; index < references.Size(); ++index)
        {
            const cell::EntityReferenceRecord& reference = references[index];
            if (reference.sourceEntityId == cell::InvalidEntityId || reference.targetEntityId == cell::InvalidEntityId || reference.slot == 0 ||
                reference.kind > cell::EntityReferenceKind::OptionalWorld || FindLookup(lookup, reference.sourceEntityId) == nullptr ||
                ((reference.kind == cell::EntityReferenceKind::RequiredLocal || reference.kind == cell::EntityReferenceKind::OptionalLocal) &&
                 FindLookup(lookup, reference.targetEntityId) == nullptr))
                return cell::Result::InvalidLayout;
            if (index > 0 && references[index - 1u].sourceEntityId == reference.sourceEntityId && references[index - 1u].slot >= reference.slot)
                return cell::Result::InvalidLayout;
        }
...
namespace vanguard::world
{
    const char* ToString(const Result result) noexcept
    {
        ...
        case Result::MissingParent:
            return "MissingParent";
        case Result::HierarchyCycle:
            return "HierarchyCycle";
        case Result::UnknownActivationGroup:
            return "UnknownActivationGroup";
        case Result::InvalidReference:
            return "InvalidReference";
        case Result::UnknownSchema:
    ...
    Result CellFile::Open(filesystem::IFile& file, const ReadLimits& limits) noexcept
    {
        ...
        const serialization::Result headerResult = serialization::ReadDocumentHeader(reader, CellMagic, {1, 0, 0}, documentLimits, header);
        ...
        if (sectionResult != serialization::Result::Success)
            return Convert(sectionResult);
        if (sections.Size() != 1 || sections[0].id != CellSection || sections[0].version != FileVersion || sections[0].codec != serialization::Codec::None ||
        ...
        u8 category = 0;
        u16 reserved = 0;
        if (!bodyReader.ReadU64(m_cellId) || !bodyReader.ReadU64(m_worldId))
            return ReaderResult(bodyReader);
        for (i32& coordinate : m_gridCoordinate)
            if (!bodyReader.ReadI32(coordinate))
                return ReaderResult(bodyReader);
        ...
        if (!ReadBounds(bodyReader, m_bounds) || !ReadDigest(bodyReader, m_sourceFingerprint) || !bodyReader.ReadU32(groupCount) ||
            !bodyReader.ReadU32(placementCount) || !bodyReader.ReadU32(lookupCount) || !bodyReader.ReadU32(overrideCount) ||
            !bodyReader.ReadU32(referenceCount) || !bodyReader.ReadU32(dependencyCount) || !bodyReader.ReadU64(overrideDataSize))
            return ReaderResult(bodyReader);
        const u32 maximumOverrideData = limits.maximumOverrideDataBytes > 0xffffffffull ? 0xffffffffu : static_cast<u32>(limits.maximumOverrideDataBytes);
        if (overrideDataSize > limits.maximumOverrideDataBytes || overrideDataSize > 0xffffffffull ||
            ...
            !ResizeChecked(m_placements, placementCount, limits.maximumPlacements) || !ResizeChecked(m_entityLookup, lookupCount, limits.maximumPlacements) ||
            ...
            !ResizeChecked(m_overrideData, static_cast<u32>(overrideDataSize), maximumOverrideData))
            return Result::LimitExceeded;
        for (ActivationGroupRecord& value : m_activationGroups)
            if (!ReadGroup(bodyReader, value))
                return Result::InvalidLayout;
        for (PlacementRecord& value : m_placements)
            if (!ReadPlacement(bodyReader, value))
                return Result::InvalidLayout;
        for (EntityLookupRecord& value : m_entityLookup)
            if (!ReadLookup(bodyReader, value))
                return Result::InvalidLayout;
        for (ComponentOverrideRecord& value : m_overrides)
            if (!ReadOverride(bodyReader, value))
                return Result::InvalidLayout;
        for (EntityReferenceRecord& value : m_entityReferences)
            if (!ReadEntityReference(bodyReader, value))
                return Result::InvalidLayout;
        for (DependencyRecord& value : m_dependencies)
            if (!ReadDependency(bodyReader, value))
                return Result::InvalidLayout;
        if (!bodyReader.ReadBytes(m_overrideData.Data(), m_overrideData.Size()))
            return ReaderResult(bodyReader);
    ...
    bool CellFile::IsOpen() const noexcept
    {
        return m_open;
    }
    u64 CellFile::GetCellId() const noexcept
    {
        return m_cellId;
    }
    u64 CellFile::GetWorldId() const noexcept
    {
        return m_worldId;
    }
    const i32* CellFile::GetGridCoordinate() const noexcept
    {
        return m_gridCoordinate;
    }
    u8 CellFile::GetHierarchyLevel() const noexcept
    {
        return m_hierarchyLevel;
    }
    CellCategory CellFile::GetCategory() const noexcept
    {
        return m_category;
    }
    const f64* CellFile::GetOrigin() const noexcept
    {
        return m_origin;
    }
    const Bounds& CellFile::GetCellBounds() const noexcept
    {
        return m_bounds;
    }
    const crypto::Digest256& CellFile::GetSourceFingerprint() const noexcept
    {
        return m_sourceFingerprint;
    }
    const crypto::Digest256& CellFile::GetContentFingerprint() const noexcept
    {
        return m_contentFingerprint;
    }
    containers::ArraySpan<const ActivationGroupRecord> CellFile::GetActivationGroups() const noexcept
    {
        return m_activationGroups;
    }
    containers::ArraySpan<const PlacementRecord> CellFile::GetPlacements() const noexcept
    {
        return m_placements;
    }
    containers::ArraySpan<const ComponentOverrideRecord> CellFile::GetOverrides() const noexcept
    {
        return m_overrides;
    }
    containers::ArraySpan<const EntityReferenceRecord> CellFile::GetEntityReferences() const noexcept
    {
        return m_entityReferences;
    }
    containers::ArraySpan<const DependencyRecord> CellFile::GetDependencies() const noexcept
    {
        return m_dependencies;
    }

    containers::ArraySpan<const PlacementRecord> CellFile::GetPlacementsInGroup(const ActivationGroupRecord& group) const noexcept
    {
        if (!m_open || group.firstPlacement > m_placements.Size() || group.placementCount > m_placements.Size() - group.firstPlacement)
            return {};
        return {m_placements.TypedData() + group.firstPlacement, group.placementCount};
    }

    containers::ArraySpan<const ComponentOverrideRecord> CellFile::GetOverridesFor(const PlacementRecord& placement) const noexcept
    {
        if (!m_open || placement.firstOverride > m_overrides.Size() || placement.overrideCount > m_overrides.Size() - placement.firstOverride)
            return {};
        return {m_overrides.TypedData() + placement.firstOverride, placement.overrideCount};
    }

    containers::ArraySpan<const EntityReferenceRecord> CellFile::GetReferencesFor(const PlacementRecord& placement) const noexcept
    {
        if (!m_open || placement.firstReference > m_entityReferences.Size() || placement.referenceCount > m_entityReferences.Size() - placement.firstReference)
            return {};
        return {m_entityReferences.TypedData() + placement.firstReference, placement.referenceCount};
    }

    containers::ArraySpan<const u8> CellFile::GetOverrideData(const ComponentOverrideRecord& record) const noexcept
    {
        if (!m_open || record.dataOffset > m_overrideData.Size() || record.dataSize > m_overrideData.Size() - record.dataOffset ||
            record.dataSize > 0xffffffffull)
            return {};
    ...
    resources::ResourceTypeId CellResource::GetType() const noexcept
    {
        return CellResourceType;
    }
    ...
    {
        if (!m_file.IsOpen() || !m_prefabs.Empty())
            return false;
        for (u32 index = 0; index < context.GetDependencyCount(); ++index)
        {
            const resources::ResourceReference reference = context.GetDependencyReference(index);
            if (reference.ExpectedType() != prefabs::PrefabResourceType)
                continue;
            const resources::ResourceHandle& dependency = context.GetDependency(index);
            if (!dependency.IsValid() || dependency.GetType() != prefabs::PrefabResourceType)
                return false;
            const u32 expectedSize = m_prefabs.Size() + 1u;
            m_prefabs.PushBack(dependency);
            if (m_prefabs.Size() != expectedSize)
                return false;
        }

        for (const PlacementRecord& placement : m_file.GetPlacements())
            if (ResolvePrefab(placement.prefab) == nullptr)
                return false;
        return true;
    }
    ...
    {
        if (!reference.IsValid() || reference.ExpectedType() != prefabs::PrefabResourceType)
            return nullptr;
        for (const resources::ResourceHandle& handle : m_prefabs)
            if (handle.GetPath() == reference.GetPath() && handle.GetType() == prefabs::PrefabResourceType)
                return &static_cast<const prefabs::PrefabResource*>(handle.Get())->GetFile();
        return nullptr;
    }

    const CellFile& CellResource::GetFile() const noexcept
    {
        return m_file;
    }
