...
namespace
{
    ...
    constexpr u32 KnownCellFlags = static_cast<u32>(world::WorldCellFlags::AlwaysLoaded) |
                                   static_cast<u32>(world::WorldCellFlags::Interior) |
                                   static_cast<u32>(world::WorldCellFlags::Mission) |
                                   static_cast<u32>(world::WorldCellFlags::TwoDimensionalStreaming) |
                                   static_cast<u32>(world::WorldCellFlags::EditorOnly) |
                                   static_cast<u32>(world::WorldCellFlags::AllowDistanceBoosting);
    constexpr u32 KnownProxyFlags = static_cast<u32>(world::DistantProxyFlags::ProxyOnly) |
                                    static_cast<u32>(world::DistantProxyFlags::Building) |
                                    static_cast<u32>(world::DistantProxyFlags::Terrain) |
                                    static_cast<u32>(world::DistantProxyFlags::Road) |
                                    static_cast<u32>(world::DistantProxyFlags::Water) |
                                    static_cast<u32>(world::DistantProxyFlags::Decoration) |
                                    static_cast<u32>(world::DistantProxyFlags::Mission) |
                                    static_cast<u32>(world::DistantProxyFlags::TwoDimensionalStreaming) |
                                    static_cast<u32>(world::DistantProxyFlags::KeepResident) |
                                    static_cast<u32>(world::DistantProxyFlags::EditorOnly) |
    ...
    struct CanonicalData
    {
        CanonicalData() noexcept
            : cells(memory::pools::World::GetInstance()), proxies(memory::pools::World::GetInstance()),
              children(memory::pools::World::GetInstance()), dependencies(memory::pools::Resources::GetInstance())
        {
        }
    ...
    [[nodiscard]] world::Result Convert(const serialization::Result result) noexcept
    {
        switch (result)
        {
        case serialization::Result::Success: return world::Result::Success;
        case serialization::Result::InvalidMagic: return world::Result::InvalidMagic;
        case serialization::Result::UnsupportedVersion:
        case serialization::Result::UnsupportedByteOrder:
        case serialization::Result::UnsupportedHeader: return world::Result::UnsupportedVersion;
        case serialization::Result::IntegrityFailure: return world::Result::IntegrityFailure;
        case serialization::Result::LimitExceeded:
        case serialization::Result::Overflow: return world::Result::LimitExceeded;
        case serialization::Result::InvalidArgument: return world::Result::InvalidArgument;
        case serialization::Result::IoFailure:
        case serialization::Result::WrongStreamMode:
        case serialization::Result::EndOfStream: return world::Result::IoFailure;
        default: return world::Result::InvalidLayout;
        }
    }

    [[nodiscard]] world::Result WriterResult(const serialization::BinaryWriter& writer) noexcept
    {
        return writer.Good() ? world::Result::Success : Convert(writer.Status());
    }

    [[nodiscard]] world::Result ReaderResult(const serialization::BinaryReader& reader) noexcept
    {
        return reader.Good() ? world::Result::Success : Convert(reader.Status());
    }
    ...
    {
        for (const f64 coordinate : value.minimum) if (!writer.WriteF64(coordinate)) return false;
        for (const f64 coordinate : value.maximum) if (!writer.WriteF64(coordinate)) return false;
        return true;
    }

    [[nodiscard]] bool ReadBounds(serialization::BinaryReader& reader, world::WorldBounds& value) noexcept
    {
        for (f64& coordinate : value.minimum) if (!reader.ReadF64(coordinate)) return false;
        for (f64& coordinate : value.maximum) if (!reader.ReadF64(coordinate)) return false;
        return true;
    }
    ...
    {
        for (u32 axis = 0; axis < 3; ++axis)
            if (inner.minimum[axis] < outer.minimum[axis] || inner.maximum[axis] > outer.maximum[axis]) return false;
        return true;
    }
    ...
    {
        if (left.proxyId != right.proxyId) return left.proxyId < right.proxyId;
    ...
    [[nodiscard]] const Record* FindById(const containers::ArraySpan<const Record> records, const Id id) noexcept
    {
        u32 first = 0;
        ...
        while (count != 0)
        {
            ...
            {
                if constexpr (requires { records[index].cellId; }) return records[index].cellId;
                else return records[index].proxyId;
            }();
            if (candidate < id) { first = index + 1u; count -= step + 1u; }
            else count = step;
        }
        if (first >= records.Size()) return nullptr;
        if constexpr (requires { records[first].cellId; }) return records[first].cellId == id ? &records[first] : nullptr;
        else return records[first].proxyId == id ? &records[first] : nullptr;
    }

    [[nodiscard]] bool AddDependency(containers::DynamicArray<world::DependencyRecord>& dependencies,
                                     const resources::ResourceReference reference) noexcept
    {
        if (!reference.IsValid() || !reference.IsTyped()) return false;
        for (const world::DependencyRecord& existing : dependencies) if (existing.resource == reference) return true;
                                                      ...
                                                      const world::WorldBounds& worldBounds) noexcept
    {
        ...
        {
            if (!Contains(worldBounds, cell.bounds)) return world::Result::InvalidLayout;
            if (cell.parentCellId == world::InvalidCellId) continue;
            const world::WorldCellRecord* parent = FindById(view, cell.parentCellId);
            if (parent == nullptr) return world::Result::MissingParent;
                                                       ...
                                                       const world::WorldBounds& worldBounds) noexcept
    {
        ...
        if (states.Size() != proxies.Size()) return world::Result::LimitExceeded;
        for (u32 index = 0; index < states.Size(); ++index) states[index] = 0;
        for (u32 start = 0; start < proxies.Size(); ++start)
        {
            if (!Contains(worldBounds, proxies[start].bounds)) return world::Result::InvalidLayout;
            u32 cursor = start;
            while (states[cursor] != 2)
            {
                if (states[cursor] == 1) return world::Result::HierarchyCycle;
                states[cursor] = 1;
                const u64 parentId = proxies[cursor].parentProxyId;
                if (parentId == world::InvalidProxyId) break;
    ...
    [[nodiscard]] world::Result Canonicalize(const world::WorldBuildDescription& description, CanonicalData& output) noexcept
    {
        ...
        for (const f64 coordinate : description.origin) if (!std::isfinite(coordinate)) return world::Result::InvalidArgument;
        output.worldId = description.worldId;
        output.bounds = description.bounds;
        output.sourceFingerprint = description.sourceFingerprint;
        for (u32 axis = 0; axis < 3; ++axis) output.origin[axis] = description.origin[axis];

        output.cells.Reserve(description.cells.Size());
        for (const world::WorldCellBuildRecord& source : description.cells)
        {
            if (source.cellId == world::InvalidCellId || !source.cell.IsValid() ||
                source.cell.ExpectedType() != world::CellResourceType || source.category > world::CellCategory::Generic ||
                !IsFiniteBounds(source.bounds) || !std::isfinite(source.activationDistance) || source.activationDistance < 0.0f ||
                !std::isfinite(source.retentionDistance) || source.retentionDistance < source.activationDistance ||
                (static_cast<u32>(source.flags) & ~KnownCellFlags) != 0) return world::Result::InvalidArgument;
            ...
            if (!validCoordinates) return world::Result::InvalidArgument;
            if (!description.includeEditorData && (static_cast<u32>(source.flags) &
                static_cast<u32>(world::WorldCellFlags::EditorOnly)) != 0) continue;
            ...
            if (!AddDependency(output.dependencies, source.cell)) return world::Result::LimitExceeded;
        }
        if (output.cells.Empty()) return world::Result::InvalidArgument;
        std::sort(output.cells.Begin(), output.cells.End(), CellLess);
        for (u32 index = 1; index < output.cells.Size(); ++index)
            if (output.cells[index - 1u].cellId == output.cells[index].cellId) return world::Result::DuplicateIdentifier;
        world::Result result = ValidateCellHierarchy(output.cells, output.bounds);
        if (result != world::Result::Success) return result;
        ...
        {
            if (source.proxyId == world::InvalidProxyId || source.ownerCellId == world::InvalidCellId ||
                !source.mesh.IsValid() || source.mesh.ExpectedType() != meshes::MeshResourceType ||
                !IsFiniteBounds(source.bounds) || !std::isfinite(source.streamingDistance) ||
                !std::isfinite(source.secondaryReferencePointDistance) || !std::isfinite(source.nearHideDistance) ||
                source.nearHideDistance < 0.0f || source.secondaryReferencePointDistance < 0.0f ||
                source.streamingDistance < source.nearHideDistance ||
                (static_cast<u32>(source.flags) & ~KnownProxyFlags) != 0) return world::Result::InvalidArgument;
            for (u32 axis = 0; axis < 3; ++axis)
                if (!std::isfinite(source.streamingReferencePoint[axis]) ||
                    !std::isfinite(source.preboostStreamingReferencePoint[axis]) ||
                    !std::isfinite(source.secondaryReferencePoint[axis])) return world::Result::InvalidArgument;
            if (!description.includeEditorData && (static_cast<u32>(source.flags) &
                static_cast<u32>(world::DistantProxyFlags::EditorOnly)) != 0) continue;
            const world::WorldCellRecord* ownerCell =
                FindById(containers::ArraySpan<const world::WorldCellRecord>(output.cells), source.ownerCellId);
            if (ownerCell == nullptr) return world::Result::InvalidReference;
            if (!Contains(ownerCell->bounds, source.bounds)) return world::Result::InvalidLayout;
            ...
            if (!AddDependency(output.dependencies, source.mesh)) return world::Result::LimitExceeded;
        }
        std::sort(output.proxies.Begin(), output.proxies.End(), ProxyLess);
        for (u32 index = 1; index < output.proxies.Size(); ++index)
            if (output.proxies[index - 1u].proxyId == output.proxies[index].proxyId) return world::Result::DuplicateIdentifier;
        ...
        for (const world::ProxyChildBuildRecord& source : description.proxyChildren)
        {
            ...
            {
                if (FindById(cells, source.childId) == nullptr) return world::Result::InvalidReference;
            }
            else
            {
                const world::DistantProxyRecord* child = FindById(proxies, source.childId);
                if (child == nullptr || child->parentProxyId != source.proxyId) return world::Result::InvalidReference;
            }
        ...
        for (u32 index = 1; index < output.children.Size(); ++index)
            if (output.children[index - 1u].proxyId == output.children[index].proxyId &&
                output.children[index - 1u].kind == output.children[index].kind &&
                output.children[index - 1u].childId == output.children[index].childId) return world::Result::DuplicateIdentifier;
        u32 childIndex = 0;
        for (world::DistantProxyRecord& proxy : output.proxies)
        {
            ...
            {
                hasRequiredChild |= (static_cast<u8>(output.children[childIndex].flags) &
                                     static_cast<u8>(world::ProxyChildFlags::RequiredForReplacement)) != 0;
                ++proxy.childCount;
                ++childIndex;
            }
            const bool proxyOnly = (static_cast<u32>(proxy.flags) & static_cast<u32>(world::DistantProxyFlags::ProxyOnly)) != 0;
            if (!proxyOnly && !hasRequiredChild) return world::Result::InvalidLayout;
        }
        if (childIndex != output.children.Size()) return world::Result::InvalidLayout;
    ...
    {
        if (!writer.WriteU64(value.cellId) || !writer.WriteU64(value.parentCellId) || !writer.WriteU64(value.name) ||
            !WriteReference(writer, value.cell)) return false;
        for (const i32 coordinate : value.gridCoordinate) if (!writer.WriteI32(coordinate)) return false;
        if (!writer.WriteU8(value.hierarchyLevel) || !writer.WriteU8(static_cast<u8>(value.category)) ||
            !writer.WriteU8(static_cast<u8>(value.streamingPriority)) || !writer.WriteU8(0)) return false;
        for (const f64 coordinate : value.origin) if (!writer.WriteF64(coordinate)) return false;
        if (!WriteBounds(writer, value.bounds)) return false;
        for (const f64 coordinate : value.streamingReferencePoint) if (!writer.WriteF64(coordinate)) return false;
        return writer.WriteF32(value.activationDistance) && writer.WriteF32(value.retentionDistance) &&
               writer.WriteU32(static_cast<u32>(value.flags)) && writer.WriteU32(0);
    }
    ...
    [[nodiscard]] bool ReadCell(serialization::BinaryReader& reader, world::WorldCellRecord& value) noexcept
    {
        ...
        u32 flags = 0;
        u32 reserved32 = 0;
        if (!reader.ReadU64(value.cellId) || !reader.ReadU64(value.parentCellId) || !reader.ReadU64(value.name) ||
            !ReadReference(reader, value.cell)) return false;
        for (i32& coordinate : value.gridCoordinate) if (!reader.ReadI32(coordinate)) return false;
        u8 streamingPriority = 0;
        if (!reader.ReadU8(value.hierarchyLevel) || !reader.ReadU8(category) || !reader.ReadU8(streamingPriority) ||
            !reader.ReadU8(reserved8)) return false;
        ...
        for (f64& coordinate : value.origin) if (!reader.ReadF64(coordinate)) return false;
        if (!ReadBounds(reader, value.bounds)) return false;
        for (f64& coordinate : value.streamingReferencePoint) if (!reader.ReadF64(coordinate)) return false;
        if (!reader.ReadF32(value.activationDistance) || !reader.ReadF32(value.retentionDistance) ||
            !reader.ReadU32(flags) || !reader.ReadU32(reserved32)) return false;
    ...
    {
        if (!writer.WriteU64(value.proxyId) || !writer.WriteU64(value.parentProxyId) ||
            !writer.WriteU64(value.ownerCellId) || !writer.WriteU64(value.name) || !WriteReference(writer, value.mesh) ||
            !WriteBounds(writer, value.bounds)) return false;
        for (const f64 coordinate : value.streamingReferencePoint) if (!writer.WriteF64(coordinate)) return false;
        for (const f64 coordinate : value.preboostStreamingReferencePoint) if (!writer.WriteF64(coordinate)) return false;
        for (const f64 coordinate : value.secondaryReferencePoint) if (!writer.WriteF64(coordinate)) return false;
        return writer.WriteF32(value.streamingDistance) && writer.WriteF32(value.secondaryReferencePointDistance) &&
               writer.WriteF32(value.nearHideDistance) && writer.WriteF32(0.0f) &&
               writer.WriteU32(value.firstChild) && writer.WriteU32(value.childCount) &&
               writer.WriteU32(static_cast<u32>(value.flags)) && writer.WriteU8(static_cast<u8>(value.streamingPriority)) &&
    ...
    [[nodiscard]] bool ReadProxy(serialization::BinaryReader& reader, world::DistantProxyRecord& value) noexcept
    {
        ...
        u8 reserved8 = 0;
        u16 reserved16 = 0;
        if (!reader.ReadU64(value.proxyId) || !reader.ReadU64(value.parentProxyId) ||
            !reader.ReadU64(value.ownerCellId) || !reader.ReadU64(value.name) || !ReadReference(reader, value.mesh) ||
            !ReadBounds(reader, value.bounds)) return false;
        for (f64& coordinate : value.streamingReferencePoint) if (!reader.ReadF64(coordinate)) return false;
        for (f64& coordinate : value.preboostStreamingReferencePoint) if (!reader.ReadF64(coordinate)) return false;
        for (f64& coordinate : value.secondaryReferencePoint) if (!reader.ReadF64(coordinate)) return false;
        u8 streamingPriority = 0;
        f32 reservedDistance = 0.0f;
        if (!reader.ReadF32(value.streamingDistance) || !reader.ReadF32(value.secondaryReferencePointDistance) ||
            !reader.ReadF32(value.nearHideDistance) || !reader.ReadF32(reservedDistance) ||
            !reader.ReadU32(value.firstChild) || !reader.ReadU32(value.childCount) || !reader.ReadU32(flags) ||
            !reader.ReadU8(streamingPriority) || !reader.ReadU8(reserved8) || !reader.ReadU16(reserved16)) return false;
    ...
    [[nodiscard]] bool ReadChild(serialization::BinaryReader& reader, world::ProxyChildRecord& value) noexcept
    {
        ...
        u16 reserved16 = 0;
        u32 reserved32 = 0;
        if (!reader.ReadU64(value.proxyId) || !reader.ReadU64(value.childId) || !reader.ReadU8(kind) ||
            !reader.ReadU8(flags) || !reader.ReadU16(reserved16) || !reader.ReadU32(reserved32)) return false;
    ...
    [[nodiscard]] world::Result BuildBody(const CanonicalData& data, ByteArray& body,
                                          crypto::Digest256& fingerprint) noexcept
    {
        filesystem::MemoryFileWriter file(body);
        serialization::BinaryWriter writer(file);
        if (!writer.WriteU64(data.worldId)) return WriterResult(writer);
        ...
        if (!WriteBounds(writer, data.bounds) || !WriteDigest(writer, data.sourceFingerprint) ||
            !writer.WriteU32(data.cells.Size()) || !writer.WriteU32(data.proxies.Size()) ||
            !writer.WriteU32(data.children.Size()) || !writer.WriteU32(data.dependencies.Size())) return WriterResult(writer);
        for (const world::WorldCellRecord& value : data.cells) if (!WriteCell(writer, value)) return WriterResult(writer);
        for (const world::DistantProxyRecord& value : data.proxies) if (!WriteProxy(writer, value)) return WriterResult(writer);
        for (const world::ProxyChildRecord& value : data.children) if (!WriteChild(writer, value)) return WriterResult(writer);
        for (const world::DependencyRecord& value : data.dependencies) if (!WriteDependency(writer, value)) return WriterResult(writer);
        fingerprint = crypto::Sha256(body.Data(), body.Size());
        return world::Result::Success;
    }

    [[nodiscard]] world::Result WriteDocument(filesystem::IFile& file, const ByteArray& body,
    ...
    [[nodiscard]] const world::DependencyRecord* FindDependency(
        const containers::ArraySpan<const world::DependencyRecord> dependencies,
        const resources::ResourceReference reference) noexcept
    {
        for (const world::DependencyRecord& dependency : dependencies)
            if (dependency.resource == reference) return &dependency;
        return nullptr;
    }

    [[nodiscard]] world::Result ValidateLoaded(const world::WorldFile& file) noexcept
    {
        const auto cells = file.Cells();
        const auto proxies = file.DistantProxies();
        const auto children = file.ProxyChildren();
        const auto dependencies = file.Dependencies();
        if (file.WorldId() == 0 || cells.Empty() || !IsFiniteBounds(file.Bounds())) return world::Result::InvalidLayout;
        for (const f64 coordinate : containers::ArraySpan<const f64>(file.Origin(), 3))
            if (!std::isfinite(coordinate)) return world::Result::InvalidLayout;
        for (u32 index = 0; index < cells.Size(); ++index)
        {
            const world::WorldCellRecord& cell = cells[index];
            if ((index > 0 && cells[index - 1u].cellId >= cell.cellId) || cell.cellId == world::InvalidCellId ||
                !cell.cell.IsValid() || cell.cell.ExpectedType() != world::CellResourceType ||
                cell.category > world::CellCategory::Generic || !IsFiniteBounds(cell.bounds) ||
                !std::isfinite(cell.activationDistance) || cell.activationDistance < 0.0f ||
                !std::isfinite(cell.retentionDistance) || cell.retentionDistance < cell.activationDistance ||
                (static_cast<u32>(cell.flags) & ~KnownCellFlags) != 0) return world::Result::InvalidLayout;
            for (u32 axis = 0; axis < 3; ++axis)
            ...
            if (dependency == nullptr || dependency->kind != resources::DependencyKind::Soft) return world::Result::InvalidLayout;
        }
        containers::DynamicArray<world::WorldCellRecord> cellCopy(memory::pools::World::GetInstance());
        cellCopy.Reserve(cells.Size());
        for (const auto& cell : cells) cellCopy.PushBack(cell);
        ...
        for (u32 index = 0; index < proxies.Size(); ++index)
        {
            ...
            if ((index > 0 && proxies[index - 1u].proxyId >= proxy.proxyId) || proxy.proxyId == world::InvalidProxyId ||
                FindById(cells, proxy.ownerCellId) == nullptr ||
                !Contains(FindById(cells, proxy.ownerCellId)->bounds, proxy.bounds) || !proxy.mesh.IsValid() ||
                proxy.mesh.ExpectedType() != meshes::MeshResourceType || !IsFiniteBounds(proxy.bounds) ||
                !std::isfinite(proxy.streamingDistance) || !std::isfinite(proxy.secondaryReferencePointDistance) ||
                !std::isfinite(proxy.nearHideDistance) || proxy.nearHideDistance < 0.0f ||
                ...
                proxy.childCount > children.Size() - expectedChild) return world::Result::InvalidLayout;
            for (u32 axis = 0; axis < 3; ++axis)
                if (!std::isfinite(proxy.streamingReferencePoint[axis]) ||
                    !std::isfinite(proxy.preboostStreamingReferencePoint[axis]) ||
                    !std::isfinite(proxy.secondaryReferencePoint[axis])) return world::Result::InvalidLayout;
            ...
            for (u32 local = 0; local < proxy.childCount; ++local)
            {
                ...
                if (child.proxyId != proxy.proxyId || child.childId == 0 || child.kind > world::ProxyChildKind::Proxy ||
                    (static_cast<u8>(child.flags) & ~KnownChildFlags) != 0 ||
                    (local > 0 && !ChildLess(children[expectedChild + local - 1u], child))) return world::Result::InvalidLayout;
                if (child.kind == world::ProxyChildKind::Cell)
                {
                    if (FindById(cells, child.childId) == nullptr) return world::Result::InvalidReference;
                }
                else
                {
                    const world::DistantProxyRecord* childProxy = FindById(proxies, child.childId);
                    if (childProxy == nullptr || childProxy->parentProxyId != proxy.proxyId) return world::Result::InvalidReference;
                }
                hasRequiredChild |= (static_cast<u8>(child.flags) &
                                     static_cast<u8>(world::ProxyChildFlags::RequiredForReplacement)) != 0;
            }
            const bool proxyOnly = (static_cast<u32>(proxy.flags) & static_cast<u32>(world::DistantProxyFlags::ProxyOnly)) != 0;
            if (!proxyOnly && !hasRequiredChild) return world::Result::InvalidLayout;
        ...
        for (u32 index = 0; index < dependencies.Size(); ++index)
            if (!dependencies[index].resource.IsValid() || !dependencies[index].resource.IsTyped() ||
                ...
                (index > 0 && !DependencyLess(dependencies[index - 1u], dependencies[index])))
                return world::Result::InvalidLayout;
        return world::Result::Success;
    }
} // namespace
...
namespace vanguard::world
{
    ...
    Result WorldFile::Open(filesystem::IFile& file, const WorldReadLimits& limits) noexcept
    {
        ...
        const serialization::Result headerResult =
            serialization::ReadDocumentHeader(reader, WorldMagic, {1, 0, 0}, documentLimits, header);
        ...
        if (sectionResult != serialization::Result::Success) return Convert(sectionResult);
        if (sections.Size() != 1 || sections[0].id != WorldSection || sections[0].version != FileVersion ||
        ...
        if (!bodyReader.ReadU64(m_worldId)) return ReaderResult(bodyReader);
        ...
        if (!ReadBounds(bodyReader, m_bounds) || !ReadDigest(bodyReader, m_sourceFingerprint) ||
            !bodyReader.ReadU32(cellCount) || !bodyReader.ReadU32(proxyCount) ||
            !bodyReader.ReadU32(childCount) || !bodyReader.ReadU32(dependencyCount)) return ReaderResult(bodyReader);
        if (!ResizeChecked(m_cells, cellCount, limits.maximumCells) ||
            !ResizeChecked(m_distantProxies, proxyCount, limits.maximumDistantProxies) ||
            !ResizeChecked(m_proxyChildren, childCount, limits.maximumProxyChildren) ||
            !ResizeChecked(m_dependencies, dependencyCount, limits.maximumDependencies)) return Result::LimitExceeded;
        for (WorldCellRecord& value : m_cells) if (!ReadCell(bodyReader, value)) return Result::InvalidLayout;
        for (DistantProxyRecord& value : m_distantProxies) if (!ReadProxy(bodyReader, value)) return Result::InvalidLayout;
        for (ProxyChildRecord& value : m_proxyChildren) if (!ReadChild(bodyReader, value)) return Result::InvalidLayout;
    ...
    {
        m_worldId = 0;
        for (f64& coordinate : m_origin) coordinate = 0.0;
        m_bounds = {};
        m_sourceFingerprint = {};
    ...
    bool WorldFile::IsOpen() const noexcept { return m_open; }

    resources::ResourceTypeId WorldResource::Type() const noexcept { return WorldResourceType; }
    ...
    const WorldFile& WorldResource::File() const noexcept { return m_file; }
    u64 WorldFile::WorldId() const noexcept { return m_worldId; }
    const f64* WorldFile::Origin() const noexcept { return m_origin; }
    const WorldBounds& WorldFile::Bounds() const noexcept { return m_bounds; }
    const crypto::Digest256& WorldFile::SourceFingerprint() const noexcept { return m_sourceFingerprint; }
    const crypto::Digest256& WorldFile::ContentFingerprint() const noexcept { return m_contentFingerprint; }
    containers::ArraySpan<const WorldCellRecord> WorldFile::Cells() const noexcept { return m_cells; }
    containers::ArraySpan<const DistantProxyRecord> WorldFile::DistantProxies() const noexcept { return m_distantProxies; }
    containers::ArraySpan<const ProxyChildRecord> WorldFile::ProxyChildren() const noexcept { return m_proxyChildren; }
    containers::ArraySpan<const DependencyRecord> WorldFile::Dependencies() const noexcept { return m_dependencies; }

    containers::ArraySpan<const ProxyChildRecord> WorldFile::ChildrenOf(const DistantProxyRecord& proxy) const noexcept
    {
        if (!m_open || proxy.firstChild > m_proxyChildren.Size() ||
            proxy.childCount > m_proxyChildren.Size() - proxy.firstChild) return {};
    ...
    Result CalculateWorldContentFingerprint(const WorldBuildDescription& description,
                                            crypto::Digest256& fingerprint) noexcept
    {
        CanonicalData canonical;
        Result result = Canonicalize(description, canonical);
