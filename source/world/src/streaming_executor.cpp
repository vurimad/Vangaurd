...
namespace
{
    ...
    template <typename Type, typename... Args> [[nodiscard]] Type* AllocateStreamingObject(Args&&... args) noexcept
    {
    ...
    template <typename Type> void DeleteStreamingObject(Type* const object) noexcept
    {
        if (object == nullptr)
            return;
        object->~Type();
    ...
    [[nodiscard]] constexpr resources::LoadPriority ToLoadPriority(const world::StreamingPriority priority) noexcept
    {
        ...
        if (value >= static_cast<u8>(world::StreamingPriority::Critical))
            return resources::LoadPriority::Critical;
        if (value >= static_cast<u8>(world::StreamingPriority::StaticMesh))
            return resources::LoadPriority::High;
        if (value == static_cast<u8>(world::StreamingPriority::Low))
            return resources::LoadPriority::Background;
...
namespace vanguard::world
{
    struct WorldStreamingExecutor::Impl
    {
        ...
        {
            if (Operation* const existing = Find(key))
                return existing;
            ...
            {
                if (operations.Size() == index + 1u)
                    static_cast<void>(operations.RemoveAt(index));
                return nullptr;
            }
        ...
        [[nodiscard]] bool Emit(containers::DynamicArray<StreamingResourceEvent>& events, const StreamingResourceEvent& event) noexcept
        {
        ...
        [[nodiscard]] bool Poll(containers::DynamicArray<StreamingResourceEvent>& events) noexcept
        {
            ...
            {
                if (operation.phase != Phase::Requesting || !operation.request.HasFinished())
                    continue;
                if (operation.request.HasLoaded())
                {
                    ...
                    if (!operation.handle)
                    {
                        ...
                        if (!grid->NotifyStreamInComplete(operation.key, false) ||
                            !Emit(events, {StreamingResourceEventType::ResourceFailed, operation.key, operation.resource, operation.failure}))
                            return false;
                        continue;
                    }
                    ...
                    if (!grid->NotifyStreamInComplete(operation.key, true, false) ||
                        !Emit(events, {StreamingResourceEventType::ResourceAvailable, operation.key, operation.resource, resources::Failure::None}))
                        return false;
                }
                else
                {
                    operation.failure = operation.request.GetError();
                    operation.failureTrace = {};
        ...
        [[nodiscard]] bool BeginStreamIn(const StreamingCommand& command, containers::DynamicArray<StreamingResourceEvent>& events) noexcept
        {
            ...
            if (operation == nullptr || operation->phase != Phase::Idle)
            {
                ...
                return grid->NotifyStreamInComplete(command.key, false) &&
                       Emit(events, {StreamingResourceEventType::ResourceFailed, command.key, command.resource, resources::Failure::InternalError});
            }
            ...
            if (!operation->request)
            {
                ...
                ++failedRequests;
                return grid->NotifyStreamInComplete(command.key, false) &&
                       Emit(events, {StreamingResourceEventType::ResourceFailed, command.key, command.resource, operation->failure});
            }
            operation->phase = Phase::Requesting;
            return true;
        }

        [[nodiscard]] bool BeginStreamOut(const StreamingCommand& command, containers::DynamicArray<StreamingResourceEvent>& events) noexcept
        {
            ...
            if (operation->phase == Phase::Requesting)
            {
                ...
                return grid->NotifyStreamOutComplete(command.key) &&
                       Emit(events, {StreamingResourceEventType::RequestCancelled, command.key, command.resource, resources::Failure::Cancelled});
            }
            ...
            if (operation->phase != Phase::Resident)
                return false;
            operation->phase = Phase::ReleasePending;
            return Emit(events, {StreamingResourceEventType::ReleaseRequested, command.key, command.resource, resources::Failure::None});
        }
    ...
    bool WorldStreamingExecutor::Initialize(WorldStreamingGrid& grid, resources::ResourcePipeline& pipeline) noexcept
    {
        if (m_impl != nullptr)
            return false;
        if (!grid.IsInitialized() || !pipeline.IsInitialized())
            return false;
    ...
    bool WorldStreamingExecutor::Shutdown() noexcept
    {
        ...
        for (const Impl::Operation& operation : m_impl->operations)
            if (operation.phase != Impl::Phase::Idle)
                return false;
        DeleteStreamingObject(m_impl);
        m_impl = nullptr;
        return true;
    ...
    bool WorldStreamingExecutor::IsInitialized() const noexcept
    {
        return m_impl != nullptr;
    }

    bool WorldStreamingExecutor::Process(const StreamingProcessInput& input, containers::DynamicArray<StreamingResourceEvent>& events) noexcept
    {
        events.Clear();
        if (m_impl == nullptr || !m_impl->Poll(events) || !m_impl->grid->Process(input, m_impl->commands))
            return false;
        events.Reserve(events.Size() + m_impl->commands.Size());
        for (const StreamingCommand& command : m_impl->commands)
        {
            const bool success =
                command.type == StreamingCommandType::StreamIn ? m_impl->BeginStreamIn(command, events) : m_impl->BeginStreamOut(command, events);
            if (!success)
                return false;
        }
        return true;
    ...
    bool WorldStreamingExecutor::CompleteRelease(const StreamingNodeKey key) noexcept
    {
        ...
        if (operation == nullptr || operation->phase != Impl::Phase::ReleasePending)
            return false;
    ...
    {
        if (m_impl == nullptr || failure == resources::Failure::None)
            return false;
        Impl::Operation* const operation = m_impl->Find(key);
        if (operation == nullptr || operation->phase != Impl::Phase::Resident || !m_impl->grid->NotifyResidentFailed(key))
            return false;
        operation->handle.Reset();
    ...
    const resources::ResourceHandle* WorldStreamingExecutor::GetResource(const StreamingNodeKey key) const noexcept
    {
        if (m_impl == nullptr)
            return nullptr;
        const Impl::Operation* const operation = m_impl->Find(key);
        return operation != nullptr && (operation->phase == Impl::Phase::Resident || operation->phase == Impl::Phase::ReleasePending) ? &operation->handle
                                                                                                                                      : nullptr;
    }

    resources::Failure WorldStreamingExecutor::GetLastFailure(const StreamingNodeKey key) const noexcept
    {
        if (m_impl == nullptr)
    ...
    bool WorldStreamingExecutor::GetFailureTrace(const StreamingNodeKey key, resources::FailureTrace& trace) const noexcept
    {
        ...
        if (m_impl == nullptr)
            return false;
        const Impl::Operation* const operation = m_impl->Find(key);
        if (operation == nullptr || operation->failure == resources::Failure::None)
            return false;
