...
namespace
{
    ...
    template <typename Type, typename... Args> [[nodiscard]] Type* AllocateApplicationObject(Args&&... args) noexcept
    {
    ...
    template <typename Type> void DeleteApplicationObject(Type* const object) noexcept
    {
        if (object == nullptr)
            return;
        object->~Type();
...
{
    LifecycleStatus Service::OnInitialize(ServiceContext&) noexcept
    {
        return LifecycleStatus::Success();
    }
    LifecycleStatus Service::OnStart(ServiceContext&) noexcept
    {
        return LifecycleStatus::Success();
    }
    LifecycleStatus Service::OnQuiesce(ServiceContext&) noexcept
    {
        return LifecycleStatus::Success();
    }
    LifecycleStatus Service::OnDrain(ServiceContext&) noexcept
    {
        return LifecycleStatus::Success();
    }
    LifecycleStatus Service::OnStop(ServiceContext&) noexcept
    {
        return LifecycleStatus::Success();
    }
    LifecycleStatus Service::OnShutdown(ServiceContext&) noexcept
    {
        return LifecycleStatus::Success();
    }
    ...
    struct EngineHost::Impl
    {
        ...
        Impl() noexcept
            : modules(memory::pools::Runtime::GetInstance()), services(memory::pools::Runtime::GetInstance()), plan(memory::pools::Runtime::GetInstance()),
              events(memory::pools::Runtime::GetInstance())
        {
        ...
        {
            for (ModuleRecord* const module : modules)
        ...
        {
            for (ServiceRecord* const service : services)
        ...
        {
            for (const CapabilityId provided : service.provides)
        ...
        [[nodiscard]] ServiceRecord* CapabilityProvider(const CapabilityId capability) const noexcept
        {
            ...
            {
                if (!Provides(*candidate, capability))
                    continue;
                if (provider == nullptr || candidate->descriptor.id < provider->descriptor.id)
                    provider = candidate;
            }
            return provider;
        }
        ...
        [[nodiscard]] ServiceRecord* SelectedCapabilityProvider(const CapabilityId capability) const noexcept
        {
            ...
            {
                if (!candidate->selected || !Provides(*candidate, capability))
                    continue;
                if (provider == nullptr || candidate->descriptor.id < provider->descriptor.id)
                    provider = candidate;
            }
        ...
        {
            for (const ServiceRecord* const service : plan)
                if (service->descriptor.id == id)
                    return true;
            return false;
        }
        ...
        [[nodiscard]] bool DependsOn(const ServiceRecord& dependent, const ServiceRecord& provider) const noexcept
        {
            ...
            {
                if (dependency.service != provider.descriptor.id)
                    continue;
                if (dependency.kind == DependencyKind::Required || dependency.kind == DependencyKind::StartAfter)
                    return true;
                if (dependency.kind == DependencyKind::Optional && provider.selected)
                    return true;
            }
            for (const CapabilityRequirement& requirement : dependent.requirements)
            {
                if (SelectedCapabilityProvider(requirement.capability) == &provider)
                    return true;
            }
            return false;
        }

        void SetFailure(HostFailure* const output, const HostFailureCode code, const ServiceId service, const ServiceId related, const CapabilityId capability,
                        const char* const message) const noexcept
        {
            ...
            VG_LOG_ERROR(diagnostics::Category::Services, "application lifecycle failure: code=%u service=%llu related=%llu capability=%llu message=%s",
                         static_cast<u32>(code), static_cast<unsigned long long>(service), static_cast<unsigned long long>(related),
                         static_cast<unsigned long long>(capability), message != nullptr ? message : "unspecified");
        }
                                    ...
                                    HostFailureCode& firstCleanupFailure) noexcept
        {
            ...
            case LifecycleStage::Initialize:
                ...
                else
                {
                    ...
                    failureCode = HostFailureCode::InitializeFailure;
                }
                break;
            case LifecycleStage::Start:
                ...
                {
                    service.state = ServiceState::Failed;
                    failureCode = HostFailureCode::StartFailure;
                }
                break;
            case LifecycleStage::Quiesce:
                ...
                if (status)
                    service.state = ServiceState::Quiesced;
                else
                {
                    service.state = ServiceState::Failed;
                    failureCode = HostFailureCode::QuiesceFailure;
                }
                break;
            case LifecycleStage::Drain:
                ...
                if (status)
                    service.state = ServiceState::Drained;
                else
                {
                    service.state = ServiceState::Failed;
                    failureCode = HostFailureCode::DrainFailure;
                }
                break;
            case LifecycleStage::Stop:
                ...
                {
                    service.state = ServiceState::Failed;
                    failureCode = HostFailureCode::StopFailure;
                }
                break;
            case LifecycleStage::Shutdown:
                ...
                {
                    service.state = ServiceState::Failed;
                    failureCode = HostFailureCode::ShutdownFailure;
                }
                break;
            default:
                return false;
            ...
            if (firstCleanupFailure == HostFailureCode::None)
                firstCleanupFailure = failureCode;
            return false;
        }
        ...
        [[nodiscard]] bool Unwind(const bool rollback, HostFailure* const cleanupFailure) noexcept
        {
            ...
            if (rollback)
                Emit(nullptr, LifecycleStage::Rollback, ServiceState::Failed);
    ...
    EngineHost::~EngineHost()
    {
        ...
        for (Impl::ModuleRecord* const module : m_impl->modules)
            DeleteApplicationObject(module);
    ...
    bool EngineHost::RegisterModule(const ModuleDescriptor& descriptor, HostFailure* const failure) noexcept
    {
        ...
        {
            if (failure != nullptr)
                *failure = {HostFailureCode::InvalidArgument, InvalidServiceId, InvalidServiceId, InvalidCapabilityId, "invalid module descriptor"};
            return false;
        }
        if (m_impl == nullptr)
        {
            ...
            {
                if (failure != nullptr)
                    failure->code = HostFailureCode::LimitExceeded;
                return false;
            }
            m_impl->owner = this;
        }
        ...
        {
            m_impl->SetFailure(failure, HostFailureCode::InvalidState, InvalidServiceId, InvalidServiceId, InvalidCapabilityId,
                               "module registration is closed");
            return false;
        }
        if (m_impl->FindModule(descriptor.id) != nullptr)
        {
            m_impl->SetFailure(failure, HostFailureCode::DuplicateModule, InvalidServiceId, InvalidServiceId, InvalidCapabilityId, "duplicate module ID");
            return false;
        }
        if (m_impl->modules.Size() >= MaximumModules)
        {
            m_impl->SetFailure(failure, HostFailureCode::LimitExceeded, InvalidServiceId, InvalidServiceId, InvalidCapabilityId, "module limit exceeded");
            return false;
        }
        Impl::ModuleRecord* const module = AllocateApplicationObject<Impl::ModuleRecord>();
        if (module == nullptr)
        {
            m_impl->SetFailure(failure, HostFailureCode::LimitExceeded, InvalidServiceId, InvalidServiceId, InvalidCapabilityId,
                               "module record allocation failed");
            return false;
        }
    ...
    bool EngineHost::RegisterService(const ModuleId owner, const ServiceDescriptor& descriptor, HostFailure* const failure) noexcept
    {
        if (failure != nullptr)
            *failure = {};
        if (m_impl == nullptr || m_impl->state != HostState::Building || descriptor.id == InvalidServiceId || !ValidName(descriptor.name) ||
            descriptor.profiles == ApplicationProfile::None || descriptor.create == nullptr || descriptor.destroy == nullptr)
        {
            if (m_impl != nullptr)
                m_impl->SetFailure(failure, m_impl->state != HostState::Building ? HostFailureCode::InvalidState : HostFailureCode::InvalidArgument,
                                   descriptor.id, InvalidServiceId, InvalidCapabilityId, "invalid service registration");
            else if (failure != nullptr)
                *failure = {HostFailureCode::InvalidState, descriptor.id, InvalidServiceId, InvalidCapabilityId, "register a module before its services"};
            return false;
        }
        if (m_impl->FindModule(owner) == nullptr)
        {
            m_impl->SetFailure(failure, HostFailureCode::UnknownModule, descriptor.id, InvalidServiceId, InvalidCapabilityId,
                               "service owner module is not registered");
            return false;
        }
        if (m_impl->FindService(descriptor.id) != nullptr)
        {
            m_impl->SetFailure(failure, HostFailureCode::DuplicateService, descriptor.id, descriptor.id, InvalidCapabilityId, "duplicate service ID");
            return false;
        }
        if (m_impl->services.Size() >= MaximumServices)
        {
            m_impl->SetFailure(failure, HostFailureCode::LimitExceeded, descriptor.id, InvalidServiceId, InvalidCapabilityId, "service limit exceeded");
            return false;
        }
        Impl::ServiceRecord* const service = AllocateApplicationObject<Impl::ServiceRecord>();
        if (service == nullptr)
        {
            m_impl->SetFailure(failure, HostFailureCode::LimitExceeded, descriptor.id, InvalidServiceId, InvalidCapabilityId,
                               "service record allocation failed");
            return false;
        }
        ...
        for (const ServiceDependency dependency : descriptor.dependencies)
        {
            ...
            {
                DeleteApplicationObject(service);
                m_impl->SetFailure(failure, HostFailureCode::InvalidArgument, descriptor.id, dependency.service, InvalidCapabilityId,
        ...
        {
            if (capability == InvalidCapabilityId)
            {
                DeleteApplicationObject(service);
                return false;
            }
        ...
        {
            if (requirement.capability == InvalidCapabilityId)
            {
                DeleteApplicationObject(service);
                return false;
            }
        ...
        {
            if (conflict == InvalidServiceId || conflict == descriptor.id)
            {
                DeleteApplicationObject(service);
                return false;
            }
    ...
    bool EngineHost::Compile(const ApplicationProfile profile, HostFailure* const failure) noexcept
    {
        ...
        {
            if (failure != nullptr)
                failure->code = HostFailureCode::InvalidState;
            return false;
        }
        ...
        for (Impl::ServiceRecord* const service : m_impl->services)
        {
            ...
            for (const ServiceDependency& dependency : service->dependencies)
            {
                ...
                if (target != nullptr || dependency.kind != DependencyKind::Required)
                    continue;
                m_impl->SetFailure(failure, HostFailureCode::MissingDependency, service->descriptor.id, dependency.service, InvalidCapabilityId,
                                   "required service is unavailable in this profile");
                return false;
            }
            for (const ServiceId conflict : service->conflicts)
            {
                if (m_impl->FindPlanned(conflict) == nullptr)
                    continue;
                m_impl->SetFailure(failure, HostFailureCode::ConflictingService, service->descriptor.id, conflict, InvalidCapabilityId,
                                   "conflicting services are selected");
                return false;
            }
            for (const CapabilityRequirement& requirement : service->requirements)
            {
                ...
                for (const Impl::ServiceRecord* const candidate : m_impl->services)
                    if (candidate->selected && m_impl->Provides(*candidate, requirement.capability))
                        ++providers;
                if (providers == 0 && !requirement.optional)
                {
                    m_impl->SetFailure(failure, HostFailureCode::MissingCapability, service->descriptor.id, InvalidServiceId, requirement.capability,
                                       "required capability has no provider");
                    return false;
                }
                if (providers > 1 && requirement.cardinality == CapabilityCardinality::ExactlyOne)
                {
                    m_impl->SetFailure(failure, HostFailureCode::AmbiguousCapability, service->descriptor.id, InvalidServiceId, requirement.capability,
                                       "capability requires exactly one provider");
                    return false;
                }
            }
        }
        while (m_impl->plan.Size() < selectedCount)
        {
            ...
            {
                if (!candidate->selected || m_impl->PlanContains(candidate->descriptor.id))
                    continue;
                bool ready = true;
                for (Impl::ServiceRecord* const provider : m_impl->services)
                {
                    if (!provider->selected || !m_impl->DependsOn(*candidate, *provider))
                        continue;
                    if (!m_impl->PlanContains(provider->descriptor.id))
                    {
                        ready = false;
                        break;
                    }
                }
                if (ready && (next == nullptr || candidate->descriptor.id < next->descriptor.id))
                    next = candidate;
            }
            if (next == nullptr)
            {
                ...
                for (const Impl::ServiceRecord* const service : m_impl->services)
                    if (service->selected && !m_impl->PlanContains(service->descriptor.id))
                    {
                        blocked = service->descriptor.id;
                        break;
                    }
                m_impl->SetFailure(failure, HostFailureCode::DependencyCycle, blocked, InvalidServiceId, InvalidCapabilityId,
    ...
    bool EngineHost::Start(HostFailure* const failure) noexcept
    {
        ...
        for (Impl::ServiceRecord* const service : m_impl->plan)
        {
            ...
            {
                service->state = ServiceState::Failed;
                m_impl->Emit(service, LifecycleStage::Construct, service->state, HostFailureCode::FactoryFailure, "service factory returned null");
                m_impl->SetFailure(failure, HostFailureCode::FactoryFailure, service->descriptor.id, InvalidServiceId, InvalidCapabilityId,
    ...
    bool EngineHost::Shutdown(HostFailure* const failure) noexcept
    {
        ...
        if (m_impl == nullptr)
            return true;
        if (m_impl->state == HostState::Stopped)
            return true;
        if (m_impl->state != HostState::Running)
        {
            m_impl->SetFailure(failure, HostFailureCode::InvalidState, InvalidServiceId, InvalidServiceId, InvalidCapabilityId, "host is not running");
            return false;
        }
    ...
    HostState EngineHost::State() const noexcept
    {
        return m_impl != nullptr ? m_impl->state : HostState::Building;
    }
    ApplicationProfile EngineHost::GetProfile() const noexcept
    {
    ...
    {
        if (m_impl == nullptr || capability == InvalidCapabilityId)
            return nullptr;
    ...
    ServiceState EngineHost::GetStateOf(const ServiceId service) const noexcept
    {
        if (m_impl == nullptr)
    ...
    void EngineHost::VisitLifecycleEvents(const LifecycleVisitor visitor, void* const userData) const noexcept
    {
        if (m_impl == nullptr || visitor == nullptr)
            return;
        for (const LifecycleEvent& event : m_impl->events)
            visitor(event, userData);
    }
    ...
    void EngineHost::VisitCompiledServices(const CompiledServiceVisitor visitor, void* const userData) const noexcept
    {
        ...
        for (u32 index = 0; index < m_impl->plan.Size(); ++index)
        {
            ...
            visitor({service.module, service.descriptor.id, service.descriptor.name, index, service.descriptor.scope, service.descriptor.affinity}, userData);
        }
    }
    ...
    ServiceContext::ServiceContext(EngineHost& host, const ServiceId currentService) noexcept : m_host(&host), m_currentService(currentService) {}
    ApplicationProfile ServiceContext::GetProfile() const noexcept
    {
        return m_host->GetProfile();
    }
    ServiceId ServiceContext::GetCurrentService() const noexcept
    {
        return m_currentService;
    }
    Service* ServiceContext::Find(const ServiceId service) const noexcept
    {
        return m_host->Find(service);
    }
    ...
    ServiceHandle ServiceContext::Acquire(const ServiceId service) const noexcept
    {
        return m_host->Acquire(service);
    }
    Service* ServiceContext::Resolve(const ServiceHandle handle) const noexcept
    {
        return m_host->Resolve(handle);
    }
} // namespace vanguard::application
