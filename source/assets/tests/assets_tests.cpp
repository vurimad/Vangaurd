...
namespace
{
    ...
    bool DiscoverDependencies(const assets::BuildRequest&, assets::DependencyCollector& collector, void* const userData) noexcept
    {
        ...
        if (state.mode == CompilerMode::DuplicateDependency)
        {
            ...
            static_cast<void>(collector.Add(
                {state.firstDependency, state.firstContent, assets::DependencyRole::Source, assets::DependencyRequirement::Required}));
            return true;
        }
    ...
    bool Compile(const assets::CompileContext& context, assets::ArtifactWriter& writer, void* const userData) noexcept
    {
        ...
        const u8 primary[] = {static_cast<u8>(context.request.target),
                              context.request.source.content.Empty() ? static_cast<u8>(0) : context.request.source.content[0],
                              state.firstContent.bytes[0], state.secondContent.bytes[0]};
        ...
        return writer.Add(state.auxiliaryOutput, 7, assets::ArtifactFlags::Streamable, 4, auxiliary, sizeof(auxiliary)) ==
...
int main()
{
    ...
    assets::CompilerDescriptor compiler{compilerId, "assets.test_compiler", 1,        SourceType,
                                        OutputType, &DiscoverDependencies,  &Compile, &state};
    ...
    assets::BuildRequest request{
        {source, {sourceBytes, 4}, {metadataBytes, 2}}, output, assets::TargetPlatform::WindowsD3D12, {settingsA, 1}};

    assets::BuildOutput first;
    Check(system.Build(request, first) == assets::Result::Success && first.disposition == assets::BuildDisposition::Built &&
              first.artifacts.Size() == 2 && !first.buildFingerprint.IsEmpty() && !first.contentFingerprint.IsEmpty() &&
    ...
    Check(system.Build(request, reordered) == assets::Result::Success && reordered.disposition == assets::BuildDisposition::CacheHit &&
              reordered.buildFingerprint == first.buildFingerprint && reordered.contentFingerprint == first.contentFingerprint &&
              state.compileCalls == 1,
    ...
    Check(system.Build(changedSource, sourceInvalidated) == assets::Result::Success &&
              sourceInvalidated.disposition == assets::BuildDisposition::Built &&
    ...
    Check(system.Build(request, dependencyInvalidated) == assets::Result::Success &&
              dependencyInvalidated.disposition == assets::BuildDisposition::Built &&
    ...
    Check(system.Build(platformChanged, platformInvalidated) == assets::Result::Success &&
              platformInvalidated.disposition == assets::BuildDisposition::Built && state.compileCalls == 4,
    ...
    Check(system.Build(settingsChanged, settingsInvalidated) == assets::Result::Success &&
              settingsInvalidated.disposition == assets::BuildDisposition::Built && state.compileCalls == 5,
    ...
    Check(system.Build(duplicateRequest, duplicateOutput) == assets::Result::DuplicateDependency,
          "duplicate dependencies fail deterministically");
    ...
    Check(system.Build(invalidArtifactRequest, invalidArtifactOutput) == assets::Result::InvalidArtifact,
          "compiler must emit an explicit primary artifact");
    ...
    Check(system.Build(request, versionInvalidated) == assets::Result::Success &&
              versionInvalidated.disposition == assets::BuildDisposition::Built &&
              versionInvalidated.buildFingerprint != dependencyInvalidated.buildFingerprint && state.compileCalls == 7,
          "compiler version invalidates derived data");

    const assets::Stats stats = system.GetStats();
    Check(stats.registeredCompilers == 1 && stats.activeBuilds == 0 && stats.buildRequests == 9 && stats.localBuilds == 6 &&
              stats.cacheHits == 1 && stats.cacheMisses == 6 && stats.failedBuilds == 2 && stats.cacheEntries == 4 &&
              stats.cacheStores == 6 && stats.cacheEvictions == 2,
