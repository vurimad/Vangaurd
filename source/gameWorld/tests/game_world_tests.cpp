...
namespace
{
    ...
    {
    public:
        RecordingSystem(const vanguard::game::RuntimeSystemDescriptor& descriptor,
                        vanguard::containers::DynamicArray<vanguard::u32>& events) noexcept
    ...
    protected:
        bool OnInitialize(vanguard::game::GameWorld&) noexcept override { Record(100); return true; }
        bool OnSetup(vanguard::game::GameWorld&) noexcept override { Record(200); return true; }
        void OnUninitialize(vanguard::game::GameWorld&) noexcept override { Record(1000); }
        void OnGameAttached(void*) noexcept override { Record(300); }
        void OnGameDetached(void*) noexcept override { Record(800); }
        void OnPostWorldDetached(vanguard::game::GameWorld&) noexcept override { Record(900); }
        void OnGainFocus(vanguard::game::GameWorld&) noexcept override { Record(400); }
        void OnLoseFocus(vanguard::game::GameWorld&) noexcept override { Record(700); }
        ...
        void OnBeginFrame(vanguard::game::GameWorld& world, vanguard::f32) noexcept override
        {
            ...
            {
                static_cast<void>(world.Entities().QueueCreate(0xabc));
                createEntityOnFirstFrame = false;
            }
        }

        void OnEndFrame(vanguard::game::GameWorld&, vanguard::f32) noexcept override { Record(600); }
        const char* ReadinessBlocker() const noexcept override { return blocker; }

    private:
        void Record(const vanguard::u32 phase) noexcept
        {
            m_events.PushBack(phase + Descriptor().id);
        }
    ...
    protected:
        bool OnInitialize(vanguard::game::GameWorld&) noexcept override { ++initializeCalls; return true; }
        bool OnSetup(vanguard::game::GameWorld&) noexcept override { return !rejectSetup; }
        void OnUninitialize(vanguard::game::GameWorld&) noexcept override { ++uninitializeCalls; }
    };
}
...
int main()
{
    ...
    Check(vanguard::diagnostics::Initialize(vanguard::diagnostics::Mode::Synchronous, "gameWorldTests"),
    ...
    Check(events.Size() == 4 && events[0] == 110 && events[1] == 120 && events[2] == 210 && events[3] == 220 &&
    ...
    world.GainFocus();
    Check(world.Tick(1.0f / 60.0f) && world.Entities().Resolve(0xabc),
          "frame boundary commits entity actions queued by runtime systems");
    ...
    const vanguard::ecs::ComponentType<WorldPosition> positionType =
        vanguard::ecs::RegisterComponent<WorldPosition>(world.Entities());
    Check(positionType && world.Entities().QueueAddComponent(0xabc, positionType) &&
              world.Entities().QueueSetComponent(0xabc, positionType, WorldPosition{42.0f}) && world.Tick(0.0f),
          "GameWorld commits queued component transactions at its frame synchronization point");
    {
        flecs::world native = vanguard::ecs::Native(world.Entities());
        const WorldPosition* const position = native.entity(world.Entities().Resolve(0xabc).value).try_get<WorldPosition>();
        Check(position != nullptr && position->x == 42.0f,
              "component transaction value is visible to the live Flecs world");
    }
    ...
    Check(world.Entities().QueueDestroy(0xabc) && world.Tick(0.0f), "destroy final entity through world sync point");
    Check(world.Shutdown(), "uninitialize systems in reverse order and shutdown empty ECS world");

    const vanguard::u32 expected[] = {
        110, 120, 210, 220,
        310, 320, 410, 420,
        510, 520, 610, 620,
        510, 520, 610, 620,
    ...
    Check(retryWorld.RegisterSystem(retrySystem) && !retryWorld.Initialize() && !retryWorld.IsInitialized() &&
              !retrySystem.IsInitialized() && retrySystem.initializeCalls == 1 && retrySystem.uninitializeCalls == 1,
          "failed setup rolls back both runtime systems and the Flecs world");
    retrySystem.rejectSetup = false;
    Check(retryWorld.Initialize() && retryWorld.Shutdown() && retrySystem.initializeCalls == 2 &&
              retrySystem.uninitializeCalls == 2,
