...
namespace
{
    ...
    template <typename Type, typename... Args> [[nodiscard]] Type* AllocateGameObject(Args&&... args) noexcept
    {
    ...
    template <typename Type> void DeleteGameObject(Type* const object) noexcept
    {
        if (object == nullptr)
            return;
        object->~Type();
    ...
    [[nodiscard]] constexpr game::RuntimeSystemFlags ModeFlag(const game::WorldMode mode) noexcept
    {
        ...
        case game::WorldMode::Game:
            return game::RuntimeSystemFlags::Game;
        case game::WorldMode::Preview:
            return game::RuntimeSystemFlags::Preview;
        case game::WorldMode::Thumbnail:
            return game::RuntimeSystemFlags::Thumbnail;
        case game::WorldMode::Headless:
            return game::RuntimeSystemFlags::Headless;
        }
        return game::RuntimeSystemFlags::None;
    }

    [[nodiscard]] constexpr bool HasFlag(const game::RuntimeSystemFlags value, const game::RuntimeSystemFlags flag) noexcept
    {
...
namespace vanguard::game
{
    ...
    const RuntimeSystemDescriptor& RuntimeSystem::GetDescriptor() const noexcept
    {
        return m_descriptor;
    }
    bool RuntimeSystem::IsInitialized() const noexcept
    {
        return m_initialized;
    }
    bool RuntimeSystem::IsGameAttached() const noexcept
    {
        return m_gameAttached;
    }
    bool RuntimeSystem::OnInitialize(GameWorld&) noexcept
    {
        return true;
    }
    bool RuntimeSystem::OnSetup(GameWorld&) noexcept
    {
        return true;
    }
    ...
    const char* RuntimeSystem::ReadinessBlocker() const noexcept
    {
        return nullptr;
    }
    ...
    struct GameWorld::Impl
    {
        ...
        Impl() noexcept : systems(memory::pools::Gameplay::GetInstance())
        {
            systems.Reserve(MaximumRuntimeSystems);
        }
    ...
    GameWorld::~GameWorld()
    {
        if (m_impl != nullptr)
        {
            for (u32 index = m_impl->systems.Size(); index > 0; --index)
            {
                ...
                if (system == nullptr || !system->m_initialized)
                    continue;
    ...
    {
        if (system.GetDescriptor().id == InvalidRuntimeSystemId || system.GetDescriptor().name == nullptr || system.GetDescriptor().name[0] == '\0' ||
        ...
        if (m_impl->initialized || m_impl->systems.Size() >= MaximumRuntimeSystems)
            return false;
        for (const Impl::SystemEntry& entry : m_impl->systems)
            if (entry.system == &system || entry.system->GetDescriptor().id == system.GetDescriptor().id)
                return false;
    ...
    bool GameWorld::Initialize(const GameWorldConfig& config) noexcept
    {
        ...
        if (m_impl->initialized || !m_impl->entities.Initialize(config.entities))
            return false;
        m_impl->config = config;
        std::sort(m_impl->systems.Begin(), m_impl->systems.End(), [](const Impl::SystemEntry& left, const Impl::SystemEntry& right) noexcept
                  { return left.system->GetDescriptor().id < right.system->GetDescriptor().id; });
        ...
        for (u32 systemIndex = 0; systemIndex < m_impl->systems.Size(); ++systemIndex)
        {
            ...
            entry.active = HasFlag(entry.system->GetDescriptor().modes, mode);
            if (!entry.active)
                continue;
            if (!entry.system->OnInitialize(*this))
            {
                for (u32 rollback = systemIndex; rollback > 0; --rollback)
                {
                    ...
                    if (previous->m_initialized)
                    {
                        previous->OnUninitialize(*this);
                        previous->m_initialized = false;
                    }
                }
                for (Impl::SystemEntry& rollbackEntry : m_impl->systems)
                    rollbackEntry.active = false;
        ...
        {
            if (!entry.active || entry.system->OnSetup(*this))
                continue;
    ...
    bool GameWorld::Shutdown() noexcept
    {
        ...
        if (m_impl->gameInstance != nullptr || m_impl->focused)
            return false;
        for (const Impl::SystemEntry& entry : m_impl->systems)
            if (entry.system->m_needsPostWorldDetached)
                return false;
        ecs::ActionReport actions;
        ecs::ComponentActionReport componentActions;
        if (!m_impl->entities.FlushActions(&actions) || !m_impl->entities.FlushComponentActions(&componentActions))
            return false;
        for (Impl::SystemEntry& entry : m_impl->systems)
            if (entry.active)
                entry.system->OnAfterWorldFlush(*this);
        if (m_impl->entities.GetStats().entities != 0)
            return false;
        for (u32 index = m_impl->systems.Size(); index > 0; --index)
        {
            RuntimeSystem* const system = m_impl->systems[index - 1u].system;
    ...
    bool GameWorld::IsInitialized() const noexcept
    {
    ...
    {
        if (!IsInitialized() || gameInstance == nullptr || m_impl->gameInstance != nullptr)
            return false;
    ...
    {
        if (!IsInitialized() || gameInstance == nullptr || m_impl->gameInstance != gameInstance)
            return false;
        for (u32 index = m_impl->systems.Size(); index > 0; --index)
        {
    ...
    {
        if (!IsInitialized() || m_impl->gameInstance != nullptr)
            return false;
        for (u32 index = m_impl->systems.Size(); index > 0; --index)
        {
            RuntimeSystem* const system = m_impl->systems[index - 1u].system;
    ...
    {
        if (!IsInitialized() || !std::isfinite(deltaSeconds) || deltaSeconds < 0.0f)
            return false;
        if (!m_impl->entities.FlushActions() || !m_impl->entities.FlushComponentActions())
            return false;
        for (Impl::SystemEntry& entry : m_impl->systems)
            if (entry.active)
                entry.system->OnBeginFrame(*this, deltaSeconds);
        if (m_impl->tickEnabled && !m_impl->entities.GetProgress(deltaSeconds))
            return false;
        for (Impl::SystemEntry& entry : m_impl->systems)
            if (entry.active)
                entry.system->OnEndFrame(*this, deltaSeconds);
        if (!m_impl->entities.FlushActions() || !m_impl->entities.FlushComponentActions())
            return false;
        for (Impl::SystemEntry& entry : m_impl->systems)
            if (entry.active)
    ...
    void GameWorld::SetTickEnabled(const bool enabled) noexcept
    {
        if (m_impl != nullptr)
            m_impl->tickEnabled = enabled;
    }

    void GameWorld::GainFocus() noexcept
    {
        if (!IsInitialized() || m_impl->focused)
            return;
        m_impl->focused = true;
        for (Impl::SystemEntry& entry : m_impl->systems)
            if (entry.active)
                entry.system->OnGainFocus(*this);
    }
    ...
    void GameWorld::LoseFocus() noexcept
    {
        ...
        for (Impl::SystemEntry& entry : m_impl->systems)
            if (entry.active)
                entry.system->OnLoseFocus(*this);
        m_impl->focused = false;
    }

    RuntimeSystem* GameWorld::GetSystem(const RuntimeSystemId id) noexcept
    {
        if (m_impl == nullptr)
            return nullptr;
        for (Impl::SystemEntry& entry : m_impl->systems)
            if (entry.active && entry.system->GetDescriptor().id == id)
                return entry.system;
        return nullptr;
    }

    const RuntimeSystem* GameWorld::GetSystem(const RuntimeSystemId id) const noexcept
    {
        return const_cast<GameWorld*>(this)->GetSystem(id);
    }

    const char* GameWorld::ReadinessBlocker() const noexcept
    {
        if (!IsInitialized())
            return "game world is not initialized";
        for (const Impl::SystemEntry& entry : m_impl->systems)
            if (entry.active)
                if (const char* const blocker = entry.system->ReadinessBlocker())
                    return blocker;
        return nullptr;
    }

    WorldMode GameWorld::Mode() const noexcept
    {
        return m_impl != nullptr ? m_impl->config.mode : WorldMode::Game;
    }
    ecs::World& GameWorld::GetEntities() noexcept
    {
        return m_impl->entities;
    }
    const ecs::World& GameWorld::GetEntities() const noexcept
    {
        return m_impl->entities;
    }
    ...
    GameWorldStats GameWorld::GetStats() const noexcept
    {
        ...
        for (const Impl::SystemEntry& entry : m_impl->systems)
            stats.activeSystems += entry.active;
