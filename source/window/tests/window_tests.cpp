...
namespace
{
    ...
    public:
        ...
        [[nodiscard]] win::BackendStatus EnumerateDisplays(win::BackendDisplaySnapshot* const destination, const vanguard::u32 capacity,
                                                           vanguard::u32& count) noexcept override
        {
            ...
            for (vanguard::u32 index = 0; index < displayCount; ++index)
                destination[index] = displays[index];
            count = displayCount;
            return {};
        }

        [[nodiscard]] win::BackendStatus Create(const win::BackendWindowDescriptor& descriptor, win::BackendWindowId& window,
                                                win::BackendWindowState& state) noexcept override
        {
            ...
            if (reenterOnCreate && manager != nullptr)
            {
                ...
                reentryRejected = !manager->ProcessBackendEvent(event, &failure) && failure.code == win::FailureCode::BackendReentry;
            }
                                                          ...
                                                          win::BackendWindowState& state) noexcept override
        {
            ...
            if (record == nullptr)
                return win::BackendStatus::Failure(3, "fake window is unavailable");
        ...
        [[nodiscard]] win::BackendStatus SetWindowTitle(const win::BackendWindowId window, const char* const title) noexcept override
        {
            if (failNext)
                return ConsumeFailure();
            Window* const record = Find(window);
        ...
        [[nodiscard]] win::BackendStatus ResolvePresentationSurface(const win::BackendWindowId window,
                                                                    win::NativePresentationSurface& surface) noexcept override
        {
            surface = {};
            if (failNext)
                return ConsumeFailure();
        ...
        [[nodiscard]] win::BackendWindowEvent Event(const win::BackendWindowId window, const win::BackendEventType type,
    ...
    {
    public:
        explicit WrongThreadRejectWorker(win::WindowManager& manager) noexcept : Thread("windowRejectWorker"), m_manager(manager) {}
        ...
        void ThreadFunction() noexcept override
        {
            for (vanguard::u32 index = 0; index < 1000; ++index)
            {
                ...
                if (failure.code != win::FailureCode::WrongThread)
                    m_failed = true;
            }
        }
    ...
    {
    public:
        PresentationOwnershipWorker(win::WindowManager& manager, const win::WindowHandle window, win::PresentationAttachmentHandle& attachment,
                                    const bool detach) noexcept
            : Thread("presentationOwner"), m_manager(manager), m_window(window), m_attachment(attachment), m_detach(detach)
        {
        }
        ...
        {
            win::Failure failure{};
            m_succeeded = m_detach ? m_manager.DetachPresentation(m_attachment, &failure) : m_manager.AttachPresentation(m_window, m_attachment, &failure);
        ...
        [[nodiscard]] win::FailureCode FailureCode() const noexcept
        {
...
int main()
{
    ...
    Check(manager.IsInitialized() && manager.GetStats().activeDisplays == 2 && manager.GetPrimaryDisplay().IsValid(), "initial display topology publication");
    ...
    Check(!manager.Create(invalidDescriptor, invalidWindow, &failure) && failure.code == win::FailureCode::InvalidDescriptor,
    ...
    Check(manager.Create(headlessDescriptor, headlessWindow, &failure) && !manager.AttachPresentation(headlessWindow, headlessPresentation, &failure) &&
              failure.code == win::FailureCode::PresentationUnavailable && manager.DestroyWindow(headlessWindow, &failure),
    ...
    primaryDescriptor.flags = win::WindowFlag::Resizable | win::WindowFlag::HighPixelDensity | win::WindowFlag::AcceptFileDrop;
    ...
    WrongThreadRejectWorker rejectWorkers[] = {WrongThreadRejectWorker(manager), WrongThreadRejectWorker(manager), WrongThreadRejectWorker(manager),
                                               WrongThreadRejectWorker(manager)};
    for (WrongThreadRejectWorker& worker : rejectWorkers)
        worker.InitThread();
    for (WrongThreadRejectWorker& worker : rejectWorkers)
        worker.JoinThread();
    ...
    Check(rejectWorkersSucceeded && manager.GetStats().rejectedOperations == rejectsBeforeWorkers.rejectedOperations + 4000u,
          "concurrent wrong-thread rejections are counted atomically");

    win::WindowSnapshot primarySnapshot;
    Check(manager.GetSnapshot(primary, primarySnapshot) && primarySnapshot.lifecycle == win::WindowLifecycleState::Alive &&
              primarySnapshot.requested.position == win::WindowPoint{-640, 80} && primarySnapshot.nativeState.pixelExtent == win::WindowExtent{1600, 900} &&
              primarySnapshot.stateRevision == 1 && primarySnapshot.pixelExtentRevision == 1 && primarySnapshot.surfaceRevision == 1,
    ...
    Check(presentationAttachWorker.Succeeded() && presentationAttachWorker.FailureCode() == win::FailureCode::None && presentation.IsValid() &&
              manager.GetSnapshot(primary, primarySnapshot) && primarySnapshot.presentation == presentation &&
    ...
    Check(manager.GetSnapshot(presentation, presentationSnapshot) && presentationSnapshot.window == primary &&
              win::HasRequirement(presentationSnapshot.requirements, win::PresentationRequirement::SurfaceReconfigure) &&
              win::HasRequirement(presentationSnapshot.requirements, win::PresentationRequirement::PixelExtentResize),
    ...
    Check(!manager.AttachPresentation(primary, duplicatePresentation, &failure) && failure.code == win::FailureCode::PresentationAlreadyAttached,
          "a native window accepts only one presentation owner");
    Check(!manager.AcknowledgePresentation(presentation, {}, &failure) && failure.code == win::FailureCode::InvalidPresentationRevision,
          "zero presentation acknowledgements are rejected");
    Check(manager.AcknowledgePresentation(presentation, {presentationSnapshot.requiredPixelExtentRevision, presentationSnapshot.requiredSurfaceRevision},
                                          &failure) &&
              manager.GetSnapshot(presentation, presentationSnapshot) && presentationSnapshot.requirements == win::PresentationRequirement::None,
    ...
    Check(!manager.DestroyWindow(primary, &failure) && failure.code == win::FailureCode::PresentationStillAttached,
    ...
    Check(manager.RequestState(primary, stateRequest, &failure) && manager.GetSnapshot(primary, primarySnapshot) &&
              primarySnapshot.requested.position == stateRequest.placement.position &&
              primarySnapshot.nativeState.placement.logicalExtent == stateRequest.placement.logicalExtent && primarySnapshot.pixelExtentRevision == 2,
          "requested and native window state transaction");
    Check(manager.GetSnapshot(presentation, presentationSnapshot) &&
              win::HasRequirement(presentationSnapshot.requirements, win::PresentationRequirement::PixelExtentResize) &&
              !win::HasRequirement(presentationSnapshot.requirements, win::PresentationRequirement::SurfaceReconfigure),
    ...
    Check(!manager.RequestState(primary, stateRequest, &failure) && failure.code == win::FailureCode::BackendFailure &&
              manager.GetSnapshot(primary, primarySnapshot) && primarySnapshot.requested.logicalExtent == stateBeforeFailure.logicalExtent,
    ...
    Check(manager.ProcessBackendEvent(pixelEvent, &failure) && manager.GetSnapshot(primary, primarySnapshot) &&
              primarySnapshot.nativeState.pixelExtent == win::WindowExtent{2560, 1440} && primarySnapshot.pixelExtentRevision == 3,
          "backend pixel extent change updates the authoritative snapshot");
    Check(manager.AcknowledgePresentation(presentation, {observedPixelRevision, 0}, &failure) && manager.GetSnapshot(presentation, presentationSnapshot) &&
    ...
    Check(manager.AcknowledgePresentation(presentation, {presentationSnapshot.requiredPixelExtentRevision, 0}, &failure),
    ...
    Check(manager.ProcessBackendEvent(compositeEvent, &failure) && manager.GetSnapshot(primary, primarySnapshot) &&
              ...
              primarySnapshot.requested.mode == win::WindowMode::BorderlessFullscreen && !primarySnapshot.requested.visible,
    ...
    Check(!manager.AcknowledgePresentation(presentation, {presentationSnapshot.requiredPixelExtentRevision + 1u, presentationSnapshot.requiredSurfaceRevision},
                                           &failure) &&
    ...
    Check(manager.AcknowledgePresentation(presentation, {presentationSnapshot.requiredPixelExtentRevision, presentationSnapshot.requiredSurfaceRevision},
                                          &failure) &&
              manager.GetSnapshot(presentation, presentationSnapshot) && presentationSnapshot.requirements == win::PresentationRequirement::Suspended,
          "completed reconfiguration leaves only the independently derived suspension state");
    Check(!manager.AcknowledgePresentation(presentation, {presentationSnapshot.acknowledgedPixelExtentRevision - 1u, 0}, &failure) &&
              failure.code == win::FailureCode::InvalidPresentationRevision,
    ...
    Check(!manager.ProcessBackendEvent(invalidStateEvent, &failure) && failure.code == win::FailureCode::BackendFailure &&
              manager.GetSnapshot(primary, primarySnapshot) && primarySnapshot.stateRevision == revisionBeforeInvalidState,
    ...
    Check(!manager.ProcessBackendEvent(invalidTypeEvent, &failure) && failure.code == win::FailureCode::BackendFailure,
    ...
    Check(presentationDetachWorker.Succeeded() && presentationDetachWorker.FailureCode() == win::FailureCode::None &&
              !manager.GetSnapshot(presentation, presentationSnapshot) && manager.GetStats().activePresentationAttachments == 0 &&
              manager.GetSnapshot(primary, primarySnapshot) && !primarySnapshot.presentation.IsValid(),
    ...
    Check(manager.ProcessBackendEvent(backend.Event(primaryBackend, win::BackendEventType::CloseRequested, 50), &failure) &&
              manager.GetSnapshot(primary, primarySnapshot) && primarySnapshot.closeRequestSerial == 1 &&
              manager.ResolveCloseRequest(primary, 1, win::CloseDecision::Defer, &failure) &&
              !manager.ResolveCloseRequest(primary, 2, win::CloseDecision::Reject, &failure) && failure.code == win::FailureCode::InvalidCloseSerial &&
    ...
    Check(manager.ProcessBackendEvent(backend.Event(primaryBackend, win::BackendEventType::CloseRequested, 60), &failure) &&
              manager.GetSnapshot(primary, primarySnapshot) && primarySnapshot.closeRequestSerial == 2 &&
              !manager.ResolveCloseRequest(primary, 2, win::CloseDecision::Accept, &failure) && failure.code == win::FailureCode::ParentHasChildren,
    ...
    backend.FailNext();
    Check(!manager.ResolveCloseRequest(primary, 2, win::CloseDecision::Accept, &failure) && failure.code == win::FailureCode::BackendFailure &&
              manager.GetSnapshot(primary, primarySnapshot) && primarySnapshot.lifecycle == win::WindowLifecycleState::CloseRequested,
          "failed native destruction restores the exact previous lifecycle");
    Check(manager.ResolveCloseRequest(primary, 2, win::CloseDecision::Accept, &failure) && !manager.GetSnapshot(primary, primarySnapshot) &&
    ...
    for (vanguard::u32 index = 0; index < displaysBeforeRemoval; ++index)
        if (!displaySnapshots[index].primary)
            secondary = displaySnapshots[index];
    ...
    Check(manager.Create(topologyWindowDescriptor, topologyWindow, &failure), "display topology fixture window creation");
    win::PresentationAttachmentHandle topologyPresentation{};
    Check(manager.AttachPresentation(topologyWindow, topologyPresentation, &failure) && manager.GetSnapshot(topologyPresentation, presentationSnapshot) &&
              manager.AcknowledgePresentation(topologyPresentation,
                                              {presentationSnapshot.requiredPixelExtentRevision, presentationSnapshot.requiredSurfaceRevision}, &failure),
    ...
    Check(manager.RefreshDisplays(&failure) && secondary.handle.IsValid() && !manager.GetSnapshot(secondary.handle, secondary) &&
              manager.GetSnapshot(topologyWindow, primarySnapshot) && !primarySnapshot.requested.display.IsValid() &&
              !primarySnapshot.nativeState.placement.display.IsValid() && manager.GetSnapshot(topologyPresentation, presentationSnapshot) &&
              win::HasRequirement(presentationSnapshot.requirements, win::PresentationRequirement::SurfaceReconfigure),
    ...
    Check(manager.ProcessBackendEvent(topologyEvent, &failure) && manager.GetSnapshot(topologyWindow, primarySnapshot) &&
              primarySnapshot.requested.display == manager.GetPrimaryDisplay(),
          "a later authoritative backend event reconciles an invalidated window placement");
    Check(manager.GetSnapshot(topologyPresentation, presentationSnapshot) &&
              manager.AcknowledgePresentation(topologyPresentation,
                                              {presentationSnapshot.requiredPixelExtentRevision, presentationSnapshot.requiredSurfaceRevision}, &failure),
    ...
    Check(manager.RefreshDisplays(&failure) && manager.GetStats().activeDisplays == 2 && manager.GetStats().topologyRevision >= 3,
          "display slot reuse receives a new generation and topology revision");

    const win::DisplayHandle primaryBeforeTopologyUpdate = manager.GetPrimaryDisplay();
    backend.displays[0].workArea.extent.height = 1380;
    Check(manager.RefreshDisplays(&failure) && manager.GetPrimaryDisplay() == primaryBeforeTopologyUpdate,
          "mutable display topology updates preserve physical display identity");

    const win::DisplayHandle primaryBeforeFingerprintReplacement = manager.GetPrimaryDisplay();
    backend.displays[0].fingerprint = 0x1112;
    Check(manager.RefreshDisplays(&failure) && manager.GetPrimaryDisplay().IsValid() && !(manager.GetPrimaryDisplay() == primaryBeforeFingerprintReplacement) &&
              !manager.GetSnapshot(primaryBeforeFingerprintReplacement, secondary) && manager.GetSnapshot(topologyWindow, primarySnapshot) &&
    ...
    topologyEvent.state.placement.display = {101};
    Check(manager.ProcessBackendEvent(topologyEvent, &failure) && !manager.DestroyWindow(topologyWindow, &failure) &&
              failure.code == win::FailureCode::PresentationStillAttached && manager.DetachPresentation(topologyPresentation, &failure) &&
    ...
    static_cast<void>(manager.GetSnapshot(manager.GetPrimaryDisplay(), replacedDisplay));
    for (vanguard::u32 index = 0; index < win::MaximumDisplays; ++index)
    {
    ...
    Check(manager.RefreshDisplays(&failure) && manager.GetStats().activeDisplays == win::MaximumDisplays &&
              !manager.GetSnapshot(replacedDisplay.handle, replacedDisplay),
    ...
    Check(manager.RefreshDisplays(&failure) && manager.GetStats().activeDisplays == 2, "display topology contracts back to the active set");
    ...
    Check(manager.CreateEventCursor(win::EventCursorOrigin::OldestAvailable, slowConsumer), "slow event consumer cursor creation");
    for (vanguard::u32 index = 0; index < win::WindowEventJournalCapacity + 8u; ++index)
        Check(manager.ProcessBackendEvent(backend.Event(journalBackend, win::BackendEventType::Exposed, index), &failure), "event journal stress publication");
    ...
    Check(firstRead.count == secondRead.count && firstRead.newestSequence == secondRead.newestSequence, "event cursors consume the journal independently");
    ...
    Check(foreignRead.invalidCursor && foreignRead.count == 0, "event cursors from another manager lifetime are rejected explicitly");
    win::WindowEventCursor invalidOriginCursor{};
    Check(!manager.CreateEventCursor(static_cast<win::EventCursorOrigin>(0xffu), invalidOriginCursor), "invalid event cursor origins are rejected");

    Check(!manager.Shutdown(&failure) && failure.code == win::FailureCode::WindowsRemainAlive, "shutdown rejects live windows");
    ...
    if (g_failures == 0)
        std::printf("[windowTests] all tests passed\n");
