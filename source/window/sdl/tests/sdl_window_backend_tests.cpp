...
int main()
{
    ...
    if (!backend.Initialize()) return 2;

    BackendDisplaySnapshot displays[MaximumDisplays]{};
    u32 displayCount = 0;
    if (!backend.EnumerateDisplays(displays, MaximumDisplays, displayCount) || displayCount == 0) return 3;
    ...
    if (!backend.Create(descriptor, window, state) || !window.IsValid()) return 4;
    if (state.placement.logicalExtent.width != 640 || state.placement.logicalExtent.height != 360) return 5;
    ...
    if (!backend.ApplyWindowState(window, request, state)) return 6;
    if (state.placement.logicalExtent.width != 800 || state.placement.logicalExtent.height != 450) return 7;
    ...
    if (!translation.windowEvent || translation.displayTopologyChanged ||
        translated.type != BackendEventType::CloseRequested || translated.window != window ||
        translated.timestampNanoseconds != 123) return 8;

    SDL_Event display{};
    display.type = SDL_EVENT_DISPLAY_CONTENT_SCALE_CHANGED;
    if (!backend.ProcessEvent(display, translated).displayTopologyChanged) return 9;
    if (!backend.DestroyWindow(window)) return 10;

    WindowManager manager;
    Failure failure;
    if (!manager.Initialize(backend, &failure)) return 11;
    WindowDescriptor managedDescriptor;
    managedDescriptor.title = "Vanguard managed SDL window";
    managedDescriptor.placement.display = manager.PrimaryDisplay();
    managedDescriptor.placement.logicalExtent = {960, 540};
    managedDescriptor.placement.visible = false;
    WindowHandle managedWindow;
    if (!manager.Create(managedDescriptor, managedWindow, &failure)) return 12;
    WindowSnapshot snapshot;
    if (!manager.Snapshot(managedWindow, snapshot) || snapshot.nativeState.pixelExtent.width == 0 ||
        snapshot.nativeState.pixelExtent.height == 0) return 13;
    if (!manager.DestroyWindow(managedWindow, &failure) || !manager.Shutdown(&failure)) return 14;
    if (!backend.Shutdown()) return 15;
