...
namespace math
{
	RED_FORCE_INLINE Vector3::Vector3( const Float f )
		: X(f), Y(f), Z(f)
	{}

	RED_FORCE_INLINE Vector3::Vector3( const Float x, const Float y, const Float z )
		: X(x), Y(y), Z(z)
	{}

	RED_FORCE_INLINE Vector3::Vector3( const Vector3& v )
		: X(v.X), Y(v.Y), Z(v.Z)
	{}

	RED_FORCE_INLINE Vector3::Vector3( const Vector2& v )
		: X(v.X), Y(v.Y), Z(0.f)
	{}

	RED_FORCE_INLINE Vector3::Vector3( const Vector4& v )
		: X(v.X), Y(v.Y), Z(v.Z)
	{}

	RED_FORCE_INLINE Vector3::Vector3( const Float f[3] )
		: X(f[0]), Y(f[1]), Z(f[2])
	{}
