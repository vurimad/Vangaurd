...
namespace simd
{
	namespace QuadHelper
	{
		...
		{
			Quad c = _mm_sub_ps(
				_mm_mul_ps( _a, _mm_shuffle_ps( _b, _b, _MM_SHUFFLE( 3, 0, 2, 1 ) ) ),
		...
		Quad QuaternionMultiplication( Quad xyzw, Quad abcd )
		{
			...
			Quad XZWY = _mm_addsub_ps( _mm_shuffle_ps( XZYnW, ZnXWY, _MM_SHUFFLE( 3, 2, 1, 0 ) ),
				_mm_shuffle_ps( ZnXWY, XZYnW, _MM_SHUFFLE( 2, 3, 0, 1 ) ) );
	...
	RED_INLINE QsTransform::QsTransform()
		: Scale( Vector4::ONES() ) // Translation and Rotation use default initialization
	{
	}
	...
	RED_INLINE QsTransform::QsTransform( const Vector4& _translation, const math::Quaternion& _rotation, const Vector4& _scale )
	...
	RED_INLINE QsTransform::QsTransform( const Vector4& _translation, const math::Quaternion& _rotation )
	...
	void QsTransform::Set( const Vector4& _translation, const math::Quaternion& _rotation, const Vector4& _scale )
	{
		Translation = _translation;
	...
	void QsTransform::Set( const math::Matrix& _m )
	{
		Translation = simd::Vector4(_m.GetTranslation().AsFloat());
		Scale = simd::Vector4(_m.GetScale33().AsFloat());
		Rotation = _m.ToQuat(); 
		RED_MATH_PARANOID_SANITY_CHECK( IsOk() );
	}
	...
	void QsTransform::SetRotation( const math::Quaternion& _r )
	{
	...
	const math::Quaternion& QsTransform::GetRotation() const
	{
		return Rotation;
	}
	...
	void QsTransform::Lerp( const QsTransform& _a, const QsTransform& _b, float _t)
	{
		...
		Rotation = math::Quaternion::Lerp( _a.Rotation, _b.Rotation , _t );
		RED_MATH_PARANOID_SANITY_CHECK( IsOk() );
	}
	...
	void QsTransform::Slerp( const QsTransform& _a, const QsTransform& _b, float _t)
	{
		Scale = Vector4::Lerp( _a.Scale, _b.Scale, _t );
		Translation = Vector4::Lerp( _a.Translation, _b.Translation, _t );
		Rotation = math::Quaternion::Slerp( _a.Rotation, _b.Rotation, _t );
		RED_MATH_PARANOID_SANITY_CHECK( IsOk() );
	}

	RED_INLINE math::Matrix QsTransform::ConvertToMatrix() const
	{

		math::Vector4 q = Rotation.AsVector();
		...
		return math::Matrix( math::Vector4( ( 1.0f - ( yy + zz ) ) * Scale.X, ( xy + wz ) * Scale.X, ( xz - wy )* Scale.X, 0.0f ),
							 math::Vector4( ( xy - wz ) * Scale.Y, ( 1.0f - ( xx + zz ) ) * Scale.Y, ( yz + wx )* Scale.Y, 0.0f ),
							 math::Vector4( ( xz + wy ) * Scale.Z, ( yz - wx ) * Scale.Z, ( 1.0f - ( xx + yy ) ) * Scale.Z, 0.0f ),
							 math::Vector4( Translation.X, Translation.Y, Translation.Z, 1.0f ) );
	}

	RED_INLINE void QsTransform::ConvertToMatrix(math::Matrix& m) const
	{
	...
	RED_INLINE void QsTransform::ConvertToMatrixFast(math::Matrix& m) const
	{
	...
	RED_INLINE math::Matrix QsTransform::ConvertToMatrixNormalized() const
	{
	...
	{
		return Translation.IsAlmostEqual(_other.Translation, _epsilon) && Rotation.IsAlmostEqual(_other.Rotation, _epsilon) && Scale.IsAlmostEqual(_other.Scale, _epsilon);
	}
	...
	{
		if( math::Abs( _totalWeight ) < FLT_EPSILON )
		{
			SetIdentity();
			return;
		}

		{
			...
			simd::SetMul(Translation, invWeight);
			simd::SetMul(Scale, invWeight);
		}
	...
	void QsTransform::FastRenormalizeBatch( QsTransform* _poseOut, float* _weight, unsigned int _numTransforms)
	{
		for (unsigned int i = 0; i < _numTransforms; ++i)
		{
			...
			simd::SetMul( _poseOut[i].Translation, invWeight );
			simd::SetMul( _poseOut[i].Scale, invWeight );
		}
	...
	{
		return { Vector4::ZERO_3D_POINT(), math::Quaternion::IDENTITY(), Vector4::ONES() };
	}
} // simd
