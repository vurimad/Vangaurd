...
namespace simd
{
	...
	public:
		...
		RED_INLINE QsTransform( const Vector4& _translation, const math::Quaternion& _rotation, const Vector4& _scale );
		RED_INLINE QsTransform( const Vector4& _translation, const math::Quaternion& _rotation) ;			

		// Operators
		RED_INLINE void operator=( const QsTransform& _other );

		RED_INLINE void Set( const Vector4& _translation, const math::Quaternion& _rotation, const Vector4& _scale );
		RED_INLINE void Set( const math::Matrix& _m );

		RED_INLINE void SetIdentity();
		RED_INLINE void SetZero();

		RED_INLINE void SetTranslation( const Vector4& _v );
		RED_INLINE void SetRotation(const math::Quaternion& _r);
		RED_INLINE void SetScale(const Vector4& _s);

		RED_INLINE const Vector4& GetTranslation() const;
		RED_INLINE const math::Quaternion& GetRotation() const;
		...
		RED_INLINE math::Matrix ConvertToMatrix() const;
		RED_INLINE void ConvertToMatrix(math::Matrix& m) const;
		RED_INLINE void ConvertToMatrixFast(math::Matrix& m) const;
		RED_INLINE math::Matrix ConvertToMatrixNormalized() const;
