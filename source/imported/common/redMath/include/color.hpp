...
namespace math
{
	...
	RED_INLINE Color::Color( Uint8 r, Uint8 g, Uint8 b, Uint8 a/*=255*/ )
		: R( r ), G( g ), B( b ), A( a )
	{}

	RED_INLINE Color::Color( const Vector4& x )
		: math::Color{ (Uint8) math::Clamp( x.X * 255.0f, 0.0f, 255.0f ), 
					   (Uint8) math::Clamp( x.Y * 255.0f, 0.0f, 255.0f ),
					   (Uint8) math::Clamp( x.Z * 255.0f, 0.0f, 255.0f ),
					   (Uint8) math::Clamp( x.W * 255.0f, 0.0f, 255.0f ) }
	{}

	RED_INLINE Color::Color( const Float f[ 4 ] )
		: math::Color{ ( Uint8 ) math::Clamp( f[ 0 ] * 255.0f, 0.0f, 255.0f ),
					   ( Uint8 ) math::Clamp( f[ 1 ] * 255.0f, 0.0f, 255.0f ),
					   ( Uint8 ) math::Clamp( f[ 2 ] * 255.0f, 0.0f, 255.0f ),
					   ( Uint8 ) math::Clamp( f[ 3 ] * 255.0f, 0.0f, 255.0f ) }
	{}
	...
	RED_FORCE_INLINE Vector4 Color::ToVectorLinearAccu() const
	{
		return
		{
			g_gammaToLinearLUT.floatLut[R],
			g_gammaToLinearLUT.floatLut[G],
			g_gammaToLinearLUT.floatLut[B],
			A / 255.0f
		};
	}
	...
	RED_FORCE_INLINE Half4 Color::ToHalfLinearAccu() const
	{
		return
		{
			g_gammaToLinearLUT.halfLut[R],
			g_gammaToLinearLUT.halfLut[G],
			g_gammaToLinearLUT.halfLut[B],
			Half( A / 255.0f )
		};
	}
	...
	Color Color::FromVectorLinear( const Vector4& linearColor )
	{
		return
		{
			(Uint8)math::Clamp( ToGamma( linearColor.X ) * 255.0f, 0.0f, 255.0f ),
			(Uint8)math::Clamp( ToGamma( linearColor.Y ) * 255.0f, 0.0f, 255.0f ),
			(Uint8)math::Clamp( ToGamma( linearColor.Z ) * 255.0f, 0.0f, 255.0f ),
			(Uint8)math::Clamp( linearColor.W * 255.0f, 0.0f, 255.0f )
		};
	}
	...
	Color Color::FromVectorLinearAccu( const Vector4& linearColor )
	{
		return
		{
			(Uint8)math::Clamp( ToGammaAccu( linearColor.X ) * 255.0f, 0.0f, 255.0f ),
			(Uint8)math::Clamp( ToGammaAccu( linearColor.Y ) * 255.0f, 0.0f, 255.0f ),
			(Uint8)math::Clamp( ToGammaAccu( linearColor.Z ) * 255.0f, 0.0f, 255.0f ),
			(Uint8)math::Clamp( linearColor.W * 255.0f, 0.0f, 255.0f )
		};
	}
	...
	RED_INLINE Color Color::Mul3( const Color& a, Float b )
	{
		...
		Color out;
		out.R = (Uint8) math::Clamp< Float >( a.R*b, 0.0f, 255.0f );
		out.G = (Uint8) math::Clamp< Float >( a.G*b, 0.0f, 255.0f );
		out.B = (Uint8) math::Clamp< Float >( a.B*b, 0.0f, 255.0f );
		out.A = a.A;
		return out;
	}
	...
	RED_INLINE Color Color::Mul4( const Color& a, Float b )
	{
		...
		out.G = (Uint8) math::Clamp< Float >( a.G*b, 0.0f, 255.0f );
		out.B = (Uint8) math::Clamp< Float >( a.B*b, 0.0f, 255.0f );
		out.A = (Uint8) math::Clamp< Float >( a.A*b, 0.0f, 255.0f );
		return out;
	}
	...
	{
		RED_ASSERT( b >= 0.0f && b <= 1.0f );
		R = (Uint8) math::Clamp< Float >( R*b, 0.0f, 255.0f );
		G = (Uint8) math::Clamp< Float >( G*b, 0.0f, 255.0f );
		B = (Uint8) math::Clamp< Float >( B*b, 0.0f, 255.0f );
	}
	...
	RED_INLINE void Color::Mul4( Float b )
	{
		...
		G = (Uint8) math::Clamp< Float >( G*b, 0.0f, 255.0f );
		B = (Uint8) math::Clamp< Float >( B*b, 0.0f, 255.0f );
		A = (Uint8) math::Clamp< Float >( A*b, 0.0f, 255.0f );
	}
	...
	RED_INLINE Color Color::Lerp( Float coef , const Color&a , const Color&b )
	{
		...
		Color out;
		out.R = (Uint8) math::Clamp< Float >( a.R * one_minut_coef + b.R * coef, 0.0f, 255.0f );
		out.G = (Uint8) math::Clamp< Float >( a.G * one_minut_coef + b.G * coef, 0.0f, 255.0f );
		out.B = (Uint8) math::Clamp< Float >( a.B * one_minut_coef + b.B * coef, 0.0f, 255.0f );
		out.A = (Uint8) math::Clamp< Float >( a.A * one_minut_coef + b.A * coef, 0.0f, 255.0f );
		return out;
	}
    ...
    RED_INLINE Color Color::operator+(const Color& c) const
    {
        ...
        result.R = math::Clamp< Uint8 >( R+c.R, 0, 255 );
		result.G = math::Clamp< Uint8 >( G+c.G, 0, 255 );
		result.B = math::Clamp< Uint8 >( B+c.B, 0, 255 );
		result.A = math::Clamp< Uint8 >( A+c.A, 0, 255 );
