namespace simd
{
	...
	RED_INLINE void CalculatePerpendicularVector( const Vector4& _in, Vector4& _out )
	{
		...
		float vA = math::Abs( _in.X );
		float vB = math::Abs( _in.Y );
		float vC = math::Abs( _in.Z );
	...
	{
		math::Vector4 redAxis( normAxis.X, normAxis.Y, normAxis.Z, 0.0f );
		math::Quaternion rotQuat(redAxis, angle);
...
namespace simd
{
	...
	RED_FORCE_INLINE __m128 Dot(
		const __m128& xxxx, const __m128& yyyy, const __m128& zzzz,
	...
	RED_FORCE_INLINE __m128 Dot(
		const __m128& xxxx, const __m128& yyyy, const __m128& zzzz, const __m128& wwww,
		const __m128& xxxx1, const __m128& yyyy1, const __m128& zzzz1, const __m128& wwww1 )
	{
		return MulAdd( wwww, wwww1, MulAdd( zzzz, zzzz1, MulAdd( xxxx, xxxx1, _mm_mul_ps( yyyy, yyyy1 ) ) ) );
	}

	RED_FORCE_INLINE void Normalize(
		__m128* outXXXX, __m128* outYYYY, __m128* outZZZZ, __m128* outWWWW,
		const __m128& xxxx, const __m128& yyyy, const __m128& zzzz, const __m128& wwww )
	{
	...
	RED_FORCE_INLINE void QuatNLerp(
		__m128* outXXXX, __m128* outYYYY, __m128* outZZZZ, __m128* outWWWW,
		const __m128& xxxx, const __m128& yyyy, const __m128& zzzz, const __m128& wwww,
		const __m128& xxxx1, const __m128& yyyy1, const __m128& zzzz1, const __m128& wwww1,
		const __m128& t )
	{
		...
		Normalize( 
			outXXXX, outYYYY, outZZZZ, outWWWW,
			tmpXXXX, tmpYYYY, tmpZZZZ, tmpWWWW
		);

	}

	// functions to write computation result to data in AoS (Array of Structs) format
	RED_FORCE_INLINE void Scatter3( ::simd::Vector4* output0, ::simd::Vector4* output1, ::simd::Vector4* output2, ::simd::Vector4* output3, const __m128 input[3], const __m128& w )
	{
		__m128 tmp[4] =
		{
			input[0], input[1], input[2], w
		};
	...
	RED_FORCE_INLINE void Scatter4( ::simd::Vector4* output0, ::simd::Vector4* output1, ::simd::Vector4* output2, ::simd::Vector4* output3, const __m128 input[4] )
	{
		__m128 tmp[4] =
		{
