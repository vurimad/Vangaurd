...
namespace math
{
	RED_FORCE_INLINE Box::Box( const Box& rhs )
		: Box{ rhs.Min, rhs.Max }
	{
	}

	RED_FORCE_INLINE Box::Box( const Vector4& min, const Vector4& max )
		: Min{ min }
		, Max{ max }
	{
	}
	...
	RED_FORCE_INLINE Box::Box( const Vector4& center, float radius )
		: Min{ center - Vector4{ radius, radius, radius, 0 } }
		, Max{ center + Vector4{ radius, radius, radius, 0 } }
	{
	}

	RED_FORCE_INLINE Box::Box( EResetState )
		: Min{ Vector4::PLUS_MAX() }
		, Max{ Vector4::MINUS_MAX() }
	{
	}

	RED_FORCE_INLINE Box::Box( EMaximize )
		: Min{ Vector4::MINUS_MAX() }
		, Max{ Vector4::PLUS_MAX() }
	{
	}
	...
	{
		Float sqrsum = 0;
		if ( pos.X < Min.X ) sqrsum += (Max.X - pos.X)*(Max.X - pos.X);
		if ( pos.X > Max.X ) sqrsum += (Min.X - pos.X)*(Min.X - pos.X);
		if ( pos.Y < Min.Y ) sqrsum += (Max.Y - pos.Y)*(Max.Y - pos.Y);
		if ( pos.Y > Max.Y ) sqrsum += (Min.Y - pos.Y)*(Min.Y - pos.Y);
		if ( pos.Z < Min.Z ) sqrsum += (Max.Z - pos.Z)*(Max.Z - pos.Z);
		if ( pos.Z > Max.Z ) sqrsum += (Min.Z - pos.Z)*(Min.Z - pos.Z);
		return (Float)sqrt( sqrsum );
	}
