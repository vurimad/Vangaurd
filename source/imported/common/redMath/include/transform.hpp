...
namespace math
 {
	RED_FORCE_INLINE Transform::Transform()
		: m_position{ Vector4::ZEROS() }
		, m_orientation{ Quaternion::IDENTITY() } 
	{}

	RED_FORCE_INLINE Transform::Transform( EIdentity )
		: m_position{ Vector4::ZEROS() }
		, m_orientation{ Quaternion::IDENTITY() }
	{}
	...
	RED_INLINE Transform::Transform( const Vector3& position, const Quaternion& orientation )
		: m_position( position.X, position.Y, position.Z, 0.f )
	...
	RED_INLINE Transform::Transform( const Quaternion& q )
		: m_position( Vector4::ZEROS() )
		, m_orientation( q.Normalized() )
	{}

	RED_INLINE Transform::Transform( const Transform& xform )
		: m_position( xform.m_position )
		, m_orientation( xform.m_orientation )
	{}
	...
	RED_INLINE Box Transform::TransformBox( const Box& box ) const
	{
		...
		return Box
		(
			Vector3::Min( xa, xb ) + Vector3::Min( ya, yb ) + Vector3::Min( za, zb ) + m_position,
