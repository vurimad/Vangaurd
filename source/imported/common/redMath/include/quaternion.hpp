...
namespace math
{
	namespace Helper
	{
		...
		///
		{
			return 
			{
				_q.r*_v.X + _q.j*_v.Z - _v.Y*_q.k,
				_q.r*_v.Y + _q.k*_v.X - _v.Z*_q.i,
				_q.r*_v.Z + _q.i*_v.Y - _v.X*_q.j,
		...
		///
		{
			...
			float vA = math::Abs( in.X );
			float vB = math::Abs( in.Y );
			float vC = math::Abs( in.Z );
	...
	RED_FORCE_INLINE Quaternion::Quaternion()
		: i( 0.0f ), j( 0.0f ), k( 0.0f ), r( 1.0f )
	{}

	RED_FORCE_INLINE Quaternion::Quaternion( const Float _i, const Float _j, const Float _k, const Float real )
		: i( _i ), j( _j ), k( _k ), r( real )
	{}

	RED_FORCE_INLINE Quaternion::Quaternion( const Vector4& v )
		: vec( v )
	{}
	...
	RED_FORCE_INLINE Quaternion::Quaternion( const Float arr[4] )
		: Quaternion{ arr[0], arr[1], arr[2], arr[3] }
	{}

	RED_FORCE_INLINE Quaternion::Quaternion( const Quaternion& q )
		: Quaternion{ q.i, q.j, q.k, q.r }
	{}
	...
	RED_INLINE Quaternion Quaternion::operator*( const Quaternion& q ) const
	{
		return
		{
			r*q.i + q.r*i + j*q.k - q.j*k,
			r*q.j + q.r*j + k*q.i - q.k*i,
			r*q.k + q.r*k + i*q.j - q.i*j,
	...
	{
		const Float angle = MAcos_safe( math::Abs( r ) ) * 2.f;
		return angle;
	}
	...
	{
		return Quaternion( simd::QuadHelper::QuaternionLerp( q1.vec.vec, q2.vec.vec, t ) );
	}

	RED_INLINE Quaternion Quaternion::Slerp( const Quaternion& q1, const Quaternion& q2, const Float t, const Float tolerance )
	{
		return Quaternion( simd::QuadHelper::QuaternionSlerp( q1.vec.vec, q2.vec.vec, t, tolerance ) );
	}
	...
	RED_INLINE Bool Quaternion::IsOk() const
	{
		...
		Bool	validLength = ( math::Abs( err ) < unitTolerance ); // changed because the vectorized math has a little bit less precision.
	...
	RED_INLINE Bool Quaternion::IsAlmostEqual( const Quaternion& q, const Float epsilon ) const
	{
		...
		return ( math::Abs(delta.r) < epsilon &&
				 math::Abs(delta.i) < epsilon &&
				 math::Abs(delta.j) < epsilon &&
				 math::Abs(delta.k) < epsilon );
	}
	...
	RED_INLINE void Quaternion::SetShortestRotation( const Vector4& from, const Vector4& to, const Vector4& perp, const Float eps )
	{
		...
		else
		{
			...
			// To get quaternion for single angle value, take identity quaternion [0, 0, 0, 1], add it to quaternion for doubled angle and normalize the result
