...
namespace math
{
	RED_FORCE_INLINE Vector4::Vector4( const Vector2& v )
		: X( v.X ), Y( v.Y ), Z( 0.f ), W( 1.f )
	{}

	RED_FORCE_INLINE Vector4::Vector4( const Vector3& v, const Float w )
		: X( v.X ), Y( v.Y ), Z( v.Z ), W( w )
	{}

	RED_FORCE_INLINE Vector4::Vector4( const Vector4& v )
		: vec( v.vec )
	{}

	RED_FORCE_INLINE Vector4::Vector4( const Float f[4] )
		: X( f[0] ), Y( f[1] ), Z( f[2] ), W( f[3] )
	{}

	RED_FORCE_INLINE Vector4::Vector4( const Float x, const Float y, const Float z, const Float w )
		: vec( _mm_set_ps( w, z, y, x ) )
	{}

	RED_FORCE_INLINE Vector4::Vector4( const Float f )
		: vec( _mm_set1_ps( f ) )
	{}

	RED_FORCE_INLINE Vector4::Vector4( const __m128& src )
		: vec( src )
	{}
	...
	{
		vec = simd::QuadHelper::Normalize4(vec);
	}
	...
	{
		return Vector4(simd::QuadHelper::Normalize4(vec));
	}
