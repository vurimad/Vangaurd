...
namespace math
{
	OrientedBox::OrientedBox( const Vector4& pos, const Vector4& edge1, const Vector4& edge2, const Vector4& edge3 )
		: m_position( pos )
	{
	...
	Bool OrientedBox::IntersectRay( const Vector4& origin, const Vector4& direction, Float& enterDistFromOrigin ) const
	{
		...
		const Float tmin = math::Max( math::Max( math::Min( t[ 0 ], t[ 1 ] ), math::Min( t[ 2 ], t[ 3 ] ) ), math::Min( t[ 4 ], t[ 5 ] ) );
		const Float tmax = math::Min( math::Min( math::Max( t[ 0 ], t[ 1 ] ), math::Max( t[ 2 ], t[ 3 ] ) ), math::Max( t[ 4 ], t[ 5 ] ) );
