...
namespace math
{
	...
	template<>
	REDMATH_API Bool Random::Get()
	{
		return GetRaw() & 0x1;
	}

	template<>
	REDMATH_API Uint32 Random::Get()
	{
		return GetRaw();
	}

	template<>
	REDMATH_API Int32 Random::Get()
	{
		return GetRaw();
	}

	template<>
	REDMATH_API Uint64 Random::Get()
	{
		return (Uint64)GetRaw() | ( (Uint64)GetRaw() << 32 );
	}

	template<>
	REDMATH_API Int64 Random::Get()
	{
		return (Uint64)GetRaw() | ( (Uint64)GetRaw() << 32 );
	}

	template<>
	REDMATH_API Float Random::Get()
	{
		union  
		{
			Uint32 i;
	...
	template<>
	REDMATH_API Vector2 Random::Get()
	{
		return { Get<Float>(), Get<Float>() };
	}

	template<>
	REDMATH_API Vector3 Random::Get()
	{
		return { Get<Float>(), Get<Float>(), Get<Float>() };
	}

	template<>
	REDMATH_API Vector4 Random::Get()
	{
		return { Get<Float>(), Get<Float>(), Get<Float>(), Get<Float>() };
	}

	template<>
	REDMATH_API Color Random::Get()
	{
	...
	template<>
	REDMATH_API Uint32 Random::Get( const Uint32 max )
	{
		return GetRaw() % max;
	}

	template<>
	REDMATH_API Int32 Random::Get( const Int32 max )
	{
		return GetRaw() % max;
	}

	template<>
	REDMATH_API Uint64 Random::Get( const Uint64 max )
	{
		return Get<Uint64>() % max;
	}

	template<>
	REDMATH_API Int64 Random::Get( const Int64 max )
	{
		return Get<Int64>() % max;
	}

	template<>
	REDMATH_API Float Random::Get( const Float max )
	{
		return max * Get<Float>();
	}

	template<>
	REDMATH_API Double Random::Get( const Double max )
	{
	...
	template<>
	REDMATH_API Uint32 Random::Get( const Uint32 min, const Uint32 max )
	{
	...
	template<>
	REDMATH_API Int32 Random::Get( const Int32 min, const Int32 max )
	{
		RED_FATAL_ASSERT( max > min, "Min is greater or equal max" );
	...
	template<>
	REDMATH_API Uint64 Random::Get( const Uint64 min, const Uint64 max )
	{
		RED_FATAL_ASSERT( max > min, "Min is greater or equal max" );
	...
	template<>
	REDMATH_API Int64 Random::Get( const Int64 min, const Int64 max )
	{
		RED_FATAL_ASSERT( max > min, "Min is greater or equal max" );
	...
	template<>
	REDMATH_API Float Random::Get( const Float min, const Float max )
	{
		return min + Get<Float>() * ( max - min );
	}

	template<>
	REDMATH_API Double Random::Get( const Double min, const Double max )
	{
		return min + Get<Double>() * ( max - min );
	}

	template<>
	REDMATH_API Vector2 Random::Get( const Vector2 min, const Vector2 max )
	{
		return min + Get<Vector2>() * ( max - min );
	}

	template<>
	REDMATH_API Vector3 Random::Get( const Vector3 min, const Vector3 max )
	{
		return min + Get<Vector3>() * ( max - min );
	}

	template<>
	REDMATH_API Vector4 Random::Get( const Vector4 min, const Vector4 max )
	{
