...
namespace math
{
	Vector3 REDMATH_API HSVToRGB(const Float h, const Float s, const Float v)
	{
		...
		if (h_i == 0) return Vector3(v, t, p);
		if (h_i == 1) return Vector3(q, v, p);
		if (h_i == 2) return Vector3(p, v, t);
		if (h_i == 3) return Vector3(p, q, v);
		if (h_i == 4) return Vector3(t, p, v);
		if (h_i == 5) return Vector3(v, p, q);
