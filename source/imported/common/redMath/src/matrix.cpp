...
#define VecShuffle_2323(vec1, vec2)        _mm_movehl_ps(vec2, vec1)
	...
	{
		return Matrix(
			b.TransformVectorWithW( a.X ),
			b.TransformVectorWithW( a.Y ),
			b.TransformVectorWithW( a.Z ),
			b.TransformVectorWithW( a.W )
		);
	}
	...
	Box Matrix::TransformBox( const Box& box ) const
	{
		...
		return Box
		(
			Vector4::Min4( xa, xb ) + Vector4::Min4( ya, yb ) + Vector4::Min4( za, zb ) + W,
	...
	Matrix& Matrix::ModifyProjectionToOblique( Vector4& clippingPlane )
	{
		...
		q.X = (math::Sgn(clippingPlane.X) + matrix[8]) / matrix[0];
		q.Y = (math::Sgn(clippingPlane.Y) + matrix[9]) / matrix[5];
		q.Z = -1.0F;
	...
	Quaternion Matrix::ToQuat() const
	{
		...
		return math::Quaternion( qx, qy, qz, qw ).Normalized();
	}
	...
	Bool Matrix::SLOW_Decompose(const Matrix& m, Vector4& translation, Matrix& rotation, Vector4& scale)
	{
		...
		do 
		{
			...
			for (int i = 0; i < 3; i++) 
			{
				Float n = fabsf(rotation[i][0] - nextRotation[i][0]) +
						  fabsf(rotation[i][1] - nextRotation[i][1]) +
	...
	Bool Matrix::SLOW_Decompose(const Matrix& m, simd::QsTransform& xform)
	{
		///
