...
namespace math
{
	Cylinder::Cylinder( const Vector4& pos1, const Vector4& pos2, Float radius)
	...
	Bool Cylinder::IntersectRay( const Vector4& origin, const Vector4& direction, Float& enterDistFromOrigin ) const
	{
		...
		Float x1;
		Float x2;
		if ( !utils::SolveQuadraticEquation( Vector4::Dot3( t2, t2 ), 2 * Vector4::Dot3( t1, t2 ), Vector4::Dot3( t1, t1 ) - m_positionAndRadius.W * m_positionAndRadius.W, x1, x2 ) || t1 == t2 )
			return false;
	...
	EulerAngles Cylinder::GetOrientation() const
	{
		return EulerAngles(
			...
			RAD2DEG( ::atan2f( m_normalAndHeight.Z, ::sqrtf( m_normalAndHeight.X * m_normalAndHeight.X + m_normalAndHeight.Y * m_normalAndHeight.Y ) ) ),
			0.0f );
	}
