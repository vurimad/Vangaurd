...
namespace math
{
	...
	Vector4 Interpolation<Vector4>::CubicHermite( const Vector4& p0, const Vector4& c0, const Vector4& c1, const Vector4& p1, const red::Float _t )
	{
		Vector4 result;
		RED_ALIGNED_VAR(Float, 16) t[] = { _t,_t,_t,_t };
	...
	Vector4 Interpolation<Vector4>::CubicBezier( const Vector4& p0, const Vector4& c0, const Vector4& c1, const Vector4& p1, const red::Float _t )
	{
		Vector4 result;
		RED_ALIGNED_VAR(Float, 16) t[] = { _t,_t,_t,_t };
	...
	Color Interpolation<Color>::CubicHermite(const Color& p0, const Color& p1, const Color& p2, const Color& p3, const Float _t)
	{
		Vector4 result;
		RED_ALIGNED_VAR(Float, 16) t[] = { _t,_t,_t,_t };
		CubicHermite_SIMD(result.AsFloat(), p0.ToVector().AsFloat(), p1.ToVector().AsFloat(), p2.ToVector().AsFloat(), p3.ToVector().AsFloat(), t);
		return Color( result );
	}
	...
	Color Interpolation<Color>::CubicBezier(const Color& p0, const Color& p1, const Color& p2, const Color& p3, const Float _t)
	{
		Vector4 result;
		RED_ALIGNED_VAR(Float, 16) t[] = { _t,_t,_t,_t };
		CubicBezier_SIMD(result.AsFloat(), p0.ToVector().AsFloat(), p1.ToVector().AsFloat(), p2.ToVector().AsFloat(), p3.ToVector().AsFloat(), t);
		return Color( result );
	}
	...
	void Linear_SIMD( Float out[4], const Float src[4], const Float dst[4], const Float t[4] )
	{
		...
		*(simd::Quad*)out = _mm_add_ps( 
			_mm_mul_ps( *(simd::Quad*)dst, *(simd::Quad*)t ),
			_mm_mul_ps( *(simd::Quad*)src, _mm_sub_ps( _mm_set_ps1(1.f), *(simd::Quad*)t ) )
			);
	}
	...
	void CubicHermite_SIMD( Float out[4], const Float A[4], const Float B[4], const Float C[4], const Float D[4], const Float t[4] )
	{
		...
		const simd::Quad half	  = _mm_set_ps1(0.5f);
		const simd::Quad one_half = _mm_set_ps1(1.5f);
		const simd::Quad neg_a    = _mm_mul_ps( _mm_set_ps1(-1.f),*(simd::Quad*)A );

		const simd::Quad a = _mm_add_ps( 
								_mm_sub_ps( _mm_add_ps( _mm_mul_ps(neg_a,half), _mm_mul_ps(one_half, *(simd::Quad*)B) ), 
								_mm_mul_ps(one_half, *(simd::Quad*)C) ), _mm_mul_ps(half, *(simd::Quad*)D )
							);

		const simd::Quad b = _mm_add_ps( 
								_mm_sub_ps( *(simd::Quad*)A, _mm_mul_ps(_mm_set_ps1(2.5f), *(simd::Quad*)B) ),
								_mm_sub_ps( _mm_mul_ps(_mm_set_ps1(2.0f), *(simd::Quad*)C), _mm_mul_ps(half, *(simd::Quad*)D ) )
							);

		const simd::Quad c = _mm_add_ps(
								_mm_mul_ps(half, neg_a),
								_mm_mul_ps(half, *(simd::Quad*)C)
							);

		const simd::Quad a_t_cubed   = _mm_mul_ps( *(simd::Quad*)t, _mm_mul_ps(*(simd::Quad*)t, _mm_mul_ps( *(simd::Quad*)t, a ) ) );
		const simd::Quad b_t_squared = _mm_mul_ps( *(simd::Quad*)t, _mm_mul_ps(*(simd::Quad*)t, b ) );
		const simd::Quad c_t		 = _mm_mul_ps( *(simd::Quad*)t, c );

		*(simd::Quad*)out = _mm_add_ps( 
								_mm_add_ps( a_t_cubed, b_t_squared ),
								_mm_add_ps( c_t, *(simd::Quad*)B ) 
							);
	}
	...
	void QuadraticBezier_SIMD( Float out[4], const Float p0[4], const Float p1[4], const Float p2[4], const Float t[4] )
	{
		...
		const simd::Quad one_minus_t = _mm_sub_ps( _mm_set_ps1(1.f), *(simd::Quad*)t );

		*(simd::Quad*)out = _mm_add_ps( 
								_mm_add_ps(
									_mm_mul_ps( _mm_mul_ps(one_minus_t, one_minus_t), *(simd::Quad*)p0 ),
									_mm_mul_ps( _mm_mul_ps( _mm_mul_ps( _mm_set_ps1(2.f), *(simd::Quad*)t ), one_minus_t), *(simd::Quad*)p1 )
								),
								_mm_mul_ps( _mm_mul_ps(*(simd::Quad*)t, *(simd::Quad*)t), *(simd::Quad*)p2 )
							);
	}
	...
	void CubicBezier_SIMD( Float out[4], const Float p0[4], const Float p1[4], const Float p2[4], const Float p3[4], const Float t[4] )
	{
		...
		*(simd::Quad*) out = _mm_add_ps( _mm_add_ps( 
									_mm_mul_ps( _mm_mul_ps( _mm_mul_ps(one_minus_t, one_minus_t), one_minus_t), *(simd::Quad*)p0 ), 
									_mm_mul_ps( _mm_mul_ps( _mm_mul_ps( _mm_set_ps1(3.f), *(simd::Quad*)t ), _mm_mul_ps(one_minus_t, one_minus_t) ), *(simd::Quad*)p1 ) 
								),
								_mm_add_ps( 
									_mm_mul_ps( _mm_mul_ps( _mm_mul_ps( _mm_set_ps1(3.f), _mm_mul_ps(*(simd::Quad*)t,*(simd::Quad*)t) ), one_minus_t ), *(simd::Quad*)p2 
									),
									_mm_mul_ps( _mm_mul_ps( _mm_mul_ps(*(simd::Quad*)t,*(simd::Quad*)t), *(simd::Quad*)t), *(simd::Quad*)p3 )
								)
							);
	}
}