...
// #define RED_MEMORY_ENABLE_TLSF_BLOCK_VALIDATION
...
namespace memory
{
	...
	RED_MEMORY_INLINE void TLSFBlockHeader::VerifyBlockState()
	{
...
#ifdef RED_MEMORY_ENABLE_TLSF_BLOCK_VALIDATION
		
		const u64 address =  reinterpret_cast< u64 >( this );
		RED_MEMORY_ASSERT( IsAligned( address, c_tlsfMinimumAlignment ) , "Misaligned block pointer" );

		if( m_previousPhysical != nullptr )
		{
			const u64 previousPhysicalAddress = reinterpret_cast< u64 >( m_previousPhysical );
			RED_MEMORY_ASSERT( previousPhysicalAddress + m_previousPhysical->GetTotalSize() == address, "A block has a bad size! Previous physical block does not point to this block" );
		...
		if( IsFreeBlock() )
		{
			if( m_freeBlockData.nextSegregated )
			{
				RED_MEMORY_ASSERT( m_freeBlockData.nextSegregated->IsFreeBlock(), "Free block linked list is broken (next free block is NOT free)" );
				RED_MEMORY_ASSERT( m_freeBlockData.nextSegregated->GetPreviousFree() == this, "Free block linked list is corrupt (next free's previous is not this)" );
			}

			if( m_freeBlockData.prevSegregated != nullptr )
			{
				RED_MEMORY_ASSERT( m_freeBlockData.prevSegregated->IsFreeBlock(), "Free block linked list is broken (previous free block is NOT free)" );
				RED_MEMORY_ASSERT( m_freeBlockData.prevSegregated->GetNextFree() == this, "Free block linked list is corrupt (prev free's next is not this)" );
			}
		}
#endif
	}
