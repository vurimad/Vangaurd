...
namespace
{
	template< typename It, typename Size, typename Value = typename std::iterator_traits<It>::value_type >
	void UninitializedValueConstruct( It first, Size n )
	{
		for ( It current = first; n > 0 ; ++current, --n )
		{
...
namespace
{
	...
	u64 TLSFAllocator::InitializeBookKeeping( const SystemBlock & block )
	{
		...
		RED_MEMORY_ASSERT( IsAligned( bookKeepingPtr, alignof( FreeListBitmap ) ), "Bad bookkeeping alignment" );
		m_secondLevelBitmap = reinterpret_cast< FreeListBitmap* >( bookKeepingPtr );
		UninitializedValueConstruct( m_secondLevelBitmap, m_firstLevelIndexWidth ); // value initialization will zero-initialize FreeListBitmap(s)
		...
		RED_MEMORY_ASSERT( IsAligned( bookKeepingPtr, alignof( TLSFBlockHeader* ) ), "Bad bookkeeping alignment" );
		m_freeListMatrix = reinterpret_cast< TLSFBlockHeader** >( bookKeepingPtr );
		UninitializedValueConstruct( m_freeListMatrix, m_firstLevelIndexWidth * m_secondLevelIndexWidth ); // value initialization will zero-initialize (nullptr) TLSFBlockHeader pointers
	...
	void TLSFAllocator::Free( Block & block )
	{
		...
		if( block.address )
		{
			RED_MEMORY_ASSERT( OwnBlock( block.address ), "TLSF do not own provided memory block." );
	...
	Block TLSFAllocator::AllocateAligned( u32 size, u32 alignment )
	{
		...
		size = red::AlignUp( size, ( Uint32 )c_tlsfBlockHeaderAlignment );
	...
	{
		return ReallocateAligned( block, size, c_tlsfDefaultAlignment );
	}

	Block TLSFAllocator::ReallocateAligned( Block & block, u32 size, u32 alignment )
	{
		if( block.address == 0 )
		{
			return size != 0 ? AllocateAligned( size, alignment ) : NullBlock();
		}
		...
		{
			ShrinkBlock( blockHeader, size, alignment );
	...
	TLSFBlockHeader * TLSFAllocator::FindFreeBlock( u32 size )
	{
		...
		if( secondLevelBitmap == 0 )	// No free blocks big enough in this list, search for the next largest free list
		{
			...
			if( firstLevelTopBits == 0 )
			{
				...
				// For big enough blocks try to use brute force search.
				if( size > c_tlsfSmallestAllocSizeToEnableBruteForceSearch )
				{
	...
	void TLSFAllocator::BuildMetrics( TLSFAllocatorMetrics & metrics )
	{
		...
		for( u32 firstLevelIndex = 0; firstLevelIndex != m_firstLevelIndexWidth; ++firstLevelIndex )
		{
			...
			freeListSize += freeBlock.totalSize;
		}

		metrics.metrics.consumedMemoryBytes = metrics.metrics.consumedSystemMemoryBytes - m_bookKeepingSize - freeListSize;
		metrics.metrics.bookKeepingBytes = m_bookKeepingSize;
		metrics.metrics.largestBlockSize = largestBlockSize;
		metrics.metrics.smallestBlockSize = smallestBlockSize;
	}

	void TLSFAllocator::SerializeMetrics( Serializer & serializer )
	{
	...
	u64 TLSFAllocator::GetBiggestBlockSize() const
	{
		const u32 firstLevel = BitUtils::BitScanReverse( m_firstLevelBitmap );
		const u32 secondLevel = BitUtils::BitScanReverse( m_secondLevelBitmap[ firstLevel ] );

		u64 minBlockSize = 1ull << ( firstLevel + c_tlsfSmallestSegregatedSizeLog2 - 1 );
		u64 maxBlockSize = minBlockSize << 1;
			
		const u64 blockSize = minBlockSize + ( ( maxBlockSize - minBlockSize ) / m_secondLevelIndexWidth ) * secondLevel;
