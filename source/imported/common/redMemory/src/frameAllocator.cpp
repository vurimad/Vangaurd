...
namespace
{
	...
	static_assert( __alignof( FrameAllocator ) == 64, "FrameAllocator is incorrectly aligned." );
	static_assert( sizeof( FrameAllocator ) <= 64, "FrameAllocator should fit in one cache line." );

	static_assert( c_frameAllocatorHeaderAlignment == FrameAllocator::DefaultAlignmentType::value, "FrameAllocator Header alignment must be equal than allocator default alignment" );

	FrameAllocator::FrameAllocator()
		: m_currentPositionInFrameBlock( 0 )
	...
	void FrameAllocator::Initialize( const FrameAllocatorParameter & parameter )
	{
		...
		m_currentPositionInFrameBlock = commitedBlock.address;
	}

	void FrameAllocator::Uninitialize()
	{
	...
	Block FrameAllocator::Allocate( u32 size )
	{
		RED_MEMORY_ASSERT( IsInitialized(), "Allocator is not initialized." );

		size = RoundUp( std::max( 1u, size ), FrameAllocator::DefaultAlignmentType::value );
		const u64 sizeWithHeader = size + c_frameAllocatorHeaderSize;
		
		u64 address = m_currentPositionInFrameBlock;
		if( address + sizeWithHeader < m_nextFrameBlockAddress )
		{
			m_currentPositionInFrameBlock += sizeWithHeader;
	...
	Block FrameAllocator::AllocateAligned( u32 size, u32 alignment )
	{
		if( alignment > FrameAllocator::DefaultAlignmentType::value )
		{
			RED_MEMORY_ASSERT( IsInitialized(), "Allocator is not initialized." );
			RED_MEMORY_ASSERT( IsPowerOf2( alignment ), "Alignment must be power of two." );
			RED_MEMORY_ASSERT( alignment > FrameAllocator::DefaultAlignmentType::value, "Alignement request is smaller than default one, 8. Call Allocate instead." );
			...
			{
				m_currentPositionInFrameBlock += sizeIncludingAlignmentPad;
	...
	Block FrameAllocator::Reallocate( Block & block, u32 size )
	{
		return ReallocateAligned( block, size, DefaultAlignmentType::value );
	}
	
	Block FrameAllocator::ReallocateAligned( Block & block, u32 size, u32 alignment )
	{
		if( size == 0 )
		{
			Free( block );
	...
	Block FrameAllocator::GrowBlock( const Block & block, u32 size )
	{
		...
		u64 position = m_currentPositionInFrameBlock;
		...
		if( position == block.address + block.size )
		{
			...
			m_currentPositionInFrameBlock = newPosition;
	...
	void FrameAllocator::Free( Block & block )
	{
		if( block.address )
		{
			...
			const u64 position = m_currentPositionInFrameBlock;
			if ( position == block.address + block.size )
			{
				// freeing in LIFO order, try to move back the current position
				m_currentPositionInFrameBlock = block.address - c_frameAllocatorHeaderSize;
			}
		}
	}

	bool FrameAllocator::OwnBlock( u64 block ) const
	{
		RED_MEMORY_ASSERT( IsInitialized(), "FrameAllocator was not initialized." );
		return block - m_virtualRange.start < GetVirtualRangeSize( m_virtualRange ); 
	}
	
	u64 FrameAllocator::GetBlockSize( u64 address ) const
	{
		FrameAllocatorHeader * header =  reinterpret_cast< FrameAllocatorHeader * >( address - c_frameAllocatorHeaderSize );
		RED_MEMORY_ASSERT( header->marker == c_frameAllocatorHeaderAllocateMarker, "Block was already deleted, or underrun occured." );	
		return header->size;
	...
	void FrameAllocator::Reset()
	{
		...
		m_currentPositionInFrameBlock = newAddress;
		m_nextFrameBlockAddress = newAddress + m_frameBlockSize;
#endif
	}

	void FrameAllocator::BuildMetrics( FrameAllocatorMetrics& metrics )
	{
		...
		if ( m_systemAllocator )
		{
			...
			const u64 currentPositionInFrameBlock = m_currentPositionInFrameBlock;
	...
	void FrameAllocator::SerializeMetrics( Serializer& serializer )
	{
	...
	bool FrameAllocator::IsInitialized() const
	{
		return m_systemAllocator != nullptr;
	}

	u32 FrameAllocator::Internal_GetNumberOfFrames() const
	{
		return m_numberOfFrames;
	}

	u64 FrameAllocator::Internal_GetCurrentPositionInFrameBlock() const
	{
		return m_currentPositionInFrameBlock;
	}

	const VirtualRange& FrameAllocator::Internal_GetVirtualRange() const
	{
