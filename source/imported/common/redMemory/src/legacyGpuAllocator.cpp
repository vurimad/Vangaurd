...
namespace Utils
{
		...
		while( currentNode != nullptr )
		{
			...
			else if( bytesToMoveAccumulated > 0 )
			{
				...
				else
				{
					...
					u64 alignedAddress = requiredAlignment == 0 ? newBaseAddress : red::AlignDown( newBaseAddress, requiredAlignment );		// Align down (i.e. may need end-padding)
		...
		while( currentNode != nullptr )
		{
			...
			else if( bytesToMoveAccumulated > 0 )
			{
				...
				else
				{
					...
					u64 requiredAlignment = currentNode->GetAlignment();
					u64 newBaseAddress = currentNode->GetAddress() - bytesToMoveAccumulated;
					u64 alignedAddress = requiredAlignment == 0 ? newBaseAddress : red::AlignUp( newBaseAddress, requiredAlignment );
	...
	void LegacyGpuAllocator::RequestDefragmentation( DefragMoveRequestCallback moveRequestCb, void* userData, DefragMode defragMode, u32 maxMemoryToMove, u32 smallestGapToFill )
	{
		m_syncLock.Acquire();		// Lock so nobody else can try to use this pool during defrag
		Int32 defragScope = m_fragmentationScope++;

		RED_MEMORY_ASSERT( defragScope == 0, "Recursive defrag, or attempting to run multiple defrags at once" );
		if( defragScope == 0 )
		{
			u32 bytesMoved = 0;
			if( defragMode == DefragAll || defragMode == DefragShortLivedRegions )
			{
				bytesMoved += DefragmentBottomRegions( maxMemoryToMove, moveRequestCb, userData, smallestGapToFill );
			}
			if( bytesMoved < maxMemoryToMove && ( defragMode == DefragAll || defragMode == DefragLongLivedRegions ) )
			{
				bytesMoved += DefragmentTopRegions( maxMemoryToMove - bytesMoved, moveRequestCb, userData, smallestGapToFill );
			}
	...
	{
		ScopedLock< Mutex > lock( m_syncLock );
		m_freeRegionHead = nullptr;
		m_freeRegionTail = nullptr;
		m_bottomPhysicalHead = nullptr;
		m_bottomPhysicalTail = nullptr;
	...
	bool LegacyGpuAllocator::NodeCanFitRequest( RegionNode* node, u64 size, u64 alignment )
	{
		if( node->GetSize() >= size )	// Region is big enough?
		{
			...
			u64 alignedUpAddr = red::AlignUp( baseAddr, alignment );
	...
	{
		RED_MEMORY_ASSERT( m_freeRegionHead, "Free region pool is full! Give this pool more CPU budget" );
	...
	LegacyGpuAllocator::RegionNode* LegacyGpuAllocator::PrepareAllocatedNode( RegionNode*& listHead, RegionNode*& listTail, RegionNode* theNode,  u64 size, u64 alignment )
	{
		...
		u64 alignedAddress = red::AlignUp( baseAddress, alignment );
	...
	RegionHandle LegacyGpuAllocator::AllocateRegionBottom( u64 size, u64 alignment )
	{
		...
		RED_MEMORY_ASSERT( ( foundNode->GetAddress() & (alignment-1) ) == 0, "Misaligned region" );
	...
	RegionHandle LegacyGpuAllocator::AllocateRegionTop( u64 size, u64 alignment )
	{
		...
		if( !foundNode )
		{
			...
			u64 alignedAddress = red::AlignUp( lowestAddress, alignment );
	...
	{
		ScopedLock< Mutex > lock( m_syncLock );
		RED_MEMORY_ASSERT( m_fragmentationScope == 0, "Allocate during defrag" );
	...
	{
		ScopedLock< Mutex > lock( m_syncLock );
		RED_MEMORY_ASSERT( m_fragmentationScope == 0, "Free during defrag" );
	...
	RegionHandle LegacyGpuAllocator::SplitRegion( RegionHandle baseRegion, u64 splitPosition, u64 splitBlockAlignment )
	{
...
#ifdef ENABLE_REGION_LOCK_PROTECTION	
		RED_MEMORY_ASSERT( theNode->IsLocked(), "Only locked regions may be split" );
#endif

		// the split block alignment must be correct upon calling thing (its too dangerous to do manually)
		RED_MEMORY_ASSERT( ( ( theNode->GetAddress() + splitPosition ) & ( splitBlockAlignment - 1 ) ) == 0, "Split offset would result in misaligned region" );

		ScopedLock< Mutex > lock( m_syncLock );
		RED_MEMORY_ASSERT( m_fragmentationScope == 0, "Split during defrag" );
		...
		RED_MEMORY_ASSERT( splitRegion != nullptr, "Splitting the memory block failed" );
	...
	Block LegacyGpuAllocator::Allocate( u32 /*size*/ )
	{
		RED_MEMORY_ASSERT( 0, "Not Supported." );
		return NullBlock();
	}
	
	Block LegacyGpuAllocator::AllocateAligned( u32 /*size*/, u32 /*alignment*/ )
	{
		RED_MEMORY_ASSERT( 0, "Not Supported." );
		return NullBlock();
	}
	
	Block LegacyGpuAllocator::Reallocate( Block & /*block*/, u32 /*size*/ )
	{
		RED_MEMORY_ASSERT( 0, "Not Supported." );
		return NullBlock();
	}

	Block LegacyGpuAllocator::ReallocateAligned( Block & /*block*/, u32 /*size*/, u32 /*alignment*/ )
	{
		RED_MEMORY_ASSERT( 0, "Not Supported." );
		return NullBlock();
	}

	void LegacyGpuAllocator::Free( Block & /*block*/ )
	{
		RED_MEMORY_ASSERT( 0, "Not Supported." );
	}
	
	u64 LegacyGpuAllocator::GetBlockSize( u64 /*address*/ ) const
	{
		RED_MEMORY_ASSERT( 0, "Not Supported." );
		return 0;
	}
