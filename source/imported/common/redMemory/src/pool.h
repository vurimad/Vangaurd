...
namespace memory
{
	...
	public:
		...
		RED_MOCKABLE Block ReallocateAligned( Block & block, u32 size, u32 alignment ) const;
		RED_MOCKABLE void Free( Block & block ) const;
		u64 GetBlockSize( u64 address ) const;
		red::memory::PoolHandle GetHandle() const;
	...
	private:
		...
		virtual Block OnReallocateAligned( Block & block, u32 size, u32 alignment ) const = 0;
		virtual void OnFree( Block & block ) const = 0;
		virtual u64 OnGetBlockSize( u64 address ) const = 0;
		virtual red::memory::PoolHandle OnGetHandle() const = 0;
	};
}
}
