...
namespace memory
{
	void ValidateInitializationParameter( const DynamicFixedSizeAllocatorParameter & parameter  )
	{
		...
		RED_MEMORY_ASSERT( parameter.initialBlockCount <= parameter.maxBlockCount, "Initial block count can't be bigger than max Block Count ." );
	...
	void DynamicFixedSizeAllocator::Initialize( const DynamicFixedSizeAllocatorParameter & parameter )
	{
		...
		const u64 rangeSize = RoundUp( parameter.blockSize + 8, parameter.blockAlignment ) * parameter.maxBlockCount;
#else
		const u64 rangeSize = RoundUp( parameter.blockSize, parameter.blockAlignment ) * parameter.maxBlockCount;
#endif
		...
		LocklessFixedSizeAllocatorParameter fixedSizeParam = 
		{
			parameter.blockSize,
	...
	void DynamicFixedSizeAllocator::Uninitialize()
	{
		...
		{
			RED_LOG_CATEGORY_WARNING( red::LoggerCategory_Engine, "Uninitialize() called on a DynamicFixedSizeAllocator that was not initialized properly." );
		}
	}

	Block DynamicFixedSizeAllocator::Allocate( u32 size )
	{
		RED_MEMORY_ASSERT( IsInitialized(), "DynamicFixedSizeAllocator is not initialized." );
	...
	Block DynamicFixedSizeAllocator::ReallocateAligned( Block & block, u32 size, u32 alignment )
	{
		RED_MEMORY_ASSERT( IsInitialized(), "DynamicFixedSizeAllocator is not initialized." );

		Block result = m_allocator.ReallocateAligned( block, size, alignment );
	...
	u64 DynamicFixedSizeAllocator::GetBlockSize() const
	{
		return m_allocator.GetBlockSize();
	}

	void DynamicFixedSizeAllocator::BuildMetrics( FixedSizeAllocatorMetrics & metrics )
	{
		m_allocator.BuildMetrics( metrics );
	}

	void DynamicFixedSizeAllocator::SerializeMetrics( Serializer & serializer )
	{
		SerializeAllocatorIdentifiers( this, serializer );
		m_allocator.SerializeMetrics( serializer );
	}

	bool DynamicFixedSizeAllocator::IsInitialized() const
	{
