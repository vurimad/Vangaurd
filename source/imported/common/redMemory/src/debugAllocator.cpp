...
namespace 
{
	...
	{
		if ( alignment < c_debugAllocatorDefaultAlignment )
		{
			alignment = c_debugAllocatorDefaultAlignment;
		}
	...
	{
		return ReallocateAligned( block, size, c_debugAllocatorDefaultAlignment );
	}

	Block DebugAllocator::ReallocateAligned( Block & block, u32 size, u32 alignment )
	{
		if ( alignment < c_debugAllocatorDefaultAlignment )
		{
			alignment = c_debugAllocatorDefaultAlignment;
	...
	bool DebugAllocator::OwnBlock( u64 block ) const
	{
		...
		// but DebugAllocator bybass all allocator so in theory, it's true.
		if( block )
		{
			return true;
		}
