...
namespace memory
{
	...
	void HookHandler::ProcessPreHooks( HookPreParameter & param, u32 disabledHooks )
	{
		if( m_rootHook )
		{
			m_rootHook->PreAllocation( param, disabledHooks );	
		}
	}

	void HookHandler::ProcessPostHooks( HookPostParameter & param, u32 disabledHooks )
	{
		if( m_rootHook )
		{
			m_rootHook->PostAllocation( param, disabledHooks );
		}
	}
