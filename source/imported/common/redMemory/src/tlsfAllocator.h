...
namespace memory
{
	...
	struct TLSFAllocatorMetrics
	{
		...
		SimpleArray< FreeBlockMetric, 32 > freeBlocksMetrics;
	};
	...
	private:
		...
		u64 m_bookKeepingSize;
	};

	static_assert( sizeof( TLSFAllocator ) == 64, "TLSFAllocator should fit in one cache line." );

	u32 RED_MEMORY_API ComputeTLSFBookKeepingSize( u32 maxAllocatedSize );
}
}
