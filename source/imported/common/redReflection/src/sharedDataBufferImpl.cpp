...
{
	return red::TSingleton< SharedDataBufferCache >::GetInstance();
}
	...
	, m_hash( hash )
{
	...
	m_data = RED_ALLOCATE( red::PoolEngine, sourceSize );
	red::Memcpy( m_data, sourceData, sourceSize );
}
...
SharedDataBufferData* SharedDataBufferCache::Request( const void* sourceData, const Uint32 sourceDataSize )
{
...

	{
		...
		newToken.m_data = RED_NEW( SharedDataBufferData )( sourceData, sourceDataSize, hash );
