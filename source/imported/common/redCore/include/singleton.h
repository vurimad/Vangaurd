...
namespace red
{
	const Uint32 DEFAULT_SINGLETON_ALIGNMENT = 16;
	...
	public:
		...
		CTrackerArray()
			: m_elements(0)
		{
			red::Memset(m_trackers, 0, sizeof(m_trackers));
		}
	};
	...
	void SetLongevity(T* pDynObject, Uint32 longevity, Destroyer d)
	{
		...
		void* ptr = PolicyMemoryAllocate( sizeof(tLifetimeTracker), DEFAULT_SINGLETON_ALIGNMENT );

		CLifetimeTracker* p = new (ptr) tLifetimeTracker(pDynObject, longevity, d);

		if (GTrackerArray == NULL)
		{
			void* ptr = PolicyMemoryAllocate( sizeof(CTrackerArray), DEFAULT_SINGLETON_ALIGNMENT );
	...
	struct TCreateUsingMalloc
	{
		static T* Create()
		{
			void* p = PolicyMemoryAllocate( sizeof(T), DEFAULT_SINGLETON_ALIGNMENT );
			if (!p) return 0;
			return new(p) T;
		}
