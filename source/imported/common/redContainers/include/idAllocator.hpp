...
template< Uint32 MaxObjects >
Uint32 IDAllocator< MaxObjects >::GetNextUsed( const Uint32 id ) const
{
	Uint32 next = m_freeIndices.FindNextClear( id + 1 );
	if ( next == MaxObjects )
	{
		return c_invalidId;
	}
...
template< Uint32 MaxObjects >
Uint32 IDAllocator< MaxObjects >::GetFirstUsed() const
{
    Uint32 next = m_freeIndices.FindNextClear( 0 );
	if ( next == MaxObjects )
	{
		return c_invalidId;
	}
