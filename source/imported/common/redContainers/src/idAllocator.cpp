...
{
	if ( maxObjects > m_freeIndices.Size() )
	{
		const Uint32 prevSize = m_freeIndices.Size();
		m_freeIndices.Resize( maxObjects );

		// set the new entries to 1
		const Uint32 curSize = m_freeIndices.Size();
...
{
	// try fast allocation
	const Uint32 maxIndex = m_freeIndices.Size();
...
{
	RED_FATAL_ASSERT( id < m_freeIndices.Size(), "Invalid ID" );
