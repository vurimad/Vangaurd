// Publicate 0 to v:\devel\projects\oodle2\include\oodle2x.h
...
{
    OodleXPriority_Never = -1, // special invalid priority level
    OodleXPriority_Background = 0, // only runs if no Normal or higher items exist
    OodleXPriority_Normal = 1, // default priority
    OodleXPriority_High   = 2, // user high priority
    OodleXPriority_Critical = 3, // generally not for client use; blocking items are critical
    OodleXPriority_Count = 4,
    OodleXPriority_Force32 = 0x40000000
} OodleXPriority;
/* Priority for async tasks.

    Setting priorities allows you to make certain tasks get run before others.
    
    It only schedules operations within a given subsystem.  eg. it orders IOQ file operation requests against other IOQ requests,
    but does not order those against the Worker threads.  Prioritizing of unrelated systems is controlled by the operating system
    thread priorities.
    
    OodleXPriority is a strict priority system; that is, no lower priority operations are done until all
    higher priority tasks are done.
    
    Tasks are generally FIFO within their priority group.  So, eg. all Critical tasks will be done in a FIFO order, then all
    High tasks, etc.
*/
...
#if defined(__RADLINUX__) || defined(__RADMAC__) || defined(__RADANDROID__) || defined(__RADIPHONE__) || defined(__RADNX__)
    #define OODLEX_WCHAR_SIZE       4
#elif defined(__RADWIN__) || defined(__RADPS4__) || defined(__RADXBOXONE__)
    #define OODLEX_WCHAR_SIZE       2
#else
...
typedef RADSTRUCT OodleXFileInfo
{
    U32     flags;      // logical or of $OODLEX_FILEINFO_FLAGS
    U32     pad;
    S64     size;       // file size; $OODLEX_FILE_SIZE_INVALID if unknown
    U64     modTime;    // modTime on different platforms doesn't necessarilly mean anything, but it should be comparable with integer < and == (on the same platform, not vs. other platforms)
} OodleXFileInfo;
...
#define OODLEX_FILEINFO_FLAG_INVALID    ((U32)-1)  /* Invalid value for $(OodleXFileInfo:flags) */

#define OODLEX_FILEINFO_MODTIME_INVALID ((U64)-1)  /* Invalid value for $(OodleXFileInfo:modTime) */

#define OODLEX_FILE_SIZE_INVALID    ((S64)-1)   /* Unknown or failure retreiving file size */
...
RADINLINE void OodleXFileInfo_Reset( OodleXFileInfo * info )
{
    info->flags = 0;
...
RR_COMPILER_ASSERT( sizeof(OodleXError) == 4 );
...
RR_COMPILER_ASSERT( sizeof(OodleXFileMode) == 4 );
...
typedef U32 OodleXLosslessFilterCode;
...
RR_COMPILER_ASSERT( sizeof(OodleXStatus) == 4 );
...
All Paths that reach low level Oodle are expected to be UTF-8
A special case of UTF-8 is plain ASCII, so if your names are ASCII that's fine.

The path combiner functions and such here generally take char *
They work on ascii and UTF-8

WARNING : Windows "A" and "OEM" encoding is *NOT* ascii or UTF-8 !! You must convert those !!

The handy thing about UTF8 is that all ascii chars are still their ascii values
in particular you can still just strchr for '/' to find paths and you can still
just strcat to put paths together

what you *can't* do is just use "++' and "--" to step a char
use "StepUTF8"

*********/

// OODLEX_PLATFORM_UNIXLIKE_PATHS means prefer forward slashes 
//      and the path "/" is root
#if defined(__RADWIN__) || defined(__RADXBOXONE__)
#define OODLEX_PLATFORM_UNIXLIKE_PATHS  0
#define OODLEX_PLATFORM_CASE_SENSITIVE 0
#elif defined(__RADLINUX__) || defined(__RADPS4__) || defined(__RADANDROID__)
#define OODLEX_PLATFORM_UNIXLIKE_PATHS  1
#define OODLEX_PLATFORM_CASE_SENSITIVE 1
#elif defined(__RADNX__) || defined(__RADMAC__)
// NX is "unixlike" (forward slashes) but not case sensitive
#define OODLEX_PLATFORM_UNIXLIKE_PATHS  1
#define OODLEX_PLATFORM_CASE_SENSITIVE 0
#else
#error need path type
#endif

#define OODLEX_PATH_DELIM "/\\" /* OODLEX_PATH_DELIM is either forward slash or back slash, whichever is preferred for the current OS.

    Oodle IO functions are path-delim agnostic.  You only need this if you are using non-Oodle OS IO functions.

*/

#undef OODLEX_PATH_DELIM

#if OODLEX_PLATFORM_UNIXLIKE_PATHS
#define OODLEX_PATH_DELIM       '/'
#define OODLEX_PATH_DELIM_STR   "/"
#define OODLEX_PATH_DELIM_BAD   '\\'
#else
#define OODLEX_PATH_DELIM       '\\'
#define OODLEX_PATH_DELIM_STR   "\\"
#define OODLEX_PATH_DELIM_BAD   '/'
...
typedef RADSTRUCT OodleXMallocVTable
{
    void * m_context;   // provided context pointer will be passed to the function pointers

    void * (OODLE_CALLBACK *m_pMalloc)( void * context, SINTa bytes ); // must return OODLE_MALLOC_MINIMUM_ALIGNMENT aligned memory
    void * (OODLE_CALLBACK *m_pMallocAligned)( void * context, SINTa bytes , S32 alignment ); // alignment will always be power of two
    void   (OODLE_CALLBACK *m_pFree)(void * context, void * ptr); // must be able to free pointers from m_pMalloc or m_pMallocAligned
    void   (OODLE_CALLBACK *m_pFreeSized)(void * context, void * ptr, SINTa bytes); // use size to make free faster
    
    S32 m_bigAlignment; // indicates the alignment provided by MallocBig ; must be a multiple of $OODLEX_IO_MAX_ALIGNMENT for OodleX
    void * (OODLE_CALLBACK *m_pMallocBig)(void * context, SINTa bytes); // must return memory aligned to m_bigAlignment
    void   (OODLE_CALLBACK *m_pFreeBig)  (void * context, void * ptr); // free a pointer allocated by m_pMallocBig
    
    rrbool (OODLE_CALLBACK *m_pValidatePointer)(void * context, void * ptr, SINTa bytes); // check on an allocation
...
OOFUNC1 OodleXStatus OOFUNC2 OodleX_GetStatus( OodleXHandle h , OodleXHandleDeleteIfDone deleteIfDone RADDEFAULT(OodleXHandleDeleteIfDone_No));
...
OOFUNC1 OodleXStatus OOFUNC2 OodleX_Wait(  OodleXHandle h , OodleXHandleDeleteIfDone deleteIfDone RADDEFAULT(OodleXHandleDeleteIfDone_No));
...
OOFUNC1 OodleXStatus OOFUNC2 OodleX_WaitAll(const OodleXHandle * handles, S32 count , OodleXHandleDeleteIfDone deleteIfDone RADDEFAULT(OodleXHandleDeleteIfDone_No));
...
OOFUNC1 OodleXStatus OOFUNC2 OodleX_CancelOrWait_AndDelete( OodleXHandle h );
/* Request cancellation of an async operation.  May block the calling thread.

  $:h       OodleXHandle weak reference
  $:return  OodleXStatus of the operation at time of cancellation
  
  If possible, cancel the handle before it is run.  If not possible, block until it is done.

  The handle is always deleted after this call.
  
  Cancel is an attempt, it is not guaranteed to stop the operation from running to completion;
  it may not be possible to cancel the handle without running it.
  Generally Cancel should only be used in error resolution.

  If OodleXStatus_Invalid is returned, the op was deleted before the Cancel.
  If OodleXStatus_Done or Error is returned, the op completed before the Cancel.
  If OodleXStatus_Pending is returned, the completion state of the op is *undefined*.
  It may have been cancelled before being started, or it may have been started and
  partially completed when it was cancelled. 
*/
...
OOFUNC1 void OOFUNC2 OodleX_WaitCancelAllPending();
/* Cancel all pending, or wait done if cancel is impossible

    Only for error exits!
*/
...
OOFUNC1 U32 OOFUNC2 OodleX_GetAvailableAsyncSelect();
...
#define OODLEX_ASYNC_SYSTEM_MASK            (((U64)7)<<OODLEX_ASYNC_SYSTEM_SHIFT)
...
OOFUNC1 U32 OOFUNC2 OodleX_GetExtensionKey(const char * filename);

OOFUNC1 U32 OOFUNC2 OodleX_MakeExtensionKey(const char * extension);
...
typedef RADSTRUCT OodleXInitOptions
{
    //---------------------------------
    // Phase 1:
    
    const OodleXMallocVTable * m_pBaseVTable;  // vtable for OodleMalloc to use [OodleXMalloc_GetVTable_OS]
    rrbool m_OodleInit_DebugAllocator; // option : put a debug allocator layer on top of m_pBaseVTable [false]

    S32 m_num_handles_log2; // log2 number of the number of handles for the OodleXHandleTable [10]

    rrbool m_OodleInit_ThreadLog; // option : enable the ThreadLog [true]
    rrbool m_OodleInit_Log;  // option : enable the Log [true]
    rrbool m_OodleInit_Log_Header;  // option : write a header to the Log at startup [true]
    const char * m_OodleInit_Log_FileName; // set the log file name (NULL for default, which is described in $Oodle_About_Platforms)
    rrbool m_OodleInit_Log_FlushEachWrite; // option : flush the log after each write [false]
    
    //-----------------------------------
    // Phase 2:

    rrbool m_OodleInit_BreakOnLogError; // option : debug break when Oodle logs an error
    rrbool m_OodleInit_Telemetry;       // option : make the Telemetry connection for tracking Oodle [false]
    void * m_OodleInit_Telemetry_Context; // the telemetry context, NULL means I will make it (if m_OodleInit_Telemetry is true)
    rrbool m_OodleInit_StackTrace;      // option : enable stack tracing in Oodle [true]
    rrbool m_OodleInit_LeakTrack;       // option : enable LeakTrack in Oodle
    rrbool m_OodleInit_SimpleProf;      // option : enable simple profiler (this is mainly for me)
    rrbool m_OodleInit_FuzzTest;        // deprecated , does nothing
    t_OodleFPVoidVoid * m_OodleInit_ThreadProfiler_funcptr; // option : enable the thread profiler
    rrbool m_OodleInit_IOQ;             // option : enable the IOQ
    rrbool m_OodleInit_IOQ_Log;         // option : enable logging operations on the IOQ
    rrbool m_OodleInit_IOQ_BreakOnError;    // option : make the IOQ issue a debug break on any error (for debugging)
    rrbool m_OodleInit_IOQ_Threaded;        // option : enable threading on IOQ (turn off for debugging)
    rrbool m_OodleInit_IOQ_CheckAlignment;  // option : should IOQ check alignment of parameters?
    //rrbool m_OodleInit_SPU;               // option : enable Oodle on the SPU (does not load the SPU ELF, you do that later)
    //rrbool m_OodleInit_SPU_Synchronous; // option : for debugging, run SPU ops synchronously
    rrbool m_OodleInit_Workers;         // option : enable the worker thread system
    S32 m_OodleInit_Workers_Count;      // number of worker threads to start (<0 for default)
...
OOFUNC1 rrbool OOFUNC2 OodleX_Init_GetDefaults(U32 oodle_header_version,OodleXInitOptions * pOptions,
    OodleX_Init_GetDefaults_DebugSystems debugSystems RADDEFAULT(OodleX_Init_GetDefaults_DebugSystems_Yes),
    OodleX_Init_GetDefaults_Threads threads RADDEFAULT(OodleX_Init_GetDefaults_Threads_Yes));
...
OOFUNC1 rrbool OOFUNC2 OodleX_Init_GetDefaults_Minimal(U32 oodle_header_version,OodleXInitOptions * pOptions);
...
OOFUNC1 rrbool OOFUNC2 OodleX_Init(U32 oodle_header_version,const OodleXInitOptions * pOptions);
/* Initialize Oodle
...
OOFUNC1 rrbool OOFUNC2 OodleX_Init_Default(U32 oodle_header_version,
    OodleX_Init_GetDefaults_DebugSystems debugSystems RADDEFAULT(OodleX_Init_GetDefaults_DebugSystems_Yes),
    OodleX_Init_GetDefaults_Threads threads RADDEFAULT(OodleX_Init_GetDefaults_Threads_Yes));
...
// pub but no doc :
OOFUNC1 rrbool OOFUNC2 OodleX_Init_Phase1(U32 oodle_header_version,const OodleXInitOptions * pOptions);
...
OOFUNC1 void OOFUNC2 OodleX_Shutdown(const char * threadProfileLogName RADDEFAULT(NULL), 
                            OodleX_Shutdown_LogLeaks logLeaks RADDEFAULT(OodleX_Shutdown_LogLeaks_Yes),
                            U64 allocStartCounter RADDEFAULT(0),
                            OodleX_Shutdown_DebugBreakOnLeaks debugBreakOnLeaks RADDEFAULT(OodleX_Shutdown_DebugBreakOnLeaks_No));
...
OOFUNC1 rrbool OOFUNC2 OodleX_Init_NoThreads(U32 oodle_header_version,const OodleXInitOptions * pOptions);
...
OOFUNC1 void OOFUNC2 OodleX_Shutdown_NoThreads(const char * threadProfileLogName RADDEFAULT(NULL), 
                            OodleX_Shutdown_LogLeaks logLeaks RADDEFAULT(OodleX_Shutdown_LogLeaks_Yes),
                            U64 allocStartCounter RADDEFAULT(0),
                            OodleX_Shutdown_DebugBreakOnLeaks debugBreakOnLeaks RADDEFAULT(OodleX_Shutdown_DebugBreakOnLeaks_No));
/* Shut down Oodle at app exit time.
...
OOFUNC1 OodleXStatus OOFUNC2 OodleXIOQ_GetStatus(OodleXHandle req,OodleXHandleDeleteIfDone andDeleteIfDone,U32 * pErrorCode RADDEFAULT(NULL), S32 * pReturnValue RADDEFAULT(NULL));
...
OOFUNC1 rrbool OOFUNC2 OodleXIOQ_GetErrorDetails(U32 code,OodleXIOQFile file,char * pMessage,int messageSize);
...
OOFUNC1 OodleXError OOFUNC2 OodleXIOQ_GetErrorEnum(U32 code,OodleXIOQFile file);
/* Convert an OS error code into a text message
...
OOFUNC1 void OOFUNC2 OodleXIOQ_LogError(U32 code,OodleXIOQFile file,const char * pName RADDEFAULT(NULL));
...
OOFUNC1 OodleXStatus OOFUNC2 OodleXIOQ_Wait(OodleXHandle req,OodleXHandleDeleteIfDone andDelete,U32 * pErrorCode RADDEFAULT(NULL));
...
OOFUNC1 rrbool OOFUNC2 OodleXIOQ_GetInfo(OodleXIOQFile file,OodleXFileInfo * pInto, S32 * pAlignmentRequired RADDEFAULT(NULL));
...
// OodleXIOQ_GetInfo will fail & return false if the open request is not yet done
OOFUNC1 rrbool OOFUNC2 OodleXIOQ_Wait_GetInfo(OodleXIOQFile file,OodleXFileInfo * pInto, S32 * pAlignmentRequired RADDEFAULT(NULL));
...
OOFUNC1 rrbool OOFUNC2 OodleXIOQ_GetName(OodleXIOQFile file,char * pInto,S32 intoSize);
...
OOFUNC1 U32 OOFUNC2 OodleXIOQ_GetLastError(OodleXIOQFile file);
...
OOFUNC1 rrbool OOFUNC2 OodleXIOQ_LogLastError(OodleXIOQFile file);
...
*/
//---------------------------------------------------------------------------------
// Hard core thread stomping :

// Pause the thread (pauses all requests and MediaBuffers)
//  note : you can really fuck yourself up by calling this, if you pause the thread and
//  then Block on a request, it will never get done and you will deadlock yourself
// (note the pause is not completely immediate, the thread needs a tick to process the request)
 // true = pause, false = resume
OOFUNC1 void OOFUNC2 OodleXIOQ_Pause( rrbool pause );
/* Pause or resume the IO thread

    $:pause true to pause the thread, false to resume
    
    Pause may not be immediate; IO ops in flight will complete.  No IO requests
    or IOQStreams will be serviced when the thread is paused.
    Blocking on IO ops will be a deadlock when the thread is paused.
    
*/

OOFUNC1 void OOFUNC2 OodleXIOQ_WaitCancelAllPending( );
/*  Cancel all pending operations

    This might be useful for error exits, for example.

    Generally you would call $OodleX_WaitCancelAllPending instead.
*/
...
//  (fileRef is optional)
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_Fence_Async(OodleXIOQFile fileRef RADDEFAULT(0),OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_OpenForRead_Async(OodleXIOQFile * pFile,const char *name, 
                        OodleXFileOpenFlags fileOpenFlags RADDEFAULT(OodleXFileOpenFlags_Default),
                        const OodleXFileOpsVTable * vtable RADDEFAULT(NULL),OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_OpenAndRead_Async(OodleXIOQFile * pFile,const char *name, 
                        void * initialReadMemory, SINTa initialReadSize, S64 initialReadPos RADDEFAULT(0),
                        OodleXFileOpenFlags fileOpenFlags RADDEFAULT(OodleXFileOpenFlags_Default),
                        const OodleXFileOpsVTable * vtable RADDEFAULT(NULL),OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_OpenForWriteCreate_Async(OodleXIOQFile * pFile,const char *name, S64 initialFileSize RADDEFAULT(OODLEX_FILE_OPEN_NO_RESERVE_SIZE),
                                OodleXFileOpenFlags fileOpenFlags RADDEFAULT(OodleXFileOpenFlags_Default),
                                const OodleXFileOpsVTable * vtable RADDEFAULT(NULL),OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_OpenForWriteTempName_Async(OodleXIOQFile * pFile,const char *nameBase RADDEFAULT(NULL), S64 initialFileSize RADDEFAULT(OODLEX_FILE_OPEN_NO_RESERVE_SIZE),
                                OodleXFileOpenFlags fileOpenFlags RADDEFAULT(OodleXFileOpenFlags_Default),
                                const OodleXFileOpsVTable * vtable RADDEFAULT(NULL),OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
/* Start opening a file for write
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_CloseFile_Async(OodleXIOQFile file,S64 truncateFileSize RADDEFAULT(OODLEX_FILE_CLOSE_NO_TRUNCATE_SIZE),OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_CloseFileRename_Async(OodleXIOQFile file,const char * renameTo,S64 truncateFileSize RADDEFAULT(OODLEX_FILE_CLOSE_NO_TRUNCATE_SIZE),OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_Read_Async(OodleXIOQFile file,void * memory,SINTa size,S64 position,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_Write_Async(OodleXIOQFile file,const void * memory,SINTa size,S64 position,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_SetFileSize_Async(OodleXIOQFile file,S64 size,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_ReserveFileSizeForWrite_Async(OodleXIOQFile file,S64 size,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_ForceWriteable_Async(const char * name,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_Delete_Async(const char * name,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_Rename_Async(const char * fm,const char *to,rrbool overwrite,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_MakeDir_Async(const char * name,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
//OodleXHandle OodleIOQ_SetFileSizeUnaligned(const char * name,S64 size);

// runs OodleXFree_IOAligned :
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_FreeBufferIOAligned_Async(OodleXIOQFile file,void * buffer,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_GetInfoByName_Async(const char * name,
    OodleFileNotFoundIsAnError errorIfNotFound, // RADDEFAULT(OodleFileNotFoundIsAnError_Yes)
    OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_SetInfoByName_Async(const char * name,U32 flags,U64 modTime,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_ReadMallocWholeFile_Async(OodleXIOQFile file,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXStatus OOFUNC2 OodleXIOQ_ReadMallocWholeFile_GetResult(OodleXHandle req,OodleXHandleDeleteIfDone andDeleteIfDone, void ** pPtr, S64 * pSize RADDEFAULT(NULL));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_OpenWriteWholeFileClose_Async(const char *name, const void * buffer, SINTa size,
                        OodleXFileOpenFlags fileOpenFlags RADDEFAULT(OodleXFileOpenFlags_Default),
                        const OodleXFileOpsVTable * vtable RADDEFAULT(NULL),OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_OpenWriteWholeFileCloseTempName_Async(const char *name, const void * buffer, SINTa size, 
                        OodleXFileOpenFlags fileOpenFlags RADDEFAULT(OodleXFileOpenFlags_Default),
                        const OodleXFileOpsVTable * vtable RADDEFAULT(NULL),OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_MakeAllDirs_Async(const char * name,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXIOQ_CopyFile_Async(const char * from,const char * to,U32 oodleCopyFileFlags,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),OodleXPriority priority RADDEFAULT(OodleXPriority_Normal),const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 void * OOFUNC2  OodleXMalloc_IOAligned(SINTa size); 
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXHandleEvent_Alloc(OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXHandleCountdown_Alloc(S32 initialCount,OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No));
...
OOFUNC1 OodleXStatus OOFUNC2 OodleXHandleCountdown_Decrement(OodleXHandle h,S32 decCount RADDEFAULT(1));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXLZ_Decompress_ThreadPhased_Narrow_Async(
                                                const void * compBuf,SINTa compSize,
                                                void * decBuf,SINTa rawSize,
                                                OodleLZ_CheckCRC checkCRC RADDEFAULT(OodleLZ_CheckCRC_No),
                                                void * decBufBase RADDEFAULT(NULL),
                                                SINTa decBufSize RADDEFAULT(0),
                                                S32 circularBufferBlockCount RADDEFAULT(-1),
                                                void * scratchBuf RADDEFAULT(NULL),
                                                rrbool synchronous_use_current_thread RADDEFAULT(false));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXLZ_Decompress_Narrow_Async(U32 asyncSelect,
                                                const void * packedDataPtr,SINTa packedLen,
                                                void * rawPtr,SINTa rawChunkLen,
                                                OodleLZ_FuzzSafe fuzzSafe RADDEFAULT(OodleLZ_FuzzSafe_No), 
                                                OodleLZ_CheckCRC checkCRC RADDEFAULT(OodleLZ_CheckCRC_No), 
                                                OodleLZ_Verbosity verbosity RADDEFAULT(OodleLZ_Verbosity_None),
                                                void * decBufBase RADDEFAULT(NULL),SINTa decBufSize RADDEFAULT(0),
                                                OodleDecompressCallback * pcb RADDEFAULT(NULL),
                                                void * pcbData RADDEFAULT(NULL),
                                                void *decMem RADDEFAULT(NULL), SINTa decMemSize RADDEFAULT(0),
                                                OodleLZ_Decode_ThreadPhase threadPhase RADDEFAULT(OodleLZ_Decode_Unthreaded),
                                                OodleXIOQFile writeToFile RADDEFAULT(0),
                                                S64 writeToFileStartPos RADDEFAULT(0),
                                                OodleXHandle writeHandleGroup RADDEFAULT(0),
                                                S32 writeHandleGroupIndex RADDEFAULT(0),
                                                OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),
                                                const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0),
                                                OodleXPriority work_priority RADDEFAULT(OodleXPriority_Normal));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXLZ_ReadAndDecompress_Wide_Async(
            ...
            const void * packedDataPtr,SINTa packedLen,SINTa packedLenPreviouslyRead,
            OodleXIOQFile packedFile,S64 packedDataStartPos,
            void * rawArray,SINTa rawArrayLen,
            OodleLZ_FuzzSafe fuzzSafe, 
            OodleLZ_CheckCRC checkCRC, 
            OodleLZ_Verbosity verbosity RADDEFAULT(OodleLZ_Verbosity_None),
            void * decBufBase RADDEFAULT(NULL),SINTa decBufSize RADDEFAULT(0),
            OodleLZ_PackedRawOverlap packedRawOverlap RADDEFAULT(OodleLZ_PackedRawOverlap_No),
            OodleXIOQFile writeToFile RADDEFAULT(0),
            S64 writeToFileStartPos RADDEFAULT(0),
            OodleXHandle * pWriteHandleGroup RADDEFAULT(0),
            OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),
            const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXLZ_Compress_Async(U32 asyncSelect,OodleLZ_Compressor compressor,
                                                    const void * rawBuf,SINTa rawLen,
                                                    void * compBuf,
                                                    OodleLZ_CompressionLevel compressSelect,const OodleLZ_CompressOptions * pOptions RADDEFAULT(NULL),
...
OOFUNC1 OodleXStatus OOFUNC2 OodleXLZ_Compress_Wait_GetResult(OodleXHandle h, SINTa * pCompLen );
...
/*
OOFUNC1 SINTa OOFUNC2 OodleXLZ_Compress_Wide_CompBufSizeNeeded(SINTa rawLen,    
                    OodleLZ_Compressor compressor,
                    OodleLZ_CompressionLevel selection,
                    const OodleLZ_CompressOptions * pOptions );
*/                  
                    
OOFUNC1 SINTa OOFUNC2 OodleXLZ_Compress_AsyncAndWait(U32 asyncSelect,OodleLZ_Compressor compressor,
                                                    const void * rawBuf,SINTa rawLen,
                                                    void * compBuf,
                                                    OodleLZ_CompressionLevel compressSelect,const OodleLZ_CompressOptions * pOptions RADDEFAULT(NULL),
                                                    const void * dictionaryBase RADDEFAULT(NULL));
...
OOFUNC1 OodleXHandle OOFUNC2 OodleXLZ_ReadCompressWrite_Async( U32 asyncSelect,OodleLZ_Compressor compressor,
                                                    
                                            const void * rawBuf,SINTa rawLen, OodleXIOQFile rawFile, const char * rawFileName,
                                            rrbool freeRawBuf,
                                                    
                                            void * compBuf, OodleXIOQFile compFile, const char * compFileName,
                                            rrbool closeCompFile,rrbool waitOnCompFileClose,rrbool freeCompBuf,
                                            ...
                                            OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),
                                            const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
/* Read a file, compress it, then write it, all async

    $:asyncSelect   logical OR of $OodleXAsyncSelect flags determine how the async is run
    $:compressor    A member of $OodleLZ_Compressor to select the compressor
    $:rawBuf        raw data to compress, pass NULL if you want me to read it
    $:rawLen        amount of raw data to compress
    $:rawFile       the file handle to read from, used if rawBuf is NULL ; can be 0 if rawBuf is already full (or rawFileName is given)
    $:rawFileName   the file name to open to make rawFile, used if rawBuf and rawFile are both 0 ; if rawFile is 0 and rawFileName is given, the open is done for you
    $:freeRawBuf    bool : should rawBuf be freed after compressing
    $:compBuf       output compressed data
    $:compFile      file handle to write compressed data to
    $:compFileName  if compFile is 0, this name will be opened for write
    $:closeCompFile         should I close comp file after writing?
    $:waitOnCompFileClose   should the returned async handle wait for the compressed file close to finish?
    $:freeCompBuf   should the compBuf be freed for you after all writing is done?  
    $:compressSelect        A member of $OodleLZ_CompressionLevel to select the compression level
    $:pOptions      (optional) compression options
    $:dictionaryBase    (optional) if not NULL, provides preceding data to prime the dictionary; must be contiguous with rawBuf, the data
                            between the pointers _dictionaryBase_ and _rawBuf_ is used as the preconditioning data
    ...
    Similar to $OodleXLZ_Compress_Async , but performs the whole read-compress-write chain of operations with maximum parallelism.
    
    Use $OodleXLZ_ReadCompressWrite_Wait_GetResult to clean up the results
    
    If _rawFile_ was passed in, it will not be closed by this call.  If this function opens rawFile, it will also close it.
*/

OOFUNC1 SINTa OOFUNC2 OodleXLZ_ReadCompressWrite_Wait_GetResult(OodleXHandle h,
                                                    void ** pCompBuf RADDEFAULT(NULL),
                                                    void ** pRawBuf RADDEFAULT(NULL), SINTa * pRawSize RADDEFAULT(NULL));
/* Wait for a $OodleXLZ_ReadCompressWrite_Async operation

    $:h     handle to the OodleXLZ_ReadCompressWrite_Async operation
    $:pCompBuf  (optional) filled with pointer to compBuf
    $:pRawBuf   (optional) filled with pointer to rawBuf
    $:pRawSize  (optional) filled with size of uncompressed file
    $:return    length of compressed data, or $OODLELZ_FAILED
    
    If pointers are provided for the buffers, then you must free them with $OodleXFree_IOAligned.  Otherwise they are freed for you.
    If the buffers were previously freed via options like _freeCompBuf_ then they are not accessible from this call.
*/

OOFUNC1 OodleXHandle OOFUNC2 OodleXLZ_ReadAndDecompress_Stream_Async(U32 asyncSelect,
                                                const void * packedDataPtr,SINTa packedLen,
                                                void * rawPtr,SINTa rawChunkLen,
                                                OodleLZ_FuzzSafe fuzzSafe,
                                                OodleLZ_CheckCRC checkCRC, 
                                                ...
                                                OodleXIOQFile readFile, void * readBuf,S64 readStartPos, 
                                                OodleXHandle readPending, SINTa alreadyReadSize,
                                                OodleXHandleAutoDelete autoDelete RADDEFAULT(OodleXHandleAutoDelete_No),
                                                const OodleXHandle * dependencies RADDEFAULT(NULL),S32 numDependencies RADDEFAULT(0));
...
typedef RADSTRUCT OodleDecompressCallback_WriteFile_Data
{
    ...
    SINTa           written;        // number of bytes written so far
    rrbool          doCloseFile;    // should the file be closed after the last write?
...
OOFUNC1 void OOFUNC2 OodleXDecompressCallback_WriteFile_Data_Init(OodleDecompressCallback_WriteFile_Data * pcbData,
    const char * fileName,
    rrbool closeFileAfterWriting,
    SINTa reserveSize RADDEFAULT(0));
...
OodleDecompressCallbackRet OODLE_CALLBACK OodleDecompressCallback_WriteFile(void * pcbData, const U8 * rawBuf,SINTa rawLen,const U8 * compBuf,SINTa compBufferSize , SINTa rawDone, SINTa compUsed);
...
OOFUNC1 S32 OOFUNC2 OodleX_IOAlignUpS32(const S32 x);
...
OOFUNC1 S64 OOFUNC2 OodleX_IOAlignUpS64(const S64 x);
/* Align up to $OODLEX_IO_MAX_ALIGNMENT
...
OOFUNC1 SINTa OOFUNC2 OodleX_IOAlignUpSINTa(const SINTa x);
/* Align up to $OODLEX_IO_MAX_ALIGNMENT
...
OOFUNC1 S32 OOFUNC2 OodleX_IOAlignDownS32(const S32 x);
...
OOFUNC1 S64 OOFUNC2 OodleX_IOAlignDownS64(const S64 x);
/* Align down to $OODLEX_IO_MAX_ALIGNMENT
...
OOFUNC1 SINTa OOFUNC2 OodleX_IOAlignDownSINTa(const SINTa x);
/* Align down to $OODLEX_IO_MAX_ALIGNMENT
...
#define OodleX_S64_to_SINTa_check(x)    (x)

#else

OOFUNC1 SINTa OOFUNC2 OodleX_S64_to_SINTa_check(const S64 x);
/* Convert S64 to SINTa and check

    Used for loading 64-bit file sizes into memory buffers.
    Converts type and checks that file size fits in memory.
    
    In 64 bit builds, this is defined to be a no-op.
...
OOFUNC1 F64 OOFUNC2 OodleX_GetSeconds();
...
OOFUNC1 rrbool OOFUNC2 OodleXUtil_ConvertUTF8ToUTF16(const char * from, U16 * to, int toSize );

OOFUNC1 rrbool OOFUNC2 OodleXUtil_ConvertUTF16ToUTF8(const U16 * from, char * to, int toSize );

//=======================================================
// OodleLog_ logging support
//
// log function bits :
//  combine with OR to create a U32 log state for OodleXLog_SetState()
typedef enum OodleXLog_StateFlags
{
    OODLEXLOG_TO_FILE               =((U32)1<<0), // log to the log file
    OODLEXLOG_ECHO                  =((U32)1<<1), // echo to a stdio file (stdout/stderr typically)
    OODLEXLOG_TO_DEBUGGER           =((U32)1<<2), // log to the debugger
    OODLEXLOG_FILE_LINE             =((U32)1<<3), // put file & line on all logs
    OODLEXLOG_CALLBACK              =((U32)1<<4), // log to the user-provided callback
    OODLEXLOG_PREFIX_THREAD_TIME    =((U32)1<<5), // prefix the thread id & time
    OODLEXLOG_AUTOFLUSH_THREADLOG   =((U32)1<<6), // flush the threadlog to the primary log automatically
    OODLEXLOG_FLUSH_EVERY_WRITE     =((U32)1<<7),  // flush log file after every write, useful for debugging crashes
    OODLEXLOG_STATE_VERBOSITY_NONE  =((U32)0<<16), // verbosity in state
    OODLEXLOG_STATE_VERBOSITY0      =((U32)1<<16),
    OODLEXLOG_STATE_VERBOSITY1      =((U32)2<<16),
    OODLEXLOG_STATE_VERBOSITY2      =((U32)3<<16)
} OodleXLog_StateFlags;
/* Flags for use with $OodleXLog_SetState
*/

RR_COMPILER_ASSERT( sizeof(OodleXLog_StateFlags) == 4 );
...
OOFUNC1 void OOFUNC2 OodleXLog_SetState(U32 options);

OOFUNC1 U32 OOFUNC2 OodleXLog_GetState();
...
// callback rrbool tells you whether to echo the log to other outputs; a false supresses all output
...
OOFUNC1 rrbool OOFUNC2 OodleX_DisplayAssertion(const char * fileName,const int line,const char * function,const char * message);

//=====================================================


typedef RADSTRUCT OodleXConfigValues
{
    S32 m_Oodle_DefaultIOBufferSize;    // the buffer size to use when none is given
    S32 m_Oodle_DefaultWriteReserveSize;// default size to reserve in files opened for write, if none is given
    S32 m_Oodle_MaxSingleIOSize;        // the maximum IO size to submit to the system; larger IO's than this are broken into several pieces; this allows other IO's to interleave, and also prevents heavy loads on kernel resources
    
    S32 m_OodleIOQStream_MaxReadSize;   // IOQStream doesn't read larger than this (unless a client is blocking on needing more than these bytes immediately).  Smaller MaxReadSize reduces IOQStream service latency, but also reduces max throughput
    S32 m_OodleIOQStream_MinReadSize;   // IOQStream tries not to read less than this in a single IO op (unless a client is blocking or we're at EOF or the loop point).  
    S32 m_OodleIOQStream_OffsetAlignment; // IOQStream tries to align all its reads this granularity; some platforms are much faster if the position of IO ops are aligned to large sectors (eg. on the PS3 DVD)

    S32 m_Oodle_very_long_wait_seconds; // seconds to consider "very long" and warn about possible deadlock
    
    S32 m_OodleLZ_Desired_Parallel_BranchFactor; // number of buffer splits for parallel compress

    rrbool m_Oodle_OSFileOpen_Default_Read_Buffered;    // should files opened with $OodleXFileOpenFlags_Default for Read be buffered or not?
    rrbool m_Oodle_OSFileOpen_Default_Write_Buffered;   // should files opened with $OodleXFileOpenFlags_Default for Write be buffered or not?

    rrbool m_Oodle_PathsCaseSensitive;  // are paths compared case-sensitive or not?  Defaults to the per-platform value OODLEX_PLATFORM_CASE_SENSITIVE.
...
OOFUNC1 void OOFUNC2 OodleX_Semaphore_Post(OodleX_Semaphore * sem, S32 count RADDEFAULT(1));
...
typedef U32 (OODLE_CALLBACK OodleX_ThreadFunc)(void * userdata);
...
OOFUNC1 void * OOFUNC2 OodleXIOQ_ReadMallocWholeFile_AsyncAndWait(const char * vfsName,S64 * pSize,
                                                                        OodleXFileOpenFlags fileOpenFlags RADDEFAULT(OodleXFileOpenFlags_Default));
/* See $OodleXIOQ_ReadMallocWholeFile_Async
*/

OOFUNC1 rrbool OOFUNC2 OodleXIOQ_WriteWholeFile_AsyncAndWait(const char * vfsName,const void * buffer,SINTa size,
                                                                        OodleXFileOpenFlags fileOpenFlags RADDEFAULT(OodleXFileOpenFlags_Default));
/* See $OodleXIOQ_OpenWriteWholeFileClose_Async
*/

OOFUNC1 rrbool OOFUNC2 OodleXIOQ_CopyFile_AsyncAndWait(const char * from,const char *to,U32 oodleCopyFileFlags);
/* See $OodleXIOQ_CopyFile_Async
*/

OOFUNC1 rrbool OOFUNC2 OodleXIOQ_GetInfoByName_AsyncAndWait(const char * vfsName,OodleXFileInfo * pInfo,OodleFileNotFoundIsAnError fnfiae);
...
OOFUNC1 rrbool OOFUNC2 OodleXIOQ_SetInfoByName_AsyncAndWait(const char * name,U32 flags,U64 modTime);
/* See $OodleXIOQ_SetInfoByName_Async
*/

// Oodle_FileHash_ReadDoubleBuffered_Async is much better :
//OOFUNC1 rrbool OOFUNC2 OodleXIOQ_ComputeFileHash_AsyncAndWait(const char * vfsName,U64 * pHash);

OOFUNC1 rrbool OOFUNC2 OodleXIOQ_MakeAllDirs_AsyncAndWait(const char * path);
/* See $OodleXIOQ_MakeAllDirs_Async
*/

OOFUNC1 rrbool OOFUNC2 OodleXIOQ_Delete_AsyncAndWait(const char * path);
/* See $OodleXIOQ_Delete_Async
*/

OOFUNC1 rrbool OOFUNC2 OodleXIOQ_Rename_AsyncAndWait(const char * fm,const char * to,rrbool overwrite);
/* See $OodleXIOQ_Rename_Async
*/

OOFUNC1 S64 OOFUNC2 OodleXIOQ_GetFileSize_AsyncAndWait(const char * vfsName,OodleFileNotFoundIsAnError fnfiae);
/* Convenience version of $OodleXIOQ_GetInfoByName_AsyncAndWait
*/

OOFUNC1 rrbool OOFUNC2 OodleXIOQ_NameIsDir_AsyncAndWait(const char * vfsName,OodleFileNotFoundIsAnError fnfiae);
/* Convenience version of $OodleXIOQ_GetInfoByName_AsyncAndWait
*/
...
OOFUNC1 void OOFUNC2 OodleX_CombinePaths(const char *base,const char *add, char * into,S32 intoSize);

// has a backslash on it
OOFUNC1 void OOFUNC2 OodleX_GetOSCwd(char * into,S32 intoSize);

// stick cwd in front of AddTo ; works with ".." in AddTo
OOFUNC1 void OOFUNC2 OodleX_PrefixOSCwd(char * addTo,S32 addToSize);

//===========================================
// OodleXMalloc

// the "Call" functions just invoke the vtable in a clean way
//  (including calling FailedHandler)
// they do NOT do leak track or debug padding or anything like that
//
// no idoc on these for now, they are "secret api"

OOFUNC1 void * OOFUNC2 OodleXMallocCall( const OodleXMallocVTable * pvt, SINTa bytes );
OOFUNC1 void * OOFUNC2 OodleXMallocAlignedCall( const OodleXMallocVTable * pvt, SINTa bytes , S32 alignment );
OOFUNC1 void   OOFUNC2 OodleXFreeCall( const OodleXMallocVTable * pvt, void * ptr);
OOFUNC1 void   OOFUNC2 OodleXFreeSizedCall( const OodleXMallocVTable * pvt, void * ptr, SINTa bytes);

OOFUNC1 void * OOFUNC2 OodleXMallocBigCall( const OodleXMallocVTable * pvt, SINTa bytes );
OOFUNC1 void   OOFUNC2 OodleXFreeBigCall( const OodleXMallocVTable * pvt, void * ptr);

OOFUNC1 rrbool OOFUNC2 OodleXMalloc_ValidatePointerCall(const OodleXMallocVTable * pvt,const void * ptr, SINTa bytes);
...
typedef rrbool (OODLE_CALLBACK OodleXMallocFailedHandler)( SINTa bytes ); 
...
OOFUNC1 void * OOFUNC2 OodleXMalloc( SINTa bytes );
...
OOFUNC1 void * OOFUNC2 OodleXMallocAligned( SINTa bytes , S32 alignment );
...
OOFUNC1 void OOFUNC2 OodleXFreeSized(void * ptr, SINTa bytes);
/* Free a pointer allocated by $OodleXMalloc or $OodleXMallocAligned
...
OOFUNC1 S32 OOFUNC2 OodleXMallocBigAlignment();
...
OOFUNC1 void * OOFUNC2 OodleXMallocBig( SINTa bytes );
...
OOFUNC1 rrbool OOFUNC2 OodleXMalloc_ValidatePointer(const void * ptr, SINTa bytes);
