...
int main()
{
    ...
    constexpr char sourceText[] = "vinput 1\n"
    ...
    Check(git::CompileSourceMapping(source, writer, &diagnostic) == git::SourceResult::Success, "compile editor source mapping");
    filesystem::MemoryFileReader reader(cooked, 0);
    game_input::MappingFile mapping;
    Check(mapping.Open(reader) == game_input::MappingResult::Success && mapping.GetContexts().Size() == 1 && mapping.GetActions().Size() == 2 &&
              mapping.Bindings().Size() == 3,
          "compiled source opens as validated vinput");
    game_input::ActionMap installed;
    Check(mapping.Install(installed) == game_input::MappingResult::Success && installed.GetStats().compiled, "compiled source installs into runtime map");
    ...
    Check(git::CompileSourceMapping(invalidSource, invalidWriter, &diagnostic) == git::SourceResult::InvalidEnum && diagnostic.line == 2 &&
              diagnostic.column != 0 && diagnostic.message != nullptr,
    ...
    Check(git::RegisterMappingCompiler(builds) == assets::Result::Success, "input mapping compiler registration");
    ...
    const assets::BuildRequest request{{sourceReference, source, {}}, outputReference, assets::TargetPlatform::WindowsD3D12, {}};
    assets::BuildOutput first;
    Check(builds.Build(request, first) == assets::Result::Success && first.disposition == assets::BuildDisposition::Built && first.artifacts.Size() == 1 &&
              first.artifacts[0].resource == outputReference && assets::HasFlag(first.artifacts[0].flags, assets::ArtifactFlags::Primary),
    ...
    if (!first.artifacts.Empty())
    {
        ...
        Check(artifact.Open(artifactReader) == game_input::MappingResult::Success, "DDC artifact is runtime-loadable vinput");
    }
