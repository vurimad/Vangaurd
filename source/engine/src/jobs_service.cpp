...
namespace
{
    ...
    {
        if (app::HasProfile(profile, app::ApplicationProfile::Editor))
            return vanguard::jobs::GetEditorConfig();
        if (app::HasProfile(profile, app::ApplicationProfile::Tool))
            return vanguard::jobs::ToolConfig();
    ...
    protected:
        app::LifecycleStatus OnInitialize(app::ServiceContext& context) noexcept override
        {
            ...
            if (!vanguard::jobs::Initialize(SelectConfig(context.GetProfile())))
        ...
        {
            if (!m_initialized || !vanguard::jobs::IsInitialized() || vanguard::jobs::GetWorkerCount() == 0)
        ...
        {
            return vanguard::jobs::GetOutstandingJobCount() == 0
        ...
        app::LifecycleStatus OnShutdown(app::ServiceContext&) noexcept override
        {
            if (!m_initialized)
                return app::LifecycleStatus::Success();
            const vanguard::jobs::SchedulerStats stats = vanguard::jobs::GetSchedulerStats();
            if (stats.outstandingJobs != 0 || stats.liveBuilders != 0 || stats.liveCounters != 0 || stats.activeDeferrals != 0)
                return app::LifecycleStatus::Failure("Jobs shutdown was blocked by live work or scheduler handles");
            if (!vanguard::jobs::Shutdown())
                return app::LifecycleStatus::Failure("Jobs shutdown failed");
            m_initialized = false;
            return app::LifecycleStatus::Success();
        }
    ...
    {
        vanguard::memory::MemoryBlock block = vanguard::memory::Allocate(vanguard::memory::PoolId::Jobs, sizeof(JobsService), alignof(JobsService));
