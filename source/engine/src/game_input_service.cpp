...
namespace
{
    ...
    [[nodiscard]] vanguard::resources::Failure ConvertMappingFailure(const game_input::MappingResult result) noexcept
    {
        ...
        switch (result)
        {
        case game_input::MappingResult::Success: return Failure::None;
        case game_input::MappingResult::InvalidMagic:
        case game_input::MappingResult::IntegrityFailure: return Failure::IntegrityFailure;
        case game_input::MappingResult::UnsupportedVersion: return Failure::UnsupportedVersion;
        case game_input::MappingResult::LimitExceeded: return Failure::OutOfMemory;
        case game_input::MappingResult::IoFailure: return Failure::IoFailure;
        default: return Failure::DeserializationFailure;
        }
    }

    [[nodiscard]] vanguard::resources::ResourceObject* DecodeMapping(
        ...
        const vanguard::resources::LoadContext&, vanguard::resources::Failure& failure, void*) noexcept
    {
        ...
        { failure = vanguard::resources::Failure::UnknownType; return nullptr; }
        vanguard::memory::MemoryBlock block = vanguard::memory::Allocate(
            vanguard::memory::PoolId::Input, sizeof(game_input::MappingResource), alignof(game_input::MappingResource));
        if (!block) { failure = vanguard::resources::Failure::OutOfMemory; return nullptr; }
        ...
        if (result == game_input::MappingResult::Success) return resource;
        resource->~MappingResource(); vanguard::memory::Free(block); failure = ConvertMappingFailure(result); return nullptr;
    }
    ...
    {
    public:
        [[nodiscard]] game_input::ActionMap& Mappings() noexcept override { return m_mappings; }
        [[nodiscard]] const game_input::ActionMap& Mappings() const noexcept override { return m_mappings; }
    ...
    protected:
        app::LifecycleStatus OnInitialize(app::ServiceContext& context) noexcept override
        {
            ...
            if (!m_streaming->Streamer().RegisterDecoder(
                    {game_input::MappingResourceType, "Vanguard input mapping", DecodeMapping, DestroyMapping, nullptr}))
            ...
            {
                static_cast<void>(m_streaming->Streamer().UnregisterDecoder(game_input::MappingResourceType));
                m_decoderRegistered = false;
                return app::LifecycleStatus::Failure(failure.message != nullptr ? failure.message
                                                                               : "Game input frame registration failed");
            }
        ...
        {
            if (m_decoderRegistered && !m_streaming->Streamer().UnregisterDecoder(game_input::MappingResourceType))
    ...
    private:
                                                            ...
                                                            void* const userData) noexcept
        {
            ...
            if (service->m_mappings.Update(service->m_input->Snapshot(), service->m_input->Events(),
                                           context.realDeltaSeconds) != game_input::Result::Success)
    ...
    {
        vanguard::memory::MemoryBlock block = vanguard::memory::Allocate(
            vanguard::memory::PoolId::Input, sizeof(ManagedGameInputService), alignof(ManagedGameInputService));
