...
namespace
{
    ...
    struct BootstrapContent final
    {
        ...
        [[nodiscard]] static resources::ResourceReference Reference(const char* const path,
        ...
        [[nodiscard]] StoredArtifact* Find(const resources::ResourceReference resource) noexcept
        {
            for (StoredArtifact* const artifact : {&input, &prefab, &cell, &startupWorld})
                if (artifact->resource == resource) return artifact;
            return nullptr;
        }
    ...
    [[nodiscard]] bool DiscoverDependencies(const assets::BuildRequest& request, assets::DependencyCollector& dependencies,
                                            void* const userData) noexcept
    {
        ...
        if (request.output == content.startupWorld.resource)
            return dependencies.Add({content.cell.resource, {}, assets::DependencyRole::Generated,
                                     assets::DependencyRequirement::Required}) == assets::Result::Success;
        if (request.output == content.cell.resource)
            return dependencies.Add({content.prefab.resource, {}, assets::DependencyRole::Generated,
                                     assets::DependencyRequirement::Required}) == assets::Result::Success;
    ...
    [[nodiscard]] bool CompileResource(const assets::CompileContext& context, assets::ArtifactWriter& artifacts,
                                       void* const userData) noexcept
    {
        const auto& content = *static_cast<const BootstrapContent*>(userData);
        ...
        {
            const prefabs::EntityBuildRecord entity{1, prefabs::InvalidStableId, 0x626f6f7473747261ull,
                                                     prefabs::EntityFlags::Root};
            prefabs::CookDescription description;
            description.name = 0x626f6f7473747261ull;
            description.entities = {&entity, 1};
            description.sourceFingerprint = crypto::Sha256(context.request.source.content.Data(),
                                                            context.request.source.content.Count());
        ...
        return cooked && artifacts.Add(context.request.output, 0,
    ...
    [[nodiscard]] assets::CompilerDescriptor Compiler(const char* const name, const resources::ResourceTypeId sourceType,
                                                       const resources::ResourceTypeId outputType,
                                                       BootstrapContent& content) noexcept
    {
        return {assets::HashCompilerName(name), name, 1, sourceType, outputType,
                &DiscoverDependencies, &CompileResource, &content};
    }
    ...
    [[nodiscard]] bool StoreBuildOutput(BootstrapContent& content, const assets::BuildOutput& output) noexcept
    {
        for (const assets::Artifact& artifact : output.artifacts)
        {
            ...
            if (destination == nullptr || artifact.segment != 0) return false;
            destination->bytes = artifact.bytes;
        }
        return true;
    }

    [[nodiscard]] bool BuildResource(assets::BuildSystem& builds, assets::DependencyIndex& index,
                                     BootstrapContent& content, const char* const sourcePath,
                                     ...
                                     const StoredArtifact* const generatedDependency = nullptr) noexcept
    {
        ...
        const assets::BuildRequest request{{BootstrapContent::Reference(sourcePath, sourceType),
                                            {reinterpret_cast<const vanguard::u8*>(SourceContent), sizeof(SourceContent) - 1u}, {}},
                                           artifact.resource, assets::TargetPlatform::WindowsD3D12, {}};
        assets::BuildPlan plan;
        if (builds.Prepare(request, plan) != assets::Result::Success) return false;
        if (generatedDependency != nullptr)
        {
            ...
            if (index.Find(generatedDependency->resource, dependencyRecord) != assets::IndexResult::Success ||
                plan.SetGeneratedDependencyContent(generatedDependency->resource, dependencyRecord.contentFingerprint) !=
                    assets::Result::Success) return false;
        }
    ...
    [[nodiscard]] bool ResolvePath(const resources::ResourceReference resource, char* const destination,
                                   const vanguard::usize capacity, vanguard::usize& written, void* const userData) noexcept
    {
        ...
        if (artifact == nullptr || artifact->path == nullptr) return false;
        written = 0;
    ...
    [[nodiscard]] bool ReadArtifact(const assets::IndexedArtifact& artifact, containers::DynamicArray<vanguard::u8>& bytes,
                                    void* const userData) noexcept
    {
        const StoredArtifact* const stored = static_cast<const BootstrapContent*>(userData)->Find(artifact.resource);
        if (stored == nullptr || artifact.segment != 0 || artifact.byteCount != stored->bytes.Size()) return false;
        bytes = stored->bytes;
        return true;
    }

    [[nodiscard]] bool ValidateImage(filesystem::Manager& files, const filesystem::AbsolutePath& path,
                                     const BootstrapContent& content) noexcept
    {
        auto file = files.CreateFileReader(path, filesystem::FOF_Buffered);
        packages::PackageReader reader;
        if (!file || reader.Open(*file) != packages::Result::Success || !reader.HasPackageSet()) return false;
        const packages::PackageSet* const packageSet = reader.GetPackageSet();
        return packageSet != nullptr && packageSet->gameId == GameId &&
               packageSet->startupWorld == content.startupWorld.resource.Path().Id() &&
               packageSet->startupWorldType == content.startupWorld.resource.ExpectedType() &&
               packageSet->defaultInput == content.input.resource.Path().Id() &&
    ...
    {
        if (runtimeRootArgument == nullptr || runtimeRootArgument[0] == '\0') return false;
        const filesystem::AbsolutePath outputDirectory = filesystem::AbsolutePath::CreateDirPath(runtimeRootArgument);
        const filesystem::AbsolutePath cacheDirectory =
            filesystem::paths::GetUserCacheDirectory().AddDirPath("bootstrapImage");
        if (!filesystem::Initialize({outputDirectory, outputDirectory, cacheDirectory})) return false;
        ...
        if (files.FileExist(packagePath))
        {
            VG_LOG_ERROR(diagnostics::Category::Resources,
                         "bootstrap image already exists at %s; committed DATA images are never overwritten implicitly",
                         packagePath.AsChar());
            filesystem::Shutdown();
            return false;
        }
        ...
        const assets::CompilerDescriptor compilers[]{
            Compiler("bootstrap.input", InputSourceType, gameInput::MappingResourceType, content),
        ...
        succeeded = succeeded && BuildResource(builds, index, content, "source/input/default.bootstrap", InputSourceType, content.input) &&
                    ...
                    BuildResource(builds, index, content, "source/world/bootstrap_cell.bootstrap", CellSourceType, content.cell,
                                  &content.prefab) &&
                    BuildResource(builds, index, content, "source/world/bootstrap_world.bootstrap", WorldSourceType,
                                  content.startupWorld, &content.cell) && index.Save() == assets::IndexResult::Success;

        assets::PackageManifest manifest;
        manifest.packageId = GameId;
        succeeded = succeeded && manifest.AddRoot({content.startupWorld.resource, assets::PackageRootFlags::Startup}) ==
                                     assets::PackagingResult::Success &&
                    manifest.AddRoot({content.input.resource, assets::PackageRootFlags::Startup}) ==
                                     assets::PackagingResult::Success;
        ...
        succeeded = succeeded && setPlanner.Prepare(packagePlan, setOptions, nullptr, setPlan, placement) ==
                                     assets::PackagingResult::Success;
        ...
        for (const assets::CompilerDescriptor& compiler : compilers)
            if (builds.IsInitialized()) static_cast<void>(builds.UnregisterCompiler(compiler.id));
        if (index.IsInitialized()) static_cast<void>(index.Shutdown());
        if (builds.IsInitialized()) static_cast<void>(builds.Shutdown());
        filesystem::Shutdown();
        return succeeded;
    }
}
...
int main(const int argumentCount, const char* const* const arguments)
{
    ...
    if (succeeded)
        ...
        VG_LOG_ERROR(diagnostics::Category::Resources,
                     "development bootstrap image generation failed; usage: bootstrapImage <runtime-image-directory>");
    if (ioInitialized) vanguard::io::Shutdown();
    if (diagnosticsInitialized) diagnostics::Shutdown();
